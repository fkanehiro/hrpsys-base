...
int main(int argc, char* argv[]) {
  /* this is c++ version example of test-preview-filter1-modified in
   * euslib/jsk/preview.l*/
  ...
  preview_dynamics_filter<extended_preview_control> df(dt, 0.8,
                                                       ref_zmp_list.front());
  ...
  while (r) {
    ...
    r = df.update(p, x, qdata, ref_zmp_list.front(), qdata,
                  !ref_zmp_list.empty());
    if (r) {
      index++;
