...
namespace rats {
void cycloid_midpoint(hrp::Vector3& ret, const double ratio,
                      const hrp::Vector3& start, const hrp::Vector3& goal,
                      const double height,
                      const double default_top_ratio = 0.5);
void multi_mid_coords(coordinates& mid_coords,
...
struct step_node {
  ...
  step_node(const leg_type _l_r, const coordinates& _worldcoords,
      ...
      : l_r(_l_r),
        ...
        toe_angle(_toe_angle),
        heel_angle(_heel_angle){};
  step_node(const std::string& _l_r, const coordinates& _worldcoords,
            const double _step_height, const double _step_time,
            const double _toe_angle, const double _heel_angle)
      : l_r((_l_r == "rleg") ? RLEG : (_l_r == "rarm") ? RARM : (_l_r == "larm")
                                                                    ? LARM
                                                                    : LLEG),
        worldcoords(_worldcoords),
        step_height(_step_height),
  ...
  friend std::ostream& operator<<(std::ostream& os, const step_node& sn) {
    ...
    os << "  name = ["
       ...
       << "]" << std::endl;
    os << "  pos =";
    os << (sn.worldcoords.pos)
              .format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "",
                                      "", " [", "]"))
       << std::endl;
    os << "  rot =";
    os << (sn.worldcoords.rot)
              .format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", "", " [",
                                      "]"))
       << std::endl;
    os << "  step_height = " << sn.step_height
       << "[m], step_time = " << sn.step_time << "[s], "
       << "toe_angle = " << sn.toe_angle
       << "[deg], heel_angle = " << sn.heel_angle << "[deg]";
    return os;
  };
};
...
struct footstep_parameter {
  ...
  footstep_parameter(const std::vector<hrp::Vector3>& _leg_pos,
                     const double _stride_fwd_x, const double _stride_y,
                     const double _stride_theta, const double _stride_bwd_x)
      : leg_default_translate_pos(_leg_pos),
        stride_fwd_x(_stride_fwd_x),
        stride_y(_stride_y),
        stride_theta(_stride_theta),
        stride_bwd_x(_stride_bwd_x){};
};
...
struct velocity_mode_parameter {
  ...
  velocity_mode_parameter() : velocity_x(0), velocity_y(0), velocity_theta(0){};
};
 ...
 *   SOLE2HEEL : Transition of foot angle (0 -> -1 * heel_angle). ZMP is on
 * heel.
 *   HEEL2SOLE : Transition of foot angle (-1 * heel_angle -> 0). ZMP is on
 * heel.
...
static double no_using_toe_heel_ratio =
    1.0;  // Ratio not to use toe and heel contact
...
struct toe_heel_types {
  ...
  toe_heel_types(const toe_heel_type _src_type = SOLE,
                 const toe_heel_type _dst_type = SOLE)
      : src_type(_src_type), dst_type(_dst_type){};
};
 ...
 public:
  ...
  bool check_toe_heel_phase_ratio_validity(const std::vector<double>& ratio) {
    ...
    if (!ret) {
      std::cerr << "toe_heel_phase_ratio is not set, "
                ...
                << ", sum_ratio = " << sum_ratio << " is not 1.0." << std::endl;
    } else {
  ...
  bool set_toe_heel_phase_ratio(const std::vector<double>& ratio) {
    if (check_toe_heel_phase_ratio_validity(ratio)) {
      for (size_t i = 0; i < NUM_TH_PHASES; i++)
        toe_heel_phase_ratio[i] = ratio[i];
      return true;
    } else {
      return false;
    }
  ...
  void get_toe_heel_phase_ratio(std::vector<double>& ratio) const {
    for (size_t i = 0; i < NUM_TH_PHASES; i++)
      ratio[i] = toe_heel_phase_ratio[i];
  };
  // functions for checking phase and calculating phase time and ratio
  bool is_phase_starting(const size_t current_count,
                         const toe_heel_phase _phase) const {
    return (current_count == toe_heel_phase_count[_phase]);
  };
  bool is_between_phases(const size_t current_count,
                         const toe_heel_phase phase0,
                         const toe_heel_phase phase1) const {
    return (toe_heel_phase_count[phase0] <= current_count) &&
           (current_count < toe_heel_phase_count[phase1]);
  };
  bool is_between_phases(const size_t current_count,
                         const toe_heel_phase phase1) const {
  ...
  bool is_no_SOLE1_phase() const {
    return toe_heel_phase_count[TOE2SOLE] == toe_heel_phase_count[SOLE1];
  };
  // Calculate period [s] between start_phase and goal_phase
  double calc_phase_period(const toe_heel_phase start_phase,
                           const toe_heel_phase goal_phase,
                           const double _dt) const {
    return _dt * (toe_heel_phase_count[goal_phase] -
                  toe_heel_phase_count[start_phase]);
  };
  // Calculate ratio between start_phase and goal_phase.
  //   If current_count is 0->goal_phase_count, start_phase : 0 -> goal_phase :
  //   1
  double calc_phase_ratio(const size_t current_count,
                          const toe_heel_phase start_phase,
                          const toe_heel_phase goal_phase) const {
    return static_cast<double>(current_count -
                               toe_heel_phase_count[start_phase]) /
           (toe_heel_phase_count[goal_phase] -
            toe_heel_phase_count[start_phase]);
  };
  ...
  double calc_phase_ratio_for_toe_heel_transition(
      const size_t current_count) const {
    if (is_between_phases(current_count, SOLE0)) {
      // ratio : 1 -> 0
      return 1 - calc_phase_ratio(current_count, SOLE0);
    } else if (is_between_phases(current_count, HEEL2SOLE, SOLE2) ||
               toe_heel_phase_count[SOLE2] == current_count) {
      // ratio : 0 -> 1
...
class toe_heel_type_checker {
 ...
 public:
  ...
  toe_heel_type_checker(const double _toe_check_thre,
                        const double _heel_check_thre)
      : toe_check_thre(_toe_check_thre), heel_check_thre(_heel_check_thre){};
  toe_heel_type check_toe_heel_type_from_swing_support_coords(
      const coordinates& swing_coords, const coordinates& support_coords,
      const double toe_pos_offset_x, const double heel_pos_offset_x) const {
    hrp::Vector3 local_toe_pos =
        support_coords.rot.transpose() *
        (swing_coords.rot * hrp::Vector3(toe_pos_offset_x, 0, 0) +
         swing_coords.pos - support_coords.pos);
    hrp::Vector3 local_heel_pos =
        support_coords.rot.transpose() *
        (swing_coords.rot * hrp::Vector3(heel_pos_offset_x, 0, 0) +
         swing_coords.pos - support_coords.pos);
    if (local_toe_pos(2) < -50 * 1e-3 &&
        (local_toe_pos(0) + toe_check_thre < 0 ||
         local_heel_pos(0) - heel_check_thre > 0)) {
      return TOE;
...
double set_value_according_to_toe_heel_type(const toe_heel_type tht,
...
class refzmp_generator {
 ...
 public:
  ...
  std::vector<std::vector<hrp::Vector3> >
      foot_x_axises_list;  // Swing foot x axis list according to
                           // refzmp_cur_list
  std::vector<std::vector<leg_type> >
      swing_leg_types_list;  // Swing leg list according to refzmp_cur_list
  std::vector<size_t>
      step_count_list;  // Swing leg list according to refzmp_cur_list
 ...
 public:
  ...
  refzmp_generator(const double _dt)
      : refzmp_cur_list(),
        ...
        dt(_dt),
        thp(),
        use_toe_heel_transition(false),
        use_toe_heel_auto_set(false) {
    ...
    zmp_weight_map = boost::assign::map_list_of<leg_type, double>(
        RLEG, zmp_weight_initial_value[0])(LLEG, zmp_weight_initial_value[1])(
        RARM, zmp_weight_initial_value[2])(LARM, zmp_weight_initial_value[3]);
    zmp_weight_interpolator =
        boost::shared_ptr<interpolator>(new interpolator(4, dt));
    zmp_weight_interpolator->set(
        zmp_weight_initial_value); /* set initial value */
  ...
  void set_zmp_weight_map(const std::map<leg_type, double> _map) {
    double zmp_weight_array[4] = {
        _map.find(RLEG)->second, _map.find(LLEG)->second,
        _map.find(RARM)->second, _map.find(LARM)->second};
    if (zmp_weight_interpolator->isEmpty()) {
      zmp_weight_interpolator->clear();
      double zmp_weight_initial_value[4] = {
          zmp_weight_map[RLEG], zmp_weight_map[LLEG], zmp_weight_map[RARM],
          zmp_weight_map[LARM]};
      zmp_weight_interpolator->set(zmp_weight_initial_value);
      zmp_weight_interpolator->setGoal(zmp_weight_array, 2.0, true);
    } else {
      std::cerr << "zmp_weight_map cannot be set because interpolating."
                << std::endl;
    }
  };
  bool set_toe_heel_phase_ratio(const std::vector<double>& ratio) {
    return thp.set_toe_heel_phase_ratio(ratio);
  };
  // getter
  bool get_current_refzmp(
      hrp::Vector3& rzmp, std::vector<hrp::Vector3>& swing_foot_zmp_offsets,
      const double default_double_support_ratio_before,
      const double default_double_support_ratio_after,
      const double default_double_support_static_ratio_before,
      const double default_double_support_static_ratio_after) {
    if (refzmp_cur_list.size() > refzmp_index)
      calc_current_refzmp(rzmp, swing_foot_zmp_offsets,
...
#ifdef FOR_TESTGAITGENERATOR
  std::vector<hrp::Vector3> get_default_zmp_offsets() const {
...
class delay_hoffarbib_trajectory_generator {
 private:
  ...
  void hoffarbib_interpolation(double& _pos, double& _vel, double& _acc,
                               const double tmp_remain_time,
                               const double tmp_goal,
                               const double tmp_goal_vel = 0,
                               const double tmp_goal_acc = 0) {
    double jerk =
        (-9.0 / tmp_remain_time) * (_acc - tmp_goal_acc / 3.0) +
        (-36.0 / (tmp_remain_time * tmp_remain_time)) *
            (tmp_goal_vel * 2.0 / 3.0 + _vel) +
        (60.0 / (tmp_remain_time * tmp_remain_time * tmp_remain_time)) *
            (tmp_goal - _pos);
    _acc = _acc + dt * jerk;
    _vel = _vel + dt * _acc;
    _pos = _pos + dt * _vel;
 ...
 protected:
  ...
  size_t one_step_count, current_count, double_support_count_before,
      double_support_count_after;  // time/dt
  virtual double calc_antecedent_path(const hrp::Vector3& start,
 ...
 public:
  delay_hoffarbib_trajectory_generator()
      : time_offset(0.35),
        final_distance_weight(1.0),
        time_offset_xy2z(0),
        one_step_count(0),
        current_count(0),
        double_support_count_before(0),
        double_support_count_after(0){};
  ~delay_hoffarbib_trajectory_generator(){};
  ...
  void set_swing_trajectory_final_distance_weight(
  ...
  void reset(const size_t _one_step_len,
    ...
    double_support_count_before =
        (default_double_support_ratio_before * one_step_count);
    double_support_count_after =
        (default_double_support_ratio_after * one_step_count);
  };
  void reset_all(const double _dt, const size_t _one_step_len,
    ...
    set_dt(_dt);
    reset(_one_step_len, default_double_support_ratio_before,
          default_double_support_ratio_after);
  ...
  void get_trajectory_point(hrp::Vector3& ret, const hrp::Vector3& start,
                            const hrp::Vector3& goal, const double height) {
    if (double_support_count_before <= current_count &&
        current_count <
            one_step_count - double_support_count_after) {  // swing phase
      size_t swing_remain_count =
          one_step_count - current_count - double_support_count_after;
      size_t swing_one_step_count = one_step_count -
                                    double_support_count_before -
                                    double_support_count_after;
      double final_path_distance_ratio =
          calc_antecedent_path(start, goal, height);
      size_t tmp_time_offset_count = time_offset / dt;
      // size_t final_path_count = 0; // Revert to previous version
      size_t final_path_count =
          final_path_distance_ratio * swing_one_step_count;
      if (final_path_count > static_cast<size_t>(time_offset_xy2z / dt))
        final_path_count = static_cast<size_t>(time_offset_xy2z / dt);
      // XY interpolation
      if (swing_remain_count >
          final_path_count + tmp_time_offset_count) {  // antecedent path is
                                                       // still interpolating
        hrp::Vector3 tmpgoal = interpolate_antecedent_path(
            (swing_one_step_count - swing_remain_count) /
            static_cast<double>(swing_one_step_count -
                                (final_path_count + tmp_time_offset_count)));
        for (size_t i = 0; i < 2; i++)
          hoffarbib_interpolation(pos(i), vel(i), acc(i), time_offset,
                                  tmpgoal(i));
      } else if (swing_remain_count >
                 final_path_count) {  // antecedent path already reached to goal
        double tmprt = (swing_remain_count - final_path_count) * dt;
        for (size_t i = 0; i < 2; i++)
          hoffarbib_interpolation(pos(i), vel(i), acc(i), tmprt, goal(i));
      } else {
        for (size_t i = 0; i < 2; i++) pos(i) = goal(i);
      }
      // Z interpolation
      if (swing_remain_count >
          tmp_time_offset_count) {  // antecedent path is still interpolating
        hrp::Vector3 tmpgoal = interpolate_antecedent_path(
            (swing_one_step_count - swing_remain_count) /
            static_cast<double>(swing_one_step_count - tmp_time_offset_count));
        hoffarbib_interpolation(pos(2), vel(2), acc(2), time_offset,
                                tmpgoal(2));
      } else if (swing_remain_count >
                 0) {  // antecedent path already reached to goal
        hoffarbib_interpolation(pos(2), vel(2), acc(2), swing_remain_count * dt,
                                goal(2));
      } else {
        pos(2) = goal(2);
      }
    } else if (current_count <
               double_support_count_before) {  // first double support phase
      pos = start;
  ...
  double get_swing_trajectory_final_distance_weight() const {
    return final_distance_weight;
  };
  double get_swing_trajectory_time_offset_xy2z() const {
    return time_offset_xy2z;
  };
  // interpolate path vector
  //   tmp_ratio : ratio value [0, 1]
  //   org_point_vec : vector of via points
  //   e.g., move tmp_ratio from 0 to 1 => move point from org_point_vec.front()
  //   to org_point_vec.back()
  double calc_antecedent_path_base(
    ...
    if (total_path_length <
        1e-5) {  // if total path is zero, return goal point.
      return 0;
    }
  ...
  hrp::Vector3 interpolate_antecedent_path(const double tmp_ratio) const {
    ...
    for (size_t i = 0; i < sum_distance_vec.size(); i++) {
      if ((sum_distance_vec[i] <= current_length) &&
          (current_length <= sum_distance_vec[i + 1])) {
        double tmpr =
            ((current_length - sum_distance_vec[i]) / distance_vec[i]);
...
class rectangle_delay_hoffarbib_trajectory_generator
    : public delay_hoffarbib_trajectory_generator {
  double calc_antecedent_path(const hrp::Vector3& start,
                              const hrp::Vector3& goal, const double height) {
...
class stair_delay_hoffarbib_trajectory_generator
  ...
  double calc_antecedent_path(const hrp::Vector3& start,
    ...
    if (diff_vec.norm() > 1e-4 && (goal(2) - start(2)) > 0.02) {
      path.push_back(hrp::Vector3(
          start + -1 * way_point_offset(0) * diff_vec.normalized() +
    ...
    if (diff_vec.norm() > 1e-4 && (start(2) - goal(2)) > 0.02) {
      path.push_back(hrp::Vector3(
          goal + way_point_offset(0) * diff_vec.normalized() +
...
class cycloid_delay_hoffarbib_trajectory_generator
  ...
  double calc_antecedent_path(const hrp::Vector3& start,
    ...
    double tmpheight =
        ((start(2) + goal(2)) / 2.0 + height - (start(2) + via_goal(2)) / 2.0);
 ...
 public:
  ...
  double calc_antecedent_path(const hrp::Vector3& start,
    ...
    if (height > 1e-4) {
      ...
      cycloid_midpoint(tmpv, 0.2, start + start_rot * kick_point_offset,
                       via_goal, tmpheight);
      cycloid_path.push_back(tmpv);
      cycloid_midpoint(tmpv, 0.4, start + start_rot * kick_point_offset,
                       via_goal, tmpheight);
      cycloid_path.push_back(tmpv);
      cycloid_midpoint(tmpv, 0.6, start + start_rot * kick_point_offset,
                       via_goal, tmpheight);
      cycloid_path.push_back(tmpv);
      cycloid_midpoint(tmpv, 0.8, start + start_rot * kick_point_offset,
                       via_goal, tmpheight);
      cycloid_path.push_back(tmpv);
    }
    cycloid_path.push_back(via_goal);
...
class cross_delay_hoffarbib_trajectory_generator
 ...
 public:
  cross_delay_hoffarbib_trajectory_generator()
      : delay_hoffarbib_trajectory_generator(),
        way_point_offset(hrp::Vector3(0.04, 0.15, 0.0)){};
  ~cross_delay_hoffarbib_trajectory_generator(){};
  ...
  double calc_antecedent_path(const hrp::Vector3& start,
    ...
    if (swing_leg == LLEG) {  // swing_leg is left
      path.push_back(hrp::Vector3(
          start + -1 * way_point_offset(0) * diff_vec.normalized() +
          hrp::Vector3(0, way_point_offset(1),
                       way_point_offset(2) + max_height - start(2))));
      path.push_back(hrp::Vector3(
          goal + way_point_offset(0) * diff_vec.normalized() +
          hrp::Vector3(0, way_point_offset(1),
                       way_point_offset(2) + max_height - goal(2))));
...
/* leg_coords_generator to generate current swing_leg_coords and
 * support_leg_coords from footstep_node_list */
class leg_coords_generator {
 ...
 public:
  ...
  // Swing leg coordinates is interpolated from swing_leg_src_coords to
  // swing_leg_dst_coords during swing phase.
  std::vector<step_node> swing_leg_steps, swing_leg_src_steps,
      swing_leg_dst_steps;
  double default_step_height, default_top_ratio, current_step_height,
  ...
  // Index for current footstep. footstep_index should be
  // [0,footstep_node_list.size()]. Current footstep is
  // footstep_node_list[footstep_index].
  size_t footstep_index;
  // one_step_count is total counter num of current steps (= step_time/dt).
  // lcg_count is counter for lcg. During one step, lcg_count decreases from
  // one_step_count to 0.
  ...
  // Map for interpolator of each swing foot rot interpolation. In constructor,
  // prepared for all limbs. In control loop, swing foot element is used.
  ...
  void calc_current_swing_leg_steps(
      std::vector<step_node>& rets, const double step_height,
      const double _current_toe_angle, const double _current_heel_angle,
      const double _default_double_support_ratio_before,
      const double _default_double_support_ratio_after);
  double calc_interpolated_toe_heel_angle(const toe_heel_phase start_phase,
                                          const toe_heel_phase goal_phase,
                                          const double start,
                                          const double goal);
  ...
  void cross_delay_midcoords(coordinates& ret, const coordinates& start,
                             const coordinates& goal, const double height,
                             leg_type lr);
  void calc_ratio_from_double_support_ratio(
      const double default_double_support_ratio_before,
      const double default_double_support_ratio_after);
 ...
 public:
  ...
  leg_coords_generator(const double _dt)
      : support_leg_steps(),
        ...
        dt(_dt),
        current_toe_angle(0),
        current_heel_angle(0),
        time_offset(0.35),
        final_distance_weight(1.0),
        time_offset_xy2z(0),
        footstep_index(0),
        lcg_count(0),
        default_orbit_type(CYCLOID),
        rdtg(),
        cdtg(),
        thp(),
        ...
        toe_pos_offset_x(0.0),
        heel_pos_offset_x(0.0),
        toe_angle(0.0),
        heel_angle(0.0),
        foot_dif_rot_angle(0.0),
        toe_heel_dif_angle(0.0),
        use_toe_joint(false),
        use_toe_heel_auto_set(false),
        current_src_toe_heel_type(SOLE),
        current_dst_toe_heel_type(SOLE) {
    ...
    if (foot_midcoords_interpolator == NULL)
      foot_midcoords_interpolator = new interpolator(6, dt);  // POS + RPY
    std::vector<leg_type> tmp_leg_types =
        boost::assign::list_of<leg_type>(RLEG)(LLEG)(RARM)(LARM);
    for (size_t i = 0; i < tmp_leg_types.size(); i++) {
      if (swing_foot_rot_interpolator.find(tmp_leg_types[i]) ==
          swing_foot_rot_interpolator.end()) {
        swing_foot_rot_interpolator.insert(std::pair<leg_type, interpolator*>(
            tmp_leg_types[i], new interpolator(3, dt)));  // RPY
        swing_foot_rot_interpolator[tmp_leg_types[i]]->setName(
            "GaitGenerator swing_foot_rot_interpolator " +
            leg_type_to_leg_type_string(tmp_leg_types[i]));
        std::cerr << "GaitGenerator swing_foot_rot_interpolator " +
                         leg_type_to_leg_type_string(tmp_leg_types[i])
                  << std::endl;
        ;
      }
    }
    // if (foot_ratio_interpolator == NULL) foot_ratio_interpolator = new
    // interpolator(1, dt, interpolator::LINEAR);
    if (toe_heel_interpolator == NULL)
      toe_heel_interpolator = new interpolator(1, dt);
    foot_midcoords_interpolator->setName(
  ...
  ~leg_coords_generator() {
    ...
    for (std::map<leg_type, interpolator*>::iterator it =
             swing_foot_rot_interpolator.begin();
         it != swing_foot_rot_interpolator.end(); it++) {
      if (it->second != NULL) {
        delete it->second;
        it->second = NULL;
      }
  ...
  void set_stair_trajectory_way_point_offset(const hrp::Vector3 _offset) {
    sdtg.set_stair_trajectory_way_point_offset(_offset);
  };
  void set_cycloid_delay_kick_point_offset(const hrp::Vector3 _offset) {
    cdktg.set_cycloid_delay_kick_point_offset(_offset);
  };
  ...
  void set_swing_support_steps_list(
      const std::vector<std::vector<step_node> >& fnsl) {
    std::vector<step_node> prev_support_leg_steps =
        support_leg_steps_list.front();
    ...
    for (size_t i = 0; i < fnsl.size(); i++) {
      if (i > 0) {
        if (is_same_footstep_nodes(fnsl.at(i), fnsl.at(i - 1))) {
          ...
          /* current support leg steps = prev swing leg dst steps + (prev
           * support leg steps without current swing leg names) */
          std::vector<step_node> tmp_support_leg_steps =
              swing_leg_dst_steps_list.at(i - 1);
          ...
          for (size_t j = 0; j < swing_leg_dst_steps_list.at(i).size(); j++) {
            std::vector<step_node>::iterator it = std::remove_if(
                tmp_support_leg_steps.begin(), tmp_support_leg_steps.end(),
                (&boost::lambda::_1->*&step_node::l_r ==
                 swing_leg_dst_steps_list.at(i).at(j).l_r));
  ...
  void reset(const size_t _one_step_count, const size_t _next_one_step_count,
    ...
    switch (default_orbit_type) {
      case RECTANGLE:
        ...
        for (size_t i = 0; i < swing_leg_dst_steps.size(); i++) {
          ...
          rdtg.back().reset_all(dt, one_step_count,
                                default_double_support_ratio_before,
                                default_double_support_ratio_after, time_offset,
                                final_distance_weight, time_offset_xy2z);
        }
        break;
      case STAIR:
      ...
      case CYCLOIDDELAYKICK:
        ...
        break;
      case CROSS:
        crdtg.reset(one_step_count, default_double_support_ratio_before,
                    default_double_support_ratio_after);
        break;
      default:
        break;
  ...
  bool is_same_footstep_nodes(const std::vector<step_node>& fns_1,
                              const std::vector<step_node>& fns_2) const;
  void update_leg_steps(const std::vector<std::vector<step_node> >& fnsl,
                        const double default_double_support_ratio_before,
                        const double default_double_support_ratio_after,
                        const toe_heel_type_checker& thtc);
  void calc_swing_leg_src_steps(
      std::vector<step_node>& ret_swing_leg_src_steps,
      const std::vector<std::vector<step_node> >& fnsl,
      const size_t _footstep_index) {
    if (_footstep_index > 0) {
      if (is_same_footstep_nodes(fnsl[_footstep_index],
                                 fnsl[_footstep_index - 1])) {
        ret_swing_leg_src_steps = swing_leg_dst_steps_list[_footstep_index - 1];
      } else {
        /* current swing leg src coords = (previout support leg coords +
         * previous swing leg dst coords) - current support leg coords */
        std::vector<step_node> tmp_swing_leg_src_steps =
            support_leg_steps_list[_footstep_index - 1];
        ...
        std::vector<step_node> tmp_support_leg_steps =
            support_leg_steps_list[_footstep_index];
        for (size_t i = 0; i < tmp_support_leg_steps.size(); i++) {
          std::vector<step_node>::iterator it = std::remove_if(
              tmp_swing_leg_src_steps.begin(), tmp_swing_leg_src_steps.end(),
              (&boost::lambda::_1->*&step_node::l_r ==
               tmp_support_leg_steps.at(i).l_r));
  ...
  void calc_swing_support_params_from_footstep_nodes_list(
      const std::vector<std::vector<step_node> >& fnsl) {
    // Get current swing coords, support coords, and support leg parameters
    size_t current_footstep_index =
        (footstep_index < fnsl.size() - 1 ? footstep_index : fnsl.size() - 1);
    swing_leg_dst_steps = fnsl[current_footstep_index];
    if (footstep_index != 0) {  // If not initial step, support_leg_coords is
                                // previous swing_leg_dst_coords // why we need
                                // this?
      support_leg_steps = support_leg_steps_list[current_footstep_index];
    }
    support_leg_types.clear();
    for (std::vector<step_node>::iterator it = support_leg_steps.begin();
         it != support_leg_steps.end(); it++) {
      support_leg_types.push_back(it->l_r);
    }
    swing_leg_types.clear();
    for (std::vector<step_node>::iterator it = swing_leg_dst_steps.begin();
         it != swing_leg_dst_steps.end(); it++) {
  ...
  double get_current_swing_time(const size_t idx) const {
    return current_swing_time.at(idx);
  };
  const std::vector<step_node>& get_swing_leg_steps() const {
    return swing_leg_steps;
  };
  const std::vector<step_node>& get_support_leg_steps() const {
    return support_leg_steps;
  };
  const std::vector<step_node>& get_swing_leg_src_steps() const {
    return swing_leg_src_steps;
  };
  const std::vector<step_node>& get_swing_leg_dst_steps() const {
    return swing_leg_dst_steps;
  };
  const std::vector<step_node>& get_swing_leg_dst_steps_idx(
      const size_t idx) const {
    return swing_leg_dst_steps_list[idx];
  };
  const std::vector<step_node>& get_support_leg_steps_idx(
      const size_t idx) const {
    return support_leg_steps_list[idx];
  };
  const std::vector<leg_type>& get_support_leg_types() const {
    return support_leg_types;
  };
  const std::vector<leg_type>& get_swing_leg_types() const {
    return swing_leg_types;
  };
  ...
  std::vector<leg_type> get_current_support_states() const {
    if (is_swing_phase) {
      ...
      std::copy(tmp_swg_types.begin(), tmp_swg_types.end(),
                std::back_inserter(tmp_sup_types));
  ...
  hrp::Vector3 get_stair_trajectory_way_point_offset() const {
    return sdtg.get_stair_trajectory_way_point_offset();
  };
  hrp::Vector3 get_cycloid_delay_kick_point_offset() const {
    return cdktg.get_cycloid_delay_kick_point_offset();
  };
  ...
  bool get_use_toe_heel_auto_set() const { return use_toe_heel_auto_set; };
  void get_toe_heel_phase_ratio(std::vector<double>& ratio) const {
    thp.get_toe_heel_phase_ratio(ratio);
  };
  double get_current_toe_heel_ratio(const bool _use_toe_heel_transition) const {
    if (_use_toe_heel_transition &&
        current_step_height > 0.0) {  // If swing phase
      return thp.calc_phase_ratio_for_toe_heel_transition(one_step_count -
                                                          lcg_count);
    } else {  // If support phase such as double support phase of starting and
              // ending.
...
class gait_generator {
 ...
 private:
  ...
  hrp::Vector3 cog, refzmp,
      prev_que_rzmp; /* cog by calculating proc_one_tick */
  ...
  double default_double_support_ratio_before,
      ...
      default_double_support_static_ratio_before,
      default_double_support_static_ratio_after;
  double default_double_support_ratio_swing_before; /*first double support time
                                                       for leg coords generator
                                                       */
  double default_double_support_ratio_swing_after;  /*last double support time
                                                       for leg coords generator
                                                       */
  ...
  //   When overwrite_footstep_index == get_overwritable_index(), overwrite
  //   footsteps after overwrite_footstep_index.
  size_t overwrite_footstep_index;
  // overwritable_footstep_index_offset is used for emergency stop and velocity
  // mode.
  //   overwritable footstep index is "footstep_index +
  //   overwritable_footstep_index_offset", which is obtained by
  ...
  bool solved;
  double leg_margin[4], stride_limitation_for_circle_type[5],
      overwritable_stride_limitation[5], footstep_modification_gain,
      cp_check_margin[2], margin_time_ratio;
  ...
  void append_go_pos_step_nodes(
      const coordinates& _ref_coords, const std::vector<leg_type>& lts,
    ...
    for (size_t i = 0; i < lts.size(); i++) {
      ...
      sns.at(i).worldcoords.pos +=
          sns.at(i).worldcoords.rot *
          footstep_param.leg_default_translate_pos[lts.at(i)];
    }
    _footstep_nodes_list.push_back(sns);
  };
  void overwrite_refzmp_queue(const std::vector<std::vector<step_node> >& fnsl);
  void calc_ref_coords_trans_vector_velocity_mode(
      coordinates& ref_coords, hrp::Vector3& trans, double& dth,
      const std::vector<step_node>& sup_fns,
      const velocity_mode_parameter& cur_vel_param) const;
  void calc_next_coords_velocity_mode(
      std::vector<std::vector<step_node> >& ret_list, const size_t idx,
 ...
 public:
  ...
  gait_generator(double _dt,
      ...
      : footstep_nodes_list(),
        ...
        rg(_dt),
        lcg(_dt),
        footstep_param(_leg_pos, _stride_fwd_x, _stride_y, _stride_theta,
                       _stride_bwd_x),
        vel_param(),
        offset_vel_param(),
        thtc(),
        cog(hrp::Vector3::Zero()),
        refzmp(hrp::Vector3::Zero()),
        prev_que_rzmp(hrp::Vector3::Zero()),
        diff_cp(hrp::Vector3::Zero()),
        modified_d_footstep(hrp::Vector3::Zero()),
        dt(_dt),
        all_limbs(_all_limbs),
        default_step_time(1.0),
        default_double_support_ratio_before(0.1),
        default_double_support_ratio_after(0.1),
        default_double_support_static_ratio_before(0.0),
        default_double_support_static_ratio_after(0.0),
        default_double_support_ratio_swing_before(0.1),
        default_double_support_ratio_swing_after(0.1),
        ...
        margin_time_ratio(0.01),
        footstep_modification_gain(5e-6),
        use_inside_step_limitation(true),
        use_stride_limitation(false),
        modify_footsteps(false),
        default_stride_limitation_type(SQUARE),
        preview_controller_ptr(NULL) {
    swing_foot_zmp_offsets =
        boost::assign::list_of<hrp::Vector3>(hrp::Vector3::Zero());
    prev_que_sfzos = boost::assign::list_of<hrp::Vector3>(hrp::Vector3::Zero());
    leg_type_map = boost::assign::map_list_of<leg_type, std::string>(
        RLEG, "rleg")(LLEG, "lleg")(RARM, "rarm")(LARM, "larm");
  ...
  void initialize_gait_parameter(
  ...
  void limit_stride(step_node& cur_fs, const step_node& prev_fs,
                    const double(&limit)[5]) const;
  void modify_footsteps_for_recovery();
  void append_footstep_nodes(const std::vector<std::string>& _legs,
                             const std::vector<coordinates>& _fss) {
    std::vector<step_node> tmp_sns;
    for (size_t i = 0; i < _legs.size(); i++) {
      tmp_sns.push_back(step_node(
          _legs[i], _fss[i], lcg.get_default_step_height(), default_step_time,
          lcg.get_toe_angle(), lcg.get_heel_angle()));
    }
  ...
  void append_footstep_nodes(const std::vector<std::string>& _legs,
    ...
    std::vector<step_node> tmp_sns;
    for (size_t i = 0; i < _legs.size(); i++) {
      tmp_sns.push_back(step_node(_legs[i], _fss[i], _step_height, _step_time,
                                  _toe_angle, _heel_angle));
    }
    footstep_nodes_list.push_back(tmp_sns);
  };
  ...
  bool go_pos_param_2_footstep_nodes_list(
      ...
      const double goal_theta, /* [mm] [mm] [deg] */
    ...
    return go_pos_param_2_footstep_nodes_list(
        goal_x, goal_y, goal_theta, initial_support_legs_coords,
        start_ref_coords, initial_support_legs, new_footstep_nodes_list,
        is_initialize);
  };
  bool go_pos_param_2_footstep_nodes_list(
      const double goal_x, const double goal_y,
      ...
      std::vector<std::vector<step_node> >& new_footstep_nodes_list,
      const bool is_initialize = true);
  void go_pos_param_2_footstep_nodes_list_core(
      const double goal_x, const double goal_y,
      const double goal_theta, /* [mm] [mm] [deg] */
      const std::vector<coordinates>& initial_support_legs_coords,
      coordinates start_ref_coords,
  ...
  void go_single_step_param_2_footstep_nodes_list(
      const double goal_x, const double goal_y, const double goal_z,
      const double goal_theta, /* [mm] [mm] [mm] [deg] */
      const std::string& tmp_swing_leg, const coordinates& _support_leg_coords);
  void initialize_velocity_mode(
      const coordinates& _ref_coords, const double vel_x, const double vel_y,
      const double vel_theta, /* [mm/s] [mm/s] [deg/s] */
  ...
  void append_finalize_footstep(
      std::vector<std::vector<step_node> >& _footstep_nodes_list) const {
    std::vector<step_node> sns =
        _footstep_nodes_list[_footstep_nodes_list.size() - 2];
  ...
  void set_default_step_time(const double _default_step_time) {
    default_step_time = _default_step_time;
  };
  void set_default_double_support_ratio_before(
      const double _default_double_support_ratio_before) {
    default_double_support_ratio_before = _default_double_support_ratio_before;
  };
  void set_default_double_support_ratio_after(
      const double _default_double_support_ratio_after) {
    default_double_support_ratio_after = _default_double_support_ratio_after;
  };
  void set_default_double_support_static_ratio_before(
      const double _default_double_support_static_ratio_before) {
    default_double_support_static_ratio_before =
        _default_double_support_static_ratio_before;
  };
  void set_default_double_support_static_ratio_after(
      const double _default_double_support_static_ratio_after) {
    default_double_support_static_ratio_after =
        _default_double_support_static_ratio_after;
  };
  void set_default_double_support_ratio_swing_before(
      const double _default_double_support_ratio_swing_before) {
    default_double_support_ratio_swing_before =
        _default_double_support_ratio_swing_before;
  };
  void set_default_double_support_ratio_swing_after(
      const double _default_double_support_ratio_swing_after) {
    default_double_support_ratio_swing_after =
        _default_double_support_ratio_swing_after;
  };
  void set_default_zmp_offsets(const std::vector<hrp::Vector3>& tmp) {
    rg.set_default_zmp_offsets(tmp);
  };
  void set_toe_zmp_offset_x(const double _off) {
    rg.set_toe_zmp_offset_x(_off);
  };
  void set_heel_zmp_offset_x(const double _off) {
    rg.set_heel_zmp_offset_x(_off);
  };
  void set_use_toe_heel_transition(const bool _u) {
    rg.set_use_toe_heel_transition(_u);
  };
  void set_use_toe_heel_auto_set(const bool _u) {
    rg.set_use_toe_heel_auto_set(_u);
    lcg.set_use_toe_heel_auto_set(_u);
  };
  void set_zmp_weight_map(const std::map<leg_type, double> _map) {
    rg.set_zmp_weight_map(_map);
  };
  void set_default_step_height(const double _tmp) {
    lcg.set_default_step_height(_tmp);
  };
  void set_default_top_ratio(const double _tmp) {
    lcg.set_default_top_ratio(_tmp);
  };
  void set_velocity_param(const double vel_x, const double vel_y,
                          const double vel_theta) /* [mm/s] [mm/s] [deg/s] */
  {
    vel_param.set(vel_x, vel_y, vel_theta);
  };
  void set_offset_velocity_param(
      const double vel_x, const double vel_y,
      const double vel_theta) /* [mm/s] [mm/s] [deg/s] */
  {
    offset_vel_param.set(vel_x, vel_y, vel_theta);
  };
  void set_stride_parameters(const double _stride_fwd_x, const double _stride_y,
                             const double _stride_theta,
  ...
  void set_use_inside_step_limitation(const bool uu) {
    use_inside_step_limitation = uu;
  };
  void set_default_orbit_type(const orbit_type type) {
    lcg.set_default_orbit_type(type);
  };
  void set_swing_trajectory_delay_time_offset(const double _time_offset) {
    lcg.set_swing_trajectory_delay_time_offset(_time_offset);
  };
  void set_swing_trajectory_final_distance_weight(
      const double _final_distance_weight) {
    lcg.set_swing_trajectory_final_distance_weight(_final_distance_weight);
  };
  void set_swing_trajectory_time_offset_xy2z(const double _tmp) {
    lcg.set_swing_trajectory_time_offset_xy2z(_tmp);
  };
  void set_stair_trajectory_way_point_offset(const hrp::Vector3 _offset) {
    lcg.set_stair_trajectory_way_point_offset(_offset);
  };
  void set_cycloid_delay_kick_point_offset(const hrp::Vector3 _offset) {
    lcg.set_cycloid_delay_kick_point_offset(_offset);
  };
  ...
  void set_toe_pos_offset_x(const double _offx) {
    lcg.set_toe_pos_offset_x(_offx);
  };
  void set_heel_pos_offset_x(const double _offx) {
    lcg.set_heel_pos_offset_x(_offx);
  };
  ...
  void set_overwrite_foot_steps_list(
      const std::vector<std::vector<step_node> >& fnsl) {
    overwrite_footstep_nodes_list.clear();
  ...
  void set_stride_limitation_for_circle_type(
      const double(&_stride_limitation_for_circle_type)[5]) {
    for (size_t i = 0; i < 5; i++) {
      stride_limitation_for_circle_type[i] =
          _stride_limitation_for_circle_type[i];
    }
  };
  void set_overwritable_stride_limitation(
      const double(&_overwritable_stride_limitation)[5]) {
    for (size_t i = 0; i < 5; i++) {
      overwritable_stride_limitation[i] = _overwritable_stride_limitation[i];
    }
  };
  void set_footstep_modification_gain(
      const double _footstep_modification_gain) {
    footstep_modification_gain = _footstep_modification_gain;
  };
  ...
  void set_act_contact_states(const std::vector<bool>& _act_contact_states) {
    if (act_contact_states.empty())
      act_contact_states.resize(_act_contact_states.size());
  ...
  /* Get overwritable footstep index. For example, if
   * overwritable_footstep_index_offset = 1, overwrite next footstep. If
   * overwritable_footstep_index_offset = 0, overwrite current swinging
   * footstep. */
  ...
  bool get_footstep_nodes_by_index(std::vector<step_node>& csl,
                                   const size_t idx) const {
  ...
  void print_footstep_nodes_list(
    ...
    for (size_t i = 0; i < _footstep_nodes_list.size(); i++) {
      for (size_t j = 0; j < _footstep_nodes_list.at(i).size(); j++) {
        std::cerr << "[" << i << "] " << _footstep_nodes_list.at(i).at(j)
                  << std::endl;
      }
    }
  };
  ...
  std::vector<std::string> convert_leg_types_to_names(
      const std::vector<leg_type>& lts) const {
    std::vector<std::string> ret;
    for (std::vector<leg_type>::const_iterator it = lts.begin();
         it != lts.end(); it++) {
  ...
  std::vector<hrp::Vector3> get_support_foot_zmp_offsets() const {
    ...
    for (size_t i = 0; i < lcg.get_support_leg_types().size(); i++) {
      ret.push_back(
          rg.get_default_zmp_offset(lcg.get_support_leg_types().at(i)));
    }
    return ret;
  };
  // Get foot zmp offsets by checking whether given EE name is swing or support
  bool get_swing_support_foot_zmp_offsets_from_ee_name(
      hrp::Vector3& ret, const std::string& ee_name) const {
    leg_type lt = get_leg_type_from_ee_name(ee_name);
    std::vector<leg_type>::const_iterator it =
        std::find(lcg.get_support_leg_types().begin(),
                  lcg.get_support_leg_types().end(), lt);
    if (it != lcg.get_support_leg_types().end()) {  // If support leg
      ret = get_support_foot_zmp_offsets()[std::distance(
          lcg.get_support_leg_types().begin(), it)];
    } else {
      it = std::find(lcg.get_swing_leg_types().begin(),
                     lcg.get_swing_leg_types().end(), lt);
      if (it != lcg.get_swing_leg_types().end()) {  // If swing leg
        ret = get_swing_foot_zmp_offsets()[std::distance(
            lcg.get_swing_leg_types().begin(), it)];
      } else {  // Otherwise
        return false;
      }
    }
  ...
  bool get_use_toe_heel_transition() const {
    return rg.get_use_toe_heel_transition();
  };
  bool get_use_toe_heel_auto_set() const {
    return rg.get_use_toe_heel_auto_set();
  };
  const std::map<leg_type, double> get_zmp_weight_map() const {
    return rg.get_zmp_weight_map();
  };
  void proc_zmp_weight_map_interpolation() {
    return rg.proc_zmp_weight_map_interpolation();
  };
  ...
  const std::vector<std::string> get_support_leg_names() const {
    return convert_leg_types_to_names(lcg.get_support_leg_types());
  };
  const std::vector<std::string> get_swing_leg_names() const {
    return convert_leg_types_to_names(lcg.get_swing_leg_types());
  };
  const std::vector<step_node>& get_swing_leg_steps() const {
    return lcg.get_swing_leg_steps();
  };
  const std::vector<step_node>& get_support_leg_steps() const {
    return lcg.get_support_leg_steps();
  };
  const std::vector<step_node>& get_swing_leg_src_steps() const {
    return lcg.get_swing_leg_src_steps();
  };
  const std::vector<step_node>& get_swing_leg_dst_steps() const {
    return lcg.get_swing_leg_dst_steps();
  };
  const coordinates get_dst_foot_midcoords()
      const /* get foot_midcoords calculated from swing_leg_dst_coords */
  {
    coordinates tmp(lcg.get_swing_leg_dst_steps().front().worldcoords);
    tmp.pos +=
        tmp.rot * hrp::Vector3(-1 *
                               footstep_param.leg_default_translate_pos
                                   [lcg.get_swing_leg_dst_steps().front().l_r]);
    return tmp;
  };
  void get_swing_support_mid_coords(coordinates& ret) const {
    lcg.get_swing_support_mid_coords(ret);
  };
  void get_stride_parameters(double& _stride_fwd_x, double& _stride_y,
                             double& _stride_theta,
                             double& _stride_bwd_x) const {
  ...
  double get_current_swing_time(const size_t idx) const {
    return lcg.get_current_swing_time(idx);
  };
  // Get current swing time by checking whether given EE name is swing or
  // support
  ...
  orbit_type get_default_orbit_type() const {
    return lcg.get_default_orbit_type();
  };
  double get_swing_trajectory_delay_time_offset() const {
    return lcg.get_swing_trajectory_delay_time_offset();
  };
  double get_swing_trajectory_final_distance_weight() const {
    return lcg.get_swing_trajectory_final_distance_weight();
  };
  double get_swing_trajectory_time_offset_xy2z() const {
    return lcg.get_swing_trajectory_time_offset_xy2z();
  };
  hrp::Vector3 get_stair_trajectory_way_point_offset() const {
    return lcg.get_stair_trajectory_way_point_offset();
  };
  hrp::Vector3 get_cycloid_delay_kick_point_offset() const {
    return lcg.get_cycloid_delay_kick_point_offset();
  };
  ...
  double get_foot_dif_rot_angle() const {
    return lcg.get_foot_dif_rot_angle();
  };
  void get_toe_heel_phase_ratio(std::vector<double>& ratio) const {
    lcg.get_toe_heel_phase_ratio(ratio);
  };
  ...
  double get_current_toe_heel_ratio() const {
    return lcg.get_current_toe_heel_ratio(get_use_toe_heel_transition());
  };
  // Get current toe heel ratio by checking whether given EE name is swing or
  // support
  bool get_current_toe_heel_ratio_from_ee_name(
      double& ret, const std::string& ee_name) const {
    leg_type lt = get_leg_type_from_ee_name(ee_name);
    if (std::find(lcg.get_support_leg_types().begin(),
                  lcg.get_support_leg_types().end(),
                  lt) != lcg.get_support_leg_types().end()) {  // If support
      ret = rats::no_using_toe_heel_ratio;
    } else if (std::find(lcg.get_swing_leg_types().begin(),
                         lcg.get_support_leg_types().end(),
                         lt) != lcg.get_swing_leg_types().end()) {  // If swing
  ...
  void get_leg_default_translate_pos(std::vector<hrp::Vector3>& off) const {
    off = footstep_param.leg_default_translate_pos;
  };
  size_t get_overwritable_footstep_index_offset() const {
    return overwritable_footstep_index_offset;
  };
  const std::vector<leg_type> calc_counter_leg_types_from_footstep_nodes(
      const std::vector<step_node>& fns,
      std::vector<std::string> _all_limbs) const;
  const std::map<leg_type, std::string> get_leg_type_map() const {
    return leg_type_map;
  };
  size_t get_optional_go_pos_finalize_footstep_num() const {
    return optional_go_pos_finalize_footstep_num;
  };
  bool is_finalizing(const double tm) const {
    return ((preview_controller_ptr->get_delay() * 2 - default_step_time / dt) -
            finalize_count) <= (tm / dt) - 1;
  };
  size_t get_overwrite_check_timing() const {
    return static_cast<size_t>(
               footstep_nodes_list[lcg.get_footstep_index()][0].step_time / dt *
               0.5) -
           1;
  };  // Almost middle of step time
  ...
  stride_limitation_type get_stride_limitation_type() const {
    return default_stride_limitation_type;
  };
  ...
  bool get_swing_support_ee_coords_from_ee_name(
      hrp::Vector3& cpos, hrp::Matrix33& crot,
    ...
    if (std::find(lcg.get_support_leg_types().begin(),
                  ...
                  lt) != lcg.get_support_leg_types().end()) {  // If support
      coordinates tmpc =
          std::find_if(lcg.get_support_leg_steps().begin(),
                       lcg.get_support_leg_steps().end(),
                       (&boost::lambda::_1->*&step_node::l_r == lt))
              ->worldcoords;
      cpos = tmpc.pos;
      crot = tmpc.rot;
    } else if (std::find(lcg.get_swing_leg_types().begin(),
                         lcg.get_swing_leg_types().end(),
                         lt) != lcg.get_swing_leg_types().end()) {  // If swing
      coordinates tmpc =
          std::find_if(lcg.get_swing_leg_steps().begin(),
                       lcg.get_swing_leg_steps().end(),
                       (&boost::lambda::_1->*&step_node::l_r == lt))
              ->worldcoords;
      cpos = tmpc.pos;
      crot = tmpc.rot;
  ...
  // Get current support state (true=support, false=swing) by checking whether
  // given EE name is swing or support
  bool get_current_support_state_from_ee_name(
      const std::string& ee_name) const {
    leg_type lt = get_leg_type_from_ee_name(ee_name);
  ...
  void get_footstep_nodes_list(
      std::vector<std::vector<step_node> >& fsl) const {
    fsl = footstep_nodes_list;
  };
  double get_toe_heel_dif_angle() const {
  ...
  void print_param(const std::string& print_str = "") const {
    ...
    std::cerr << "[" << print_str << "]   stride_parameter = " << stride_fwd_x
              << "[m], " << stride_y << "[m], " << stride_th << "[deg], "
              << stride_bwd_x << "[m]" << std::endl;
    std::cerr << "[" << print_str << "]   leg_default_translate_pos = ";
    for (size_t i = 0; i < footstep_param.leg_default_translate_pos.size();
         i++) {
      std::cerr << footstep_param.leg_default_translate_pos[i].format(
          Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "", "",
                          "    [", "]"));
    }
    std::cerr << std::endl;
    std::cerr << "[" << print_str
              << "]   default_step_time = " << get_default_step_time() << "[s]"
              << std::endl;
    std::cerr << "[" << print_str
    ...
    std::cerr << "[" << print_str
              ...
              << ", default_double_support_static_ratio_swing_before = "
              << get_default_double_support_ratio_swing_before()
              << ", default_double_support_static_ratio_swing_after = "
              << get_default_double_support_ratio_swing_after() << std::endl;
    ...
    std::cerr << "[" << print_str << "]   swing_trajectory_delay_time_offset = "
              << get_swing_trajectory_delay_time_offset()
              << "[s], swing_trajectory_final_distance_weight = "
              << get_swing_trajectory_final_distance_weight()
              << ", swing_trajectory_time_offset_xy2z = "
              << get_swing_trajectory_time_offset_xy2z() << std::endl;
    hrp::Vector3 tmpv;
    tmpv = get_stair_trajectory_way_point_offset();
    std::cerr << "[" << print_str << "]   stair_trajectory_way_point_offset = "
              << tmpv.format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ",
                                             ", ", "", "", "    [", "]"))
              << "[m]" << std::endl;
    tmpv = get_cycloid_delay_kick_point_offset();
    std::cerr << "[" << print_str << "]   cycloid_delay_kick_point_offset = "
              << tmpv.format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ",
                                             ", ", "", "", "    [", "]"))
              << "[m]" << std::endl;
    std::cerr << "[" << print_str << "]   gravitational_acceleration = "
              << get_gravitational_acceleration() << "[m/s^2]" << std::endl;
    std::cerr << "[" << print_str
    ...
    std::cerr << "[" << print_str
              ...
              << "[mm]" << std::endl;
    std::cerr << "[" << print_str << "]   toe_angle = " << get_toe_angle()
              << "[deg]" << std::endl;
    std::cerr << "[" << print_str << "]   heel_angle = " << get_heel_angle()
              << "[deg]" << std::endl;
    std::cerr << "[" << print_str << "]   use_toe_joint = "
              << (get_use_toe_joint() ? "true" : "false")
              << ", use_toe_heel_transition = "
              << (get_use_toe_heel_transition() ? "true" : "false")
              << ", use_toe_heel_auto_set = "
              << (get_use_toe_heel_auto_set() ? "true" : "false") << std::endl;
    ...
    for (int i = 0; i < get_NUM_TH_PHASES(); i++)
      std::cerr << tmp_ratio[i] << " ";
    std::cerr << "]" << std::endl;
    std::cerr << "[" << print_str
              << "]   optional_go_pos_finalize_footstep_num = "
              << optional_go_pos_finalize_footstep_num
              << ", overwritable_footstep_index_offset = "
              << overwritable_footstep_index_offset << std::endl;
