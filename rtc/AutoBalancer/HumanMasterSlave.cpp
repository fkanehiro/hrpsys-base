...
template <class T>
IIRFilter_vec<T>::IIRFilter_vec(const std::string& error_prefix) :
    m_initialized(false) {
    m_error_prefix = error_prefix;
}
template <class T>
bool IIRFilter_vec<T>::setParameter(int dim, std::vector<double>& A, std::vector<double>& B) {
    m_dimension = dim;
    // init coefficients
    ...
    m_previous_values.assign(dim, T::Zero());
    m_initialized = true;
    return true;
}
template <class T>
bool IIRFilter_vec<T>::setParameterAsBiquad(const double f_cutoff, const double Q, const double hz){
...
template <class T>
void IIRFilter_vec<T>::getParameter(int &dim, std::vector<double>& A, std::vector<double>& B)
{
    dim = m_dimension;
...
template <class T>
void IIRFilter_vec<T>::reset(T initial_input)
{
...
template <class T>
T IIRFilter_vec<T>::passFilter(T input)
{
    if (! m_initialized) {
        return 0.0;
    }
