...
void preview_control_base<dim>::update_x_k(
    const hrp::Vector3& pr, const std::vector<hrp::Vector3>& _qdata) {
  zmp_z = pr(2);
...
void preview_control::calc_f() {
  ...
  for (size_t i = 0; i < delay; i++) {
    fa = riccati.R_btPb_inv * riccati.b.transpose() *
         (gsi * riccati.Q * riccati.c.transpose());
...
void preview_control::calc_u() {
  ...
  for (size_t i = 0; i < 1 + delay; i++) gfp += f(i) * p[i];
...
void extended_preview_control::calc_x_k() {
  ...
  for (size_t i = 0; i < 3; i++)
    for (size_t j = 0; j < 2; j++) x_k(i, j) += x_k_e(i + 1, j);
}
