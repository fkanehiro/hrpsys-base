...
struct riccati_equation {
  ...
  riccati_equation()
      : A(Eigen::Matrix<double, dim, dim>::Zero()),
        b(Eigen::Matrix<double, dim, 1>::Zero()),
        c(Eigen::Matrix<double, 1, dim>::Zero()),
        P(Eigen::Matrix<double, dim, dim>::Zero()),
        K(Eigen::Matrix<double, 1, dim>::Zero()),
        A_minus_bKt(Eigen::Matrix<double, dim, dim>::Zero()),
        Q(0),
        R(0),
        R_btPb_inv(0){};
  riccati_equation(const Eigen::Matrix<double, dim, dim>& _A,
                   const Eigen::Matrix<double, dim, 1>& _b,
                   const Eigen::Matrix<double, 1, dim>& _c, const double _Q,
                   const double _R)
      : A(_A),
        b(_b),
        c(_c),
  ...
  bool solve() {
    ...
    for (int i = 0; i < 10000; i++) {
      ...
      prev_P = A.transpose() * tmp_pa + c.transpose() * Q * c -
               A.transpose() * P * b * K;
 ...
 public:
  ...
  preview_control_base(const double dt, const double zc,
      ...
      : riccati(),
        ...
        u_k(Eigen::Matrix<double, 1, 2>::Zero()),
        p(),
        pz(),
        qdata(),
        zmp_z(0),
        cog_z(zc),
        delay(static_cast<size_t>(round(d / dt))),
        ending_count(1 + delay) {
    tcA << 1, dt, 0.5 * dt * dt, 0, 1, dt, 0, 0, 1;
    tcb << 1 / 6.0 * dt * dt * dt, 0.5 * dt * dt, dt;
  ...
  void set_preview_queue(const hrp::Vector3& pr,
                         const std::vector<hrp::Vector3>& q, const size_t idx) {
  ...
  size_t get_preview_queue_size() { return p.size(); };
...
class preview_control : public preview_control_base<3> {
 ...
 public:
  preview_control(const double dt, const double zc, const hrp::Vector3& init_xk,
                  const double _gravitational_acceleration =
                      DEFAULT_GRAVITATIONAL_ACCELERATION,
                  const double q = 1.0, const double r = 1.0e-6,
                  const double d = 1.6)
      : preview_control_base<3>(dt, zc, init_xk, _gravitational_acceleration,
                                d) {
...
class extended_preview_control : public preview_control_base<4> {
 ...
 public:
  extended_preview_control(const double dt, const double zc,
    ...
    A << 1.0, tmpca(0, 0), tmpca(0, 1), tmpca(0, 2), 0.0, tcA(0, 0), tcA(0, 1),
        tcA(0, 2), 0.0, tcA(1, 0), tcA(1, 1), tcA(1, 2), 0.0, tcA(2, 0),
        tcA(2, 1), tcA(2, 2);
    b << tmpcb(0, 0), tcb(0, 0), tcb(1, 0), tcb(2, 0);
    c << 1, 0, 0, 0;
    x_k_e(0, 0) = init_xk(0);
    x_k_e(0, 1) = init_xk(1);
...
class preview_dynamics_filter {
 ...
 public:
  ...
  preview_dynamics_filter(const double dt, const double zc,
                          const hrp::Vector3& init_xk,
                          const double _gravitational_acceleration =
                              DEFAULT_GRAVITATIONAL_ACCELERATION,
                          const double q = 1.0, const double r = 1.0e-6,
                          const double d = 1.6)
      : preview_controller(dt, zc, init_xk, _gravitational_acceleration, q, r,
                           d),
        finishedp(false){};
  ~preview_dynamics_filter(){};
  bool update(hrp::Vector3& p_ret, hrp::Vector3& x_ret,
              std::vector<hrp::Vector3>& qdata_ret, const hrp::Vector3& pr,
              const std::vector<hrp::Vector3>& qdata, const bool updatep) {
    bool flg;
    if (updatep) {
  ...
  void remove_preview_queue() { preview_controller.remove_preview_queue(); };
  void set_preview_queue(const hrp::Vector3& pr,
                         const std::vector<hrp::Vector3>& qdata,
                         const size_t idx) {
  ...
  void print_all_queue() { preview_controller.print_all_queue(); }
  ...
  void get_current_refzmp(double* ret) {
    preview_controller.get_current_refzmp(ret);
  }
  // void get_current_qdata (double* ret) {
  // preview_controller.get_current_qdata(ret);}
