...
static const char* autobalancer_spec[] = {
    "implementation_id", "AutoBalancer", "type_name", "AutoBalancer",
    "description", "autobalancer component", "version", HRPSYS_PACKAGE_VERSION,
...
AutoBalancer::AutoBalancer(RTC::Manager* manager)
    : RTC::DataFlowComponentBase(manager),
      ...
      m_refFootOriginExtMomentIn("refFootOriginExtMoment",
      ...
      m_controlSwingSupportTimeOut("controlSwingSupportTime",
...
RTC::ReturnCode_t AutoBalancer::onInitialize() {
  ...
  m_AutoBalancerServicePort.registerProvider("service0", "AutoBalancerService",
                                             m_service0);
  ...
  // GaitGenerator requires abc_leg_offset and abc_stride_parameter in robot
  // conf file
  ...
  // Generate FIK
  fik = fikPtr(new SimpleFullbodyInverseKinematicsSolver(
      m_robot, std::string(m_profile.instance_name), m_dt));
  ...
  if (end_effectors_str.size() > 0) {
    ...
    for (size_t i = 0; i < num; i++) {
      ...
      tp.localR = Eigen::AngleAxis<double>(
      ...
      tmp_fikp.manip = hrp::JointPathExPtr(new hrp::JointPathEx(
          m_robot, m_robot->link(ee_base), m_robot->link(ee_target), m_dt,
          false, std::string(m_profile.instance_name)));
      ...
      fik->ikp.insert(std::pair<std::string,
                                SimpleFullbodyInverseKinematicsSolver::IKparam>(
          ee_name, tmp_fikp));
      // Fix for toe joint
      //   Toe joint is defined as end-link joint in the case that end-effector
      //   link != force-sensor link
      //   Without toe joints, "end-effector link == force-sensor link" is
      //   assumed.
      //   With toe joints, "end-effector link != force-sensor link" is assumed.
      if (m_robot->link(ee_target)->sensors.size() ==
          0) {  // If end-effector link has no force sensor
  ...
  if (leg_offset_str.size() > 0) {
    ...
    for (size_t i = 0; i < 3; i++)
      coil::stringTo(leg_offset(i), leg_offset_str[i].c_str());
  ...
  zmp_offset_interpolator->setName(std::string(m_profile.instance_name) +
                                   " zmp_offset_interpolator");
  zmp_transition_time = 1.0;
  transition_interpolator =
      new interpolator(1, m_dt, interpolator::HOFFARBIB, 1);
  transition_interpolator->setName(std::string(m_profile.instance_name) +
                                   " transition_interpolator");
  transition_interpolator_ratio = 0.0;
  adjust_footstep_interpolator =
      new interpolator(1, m_dt, interpolator::HOFFARBIB, 1);
  adjust_footstep_interpolator->setName(std::string(m_profile.instance_name) +
                                        " adjust_footstep_interpolator");
  transition_time = 2.0;
  adjust_footstep_transition_time = 2.0;
  leg_names_interpolator =
      new interpolator(1, m_dt, interpolator::HOFFARBIB, 1);
  leg_names_interpolator->setName(std::string(m_profile.instance_name) +
                                  " leg_names_interpolator");
  ...
  std::cerr << "[" << m_profile.instance_name
            << "] abc_stride_parameter : " << stride_fwd_x_limit << "[m], "
            << stride_y_limit << "[m], " << stride_th_limit << "[deg], "
            << stride_bwd_x_limit << "[m]" << std::endl;
  if (default_zmp_offsets.size() == 0) {
    for (size_t i = 0; i < ikp.size(); i++)
      default_zmp_offsets.push_back(hrp::Vector3::Zero());
  }
  if (leg_offset_str.size() > 0) {
    gg = ggPtr(new rats::gait_generator(
        m_dt, leg_pos, leg_names, stride_fwd_x_limit /*[m]*/,
        stride_y_limit /*[m]*/, stride_th_limit /*[deg]*/,
  ...
  if (nforce < m_contactStates.data.length()) {
    std::cerr << "[" << m_profile.instance_name
              ...
              << nforce;
    std::cerr << ") than end-effector settings("
              << m_contactStates.data.length() << ") !" << std::endl;
  }
  ...
  for (unsigned int i = 0; i < nforce; i++) {
    m_ref_forceIn[i] = new InPort<TimedDoubleSeq>(
        std::string("ref_" + sensor_names[i]).c_str(), m_ref_force[i]);
    m_ref_force[i].data.length(6);
    registerInPort(std::string("ref_" + sensor_names[i]).c_str(),
                   *m_ref_forceIn[i]);
    std::cerr << "[" << m_profile.instance_name
              << "]   name = " << std::string("ref_" + sensor_names[i])
              << std::endl;
    ref_forces.push_back(hrp::Vector3(0, 0, 0));
  }
  // set force port
  for (unsigned int i = 0; i < nforce; i++) {
    m_ref_forceOut[i] = new OutPort<TimedDoubleSeq>(
        std::string(sensor_names[i]).c_str(), m_force[i]);
    ...
    std::cerr << "[" << m_profile.instance_name
              << "]   name = " << std::string(sensor_names[i]) << std::endl;
  }
  ...
  for (unsigned int i = 0; i < nforce; i++) {
    ...
    m_limbCOPOffsetOut[i] =
        new OutPort<TimedPoint3D>(nm.c_str(), m_limbCOPOffset[i]);
    registerOutPort(nm.c_str(), *m_limbCOPOffsetOut[i]);
    m_limbCOPOffset[i].data.x = m_limbCOPOffset[i].data.y =
        m_limbCOPOffset[i].data.z = 0.0;
...
RTC::ReturnCode_t AutoBalancer::onDeactivated(RTC::UniqueId ec_id) {
  ...
  if (control_mode == MODE_ABC) {
    ...
    transition_interpolator->setGoal(&tmp_ratio, m_dt,
                                     true);  // sync in one controller loop
  }
  return RTC::RTC_OK;
}
...
RTC::ReturnCode_t AutoBalancer::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_emergencySignalIn.isNew()) {
    ...
    // if (!is_stop_mode) {
    //     std::cerr << "[" << m_profile.instance_name << "] emergencySignal is
    //     set!" << std::endl;
    //     is_stop_mode = true;
    //     gg->emergency_stop();
    // }
  ...
  if (m_diffCPIn.isNew()) {
    m_diffCPIn.read();
    gg->set_diff_cp(
        hrp::Vector3(m_diffCP.data.x, m_diffCP.data.y, m_diffCP.data.z));
  }
  ...
  if (is_legged_robot) {
    ...
    if (control_mode != MODE_IDLE) {
      ...
      //          for(int i=0;i<3;i++){
      //            invdyn_zmp_filters[i].setParameterAsBiquad(25,
      //            1/std::sqrt(2), 1.0/m_dt);
      ...
      //          for(int i=0;i<3;i++) ref_zmp(i) =
      //          invdyn_zmp_filters[i].passFilter(ref_zmp(i));
      //        }
      //        updateInvDynStateBuffer(idsb);

      rel_ref_zmp = m_robot->rootLink()->R.transpose() *
                    (ref_zmp - m_robot->rootLink()->p);
    } else {
    ...
    if (!is_transition_interpolator_empty) {
      ...
      ref_basePos = (1 - transition_interpolator_ratio) * input_basePos +
                    transition_interpolator_ratio * m_robot->rootLink()->p;
      rel_ref_zmp = (1 - transition_interpolator_ratio) * input_zmp +
                    transition_interpolator_ratio * rel_ref_zmp;
      rats::mid_rot(ref_baseRot, transition_interpolator_ratio, input_baseRot,
                    m_robot->rootLink()->R);
      for (unsigned int i = 0; i < m_robot->numJoints(); i++) {
        m_robot->joint(i)->q =
            (1 - transition_interpolator_ratio) * m_qRef.data[i] +
            transition_interpolator_ratio * m_robot->joint(i)->q;
      }
      for (unsigned int i = 0; i < m_force.size(); i++) {
        for (unsigned int j = 0; j < 6; j++) {
          m_force[i].data[j] =
              transition_interpolator_ratio * m_force[i].data[j] +
              (1 - transition_interpolator_ratio) * m_ref_force[i].data[j];
        }
      }
      for (unsigned int i = 0; i < m_limbCOPOffset.size(); i++) {
        ...
        m_limbCOPOffset[i].data.x =
            ...
            m_limbCOPOffset[i]
                .data.x;  // + (1-transition_interpolator_ratio) * 0;
        m_limbCOPOffset[i].data.y =
            transition_interpolator_ratio *
            m_limbCOPOffset[i]
                .data.y;  // + (1-transition_interpolator_ratio) * 0;
        m_limbCOPOffset[i].data.z =
            transition_interpolator_ratio *
            m_limbCOPOffset[i]
                .data.z;  // + (1-transition_interpolator_ratio) * 0;
      }
    } else {
    ...
    } else if (control_mode == MODE_SYNC_TO_IDLE &&
               transition_interpolator->isEmpty()) {
  ...
  if (is_legged_robot) {
    ...
    if (sen != NULL) {
      ...
      hrp::Vector3 imu_sensor_vel =
          (imu_sensor_pos - prev_imu_sensor_pos) / m_dt;
      // convert to imu sensor local acceleration
      hrp::Vector3 acc = (sen->link->R * sen->localR).transpose() *
                         (imu_sensor_vel - prev_imu_sensor_vel) / m_dt;
  ...
  if (control_mode != MODE_IDLE) {
    ...
    if (control_mode == MODE_SYNC_TO_ABC) {
      ...
      for (std::map<std::string, ABCIKparam>::iterator it = ikp.begin();
           it != ikp.end(); it++) {
        if (std::find(leg_names.begin(), leg_names.end(), it->first) !=
            leg_names.end()) {
          it->second.target_p0 =
              it->second.target_link->p +
              it->second.target_link->R * it->second.localPos;
    ...
    for (std::map<std::string, ABCIKparam>::iterator it = ikp.begin();
      ...
      if (std::find(leg_names.begin(), leg_names.end(), it->first) !=
          leg_names.end()) {
        tmp_end_coords_list.push_back(
            coordinates(it->second.target_link->p +
                            it->second.target_link->R * it->second.localPos,
                        it->second.target_link->R * it->second.localR));
      }
    }
...
void AutoBalancer::getOutputParametersForWalking() {
  for (std::map<std::string, ABCIKparam>::iterator it = ikp.begin();
    ...
    // Check whether "it->first" ee_name is included in leg_names. leg_names is
    // equivalent to "swing" + "support" in gg.
    if (std::find(leg_names.begin(), leg_names.end(), it->first) !=
        leg_names.end()) {
      // Set EE coords
      gg->get_swing_support_ee_coords_from_ee_name(
          it->second.target_p0, it->second.target_r0, it->first);
      // Set contactStates
      m_contactStates.data[idx] =
          gg->get_current_support_state_from_ee_name(it->first);
      // Set controlSwingSupportTime
      m_controlSwingSupportTime.data[idx] =
          gg->get_current_swing_time_from_ee_name(it->first);
      // Set limbCOPOffset
      hrp::Vector3 tmpzmpoff(m_limbCOPOffset[idx].data.x,
                             m_limbCOPOffset[idx].data.y,
                             m_limbCOPOffset[idx].data.z);
      ...
      // controlSwingSupportTime is not used while double support period, 1.0 is
      // neglected
...
void AutoBalancer::getOutputParametersForABC() {
  ...
  for (std::map<std::string, ABCIKparam>::iterator it = ikp.begin();
    ...
    size_t idx = contact_states_index_map[it->first];
    // Set EE coords.
    //   If not included in leg_names, calculate EE coords because of not being
    //   used in GaitGenerator.
    //   Otherwise, keep previous EE coords derived from GaitGenerator or
    //   initial value.
    if (std::find(leg_names.begin(), leg_names.end(), it->first) ==
        leg_names.end()) {
      it->second.target_p0 = it->second.target_link->p +
    ...
    // Set contactStates
    std::vector<std::string>::const_iterator dst = std::find_if(
        leg_names.begin(), leg_names.end(), (boost::lambda::_1 == it->first));
...
void AutoBalancer::getOutputParametersForIDLE() {
  ...
  if (m_optionalData.data.length() >= contact_states_index_map.size() * 2) {
    // current optionalData is contactstates x limb and controlSwingSupportTime
    // x limb
    //   If contactStates in optionalData is 1.0, m_contactStates is true.
    //   Otherwise, false.
    for (std::map<std::string, ABCIKparam>::iterator it = ikp.begin();
         it != ikp.end(); it++) {
      m_contactStates.data[contact_states_index_map[it->first]] =
          isOptionalDataContact(it->first);
      m_controlSwingSupportTime.data[contact_states_index_map[it->first]] =
          m_optionalData.data[contact_states_index_map[it->first] +
                              contact_states_index_map.size()];
    }
    // If two feet have no contact, force set double support contact
    if (!m_contactStates.data[contact_states_index_map["rleg"]] &&
        !m_contactStates.data[contact_states_index_map["lleg"]]) {
      m_contactStates.data[contact_states_index_map["rleg"]] = true;
      m_contactStates.data[contact_states_index_map["lleg"]] = true;
    }
  }
  ...
  if (!zmp_offset_interpolator->isEmpty()) {
    ...
    if (DEBUGP) {
      ...
      for (std::map<std::string, ABCIKparam>::iterator it = ikp.begin();
           it != ikp.end(); it++) {
        std::cerr
            << "[" << m_profile.instance_name << "]   " << it->first << " = "
            << default_zmp_offsets[contact_states_index_map[it->first]].format(
...
void AutoBalancer::calcFixCoordsForAdjustFootstep(coordinates& tmp_fix_coords) {
  ...
  // std::cerr << "[" << m_profile.instance_name << "] adjust ratio " << tmp <<
  // std::endl;
  ikp["rleg"].target_p0 = (1 - tmp) * ikp["rleg"].adjust_interpolation_org_p0 +
                          tmp * ikp["rleg"].adjust_interpolation_target_p0;
  ikp["lleg"].target_p0 = (1 - tmp) * ikp["lleg"].adjust_interpolation_org_p0 +
                          tmp * ikp["lleg"].adjust_interpolation_target_p0;
  rats::mid_rot(ikp["rleg"].target_r0, tmp,
                ikp["rleg"].adjust_interpolation_org_r0,
                ikp["rleg"].adjust_interpolation_target_r0);
  rats::mid_rot(ikp["lleg"].target_r0, tmp,
                ikp["lleg"].adjust_interpolation_org_r0,
                ikp["lleg"].adjust_interpolation_target_r0);
...
void AutoBalancer::rotateRefForcesForFixCoords(coordinates& tmp_fix_coords) {
  ...
  for (size_t i = 0; i < m_ref_forceIn.size(); i++) {
    ...
    // m_robot->sensor<hrp::ForceSensor>(sensor_names[i]);
    // if (sensor) parentlink = sensor->link;
    // else parentlink = m_vfs[sensor_names[i]].link;
    // for ( std::map<std::string, ABCIKparam>::iterator it = ikp.begin(); it !=
    // ikp.end(); it++ ) {
    //     if (it->second.target_link->name == parentlink->name) eeR =
    //     parentlink->R * it->second.localR;
    // }
    // End effector frame
    // ref_forces[i] = eeR * hrp::Vector3(m_ref_force[i].data[0],
    // m_ref_force[i].data[1], m_ref_force[i].data[2]);
    // world frame
    ref_forces[i] = tmp_fix_coords.rot * hrp::Vector3(m_ref_force[i].data[0],
                                                      m_ref_force[i].data[1],
                                                      m_ref_force[i].data[2]);
  }
  sbp_offset = tmp_fix_coords.rot * hrp::Vector3(sbp_offset);
};

void AutoBalancer::updateTargetCoordsForHandFixMode(
    coordinates& tmp_fix_coords) {
  // Move hand for hand fix mode
  //   If arms' ABCIKparam.is_active is true, move hands according to cog
  //   velocity.
  //   If is_hand_fix_mode is false, no hand fix mode and move hands according
  //   to cog velocity.
  //   If is_hand_fix_mode is true, hand fix mode and do not move hands in Y
  //   axis in tmp_fix_coords.rot.
  if (gg_is_walking) {
    // hand control while walking = solve hand ik with is_hand_fix_mode and
    // solve hand ik without is_hand_fix_mode
    ...
    if (is_hand_control_while_walking) {
      ...
      if (is_hand_fix_initial)
        hand_fix_initial_offset =
            tmp_fix_coords.rot.transpose() *
            (hrp::Vector3(gg->get_cog()(0), gg->get_cog()(1),
                          tmp_fix_coords.pos(2)) -
             tmp_fix_coords.pos);
      is_hand_fix_initial = false;
      hrp::Vector3 dif_p = hrp::Vector3(gg->get_cog()(0), gg->get_cog()(1),
                                        tmp_fix_coords.pos(2)) -
                           tmp_fix_coords.pos -
                           tmp_fix_coords.rot * hand_fix_initial_offset;
      if (is_hand_fix_mode) {
...
void AutoBalancer::calculateOutputRefForces() {
  ...
  for (size_t i = 0; i < leg_names.size(); i++) {
    ...
    ee_pos.push_back(tmpikp.target_p0 +
                     tmpikp.target_r0 * default_zmp_offsets[i]);
  }
  double alpha = (ref_zmp - ee_pos[1]).norm() /
                 ((ee_pos[0] - ref_zmp).norm() + (ee_pos[1] - ref_zmp).norm());
  if (alpha > 1.0) alpha = 1.0;
  if (alpha < 0.0) alpha = 0.0;
  if (DEBUGP) {
    std::cerr << "[" << m_profile.instance_name << "] alpha:" << alpha
              << std::endl;
  }
  ...
  if (use_force == MODE_REF_FORCE_WITH_FOOT ||
      use_force ==
          MODE_REF_FORCE_RFU_EXT_MOMENT) {  // TODO : use other use_force mode.
                                            // This should be depends on
                                            // Stabilizer distribution mode.
...
hrp::Vector3 AutoBalancer::calcFootMidPosUsingZMPWeightMap() {
  ...
  for (size_t i = 0; i < leg_names.size(); i++) {
    ABCIKparam& tmpikp = ikp[leg_names[i]];
    // for foot_mid_pos
    std::map<leg_type, std::string>::const_iterator dst = std::find_if(
        leg_type_map.begin(), leg_type_map.end(),
        (&boost::lambda::_1
                 ->*&std::map<leg_type, std::string>::value_type::second ==
         leg_names[i]));
    tmp_foot_mid_pos +=
        (tmpikp.target_p0 + tmpikp.target_r0 * default_zmp_offsets[i]) *
        zmp_weight_map[dst->first];
...
void AutoBalancer::updateWalkingVelocityFromHandError(
    coordinates& tmp_fix_coords) {
  // TODO : check frame and robot state for this calculation
  if (gg_is_walking &&
      gg->get_lcg_count() == gg->get_overwrite_check_timing() + 2) {
    hrp::Vector3 vel_htc(calc_vel_from_hand_error(tmp_fix_coords));
    gg->set_offset_velocity_param(vel_htc(0), vel_htc(1), vel_htc(2));
  }  //  else {
     //     if ( gg_is_walking && gg->get_lcg_count() ==
     //     static_cast<size_t>(gg->get_default_step_time()/(2*m_dt))-1) {
...
void AutoBalancer::calcReferenceJointAnglesForIK() {
  ...
  for (std::map<std::string, ABCIKparam>::iterator it = ikp.begin();
       it != ikp.end(); it++) {
    if (it->second.is_active && it->second.has_toe_joint &&
...
void AutoBalancer::fixLegToCoords(const hrp::Vector3& fix_pos,
  ...
  for (size_t i = 0; i < leg_names.size(); i++) {
    ...
    if (leg_names[i].find("leg") != std::string::npos)
      foot_coords.push_back(coordinates(
          (tmpikp.target_link->p + tmpikp.target_link->R * tmpikp.localPos),
  ...
  m_robot->rootLink()->p =
      fix_pos + tmpR * (m_robot->rootLink()->p - current_foot_mid_pos);
...
void AutoBalancer::fixLegToCoords2(coordinates& tmp_fix_coords) {
  ...
  // This will be removed after seq outputs adequate waistRPY discussed in
  // https://github.com/fkanehiro/hrpsys-base/issues/272
  // Snap input tmp_fix_coords to XY plan projection.
  {
    ...
    tmp_fix_coords.rot(0, 0) = xv1(0);
    tmp_fix_coords.rot(1, 0) = xv1(1);
    tmp_fix_coords.rot(2, 0) = xv1(2);
    tmp_fix_coords.rot(0, 1) = yv1(0);
    tmp_fix_coords.rot(1, 1) = yv1(1);
    tmp_fix_coords.rot(2, 1) = yv1(2);
    tmp_fix_coords.rot(0, 2) = ez(0);
    tmp_fix_coords.rot(1, 2) = ez(1);
    tmp_fix_coords.rot(2, 2) = ez(2);
  }
...
void AutoBalancer::solveFullbodyIK() {
  ...
  for (std::map<std::string,
                SimpleFullbodyInverseKinematicsSolver::IKparam>::iterator it =
           fik->ikp.begin();
       it != fik->ikp.end(); it++) {
...
void AutoBalancer::startABCparam(
    const OpenHRP::AutoBalancerService::StrSequence& limbs) {
  std::cerr << "[" << m_profile.instance_name << "] start auto balancer mode"
            << std::endl;
  Guard guard(m_mutex);
  double tmp_ratio = 0.0;
  ...
  for (size_t i = 0; i < limbs.length(); i++) {
    ...
    tmp.is_active = true;
    std::cerr << "[" << m_profile.instance_name << "]   limb ["
              << std::string(limbs[i]) << "]" << std::endl;
  }
...
void AutoBalancer::stopABCparam() {
  std::cerr << "[" << m_profile.instance_name << "] stop auto balancer mode"
            << std::endl;
  // Guard guard(m_mutex);
...
bool AutoBalancer::startWalking() {
  if (control_mode != MODE_ABC) {
    std::cerr
        << "[" << m_profile.instance_name
        << "] Cannot start walking without MODE_ABC. Please startAutoBalancer."
        << std::endl;
    return false;
  }
  {
    ...
    std::vector<std::string> init_swing_leg_names(
    ...
    std::set_difference(tmp_all_limbs.begin(), tmp_all_limbs.end(),
                        init_swing_leg_names.begin(),
                        init_swing_leg_names.end(),
                        std::back_inserter(init_support_leg_names));
    std::vector<step_node> init_support_leg_steps, init_swing_leg_dst_steps;
    for (std::vector<std::string>::iterator it = init_support_leg_names.begin();
         it != init_support_leg_names.end(); it++)
      init_support_leg_steps.push_back(
          step_node(*it, coordinates(ikp[*it].target_p0, ikp[*it].target_r0), 0,
                    0, 0, 0));
    for (std::vector<std::string>::iterator it = init_swing_leg_names.begin();
         it != init_swing_leg_names.end(); it++)
      init_swing_leg_dst_steps.push_back(
          step_node(*it, coordinates(ikp[*it].target_p0, ikp[*it].target_r0), 0,
                    0, 0, 0));
    gg->set_default_zmp_offsets(default_zmp_offsets);
    gg->initialize_gait_parameter(ref_cog, init_support_leg_steps,
                                  init_swing_leg_dst_steps);
  }
...
void AutoBalancer::stopWalking() {
  std::vector<coordinates> tmp_end_coords_list;
  for (std::vector<string>::iterator it = leg_names.begin();
       it != leg_names.end(); it++) {
    if ((*it).find("leg") != std::string::npos)
      tmp_end_coords_list.push_back(
          coordinates(ikp[*it].target_p0, ikp[*it].target_r0));
  }
  multi_mid_coords(fix_leg_coords, tmp_end_coords_list);
...
bool AutoBalancer::startAutoBalancer(
    const OpenHRP::AutoBalancerService::StrSequence& limbs) {
  if (control_mode == MODE_IDLE) {
    fik->resetIKFailParam();
    startABCparam(limbs);
...
bool AutoBalancer::goPos(const double& x, const double& y, const double& th) {
  ...
  if (!is_stop_mode) {
    ...
    bool is_valid_gait_type = calc_inital_support_legs(
        y, initial_support_legs_coords, initial_support_legs, start_ref_coords);
    if (is_valid_gait_type == false) return false;
    bool ret = gg->go_pos_param_2_footstep_nodes_list(
        x, y, th,
        ...
        (!gg_is_walking));  // If gg_is_walking, initialize. Otherwise, not
                            // initialize and overwrite footsteps.
    if (!ret) {
      std::cerr << "[" << m_profile.instance_name
...
bool AutoBalancer::goVelocity(const double& vx, const double& vy,
                              const double& vth) {
  gg->set_all_limbs(leg_names);
  bool ret = true;
  if (gg_is_walking && gg_solved) {
    ...
    switch (gait_type) {
      case BIPED:
        current_legs = (vy > 0 ? boost::assign::list_of(RLEG)
                               : boost::assign::list_of(LLEG));
        break;
      case TROT:
        current_legs = (vy > 0 ? boost::assign::list_of(RLEG)(LARM)
                               : boost::assign::list_of(LLEG)(RARM));
        break;
      case PACE:
        current_legs = (vy > 0 ? boost::assign::list_of(RLEG)(RARM)
                               : boost::assign::list_of(LLEG)(LARM));
        break;
      case CRAWL:
        std::cerr << "[" << m_profile.instance_name << "] crawl walk["
                  << gait_type << "] is not implemented yet." << std::endl;
        return false;
      case GALLOP:
        /* at least one leg shoud be in contact */
        std::cerr << "[" << m_profile.instance_name << "] gallop walk["
                  << gait_type << "] is not implemented yet." << std::endl;
        return false;
      default:
        break;
...
bool AutoBalancer::releaseEmergencyStop() {
  if (is_stop_mode) {
    std::cerr << "[" << m_profile.instance_name << "] releaseEmergencyStop"
              << std::endl;
    is_stop_mode = false;
  }
  return true;
...
bool AutoBalancer::setFootSteps(
  ...
  // If gg_is_walking is false, initial footstep will be double support. So, set
  // 0 for step_height and toe heel angles.
  ...
  for (size_t i = 0; i < spss.length(); i++) {
    ...
    for (size_t j = 0; j < spss[i].sps.length(); j++) {
      spss[i].sps[j].step_height =
          ((!gg_is_walking && i == 0) ? 0.0 : gg->get_default_step_height());
      spss[i].sps[j].step_time = gg->get_default_step_time();
      spss[i].sps[j].toe_angle =
          ((!gg_is_walking && i == 0) ? 0.0 : gg->get_toe_angle());
      spss[i].sps[j].heel_angle =
...
bool AutoBalancer::setFootStepsWithParam(
    const OpenHRP::AutoBalancerService::FootstepsSequence& fss,
    const OpenHRP::AutoBalancerService::StepParamsSequence& spss,
    CORBA::Long overwrite_fs_idx) {
  if (!is_stop_mode) {
    std::cerr << "[" << m_profile.instance_name << "] setFootStepsList"
              << std::endl;
    ...
    step_node initial_support_step, initial_input_step;
    {
      ...
      if (gg_is_walking) {
        if (overwrite_fs_idx <= 0) {
          std::cerr << "[" << m_profile.instance_name
                    << "]   Invalid overwrite index = " << overwrite_fs_idx
                    << std::endl;
          return false;
        }
        if (!gg->get_footstep_nodes_by_index(initial_support_steps,
                                             overwrite_fs_idx - 1)) {
          std::cerr << "[" << m_profile.instance_name
                    << "]   Invalid overwrite index = " << overwrite_fs_idx
                    << std::endl;
        ...
        for (size_t i = 0; i < fss[0].fs.length(); i++) {
          ...
          initial_support_steps.push_back(step_node(
              std::string(fss[0].fs[i].leg),
              coordinates(tmpikp.target_p0, tmpikp.target_r0), 0, 0, 0, 0));
        }
      }
      initial_support_step =
          initial_support_steps
              .front(); /* use only one leg for representation */
    }
    {
      std::map<leg_type, std::string> leg_type_map = gg->get_leg_type_map();
      for (size_t i = 0; i < fss[0].fs.length(); i++) {
        if (std::string(fss[0].fs[i].leg) ==
            leg_type_map[initial_support_step.l_r]) {
          coordinates tmp;
          memcpy(tmp.pos.data(), fss[0].fs[i].pos, sizeof(double) * 3);
          tmp.rot = (Eigen::Quaternion<double>(
                         fss[0].fs[i].rot[0], fss[0].fs[i].rot[1],
                         fss[0].fs[i].rot[2], fss[0].fs[i].rot[3]))
                        .normalized()
                        .toRotationMatrix();  // rtc: (x, y, z, w) but eigen:
                                              // (w, x, y, z)
          initial_input_step =
              step_node(std::string(fss[0].fs[i].leg), tmp, 0, 0, 0, 0);
        }
      }
    }
    ...
    for (size_t i = 0; i < fss.length(); i++) {
      ...
      for (size_t j = 0; j < fss[i].fs.length(); j++) {
        ...
        if (std::find(leg_names.begin(), leg_names.end(), leg) !=
            leg_names.end()) {
          memcpy(tmpfs.pos.data(), fss[i].fs[j].pos, sizeof(double) * 3);
          tmpfs.rot = (Eigen::Quaternion<double>(
                           fss[i].fs[j].rot[0], fss[i].fs[j].rot[1],
                           fss[i].fs[j].rot[2], fss[i].fs[j].rot[3]))
                          .normalized()
                          .toRotationMatrix();  // rtc: (x, y, z, w) but eigen:
                                                // (w, x, y, z)
    ...
    if (spss.length() != fs_vec_list.size()) {
      std::cerr << "[" << m_profile.instance_name << "]   StepParam length "
                << spss.length() << " != Footstep length " << fs_vec_list.size()
                << std::endl;
      return false;
    }
    std::cerr << "[" << m_profile.instance_name << "] print footsteps "
              << std::endl;
    std::vector<std::vector<step_node> > fnsl;
    for (size_t i = 0; i < fs_vec_list.size(); i++) {
      if (!(gg_is_walking && i == 0)) {  // If initial footstep, e.g., not
                                         // walking, pass user-defined footstep
                                         // list. If walking, pass cdr footsteps
                                         // in order to neglect initial double
                                         // support leg.
        std::vector<step_node> tmp_fns;
        for (size_t j = 0; j < fs_vec_list.at(i).size(); j++) {
          tmp_fns.push_back(
              step_node(leg_name_vec_list[i][j], fs_vec_list[i][j],
                        spss[i].sps[j].step_height, spss[i].sps[j].step_time,
                        spss[i].sps[j].toe_angle, spss[i].sps[j].heel_angle));
        }
    ...
    if (gg_is_walking) {
      ...
      std::cerr << "[" << m_profile.instance_name << "]  Set normal footsteps"
                << std::endl;
...
bool AutoBalancer::setGaitGeneratorParam(
    const OpenHRP::AutoBalancerService::GaitGeneratorParam& i_param) {
  std::cerr << "[" << m_profile.instance_name << "] setGaitGeneratorParam"
            << std::endl;
  gg->set_stride_parameters(
      i_param.stride_parameter[0], i_param.stride_parameter[1],
      i_param.stride_parameter[2], i_param.stride_parameter[3]);
  std::vector<hrp::Vector3> off;
  for (size_t i = 0; i < i_param.leg_default_translate_pos.length(); i++) {
    off.push_back(hrp::Vector3(i_param.leg_default_translate_pos[i][0],
                               i_param.leg_default_translate_pos[i][1],
                               i_param.leg_default_translate_pos[i][2]));
  }
  ...
  gg->set_default_double_support_ratio_before(
      i_param.default_double_support_ratio / 2.0);
  gg->set_default_double_support_ratio_after(
      i_param.default_double_support_ratio / 2.0);
  gg->set_default_double_support_static_ratio_before(
      i_param.default_double_support_static_ratio / 2.0);
  gg->set_default_double_support_static_ratio_after(
      i_param.default_double_support_static_ratio / 2.0);
  gg->set_default_double_support_ratio_swing_before(
      i_param.default_double_support_ratio / 2.0);
  gg->set_default_double_support_ratio_swing_after(
      i_param.default_double_support_ratio / 2.0);
  ...
  } else if (i_param.default_orbit_type ==
             OpenHRP::AutoBalancerService::CYCLOID) {
    gg->set_default_orbit_type(CYCLOID);
  } else if (i_param.default_orbit_type ==
             OpenHRP::AutoBalancerService::RECTANGLE) {
    gg->set_default_orbit_type(RECTANGLE);
  } else if (i_param.default_orbit_type ==
             OpenHRP::AutoBalancerService::STAIR) {
    gg->set_default_orbit_type(STAIR);
  } else if (i_param.default_orbit_type ==
             OpenHRP::AutoBalancerService::CYCLOIDDELAY) {
    gg->set_default_orbit_type(CYCLOIDDELAY);
  } else if (i_param.default_orbit_type ==
             OpenHRP::AutoBalancerService::CYCLOIDDELAYKICK) {
    gg->set_default_orbit_type(CYCLOIDDELAYKICK);
  } else if (i_param.default_orbit_type ==
             OpenHRP::AutoBalancerService::CROSS) {
    gg->set_default_orbit_type(CROSS);
  }
  gg->set_swing_trajectory_delay_time_offset(
      i_param.swing_trajectory_delay_time_offset);
  gg->set_swing_trajectory_final_distance_weight(
      i_param.swing_trajectory_final_distance_weight);
  gg->set_swing_trajectory_time_offset_xy2z(
      i_param.swing_trajectory_time_offset_xy2z);
  gg->set_stair_trajectory_way_point_offset(
      hrp::Vector3(i_param.stair_trajectory_way_point_offset[0],
                   i_param.stair_trajectory_way_point_offset[1],
                   i_param.stair_trajectory_way_point_offset[2]));
  gg->set_cycloid_delay_kick_point_offset(
      hrp::Vector3(i_param.cycloid_delay_kick_point_offset[0],
                   i_param.cycloid_delay_kick_point_offset[1],
                   i_param.cycloid_delay_kick_point_offset[2]));
  ...
  std::vector<double> tmp_ratio(i_param.toe_heel_phase_ratio.get_buffer(),
                                i_param.toe_heel_phase_ratio.get_buffer() +
                                    i_param.toe_heel_phase_ratio.length());
  std::cerr << "[" << m_profile.instance_name
            << "]   ";  // for set_toe_heel_phase_ratio
  ...
  gg->set_zmp_weight_map(boost::assign::map_list_of<leg_type, double>(
      RLEG, i_param.zmp_weight_map[0])(LLEG, i_param.zmp_weight_map[1])(
      RARM, i_param.zmp_weight_map[2])(LARM, i_param.zmp_weight_map[3]));
  gg->set_optional_go_pos_finalize_footstep_num(
      i_param.optional_go_pos_finalize_footstep_num);
  gg->set_overwritable_footstep_index_offset(
      i_param.overwritable_footstep_index_offset);
  gg->set_leg_margin(i_param.leg_margin);
  gg->set_stride_limitation_for_circle_type(
      i_param.stride_limitation_for_circle_type);
  gg->set_overwritable_stride_limitation(
      i_param.overwritable_stride_limitation);
  ...
  } else if (i_param.stride_limitation_type ==
...
bool AutoBalancer::getGaitGeneratorParam(
    OpenHRP::AutoBalancerService::GaitGeneratorParam& i_param) {
  gg->get_stride_parameters(
      i_param.stride_parameter[0], i_param.stride_parameter[1],
      i_param.stride_parameter[2], i_param.stride_parameter[3]);
  std::vector<hrp::Vector3> off;
  ...
  i_param.default_double_support_ratio_before =
      gg->get_default_double_support_ratio_before();
  i_param.default_double_support_ratio_after =
      gg->get_default_double_support_ratio_after();
  i_param.default_double_support_static_ratio_before =
      gg->get_default_double_support_static_ratio_before();
  i_param.default_double_support_static_ratio_after =
      gg->get_default_double_support_static_ratio_after();
  i_param.default_double_support_ratio_swing_before =
      gg->get_default_double_support_ratio_swing_before();
  i_param.default_double_support_ratio_swing_after =
      gg->get_default_double_support_ratio_swing_after();
  i_param.default_double_support_ratio =
      i_param.default_double_support_ratio_before +
      i_param.default_double_support_ratio_after;
  i_param.default_double_support_static_ratio =
      i_param.default_double_support_static_ratio_before +
      i_param.default_double_support_static_ratio_after;
  ...
  for (size_t i = 0; i < 3; i++)
    i_param.stair_trajectory_way_point_offset[i] = tmpv(i);
  tmpv = gg->get_cycloid_delay_kick_point_offset();
  for (size_t i = 0; i < 3; i++)
    i_param.cycloid_delay_kick_point_offset[i] = tmpv(i);
  i_param.swing_trajectory_delay_time_offset =
  ...
  for (int i = 0; i < gg->get_NUM_TH_PHASES(); i++)
    i_param.toe_heel_phase_ratio[i] = ratio[i];
  ...
  for (size_t i = 0; i < 5; i++) {
    i_param.stride_limitation_for_circle_type[i] =
        gg->get_stride_limitation_for_circle_type(i);
  }
  for (size_t i = 0; i < 5; i++) {
    i_param.overwritable_stride_limitation[i] =
        gg->get_overwritable_stride_limitation(i);
  }
...
bool AutoBalancer::setAutoBalancerParam(
    const OpenHRP::AutoBalancerService::AutoBalancerParam& i_param) {
  Guard guard(m_mutex);
  std::cerr << "[" << m_profile.instance_name << "] setAutoBalancerParam"
            << std::endl;
  ...
  if (zmp_offset_interpolator->isEmpty()) {
    ...
    zmp_offset_interpolator->setGoal(default_zmp_offsets_array,
                                     zmp_transition_time, true);
  } else {
    std::cerr << "[" << m_profile.instance_name
              << "]   default_zmp_offsets cannot be set because interpolating."
              << std::endl;
  }
  if (control_mode == MODE_IDLE) {
    ...
    std::cerr << "[" << m_profile.instance_name
              << "]   use_force_mode cannot be changed to ["
              << i_param.use_force_mode
              << "] during MODE_ABC, MODE_SYNC_TO_IDLE or MODE_SYNC_TO_ABC."
              << std::endl;
  }
  ...
  for (size_t j = 0; j < 3; j++)
    graspless_manip_reference_trans_coords.pos[j] =
        i_param.graspless_manip_reference_trans_pos[j];
  graspless_manip_reference_trans_coords.rot =
      (Eigen::Quaternion<double>(i_param.graspless_manip_reference_trans_rot[0],
                                 i_param.graspless_manip_reference_trans_rot[1],
                                 i_param.graspless_manip_reference_trans_rot[2],
                                 i_param.graspless_manip_reference_trans_rot[3])
           .normalized()
           .toRotationMatrix());  // rtc: (x, y, z, w) but eigen: (w, x, y, z)
  ...
  if (cur_leg_names != dst_leg_names) {
    ...
    std::cerr << "[" << m_profile.instance_name
              << "]   leg_names cannot be set because interpolating."
              << std::endl;
  }
  if (!gg_is_walking) {
    is_hand_fix_mode = i_param.is_hand_fix_mode;
    std::cerr << "[" << m_profile.instance_name
              << "]   is_hand_fix_mode = " << is_hand_fix_mode << std::endl;
  } else {
    std::cerr << "[" << m_profile.instance_name
              << "]   is_hand_fix_mode cannot be set in (gg_is_walking = "
                 "true). Current is_hand_fix_mode is "
              << (is_hand_fix_mode ? "true" : "false") << std::endl;
  }
  if (control_mode == MODE_IDLE) {
    for (size_t i = 0; i < i_param.end_effector_list.length(); i++) {
      std::map<std::string, ABCIKparam>::iterator it =
          ikp.find(std::string(i_param.end_effector_list[i].leg));
      memcpy(it->second.localPos.data(), i_param.end_effector_list[i].pos,
             sizeof(double) * 3);
      it->second.localR =
          (Eigen::Quaternion<double>(i_param.end_effector_list[i].rot[0],
                                     i_param.end_effector_list[i].rot[1],
                                     i_param.end_effector_list[i].rot[2],
                                     i_param.end_effector_list[i].rot[3]))
              .normalized()
              .toRotationMatrix();
    }
  } else {
  ...
  if (i_param.default_gait_type == OpenHRP::AutoBalancerService::BIPED) {
    ...
    gait_type = CRAWL;
  } else if (i_param.default_gait_type ==
             OpenHRP::AutoBalancerService::GALLOP) {
    gait_type = GALLOP;
  }
  for (std::map<std::string, ABCIKparam>::iterator it = ikp.begin();
       it != ikp.end(); it++) {
    ...
    std::cerr << "[" << m_profile.instance_name << "]   localpos = "
              << it->second.localPos.format(
                     Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "",
                                     "", "    [", "]"))
              << "[m]" << std::endl;
    std::cerr << "[" << m_profile.instance_name << "]   localR = "
              << it->second.localR.format(Eigen::IOFormat(
  ...
  std::cerr << "[" << m_profile.instance_name
            << "]   use_force_mode = " << getUseForceModeString() << std::endl;
  std::cerr << "[" << m_profile.instance_name
            << "]   graspless_manip_mode = " << graspless_manip_mode
            << std::endl;
  std::cerr << "[" << m_profile.instance_name
            << "]   graspless_manip_arm = " << graspless_manip_arm << std::endl;
  std::cerr << "[" << m_profile.instance_name << "]   graspless_manip_p_gain = "
            << graspless_manip_p_gain.format(Eigen::IOFormat(
                   Eigen::StreamPrecision, 0, ", ", ", ", "", "", "    [", "]"))
            << std::endl;
  std::cerr << "[" << m_profile.instance_name
            << "]   graspless_manip_reference_trans_pos = "
            << graspless_manip_reference_trans_coords.pos.format(
                   Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "",
                                   "", "    [", "]"))
            << std::endl;
  std::cerr << "[" << m_profile.instance_name
            << "]   graspless_manip_reference_trans_rot = "
            << graspless_manip_reference_trans_coords.rot.format(
                   Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", "\n",
                                   "    [", "]"))
            << std::endl;
  std::cerr << "[" << m_profile.instance_name
            << "]   transition_time = " << transition_time
            << "[s], zmp_transition_time = " << zmp_transition_time
            << "[s], adjust_footstep_transition_time = "
            << adjust_footstep_transition_time << "[s]" << std::endl;
  for (std::vector<std::string>::iterator it = leg_names.begin();
       it != leg_names.end(); it++)
    std::cerr << "[" << m_profile.instance_name << "]   leg_names [" << *it
              << "]" << std::endl;
  std::cerr << "[" << m_profile.instance_name
            << "]   default_gait_type = " << gait_type << std::endl;
  // FIK
  ...
  fik->limb_stretch_avoidance_time_const =
      i_param.limb_stretch_avoidance_time_const;
  for (size_t i = 0; i < 2; i++) {
    fik->limb_stretch_avoidance_vlimit[i] =
        i_param.limb_stretch_avoidance_vlimit[i];
  }
...
bool AutoBalancer::getAutoBalancerParam(
  ...
  switch (control_mode) {
    case MODE_IDLE:
      i_param.controller_mode = OpenHRP::AutoBalancerService::MODE_IDLE;
      break;
    case MODE_ABC:
      i_param.controller_mode = OpenHRP::AutoBalancerService::MODE_ABC;
      break;
    case MODE_SYNC_TO_IDLE:
      i_param.controller_mode = OpenHRP::AutoBalancerService::MODE_SYNC_TO_IDLE;
      break;
    case MODE_SYNC_TO_ABC:
      i_param.controller_mode = OpenHRP::AutoBalancerService::MODE_SYNC_TO_ABC;
      break;
    default:
      break;
  }
  switch (use_force) {
    case MODE_NO_FORCE:
      i_param.use_force_mode = OpenHRP::AutoBalancerService::MODE_NO_FORCE;
      break;
    case MODE_REF_FORCE:
      i_param.use_force_mode = OpenHRP::AutoBalancerService::MODE_REF_FORCE;
      break;
    case MODE_REF_FORCE_WITH_FOOT:
      i_param.use_force_mode =
          OpenHRP::AutoBalancerService::MODE_REF_FORCE_WITH_FOOT;
      break;
    case MODE_REF_FORCE_RFU_EXT_MOMENT:
      i_param.use_force_mode =
  ...
  for (size_t i = 0; i < leg_names.size(); i++)
    i_param.leg_names[i] = leg_names.at(i).c_str();
  i_param.is_hand_fix_mode = is_hand_fix_mode;
  i_param.end_effector_list.length(ikp.size());
  {
    size_t i = 0;
    for (std::map<std::string, ABCIKparam>::const_iterator it = ikp.begin();
         it != ikp.end(); it++) {
      copyRatscoords2Footstep(
          i_param.end_effector_list[i],
  ...
  switch (gait_type) {
    case BIPED:
      i_param.default_gait_type = OpenHRP::AutoBalancerService::BIPED;
      break;
    case TROT:
      i_param.default_gait_type = OpenHRP::AutoBalancerService::TROT;
      break;
    case PACE:
      i_param.default_gait_type = OpenHRP::AutoBalancerService::PACE;
      break;
    case CRAWL:
      i_param.default_gait_type = OpenHRP::AutoBalancerService::CRAWL;
      break;
    case GALLOP:
      i_param.default_gait_type = OpenHRP::AutoBalancerService::GALLOP;
      break;
    default:
      break;
  }
...
void AutoBalancer::copyRatscoords2Footstep(
    OpenHRP::AutoBalancerService::Footstep& out_fs,
...
bool AutoBalancer::getFootstepParam(
    OpenHRP::AutoBalancerService::FootstepParam& i_param) {
  copyRatscoords2Footstep(i_param.support_leg_coords,
                          gg->get_support_leg_steps().front().worldcoords);
  copyRatscoords2Footstep(i_param.swing_leg_coords,
                          gg->get_swing_leg_steps().front().worldcoords);
  copyRatscoords2Footstep(i_param.swing_leg_src_coords,
                          gg->get_swing_leg_src_steps().front().worldcoords);
  copyRatscoords2Footstep(i_param.swing_leg_dst_coords,
                          gg->get_swing_leg_dst_steps().front().worldcoords);
  copyRatscoords2Footstep(i_param.dst_foot_midcoords,
                          gg->get_dst_foot_midcoords());
  ...
  switch (gg->get_current_support_states().front()) {
    case BOTH:
      i_param.support_leg_with_both = OpenHRP::AutoBalancerService::BOTH;
      break;
    case RLEG:
      i_param.support_leg_with_both = OpenHRP::AutoBalancerService::RLEG;
      break;
    case LLEG:
      i_param.support_leg_with_both = OpenHRP::AutoBalancerService::LLEG;
      break;
    default:
      break;
  }
...
bool AutoBalancer::adjustFootSteps(
  ...
  if (control_mode == MODE_ABC && !gg_is_walking &&
      adjust_footstep_interpolator->isEmpty()) {
    Guard guard(m_mutex);
    //
    ...
    mid_coords(org_mid_coords, 0.5,
               coordinates(ikp["rleg"].adjust_interpolation_org_p0,
                           ikp["rleg"].adjust_interpolation_org_r0),
               coordinates(ikp["lleg"].adjust_interpolation_org_p0,
                           ikp["lleg"].adjust_interpolation_org_r0));
    ...
    eerot = (Eigen::Quaternion<double>(rfootstep.rot[0], rfootstep.rot[1],
                                       rfootstep.rot[2], rfootstep.rot[3]))
                .normalized()
                .toRotationMatrix();  // rtc:
    ...
    eerot = (Eigen::Quaternion<double>(lfootstep.rot[0], lfootstep.rot[1],
                                       lfootstep.rot[2], lfootstep.rot[3]))
                .normalized()
                .toRotationMatrix();  // rtc:
    ...
    mid_coords(target_mid_coords, 0.5,
               coordinates(ikp["rleg"].adjust_interpolation_target_p0,
                           ikp["rleg"].adjust_interpolation_target_r0),
               coordinates(ikp["lleg"].adjust_interpolation_target_p0,
                           ikp["lleg"].adjust_interpolation_target_r0));
    coordinates rtrans, ltrans;
    target_mid_coords.transformation(
        rtrans, coordinates(ikp["rleg"].adjust_interpolation_target_p0,
                            ikp["rleg"].adjust_interpolation_target_r0));
    target_mid_coords.transformation(
        ltrans, coordinates(ikp["lleg"].adjust_interpolation_target_p0,
                            ikp["lleg"].adjust_interpolation_target_r0));
    ...
    tmp = 1.0;
    adjust_footstep_interpolator->setGoal(&tmp, adjust_footstep_transition_time,
                                          true);
  }
  while (!adjust_footstep_interpolator->isEmpty()) usleep(1000);
  usleep(1000);
  return true;
};

bool AutoBalancer::getRemainingFootstepSequence(
...
bool AutoBalancer::getGoPosFootstepsSequence(
  ...
  if (gg_is_walking) {
    ...
    gg->go_pos_param_2_footstep_nodes_list_core(
        x, y, th, initial_support_legs_coords, start_ref_coords,
        initial_support_legs, new_footstep_nodes_list, true, 0);
    ...
    for (size_t i = 0; i < new_footstep_nodes_list.size(); i++) {
      ...
      for (size_t j = 0; j < new_footstep_nodes_list.at(i).size(); j++) {
        ...
        o_footstep[i].fs[j].leg =
            ((tmp_leg_type == RLEG) ? "rleg" : (tmp_leg_type == LLEG)
                                                   ? "lleg"
                                                   : (tmp_leg_type == RARM)
                                                         ? "rarm"
                                                         : "larm");
        copyRatscoords2Footstep(
            o_footstep[i].fs[j],
            new_footstep_nodes_list.at(i).at(j).worldcoords);
      }
    }
    return true;
  }
...
void AutoBalancer::calc_static_balance_point_from_forces(
  ...
  hrp::Vector3 total_nosensor_ref_force =
      mg * hrp::Vector3::UnitZ() - total_sensor_ref_force;  // total ref force
                                                            // at the point
                                                            // without sensors,
                                                            // such as torso
  hrp::Vector3 tmp_ext_moment =
      fix_leg_coords2.pos.cross(total_nosensor_ref_force) +
      fix_leg_coords2.rot * hrp::Vector3(m_refFootOriginExtMoment.data.x,
                                         m_refFootOriginExtMoment.data.y,
                                         m_refFootOriginExtMoment.data.z);
  for (size_t j = 0; j < 2; j++) {
    nume(j) = mg * tmpcog(j);
    denom(j) = mg;
    for (size_t i = 0; i < sensor_names.size(); i++) {
      if (sensor_names[i].find("hsensor") != std::string::npos ||
          sensor_names[i].find("asensor") !=
              std::string::npos) {  // tempolary to get arm force coords
        hrp::Link* parentlink;
        hrp::ForceSensor* sensor =
            m_robot->sensor<hrp::ForceSensor>(sensor_names[i]);
        if (sensor)
          ...
          parentlink = m_vfs[sensor_names[i]].link;
        for (std::map<std::string, ABCIKparam>::iterator it = ikp.begin();
             it != ikp.end(); it++) {
          if (it->second.target_link->name == parentlink->name) {
            hrp::Vector3 fpos =
                parentlink->p + parentlink->R * it->second.localPos;
            nume(j) += ((fpos(2) - ref_com_height) * tmp_forces[i](j)-fpos(j) *
                        tmp_forces[i](2));
    ...
    if (use_force == MODE_REF_FORCE_WITH_FOOT) {
      ...
      nume(j) += ((fpos(2) - ref_com_height) * total_nosensor_ref_force(j) -
                  fpos(j) * total_nosensor_ref_force(2));
...
hrp::Vector3 AutoBalancer::calc_vel_from_hand_error(
    const coordinates& tmp_fix_coords) {
  if (graspless_manip_mode) {
    hrp::Vector3 dp, dr;
    coordinates ref_hand_coords(gg->get_dst_foot_midcoords()), act_hand_coords;
    ref_hand_coords.transform(
        graspless_manip_reference_trans_coords);  // desired arm coords
    ...
    if (graspless_manip_arm == "arms") {
      ...
      ref_y.normalize();
      dr = hrp::Vector3(0, 0,
                        (hrp::Vector3(ref_y.cross(act_y))(2) > 0 ? 1.0 : -1.0) *
                            std::acos(ref_y.dot(act_y)));  // fix for rotation
    } else {
      ABCIKparam& tmpikp = ikp[graspless_manip_arm];
      act_hand_coords = rats::coordinates(tmpikp.target_p0, tmpikp.target_r0);
    ...
    // alias(dr) = foot_mt * dr;
    return hrp::Vector3(
        graspless_manip_p_gain[0] * dp2(0) / gg->get_default_step_time(),
        graspless_manip_p_gain[1] * dp2(1) / gg->get_default_step_time(),
        graspless_manip_p_gain[2] * rad2deg(dr(2)) /
...
bool AutoBalancer::calc_inital_support_legs(
    const double& y, std::vector<coordinates>& initial_support_legs_coords,
    std::vector<leg_type>& initial_support_legs,
    coordinates& start_ref_coords) {
  switch (gait_type) {
    case BIPED:
      initial_support_legs_coords =
          (y > 0 ? boost::assign::list_of(coordinates(ikp["rleg"].target_p0,
                                                      ikp["rleg"].target_r0))
                 : boost::assign::list_of(coordinates(ikp["lleg"].target_p0,
                                                      ikp["lleg"].target_r0)));
      ...
      initial_support_legs_coords =
          (y > 0
               ? boost::assign::list_of(
                     ...
                     coordinates(ikp["larm"].target_p0, ikp["larm"].target_r0))
               : boost::assign::list_of(
                     coordinates(ikp["lleg"].target_p0, ikp["lleg"].target_r0))(
                     coordinates(ikp["rarm"].target_p0,
                                 ikp["rarm"].target_r0)));
      initial_support_legs = (y > 0 ? boost::assign::list_of(RLEG)(LARM)
                                    : boost::assign::list_of(LLEG)(RARM));
      break;
    case PACE:
      initial_support_legs_coords =
          (y > 0
               ? boost::assign::list_of(
                     coordinates(ikp["rleg"].target_p0, ikp["rleg"].target_r0))(
                     coordinates(ikp["rarm"].target_p0, ikp["rarm"].target_r0))
               : boost::assign::list_of(
                     coordinates(ikp["lleg"].target_p0, ikp["lleg"].target_r0))(
                     coordinates(ikp["larm"].target_p0,
                                 ikp["larm"].target_r0)));
      initial_support_legs = (y > 0 ? boost::assign::list_of(RLEG)(RARM)
                                    : boost::assign::list_of(LLEG)(LARM));
      break;
    case CRAWL:
      std::cerr << "[" << m_profile.instance_name << "] crawl walk["
  ...
  mid_rot(start_ref_coords.rot, 0.5, ikp["rleg"].target_r0,
          ikp["lleg"].target_r0);
  return true;
};
...
void calcWeightedLinearEquation(hrp::dvector& ret, const hrp::dmatrix& A,
                                const hrp::dmatrix& W, const hrp::dvector& b) {
...
void AutoBalancer::distributeReferenceZMPToWrenches(
    const hrp::Vector3& _ref_zmp) {
  // apply inverse system
  // TODO : fix 0.055 (zmp delay)
  hrp::Vector3 tmp_ref_zmp =
      _ref_zmp + 0.055 * (_ref_zmp - prev_ref_zmp) / m_dt;

  std::vector<hrp::Vector3> cop_pos;
  std::vector<double> limb_gains;
  for (size_t i = 0; i < leg_names.size(); i++) {
    ABCIKparam& tmpikp = ikp[leg_names[i]];
    cop_pos.push_back(tmpikp.target_p0 +
                      tmpikp.target_r0 * tmpikp.localR *
                          default_zmp_offsets[i]);
    limb_gains.push_back(
        m_contactStates.data[contact_states_index_map[leg_names[i]]] ? 1.0
                                                                     : 0.0);
  }
  size_t ee_num = leg_names.size();
  size_t state_dim = 6 * ee_num;
  size_t total_wrench_dim = 5;
  // size_t total_fz = m_robot->totalMass() *
  ...
  for (size_t j = 0; j < ee_num; j++) {
    for (size_t i = 0; i < 3; i++) {
      if (ee_num == 2)
        Wmat(i + j * 3, i + j * 3) =
            Wmat(i + j * 3, i + j * 3) * limb_gains[j] * (i == 2 ? 1.0 : 0.01);
      else
  ...
  total_wrench(total_wrench_dim - 5) =
      m_ref_force[0].data[0] + m_ref_force[1].data[0];
  total_wrench(total_wrench_dim - 4) =
      m_ref_force[0].data[1] + m_ref_force[1].data[1];
  ...
  for (size_t i = 0; i < leg_names.size(); i++) {
    ...
    // hrp::Vector3 tmp_ee_pos = tmpikp.target_p0 + tmpikp.target_r0 *
    // tmpikp.localPos;
  ...
  if (DEBUGP) {
    std::cerr << "[" << m_profile.instance_name << "]   Gmat = "
              << Gmat.format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ",
                                             ", ", "", "", "[", "]"))
              << "[N,Nm]" << std::endl;
    std::cerr << "[" << m_profile.instance_name << "]   total_wrench = "
              << total_wrench.format(Eigen::IOFormat(
                     Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]"))
              << "[N,Nm]" << std::endl;
    hrp::dvector tmp(total_wrench.size());
    tmp = Gmat * ret;
    std::cerr << "[" << m_profile.instance_name << "]   Gmat*ret = "
              << tmp.format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ",
                                            ", ", "", "", "[", "]"))
              << "[N,Nm]" << std::endl;
    std::cerr << "[" << m_profile.instance_name
              << "]   (Gmat*ret-total_wrench) = "
              << (tmp - total_wrench)
                     .format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ",
                                             ", ", "", "", "[", "]"))
              << "[N,Nm]" << std::endl;
    std::cerr << "[" << m_profile.instance_name << "]   ret = "
              << ret.format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ",
                                            ", ", "", "", "[", "]"))
              << "[N,Nm]" << std::endl;
...
void AutoBalancerInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<AutoBalancer>,
