...
//   Input : target root pos and rot, target COG, target joint angles, target EE
//   coords
//   Output : joint angles
 ...
 public:
  ...
  struct IKparam {
    ...
    IKparam()
        : avoid_gain(0.001),
          reference_gain(0.01),
          pos_ik_error_count(0),
          rot_ik_error_count(0),
  ...
  double d_root_height, limb_stretch_avoidance_time_const,
      limb_stretch_avoidance_vlimit[2], m_dt;
  bool use_limb_stretch_avoidance;

  SimpleFullbodyInverseKinematicsSolver(hrp::BodyPtr& _robot,
                                        const std::string& _print_str,
                                        const double _dt)
      : m_robot(_robot),
        ik_error_debug_print_freq(
            static_cast<int>(0.2 / _dt)),  // once per 0.2 [s]
        has_ik_failed(false),
  ...
  void initializeInterlockingJoints(
      std::vector<std::pair<hrp::Link*, hrp::Link*> >& interlocking_joints) {
    for (std::map<std::string, IKparam>::iterator it = ikp.begin();
         it != ikp.end(); it++) {
      std::cerr << "[" << print_str << "] Interlocking Joints for ["
                << it->first << "]" << std::endl;
      it->second.manip->setInterlockingJointPairIndices(interlocking_joints,
                                                        print_str);
    }
  };
  ...
  void solveFullbodyIK(const hrp::Vector3& _dif_cog, const bool is_transition) {
    ...
    m_robot->rootLink()->p =
        m_robot->rootLink()->p + -1 * move_base_gain * dif_cog;
    ...
    for (size_t i = 0; i < overwrite_ref_ja_index_vec.size(); i++) {
      m_robot->joint(overwrite_ref_ja_index_vec[i])->q =
          qrefv[overwrite_ref_ja_index_vec[i]];
    }
    m_robot->calcForwardKinematics();
    for (std::map<std::string, IKparam>::iterator it = ikp.begin();
         it != ikp.end(); it++) {
  ...
  // Solve limb IK
  bool solveLimbIK(IKparam& param, const std::string& limb_name,
                   const double ratio_for_vel, const bool is_transition) {
    param.manip->calcInverseKinematics2Loop(
        param.target_p0, param.target_r0, 1.0, param.avoid_gain,
        param.reference_gain, &qrefv, ratio_for_vel, param.localPos,
        param.localR);
    checkIKTracking(param, limb_name, is_transition);
    return true;
  }
  // IK fail check
  void checkIKTracking(IKparam& param, const std::string& limb_name,
                       const bool is_transition) {
    hrp::Vector3 vel_p, vel_r;
    vel_p = param.target_p0 -
            (param.target_link->p + param.target_link->R * param.localPos);
    rats::difference_rotation(vel_r, (param.target_link->R * param.localR),
                              param.target_r0);
    if (vel_p.norm() > pos_ik_thre && is_transition) {
      if (param.pos_ik_error_count % ik_error_debug_print_freq == 0) {
        std::cerr << "[" << print_str << "] Too large IK error in " << limb_name
                  << " (vel_p) = [" << vel_p(0) << " " << vel_p(1) << " "
                  << vel_p(2) << "][m], count = " << param.pos_ik_error_count
                  << std::endl;
      }
    ...
    if (vel_r.norm() > rot_ik_thre && is_transition) {
      if (param.rot_ik_error_count % ik_error_debug_print_freq == 0) {
        std::cerr << "[" << print_str << "] Too large IK error in " << limb_name
                  << " (vel_r) = [" << vel_r(0) << " " << vel_r(1) << " "
                  << vel_r(2) << "][rad], count = " << param.rot_ik_error_count
                  << std::endl;
      }
  ...
  void getIKParam(
    ...
    for (size_t i = 0; i < ee_vec.size(); i++) {
      ...
      OpenHRP::AutoBalancerService::IKLimbParameters& ilp =
          ik_limb_parameters[i];
  ...
  void setIKParam(
      std::vector<std::string>& ee_vec,
      const _CORBA_Unbounded_Sequence<
          OpenHRP::AutoBalancerService::IKLimbParameters>& ik_limb_parameters) {
    ...
    if (ik_limb_parameters.length() != ee_vec.size()) {
      ...
      std::cerr
          ...
          << ik_limb_parameters.length() << ", desired = " << ee_vec.size()
          << ")" << std::endl;
    } else {
      for (size_t i = 0; i < ee_vec.size(); i++) {
        if (ikp[ee_vec[i]].manip->numJoints() !=
            ik_limb_parameters[i].ik_optional_weight_vector.length())
          is_ik_limb_parameter_valid_length = false;
      }
      if (is_ik_limb_parameter_valid_length) {
        for (size_t i = 0; i < ee_vec.size(); i++) {
          IKparam& param = ikp[ee_vec[i]];
          const OpenHRP::AutoBalancerService::IKLimbParameters& ilp =
              ik_limb_parameters[i];
          std::vector<double> ov;
          ov.resize(param.manip->numJoints());
          for (size_t j = 0; j < param.manip->numJoints(); j++) {
        ...
        std::cerr << "[" << print_str << "]   ik_optional_weight_vector "
                                         "invalid length! Cannot be set. "
                                         "(input = [";
        for (size_t i = 0; i < ee_vec.size(); i++) {
  ...
  void limbStretchAvoidanceControl(
      const std::vector<hrp::Vector3>& target_p,
    ...
    if (use_limb_stretch_avoidance) {
      for (size_t i = 0; i < target_p.size(); i++) {
        ...
        hrp::Vector3 rel_target_p =
            target_p[i] -
            parent_link
                ->p;  // position from parent to target link (world frame)
        double limb_length_limitation = ikp[target_name[i]].max_limb_length -
                                        ikp[target_name[i]].limb_length_margin;
        double tmp = limb_length_limitation * limb_length_limitation -
                     rel_target_p(0) * rel_target_p(0) -
                     rel_target_p(1) * rel_target_p(1);
      ...
      d_root_height =
          tmp_d_root_height == 0.0
              ? (-1 / limb_stretch_avoidance_time_const * d_root_height * m_dt +
                 d_root_height)
              : tmp_d_root_height;
    } else {
      d_root_height =
    ...
    d_root_height = vlimit(
        d_root_height, prev_d_root_height + limb_stretch_avoidance_vlimit[0],
        prev_d_root_height + limb_stretch_avoidance_vlimit[1]);
    m_robot->rootLink()->p(2) += d_root_height;
  };
  double vlimit(const double value, const double llimit_value,
  ...
  void printParam() const {
    std::cerr << "[" << print_str << "]   move_base_gain = " << move_base_gain
              << std::endl;
    std::cerr << "[" << print_str << "]   pos_ik_thre = " << pos_ik_thre
              << "[m], rot_ik_thre = " << rot_ik_thre << "[rad]" << std::endl;
  };
  // Set IKparam
  ...
  hrp::Vector3 getEndEffectorPos(const std::string& limb_name) {
    return ikp[limb_name].target_link->p +
           ikp[limb_name].target_link->R * ikp[limb_name].localPos;
  }
