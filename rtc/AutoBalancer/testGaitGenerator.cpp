...
template <>
double ValueDifferenceChecker<hrp::Vector3>::calcDiff(
    hrp::Vector3& value) const {
  return (value - prev_value).norm();
};

template <>
double ValueDifferenceChecker<std::vector<hrp::Vector3> >::calcDiff(
...
class ValueErrorChecker {
 ...
 public:
  ValueErrorChecker(double _thre)
      : error_thre(_thre), max_value_error(0), is_small_error(true){};
  ~ValueErrorChecker(){};
  void checkValueError(
      const hrp::Vector3& p0, const hrp::Vector3& p1,
      std::vector<size_t> neglect_index = std::vector<size_t>()) {
    hrp::Vector3 errorv(p0 - p1);
    for (size_t i = 0; i < neglect_index.size(); i++)
      errorv(neglect_index[i]) = 0.0;
...
class testGaitGenerator {
 protected:
  ...
  std::vector<hrp::Vector3>
      leg_pos; /* default footstep transformations are necessary */
  ...
  double min_toe_heel_dif_angle, max_toe_heel_dif_angle, min_zmp_offset_x,
      max_zmp_offset_x;
  // Value checker
  bool is_contact_states_swing_support_time_validity;
  //   Check difference of value
  ValueDifferenceChecker<hrp::Vector3> refzmp_diff_checker,
      cartzmp_diff_checker, cog_diff_checker, ssmcpos_diff_checker,
      ssmcrot_diff_checker, ssmcposvel_diff_checker, ssmcrotvel_diff_checker;
  ValueDifferenceChecker<std::vector<hrp::Vector3> > footpos_diff_checker,
      footrot_diff_checker, footposvel_diff_checker, footrotvel_diff_checker,
  ...
  std::vector<double> step_time_list, min_toe_heel_dif_angle_list,
      max_toe_heel_dif_angle_list, min_zmp_offset_x_list, max_zmp_offset_x_list;
  bool is_step_time_valid, is_toe_heel_dif_angle_valid,
 ...
 private:
  ...
  void plot_and_save(FILE* gp, const std::string graph_fname,
  ...
  void proc_one_walking_motion(size_t i) {
    ...
    //   gg->update_refzmp_queue(coordinates(hrp::Vector3(150, 105, 0)),
    //   coordinates(hrp::Vector3(150, -105, 0)));
    // }
    ...
    hrp::Vector3 rfoot_pos =
        (gg->get_support_leg_names() == tmp_string_vector)
            ? gg->get_support_leg_steps().front().worldcoords.pos
            : gg->get_swing_leg_steps().front().worldcoords.pos;
    for (size_t ii = 0; ii < 3; ii++) {
    ...
    hrp::Matrix33 rfoot_rot =
        (gg->get_support_leg_names() == tmp_string_vector)
            ? gg->get_support_leg_steps().front().worldcoords.rot
            : gg->get_swing_leg_steps().front().worldcoords.rot;
    hrp::Vector3 rfoot_rpy = hrp::rpyFromRot(rfoot_rot);
    for (size_t ii = 0; ii < 3; ii++) {
      fprintf(fp_frot, "%f ", rad2deg(rfoot_rpy(ii)));
    }
    tmp_string_vector = boost::assign::list_of("lleg");
    hrp::Matrix33 lfoot_rot =
        (gg->get_support_leg_names() == tmp_string_vector)
            ? gg->get_support_leg_steps().front().worldcoords.rot
            : gg->get_swing_leg_steps().front().worldcoords.rot;
    ...
    tmp_string_vector = boost::assign::list_of("rleg");
    hrp::Vector3 rfoot_zmp_offset =
        (gg->get_support_leg_names() == tmp_string_vector)
            ? gg->get_support_foot_zmp_offsets().front()
            : gg->get_swing_foot_zmp_offsets().front();
    for (size_t ii = 0; ii < 3; ii++) {
      fprintf(fp_zoff, "%f ", rfoot_zmp_offset(ii));
    }
    tmp_string_vector = boost::assign::list_of("lleg");
    hrp::Vector3 lfoot_zmp_offset =
        (gg->get_support_leg_names() == tmp_string_vector)
            ? gg->get_support_foot_zmp_offsets().front()
            : gg->get_swing_foot_zmp_offsets().front();
    for (size_t ii = 0; ii < 3; ii++) {
    ...
    fflush(fp_zoff);
    double tmpzoff;
    if (gg->get_support_leg_names()[0] == "lleg")
      tmpzoff = rfoot_zmp_offset(0);
    else
      tmpzoff = lfoot_zmp_offset(0);
    ...
    hrp::Vector3 tmppos;
    tmppos =
        rfoot_pos + rfoot_rot * hrp::Vector3(gg->get_toe_pos_offset_x(), 0, 0);
    for (size_t ii = 0; ii < 3; ii++) {
    ...
    tmppos =
        lfoot_pos + lfoot_rot * hrp::Vector3(gg->get_toe_pos_offset_x(), 0, 0);
    for (size_t ii = 0; ii < 3; ii++) {
      fprintf(fp_thpos, "%f ", tmppos(ii));
    ...
    tmppos =
        rfoot_pos + rfoot_rot * hrp::Vector3(gg->get_heel_pos_offset_x(), 0, 0);
    for (size_t ii = 0; ii < 3; ii++) {
      fprintf(fp_thpos, "%f ", tmppos(ii));
      min_rfoot_pos(ii) = std::min(min_rfoot_pos(ii), tmppos(ii));
      max_rfoot_pos(ii) = std::max(max_rfoot_pos(ii), tmppos(ii));
    ...
    tmppos =
        lfoot_pos + lfoot_rot * hrp::Vector3(gg->get_heel_pos_offset_x(), 0, 0);
    for (size_t ii = 0; ii < 3; ii++) {
      fprintf(fp_thpos, "%f ", tmppos(ii));
      min_lfoot_pos(ii) = std::min(min_lfoot_pos(ii), tmppos(ii));
      max_lfoot_pos(ii) = std::max(max_lfoot_pos(ii), tmppos(ii));
    ...
    fprintf(fp_sstime, "%f %f ", gg->get_current_swing_time(RLEG),
            gg->get_current_swing_time(LLEG));
    // Contact States
    std::vector<leg_type> tmp_current_support_states =
        gg->get_current_support_states();
    bool rleg_contact_states = std::find_if(tmp_current_support_states.begin(),
                                            tmp_current_support_states.end(),
                                            boost::lambda::_1 == RLEG) !=
                               tmp_current_support_states.end();
    bool lleg_contact_states = std::find_if(tmp_current_support_states.begin(),
                                            tmp_current_support_states.end(),
                                            boost::lambda::_1 == LLEG) !=
                               tmp_current_support_states.end();
    fprintf(fp_sstime, "%d %d %f", (rleg_contact_states ? 1 : 0),
            (lleg_contact_states ? 1 : 0),
            0.8 * gg->get_current_toe_heel_ratio() +
                0.1);  // scale+translation just for visualization
    fprintf(fp_sstime, "\n");
    fflush(fp_sstime);
    ...
    if (gg->get_lcg_count() == 0) {
      ...
      if (!eps_eq(min_zmp_offset_x, gg->get_default_zmp_offsets()[0](0), 1e-5))
        min_zmp_offset_x_list.push_back(min_zmp_offset_x);
      else
        min_zmp_offset_x_list.push_back(0.0);
      if (!eps_eq(max_zmp_offset_x, gg->get_default_zmp_offsets()[0](0), 1e-5))
        max_zmp_offset_x_list.push_back(max_zmp_offset_x);
      else
        max_zmp_offset_x_list.push_back(0.0);
    ...
    // Error checking
    {
      // Check error between RefZMP and CartZMP. If too large error,
      // PreviewControl tracking is not enough.
      ...
      //   Foot pos and rot
      std::vector<hrp::Vector3> tmpvec =
          boost::assign::list_of(rfoot_pos)(lfoot_pos);
    ...
    //   If contact states are not change, prev_swing_support_time is not dt,
    //   otherwise prev_swing_support_time is dt.
  ...
  void plot_and_print_errorcheck() {
    ...
    if (use_gnuplot) {
      ...
      }
      {
        ...
        size_t tmp_start = 2;
        oss << "set multiplot layout 3, 1 title '" << gtitle << "'"
            << std::endl;
        ...
        for (size_t ii = 0; ii < 3; ii++) {
          ...
          oss << "plot "
              << "'" << fname_cogzmp << "' using 1:" << (tmp_start + ii)
              << " with lines title 'REFZMP',"
              << "'" << fname_cogzmp << "' using 1:" << (tmp_start + 3 + ii)
              << " with lines title 'CARTZMP',"
              << "'" << fname_cogzmp << "' using 1:" << (tmp_start + 6 + ii)
              << " with lines title 'COG'" << std::endl;
        }
      ...
      }
      {
        ...
        for (size_t ii = 0; ii < 3; ii++) {
          ...
          oss << "set ylabel '" << titles[ii] << "[m]'" << std::endl;
          oss << "plot "
              << "'" << fname_fpos << "' using 1:" << (tmp_start + ii)
              << " with lines title 'rleg',"
              << "'" << fname_fpos << "' using 1:" << (tmp_start + 3 + ii)
              << " with lines title 'lleg'" << std::endl;
        }
      ...
      }
      {
        ...
        for (size_t ii = 0; ii < 3; ii++) {
          ...
          oss << "plot "
              << "'" << fname_frot << "' using 1:" << (tmp_start + ii)
              << " with lines title 'rleg',"
              << "'" << fname_frot << "' using 1:" << (tmp_start + 3 + ii)
              << " with lines title 'lleg'" << std::endl;
        }
      ...
      }
      {
        ...
        for (size_t ii = 0; ii < 3; ii++) {
          ...
          oss << "set ylabel '" << titles[ii] << "[m]'" << std::endl;
          oss << "plot "
              << "'" << fname_zoff << "' using 1:" << (tmp_start + ii)
              << " with lines title 'rleg',"
              << "'" << fname_zoff << "' using 1:" << (tmp_start + 3 + ii)
              << " with lines title 'lleg'" << std::endl;
        }
      ...
      }
      {
        ...
        for (size_t ii = 0; ii < 3; ii++) {
          ...
          oss << "plot "
              << "'" << fname_fposvel << "' using 1:" << (tmp_start + ii)
              << " with lines title 'rleg',"
              << "'" << fname_fposvel << "' using 1:" << (tmp_start + 3 + ii)
              << " with lines title 'lleg'" << std::endl;
        }
      ...
      }
      {
        ...
        for (size_t ii = 0; ii < 3; ii++) {
          ...
          oss << "plot "
              << "'" << fname_frotvel << "' using 1:" << (tmp_start + ii)
              << " with lines title 'rleg',"
              << "'" << fname_frotvel << "' using 1:" << (tmp_start + 3 + ii)
              << " with lines title 'lleg'" << std::endl;
        }
      ...
      }
      {
        ...
        oss << "set multiplot layout 2, 1 title '" << gtitle << "'"
            << std::endl;
        ...
        oss << "plot "
            << "[" << min_v[0] << ":" << max_v[0] << "]"
            << "[" << min_v[2] << ":" << max_v[2] << "]"
            << "'" << fname_fpos << "' using " << (2) << ":" << (2 + 2)
            << " with lines title 'rleg ee',"
            << "'" << fname_fpos << "' using " << (2 + 3) << ":" << (2 + 3 + 2)
            << " with lines title 'lleg ee',"
            << "'" << fname_thpos << "' using " << (tmp_thpos_start) << ":"
            << (tmp_thpos_start + 2) << " with lines title 'rleg toe',"
            << "'" << fname_thpos << "' using " << (tmp_thpos_start + 3) << ":"
            << (tmp_thpos_start + 3 + 2) << " with lines title 'lleg toe',"
            << "'" << fname_thpos << "' using " << (tmp_thpos_start + 3 + 3)
            << ":" << (tmp_thpos_start + 3 + 3 + 2)
            << " with lines title 'rleg heel',"
            << "'" << fname_thpos << "' using " << (tmp_thpos_start + 3 + 3 + 3)
            << ":" << (tmp_thpos_start + 3 + 3 + 3 + 2)
            << " with lines title 'lleg heel'" << std::endl;
        ...
        oss << "plot "
            << "[" << min_v[1] << ":" << max_v[1] << "]"
            << "[" << min_v[2] << ":" << max_v[2] << "]"
            << "'" << fname_fpos << "' using " << (2 + 1) << ":" << (2 + 2)
            << " with lines title 'rleg ee',"
            << "'" << fname_fpos << "' using " << (2 + 3 + 1) << ":"
            << (2 + 3 + 2) << " with lines title 'lleg ee',"
            << "'" << fname_thpos << "' using " << (tmp_thpos_start + 1) << ":"
            << (tmp_thpos_start + 2) << " with lines title 'rleg toe',"
            << "'" << fname_thpos << "' using " << (tmp_thpos_start + 3 + 1)
            << ":" << (tmp_thpos_start + 3 + 2)
            << " with lines title 'lleg toe',"
            << "'" << fname_thpos << "' using " << (tmp_thpos_start + 3 + 3 + 1)
            << ":" << (tmp_thpos_start + 3 + 3 + 2)
            << " with lines title 'rleg heel',"
            << "'" << fname_thpos << "' using "
            << (tmp_thpos_start + 3 + 3 + 3 + 1) << ":"
            << (tmp_thpos_start + 3 + 3 + 3 + 2)
            << " with lines title 'lleg heel'" << std::endl;
      ...
      }
      {
        ...
        oss << "set multiplot layout 1, 1 title '" << gtitle << "'"
            << std::endl;
        oss << "set title 'Remain Time'" << std::endl;
        oss << "set xlabel 'Time [s]'" << std::endl;
        oss << "set ylabel 'Time [s]'" << std::endl;
        oss << "plot "
            << "'" << fname_sstime << "' using 1:" << 2
            << " with lines title 'rleg remain time',"
            << "'" << fname_sstime << "' using 1:" << 3
            << " with lines title 'lleg remain time',"
            << "'" << fname_sstime << "' using 1:" << 4
            << " with lines title 'rleg contact states',"
            << "'" << fname_sstime << "' using 1:" << 5
            << " with lines title 'lleg contact states',"
            << "'" << fname_sstime << "' using 1:" << 6
            << " with lines title 'toe_heel_ratio*0.8+0.1'" << std::endl;
      ...
      }
      {
        ...
        for (size_t ii = 0; ii < 3; ii++) {
          ...
          oss << "plot "
              << "'" << fname_ssmc << "' using 1:" << (tmp_start + ii)
              << " with lines title ''" << std::endl;
        }
      ...
      }
      {
        ...
        for (size_t ii = 0; ii < 3; ii++) {
          ...
          oss << "plot "
              << "'" << fname_ssmc << "' using 1:" << (tmp_start + ii + 3)
              << " with lines title ''" << std::endl;
        }
      ...
      }
      {
        ...
        for (size_t ii = 0; ii < 3; ii++) {
          ...
          oss << "plot "
              << "'" << fname_ssmcvel << "' using 1:" << (tmp_start + ii)
              << " with lines title ''" << std::endl;
        }
      ...
      }
      {
        ...
        for (size_t ii = 0; ii < 3; ii++) {
          ...
          oss << "plot "
              << "'" << fname_ssmcvel << "' using 1:" << (tmp_start + ii + 3)
              << " with lines title ''" << std::endl;
        }
      ...
      if (use_graph_append) {
        ...
        ret = system(
            "bash -c 'for f in /tmp/*.eps; do convert -density 250x250 $f "
            "${f//eps}jpg; done'");
        ret = system(
            "(cd /tmp; convert +append Swing_support_mid_coords_pos.jpg "
            "Swing_support_mid_coords_pos_vel.jpg "
            "Swing_support_mid_coords_rot.jpg "
            "Swing_support_mid_coords_rot_vel.jpg img1.jpg)");
        ret = system(
            "(cd /tmp/; convert +append Swing_support_pos.jpg "
            "Swing_support_pos_vel.jpg Swing_support_rot.jpg "
            "Swing_support_rot_vel.jpg img2.jpg)");
        ret = system(
            "(cd /tmp/; convert +append Swing_support_zmp_offset.jpg "
            "Swing_support_remain_time.jpg COG_and_ZMP.jpg "
            "Swing_support_pos_trajectory.jpg img3.jpg)");
        std::string tmpstr =
            test_doc_string.substr(0, test_doc_string.find(":"));
        for (size_t c = tmpstr.find_first_of(" "); c != std::string::npos;
             c = c = tmpstr.find_first_of(" ")) {
          tmpstr.erase(c, 1);
        }
        ret = system(
            std::string("(cd /tmp/; convert -append img1.jpg img2.jpg "
                        "img3.jpg testGaitGeneratorResults_" +
                        tmpstr +
                        ".jpg; rm -f /tmp/img[123].jpg /tmp/COG_and_ZMP.jpg "
                        "/tmp/Swing_support*.jpg)")
                .c_str());
      } else {
        double tmp;
        std::cin >> tmp;
    ...
    std::cerr << "Checking of " << tmpstr
              << " : all results = " << (check_all_results() ? "true" : "false")
              << std::endl;
    std::cerr << "  ZMP error : "
              << (zmp_error_checker.isSmallError() ? "true" : "false")
              << ", max_error : " << zmp_error_checker.getMaxValue() * 1e3
              << "[mm], thre : " << zmp_error_checker.getErrorThre() * 1e3
              << "[mm]" << std::endl;
    std::cerr << "  COGZMP error : "
              << (cogzmp_error_checker.isSmallError() ? "true" : "false")
              << ", max_error : " << cogzmp_error_checker.getMaxValue() * 1e3
              << "[mm], thre : " << cogzmp_error_checker.getErrorThre() * 1e3
              << "[mm]" << std::endl;
    std::cerr << "  RefZMP diff : "
              << (refzmp_diff_checker.isSmallDiff() ? "true" : "false")
              << ", max_diff : " << refzmp_diff_checker.getMaxValue() * 1e3
              << "[mm], thre : " << refzmp_diff_checker.getDiffThre() * 1e3
              << "[mm]" << std::endl;
    std::cerr << "  CartZMP diff : "
              << (cartzmp_diff_checker.isSmallDiff() ? "true" : "false")
              << ", max_diff : " << cartzmp_diff_checker.getMaxValue() * 1e3
              << "[mm], thre : " << cartzmp_diff_checker.getDiffThre() * 1e3
              << "[mm]" << std::endl;
    std::cerr << "  COG diff : "
              << (cog_diff_checker.isSmallDiff() ? "true" : "false")
              << ", max_diff : " << cog_diff_checker.getMaxValue() * 1e3
              << "[mm], thre : " << cog_diff_checker.getDiffThre() * 1e3
              << "[mm]" << std::endl;
    std::cerr << "  FootPos diff : "
              << (footpos_diff_checker.isSmallDiff() ? "true" : "false")
              << ", max_diff : " << footpos_diff_checker.getMaxValue() * 1e3
              << "[mm], thre : " << footpos_diff_checker.getDiffThre() * 1e3
              << "[mm]" << std::endl;
    std::cerr << "  FootRot diff : "
              << (footrot_diff_checker.isSmallDiff() ? "true" : "false")
              << ", max_diff : " << rad2deg(footrot_diff_checker.getMaxValue())
              << "[deg], thre : " << rad2deg(footrot_diff_checker.getDiffThre())
              << "[deg]" << std::endl;
    std::cerr << "  FootPosVel diff : "
              << (footposvel_diff_checker.isSmallDiff() ? "true" : "false")
              << ", max_diff : " << footposvel_diff_checker.getMaxValue() * 1e3
              << "[mm/s], thre : "
              << footposvel_diff_checker.getDiffThre() * 1e3 << "[mm/s]"
              << std::endl;
    std::cerr << "  FootRotVel diff : "
              << (footrotvel_diff_checker.isSmallDiff() ? "true" : "false")
              << ", max_diff : "
              << rad2deg(footrotvel_diff_checker.getMaxValue())
              << "[deg/s], thre : "
              << rad2deg(footrotvel_diff_checker.getDiffThre()) << "[deg/s]"
              << std::endl;
    std::cerr << "  SwingSupportFootMidPos diff : "
              << (ssmcpos_diff_checker.isSmallDiff() ? "true" : "false")
              << ", max_diff : " << ssmcpos_diff_checker.getMaxValue() * 1e3
              << "[mm], thre : " << ssmcpos_diff_checker.getDiffThre() * 1e3
              << "[mm]" << std::endl;
    std::cerr << "  SwingSupportFootMidRot diff : "
              << (ssmcrot_diff_checker.isSmallDiff() ? "true" : "false")
              << ", max_diff : " << rad2deg(ssmcrot_diff_checker.getMaxValue())
              << "[deg], thre : " << rad2deg(ssmcrot_diff_checker.getDiffThre())
              << "[deg]" << std::endl;
    std::cerr << "  SwingSupportFootMidPosVel diff : "
              << (ssmcposvel_diff_checker.isSmallDiff() ? "true" : "false")
              << ", max_diff : " << ssmcposvel_diff_checker.getMaxValue() * 1e3
              << "[mm/s], thre : "
              << ssmcposvel_diff_checker.getDiffThre() * 1e3 << "[mm/s]"
              << std::endl;
    std::cerr << "  SwingSupportFootMidRotVel diff : "
              << (ssmcrotvel_diff_checker.isSmallDiff() ? "true" : "false")
              << ", max_diff : "
              << rad2deg(ssmcrotvel_diff_checker.getMaxValue())
              << "[deg/s], thre : "
              << rad2deg(ssmcrotvel_diff_checker.getDiffThre()) << "[deg/s]"
              << std::endl;
    std::cerr << "  ZMPOffset diff : "
              << (zmpoffset_diff_checker.isSmallDiff() ? "true" : "false")
              << ", max_diff : " << zmpoffset_diff_checker.getMaxValue() * 1e3
              << "[mm/s], thre : " << zmpoffset_diff_checker.getDiffThre() * 1e3
              << "[mm/s]" << std::endl;
    std::cerr << "  Contact states & swing support time validity : "
              << (is_contact_states_swing_support_time_validity ? "true"
                                                                : "false")
              << std::endl;
    std::cerr << "  Step time validity : "
              << (is_step_time_valid ? "true" : "false") << std::endl;
    std::cerr << "  ToeHeel angle validity : "
              << (is_toe_heel_dif_angle_valid ? "true" : "false") << std::endl;
    std::cerr << "  ToeHeel+ZMPoffset validity : "
              << (is_toe_heel_zmp_offset_x_valid ? "true" : "false")
              << std::endl;
  };
  ...
  void check_start_values() {
    ...
    cogzmp_error_checker.checkValueError(gg->get_cog(), gg->get_refzmp(),
                                         neglect_index);
  };
  ...
  void check_end_values() {
    ...
    cogzmp_error_checker.checkValueError(gg->get_cog(), gg->get_refzmp(),
                                         neglect_index);
    // Check step times by comparing calculated step time vs input footsteps
    // step times
    ...
    if (is_step_time_valid) {
      for (size_t i = 0; i < step_time_list.size(); i++) {
        is_step_time_valid =
            eps_eq(step_time_list[i], fsl[i][0].step_time, 1e-5) &&
            is_step_time_valid;
      }
    }
    // Check toe heel angle by comparing calculated toe heel angle (min/max =
    // heel/toe) vs input footsteps toe heel angles
    is_toe_heel_dif_angle_valid =
        (min_toe_heel_dif_angle_list.size() == fsl.size()) &&
        (max_toe_heel_dif_angle_list.size() == fsl.size());
    if (is_toe_heel_dif_angle_valid) {
      if (gg->get_use_toe_heel_auto_set()) {
        ...
        for (size_t i = 1; i < min_toe_heel_dif_angle_list.size(); i++) {
          // std::cerr << "[" << min_toe_heel_dif_angle_list[i] << " " <<
          // max_toe_heel_dif_angle_list[i] << " "
          //           << -fsl[i][0].heel_angle << " " << fsl[i][0].toe_angle <<
          //           "]" << std::endl;
          is_toe_heel_dif_angle_valid = eps_eq(min_toe_heel_dif_angle_list[i],
                                               (-fsl[i][0].heel_angle), 1e-5) &&
                                        eps_eq(max_toe_heel_dif_angle_list[i],
                                               fsl[i][0].toe_angle, 1e-5) &&
                                        is_toe_heel_dif_angle_valid;
        }
      }
    }
    // Check validity of toe heel + zmp offset
    //   If use_toe_heel_transition is true, use/not-use of toe/heel angle
    //   corresponds to use/not-use zmp transition of toe/heel.
    is_toe_heel_zmp_offset_x_valid =
        (min_zmp_offset_x_list.size() == fsl.size()) &&
        (max_zmp_offset_x_list.size() == fsl.size());
    if (gg->get_use_toe_heel_transition()) {
      for (size_t i = 0; i < min_zmp_offset_x_list.size(); i++) {
        // std::cerr << "[" << min_zmp_offset_x_list[i] << " " <<
        // max_zmp_offset_x_list[i] << " " << gg->get_toe_zmp_offset_x() << " "
        // << gg->get_heel_zmp_offset_x() << "]" << std::endl;
        bool heel_valid = true;
        if (!eps_eq(min_toe_heel_dif_angle_list[i], 0.0,
                    1e-5)) {  // If use heel, zmp offset should be same as heel
                              // zmp offset
          heel_valid = eps_eq(min_zmp_offset_x_list[i],
                              gg->get_heel_zmp_offset_x(), 1e-5);
        } else {  // If not use heel, zmp offset should not be same as heel zmp
                  // offset
          heel_valid = !eps_eq(min_zmp_offset_x_list[i],
                               gg->get_heel_zmp_offset_x(), 1e-5);
        }
        bool toe_valid = true;
        if (!eps_eq(max_toe_heel_dif_angle_list[i], 0.0,
                    1e-5)) {  // If use toe, zmp offset should be same as toe
                              // zmp offset
          toe_valid = eps_eq(max_zmp_offset_x_list[i],
                             gg->get_toe_zmp_offset_x(), 1e-5);
        } else {  // If not use toe, zmp offset should not be same as toe zmp
                  // offset
          toe_valid = !eps_eq(max_zmp_offset_x_list[i],
                              gg->get_toe_zmp_offset_x(), 1e-5);
        }
        ...
        is_toe_heel_zmp_offset_x_valid =
            heel_valid && toe_valid && is_toe_heel_zmp_offset_x_valid;
      }
    } else {
      // TODO : not implemented yet
    }
  ...
  void gen_and_plot_walk_pattern(const step_node& initial_support_leg_step,
                                 const step_node& initial_swing_leg_dst_step) {
    parse_params(false);
    gg->print_param();
    gg->initialize_gait_parameter(
        cog, boost::assign::list_of(initial_support_leg_step),
        boost::assign::list_of(initial_swing_leg_dst_step));
  ...
  void gen_and_plot_walk_pattern() {
    ...
    if (gg->get_footstep_front_leg_names() == tmp_string_vector) {
      step_node initial_support_leg_step =
          step_node(LLEG, coordinates(leg_pos[1]), 0, 0, 0, 0);
      step_node initial_swing_leg_dst_step =
          step_node(RLEG, coordinates(leg_pos[0]), 0, 0, 0, 0);
      gen_and_plot_walk_pattern(initial_support_leg_step,
                                initial_swing_leg_dst_step);
    } else {
      step_node initial_support_leg_step =
          step_node(RLEG, coordinates(leg_pos[0]), 0, 0, 0, 0);
      step_node initial_swing_leg_dst_step =
 ...
 public:
  ...
  testGaitGenerator(double _dt)
      : dt(_dt),
        ...
        is_contact_states_swing_support_time_validity(true),
        refzmp_diff_checker(20.0 * 1e-3),
        cartzmp_diff_checker(20.0 * 1e-3),
        cog_diff_checker(10.0 * 1e-3),  // [mm]
        ssmcpos_diff_checker(10.0 * 1e-3),
        ssmcrot_diff_checker(deg2rad(0.1)),
        ssmcposvel_diff_checker(10.0 * 1e-3),
        ssmcrotvel_diff_checker(deg2rad(1)),
        footpos_diff_checker(10.0 * 1e-3),
        footrot_diff_checker(deg2rad(1)),
        footposvel_diff_checker(40 * 1e-3),
        footrotvel_diff_checker(deg2rad(15)),
        zmpoffset_diff_checker(20.0 * 1e-3),
        zmp_error_checker(50 * 1e-3),
        cogzmp_error_checker(1.5 * 1e-3),
        min_rfoot_pos(1e10, 1e10, 1e10),
        min_lfoot_pos(1e10, 1e10, 1e10),
        max_rfoot_pos(-1e10, -1e10, -1e10),
        max_lfoot_pos(-1e10, -1e10, -1e10),
        min_toe_heel_dif_angle(1e10),
        max_toe_heel_dif_angle(-1e10),
        min_zmp_offset_x(1e10),
        max_zmp_offset_x(-1e10),
        ...
        fname_cogzmp("/tmp/plot-cogzmp.dat"),
        fp_cogzmp(fopen(fname_cogzmp.c_str(), "w")),
        fname_fpos("/tmp/plot-fpos.dat"),
        fp_fpos(fopen(fname_fpos.c_str(), "w")),
        fname_frot("/tmp/plot-frot.dat"),
        fp_frot(fopen(fname_frot.c_str(), "w")),
        fname_zoff("/tmp/plot-zoff.dat"),
        fp_zoff(fopen(fname_zoff.c_str(), "w")),
        fname_fposvel("/tmp/plot-fposvel.dat"),
        fp_fposvel(fopen(fname_fposvel.c_str(), "w")),
        fname_frotvel("/tmp/plot-frotvel.dat"),
        fp_frotvel(fopen(fname_frotvel.c_str(), "w")),
        fname_thpos("/tmp/plot-thpos.dat"),
        fp_thpos(fopen(fname_thpos.c_str(), "w")),
        fname_sstime("/tmp/plot-sstime.dat"),
        fp_sstime(fopen(fname_sstime.c_str(), "w")),
        fname_ssmc("/tmp/plot-ssmc.dat"),
        fp_ssmc(fopen(fname_ssmc.c_str(), "w")),
        fname_ssmcvel("/tmp/plot-ssmcvel.dat"),
        fp_ssmcvel(fopen(fname_ssmcvel.c_str(), "w")){};
  ...
  void test0() {
    ...
    fnsl.push_back(boost::assign::list_of(step_node(
        "rleg", coordinates(hrp::Vector3(hrp::Vector3(0, 0, 0) + leg_pos[0])),
        gg->get_default_step_height(), gg->get_default_step_time(),
        gg->get_toe_angle(), gg->get_heel_angle())));
    fnsl.push_back(boost::assign::list_of(step_node(
        "lleg", coordinates(hrp::Vector3(hrp::Vector3(0, 0, 0) + leg_pos[1])),
        gg->get_default_step_height(), gg->get_default_step_time(),
        gg->get_toe_angle(), gg->get_heel_angle())));
    fnsl.push_back(boost::assign::list_of(step_node(
        "rleg",
        coordinates(hrp::Vector3(hrp::Vector3(100 * 1e-3, 0, 0) + leg_pos[0])),
        gg->get_default_step_height(), gg->get_default_step_time(),
        gg->get_toe_angle(), gg->get_heel_angle())));
    fnsl.push_back(boost::assign::list_of(step_node(
        "lleg",
        coordinates(hrp::Vector3(hrp::Vector3(200 * 1e-3, 0, 0) + leg_pos[1])),
        gg->get_default_step_height(), gg->get_default_step_time(),
        gg->get_toe_angle(), gg->get_heel_angle())));
    fnsl.push_back(boost::assign::list_of(step_node(
        "rleg",
        coordinates(hrp::Vector3(hrp::Vector3(200 * 1e-3, 0, 0) + leg_pos[0])),
        gg->get_default_step_height(), gg->get_default_step_time(),
        gg->get_toe_angle(), gg->get_heel_angle())));
  ...
  void test1() {
    ...
    mid_coords(start_ref_coords, 0.5, coordinates(leg_pos[0]),
               coordinates(leg_pos[1]));
    gg->go_pos_param_2_footstep_nodes_list(
        200 * 1e-3, 100 * 1e-3, 20,
        boost::assign::list_of(coordinates(leg_pos[0])), start_ref_coords,
        boost::assign::list_of(RLEG));
    gen_and_plot_walk_pattern();
  };
  ...
  void test2() {
    ...
    gg->clear_footstep_nodes_list();
    coordinates start_ref_coords;
    mid_coords(start_ref_coords, 0.5, coordinates(leg_pos[1]),
               coordinates(leg_pos[0]));
    gg->go_pos_param_2_footstep_nodes_list(
        300 * 1e-3, 0, 0, boost::assign::list_of(coordinates(leg_pos[1])),
        start_ref_coords, boost::assign::list_of(LLEG));
    gen_and_plot_walk_pattern();
  };
  ...
  void test3() {
    ...
    mid_coords(start_ref_coords, 0.5, coordinates(leg_pos[0]),
               coordinates(leg_pos[1]));
    gg->go_pos_param_2_footstep_nodes_list(
        0, 150 * 1e-3, 0, boost::assign::list_of(coordinates(leg_pos[0])),
        start_ref_coords, boost::assign::list_of(RLEG));
    gen_and_plot_walk_pattern();
  };
  ...
  void test4() {
    ...
    mid_coords(start_ref_coords, 0.5, coordinates(leg_pos[1]),
               coordinates(leg_pos[0]));
    gg->go_pos_param_2_footstep_nodes_list(
        0, 0, 30, boost::assign::list_of(coordinates(leg_pos[1])),
        start_ref_coords, boost::assign::list_of(LLEG));
    gen_and_plot_walk_pattern();
  };
  ...
  void test5() {
    ...
    fnsl.push_back(boost::assign::list_of(step_node(
        ...
        gg->get_toe_angle(), gg->get_heel_angle())));
    fnsl.push_back(boost::assign::list_of(step_node(
        "rleg", coordinates(hrp::Vector3(
                    hrp::Vector3(100 * 1e-3, 0, 100 * 1e-3) + leg_pos[0])),
        gg->get_default_step_height(), gg->get_default_step_time(),
        gg->get_toe_angle(), gg->get_heel_angle())));
    fnsl.push_back(boost::assign::list_of(step_node(
        "lleg", coordinates(hrp::Vector3(
                    hrp::Vector3(200 * 1e-3, 0, 200 * 1e-3) + leg_pos[1])),
        gg->get_default_step_height(), gg->get_default_step_time(),
        gg->get_toe_angle(), gg->get_heel_angle())));
    fnsl.push_back(boost::assign::list_of(step_node(
        "rleg", coordinates(hrp::Vector3(
                    hrp::Vector3(300 * 1e-3, 0, 300 * 1e-3) + leg_pos[0])),
        gg->get_default_step_height(), gg->get_default_step_time(),
        gg->get_toe_angle(), gg->get_heel_angle())));
    fnsl.push_back(boost::assign::list_of(step_node(
        "lleg", coordinates(hrp::Vector3(
                    hrp::Vector3(300 * 1e-3, 0, 300 * 1e-3) + leg_pos[1])),
        gg->get_default_step_height(), gg->get_default_step_time(),
        gg->get_toe_angle(), gg->get_heel_angle())));
    gg->set_foot_steps_list(fnsl);
    gen_and_plot_walk_pattern();
  ...
  void test6() {
    ...
    fnsl.push_back(boost::assign::list_of(step_node(
        "lleg",
        coordinates(
            hrp::Vector3(hrp::Vector3(100 * 1e-3, 75 * 1e-3, 0) + leg_pos[1]),
            hrp::rotFromRpy(
                hrp::Vector3(deg2rad(5), deg2rad(-20), deg2rad(10)))),
        gg->get_default_step_height(), gg->get_default_step_time(),
        gg->get_toe_angle(), gg->get_heel_angle())));
    gg->set_foot_steps_list(fnsl);
    gen_and_plot_walk_pattern();
  ...
  void test7() {
    ...
    mid_coords(start_ref_coords, 0.5, coordinates(leg_pos[1]),
               coordinates(leg_pos[0]));
    gg->go_pos_param_2_footstep_nodes_list(
        400 * 1e-3, 0, 0, boost::assign::list_of(coordinates(leg_pos[1])),
        start_ref_coords, boost::assign::list_of(LLEG));
    gen_and_plot_walk_pattern();
  };
  ...
  void test8() {
    ...
    gg->set_use_toe_heel_transition(true);
    gg->clear_footstep_nodes_list();
    hrp::Matrix33 initial_foot_mid_rot =
        Eigen::AngleAxis<double>(M_PI / 2, hrp::Vector3::UnitZ())
            .toRotationMatrix();
    // hrp::Matrix33 initial_foot_mid_rot = Eigen::AngleAxis<double>(M_PI,
    // hrp::Vector3::UnitZ()).toRotationMatrix();
    coordinates start_ref_coords;
    mid_coords(
        start_ref_coords, 0.5,
    ...
    gg->go_pos_param_2_footstep_nodes_list(
        100 * 1e-3, 0, 0,
        boost::assign::list_of(coordinates(initial_foot_mid_rot * leg_pos[1],
                                           initial_foot_mid_rot)),
        start_ref_coords, boost::assign::list_of(LLEG));

    std::vector<std::string> tmp_string_vector = boost::assign::list_of("rleg");
    if (gg->get_footstep_front_leg_names() == tmp_string_vector) {
      step_node initial_support_leg_step = step_node(
          LLEG, coordinates(hrp::Vector3(initial_foot_mid_rot * leg_pos[1]),
                            initial_foot_mid_rot),
          0, 0, 0, 0);
      step_node initial_swing_leg_dst_step = step_node(
          RLEG, coordinates(hrp::Vector3(initial_foot_mid_rot * leg_pos[0]),
                            initial_foot_mid_rot),
          0, 0, 0, 0);
      gen_and_plot_walk_pattern(initial_support_leg_step,
                                initial_swing_leg_dst_step);
  ...
  void test9() {
    ...
    fnsl.push_back(boost::assign::list_of(step_node(
        "rleg", coordinates(hrp::Vector3(
                    hrp::Vector3(250 * 1e-3, 0, 200 * 1e-3) + leg_pos[0])),
        gg->get_default_step_height(), gg->get_default_step_time(),
        gg->get_toe_angle(), gg->get_heel_angle())));
    fnsl.push_back(boost::assign::list_of(step_node(
        "lleg", coordinates(hrp::Vector3(
                    hrp::Vector3(250 * 1e-3, 0, 200 * 1e-3) + leg_pos[1])),
        gg->get_default_step_height(), gg->get_default_step_time(),
        gg->get_toe_angle(), gg->get_heel_angle())));
    fnsl.push_back(boost::assign::list_of(step_node(
        "rleg", coordinates(hrp::Vector3(
                    hrp::Vector3(500 * 1e-3, 0, 400 * 1e-3) + leg_pos[0])),
        gg->get_default_step_height(), gg->get_default_step_time(),
        gg->get_toe_angle(), gg->get_heel_angle())));
    fnsl.push_back(boost::assign::list_of(step_node(
        "lleg", coordinates(hrp::Vector3(
                    hrp::Vector3(500 * 1e-3, 0, 400 * 1e-3) + leg_pos[1])),
        gg->get_default_step_height(), gg->get_default_step_time(),
        gg->get_toe_angle(), gg->get_heel_angle())));
    fnsl.push_back(boost::assign::list_of(step_node(
        "rleg", coordinates(hrp::Vector3(
                    hrp::Vector3(750 * 1e-3, 0, 600 * 1e-3) + leg_pos[0])),
        gg->get_default_step_height(), gg->get_default_step_time(),
        gg->get_toe_angle(), gg->get_heel_angle())));
    fnsl.push_back(boost::assign::list_of(step_node(
        "lleg", coordinates(hrp::Vector3(
                    hrp::Vector3(750 * 1e-3, 0, 600 * 1e-3) + leg_pos[1])),
        gg->get_default_step_height(), gg->get_default_step_time(),
        gg->get_toe_angle(), gg->get_heel_angle())));
    gg->set_foot_steps_list(fnsl);
    gen_and_plot_walk_pattern();
  ...
  void test11() {
    ...
    fnsl.push_back(boost::assign::list_of(step_node(
        "rleg", coordinates(hrp::Vector3(hrp::Vector3(250 * 1e-3, 0, 0 * 1e-3) +
                                         leg_pos[0]),
                            tmpr),
        gg->get_default_step_height(), gg->get_default_step_time(),
        gg->get_toe_angle(), gg->get_heel_angle())));
    tmpr = hrp::rotFromRpy(-5 * M_PI / 180.0, -15 * M_PI / 180.0, 0);
    fnsl.push_back(boost::assign::list_of(step_node(
        "lleg", coordinates(hrp::Vector3(hrp::Vector3(250 * 1e-3, 0, 0 * 1e-3) +
                                         leg_pos[1]),
                            tmpr),
        gg->get_default_step_height(), gg->get_default_step_time(),
        gg->get_toe_angle(), gg->get_heel_angle())));
  ...
  void test12() {
    ...
    fnsl.push_back(boost::assign::list_of(step_node(
        "rleg", coordinates(hrp::Vector3(hrp::Vector3(0, 0, 0) + leg_pos[0])),
        gg->get_default_step_height(), 1.0, 0, 0)));
    fnsl.push_back(boost::assign::list_of(step_node(
        "lleg", coordinates(hrp::Vector3(hrp::Vector3(0, 0, 0) + leg_pos[1])),
        gg->get_default_step_height(), 2.0, 0, 0)));
    fnsl.push_back(boost::assign::list_of(step_node(
        "rleg",
        coordinates(hrp::Vector3(hrp::Vector3(100 * 1e-3, 0, 0) + leg_pos[0])),
        gg->get_default_step_height() * 2, 1.5, 0, 0)));
    fnsl.push_back(boost::assign::list_of(step_node(
        "lleg",
        coordinates(hrp::Vector3(hrp::Vector3(200 * 1e-3, 0, 0) + leg_pos[1])),
        gg->get_default_step_height(), 2.5, 0, 0)));
    fnsl.push_back(boost::assign::list_of(step_node(
        "rleg",
        coordinates(hrp::Vector3(hrp::Vector3(300 * 1e-3, 0, 0) + leg_pos[0])),
        gg->get_default_step_height(), 1.0, 20, 5)));
    fnsl.push_back(boost::assign::list_of(step_node(
        "lleg",
        coordinates(hrp::Vector3(hrp::Vector3(300 * 1e-3, 0, 0) + leg_pos[1])),
        gg->get_default_step_height(), 2.0, 0, 0)));
    gg->set_foot_steps_list(fnsl);
    gen_and_plot_walk_pattern();
  };
  ...
  void test13() {
    ...
    fnsl.push_back(boost::assign::list_of(step_node(
        "rleg", coordinates(hrp::Vector3(hrp::Vector3(0, 0, 0) + leg_pos[0])),
        gg->get_default_step_height(), gg->get_default_step_time(), 0, 0)));
    fnsl.push_back(boost::assign::list_of(step_node(
        "lleg", coordinates(hrp::Vector3(hrp::Vector3(0, 0, 0) + leg_pos[1])),
        gg->get_default_step_height(), gg->get_default_step_time(), 0, 0)));
    fnsl.push_back(boost::assign::list_of(step_node(
        "lleg",
        coordinates(hrp::Vector3(hrp::Vector3(100 * 1e-3, 0, 0) + leg_pos[1])),
        gg->get_default_step_height(), gg->get_default_step_time(), 0, 0)));
    fnsl.push_back(boost::assign::list_of(step_node(
        "rleg",
        coordinates(hrp::Vector3(hrp::Vector3(100 * 1e-3, 0, 0) + leg_pos[0])),
  ...
  void test16() {
    ...
    fnsl.push_back(boost::assign::list_of(step_node(
        "rleg", coordinates(hrp::Vector3(hrp::Vector3(0, 0, 0) + leg_pos[0])),
        gg->get_default_step_height() * 0.5, gg->get_default_step_time() * 0.5,
        gg->get_toe_angle() * 0.5, gg->get_heel_angle() * 0.5)));
    fnsl.push_back(boost::assign::list_of(step_node(
        "lleg",
        coordinates(hrp::Vector3(hrp::Vector3(100 * 1e-3, 0, 0) + leg_pos[1])),
        gg->get_default_step_height() * 2.0, gg->get_default_step_time() * 2.0,
        gg->get_toe_angle() * 2.0, gg->get_heel_angle() * 2.0)));
    fnsl.push_back(boost::assign::list_of(step_node(
        "rleg",
        coordinates(hrp::Vector3(hrp::Vector3(400 * 1e-3, 0, 0) + leg_pos[0])),
        gg->get_default_step_height() * 0.5, gg->get_default_step_time() * 0.5,
        gg->get_toe_angle() * 0.5, gg->get_heel_angle() * 0.5)));
    fnsl.push_back(boost::assign::list_of(step_node(
        "lleg",
        coordinates(hrp::Vector3(hrp::Vector3(400 * 1e-3, 0, 0) + leg_pos[1])),
        gg->get_default_step_height() * 2.0, gg->get_default_step_time() * 2.0,
        gg->get_toe_angle() * 2.0, gg->get_heel_angle() * 2.0)));
    gg->set_foot_steps_list(fnsl);
    gen_and_plot_walk_pattern();
  ...
  void test17() {
    ...
    mid_coords(fm_coords, 0.5, initial_swing_leg_dst_step.worldcoords,
               initial_support_leg_step.worldcoords);
    gg->initialize_velocity_mode(fm_coords, 0.1, 0.05, 10.0,
                                 boost::assign::list_of(RLEG));
    gg->initialize_gait_parameter(
        cog, boost::assign::list_of(initial_support_leg_step),
        boost::assign::list_of(initial_swing_leg_dst_step));
    ...
    while (gg->proc_one_tick()) {
      proc_one_walking_motion(i);
      i++;
      if (i > static_cast<size_t>(gg->get_default_step_time() / dt) &&
          gg->get_overwrite_check_timing()) {
  ...
  void test18() {
    test_doc_string =
        "test18 : Test goVelocity with changing velocity (translation and "
        "rotation)";
    /* initialize sample footstep_list */
    parse_params();
    gg->clear_footstep_nodes_list();
    ...
    mid_coords(fm_coords, 0.5, initial_swing_leg_dst_step.worldcoords,
               initial_support_leg_step.worldcoords);
    gg->initialize_velocity_mode(fm_coords, 0, 0, 0,
                                 boost::assign::list_of(RLEG));
    gg->initialize_gait_parameter(
        cog, boost::assign::list_of(initial_support_leg_step),
        boost::assign::list_of(initial_swing_leg_dst_step));
    ...
    while (gg->proc_one_tick()) {
      proc_one_walking_motion(i);
      i++;
      if (i > static_cast<size_t>(gg->get_default_step_time() / dt) * 5 &&
          gg->get_overwrite_check_timing()) {
        gg->finalize_velocity_mode();
      } else if (i > static_cast<size_t>(gg->get_default_step_time() / dt) *
                         4 &&
                 gg->get_overwrite_check_timing()) {
      ...
      if (arg_strs[i] == "--default-step-time") {
        ...
        if (++i < arg_strs.size()) {
          if (arg_strs[i] == "SHUFFLING") {
            ...
            std::cerr << "No such default-orbit-type " << arg_strs[i]
                      << std::endl;
          }
        }
      } else if (arg_strs[i] ==
        ...
        if (++i < arg_strs.size())
          gg->set_swing_trajectory_delay_time_offset(atof(arg_strs[i].c_str()));
        ...
        if (++i < arg_strs.size())
          gg->set_swing_trajectory_time_offset_xy2z(atof(arg_strs[i].c_str()));
      } else if (arg_strs[i] == "--stair-trajectory-way-point-offset") {
        if (++i < arg_strs.size()) {
          coil::vstring strs =
              coil::split(std::string(arg_strs[i].c_str()), ",");
          gg->set_stair_trajectory_way_point_offset(
              hrp::Vector3(atof(strs[0].c_str()), atof(strs[1].c_str()),
                           atof(strs[2].c_str())));
        }
        ...
        if (++i < arg_strs.size())
          gg->set_heel_angle(atof(arg_strs[i].c_str()));
  ...
  bool check_all_results() const {
    return refzmp_diff_checker.isSmallDiff() &&
           cartzmp_diff_checker.isSmallDiff() &&
           cog_diff_checker.isSmallDiff() &&
           ssmcpos_diff_checker.isSmallDiff() &&
           ssmcrot_diff_checker.isSmallDiff() &&
           ssmcposvel_diff_checker.isSmallDiff() &&
           ssmcrotvel_diff_checker.isSmallDiff() &&
           footpos_diff_checker.isSmallDiff() &&
           footrot_diff_checker.isSmallDiff() &&
           zmpoffset_diff_checker.isSmallDiff() &&
           footposvel_diff_checker.isSmallDiff() &&
           footrotvel_diff_checker.isSmallDiff() &&
           zmp_error_checker.isSmallError() &&
           cogzmp_error_checker.isSmallError() && is_step_time_valid &&
           is_toe_heel_dif_angle_valid && is_toe_heel_zmp_offset_x_valid &&
           is_contact_states_swing_support_time_validity;
  };
};
...
class testGaitGeneratorHRP2JSK : public testGaitGenerator {
 public:
  testGaitGeneratorHRP2JSK() : testGaitGenerator(0.004) {
    ...
    gg = new gait_generator(dt, leg_pos, all_limbs, 1e-3 * 150, 1e-3 * 50, 10,
                            1e-3 * 50);
  };
};
...
void print_usage() {
  ...
  std::cerr << "  --test16 : Set foot steps with param (toe heel contact)"
            << std::endl;
  std::cerr << "  --test17 : Test goVelocity (dx = 0.1, dy = 0.05, dth = 10.0)"
            << std::endl;
  std::cerr << "  --test18 : Test goVelocity with changing velocity "
               "(translation and rotation)"
            << std::endl;
  std::cerr << " [option] should be:" << std::endl;
  std::cerr << "  --use-gnuplot : Use gnuplot and dump eps file to /tmp. "
               "(true/false, true by default)"
            << std::endl;
  std::cerr << "  --use-graph-append : Append generated graph to "
               "/tmp/testGaitGenerator.jpg. (true/false, false by default)"
            << std::endl;
