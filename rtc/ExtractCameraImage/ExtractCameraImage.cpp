...
static const char* extractcameraimage_spec[] = {
    "implementation_id", "ExtractCameraImage", "type_name",
    "ExtractCameraImage", "description", "extractcameraimage component",
...
RTC::ReturnCode_t ExtractCameraImage::onExecute(RTC::UniqueId ec_id) {
  if (m_imagesIn.isNew()) {
    ...
    if (m_images.data.image_seq.length() > m_index) {
      ...
      std::cerr << m_profile.instance_name << ": invalid index of image("
                << m_index
                << "), length of images = " << m_images.data.image_seq.length()
                << std::endl;
    }
  }
  return RTC::RTC_OK;
...
void ExtractCameraImageInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<ExtractCameraImage>,
