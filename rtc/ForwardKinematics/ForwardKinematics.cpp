...
static const char* nullcomponent_spec[] = {
    "implementation_id", "ForwardKinematics", "type_name", "ForwardKinematics",
    "description", "forward kinematics component", "version",
...
RTC::ReturnCode_t ForwardKinematics::onInitialize() {
  ...
  bindParameter("sensorAttachedLink", m_sensorAttachedLinkName,
                ref["conf.default.sensorAttachedLink"].c_str());
  ...
  m_ForwardKinematicsServicePort.registerProvider(
      "service0", "ForwardKinematicsService", m_service0);
...
RTC::ReturnCode_t ForwardKinematics::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_sensorRpyIn.isNew()) {
    ...
    hrp::Matrix33 sensorR = hrp::rotFromRpy(
        m_sensorRpy.data.r, m_sensorRpy.data.p, m_sensorRpy.data.y);
    if (m_sensorAttachedLink) {
      hrp::Matrix33 sensor2base(m_sensorAttachedLink->R.transpose() *
                                m_actBody->rootLink()->R);
...
::CORBA::Boolean ForwardKinematics::getReferencePose(
  ...
  if (frame_name) {
    ...
    if (!f) {
      std::cerr << "[getReferencePose] ERROR Could not find frame_name = "
                << frame_name << std::endl;
      return false;
    }
  }
  std::cerr << "[getReferencePose] linkaname = " << linkname
...
::CORBA::Boolean ForwardKinematics::getCurrentPose(const char* linkname,
  ...
  if (frame_name) {
    ...
    if (!f) {
      std::cerr << "[getCurrentPose] ERROR Could not find frame_name = "
                << frame_name << std::endl;
      return false;
    }
  }
  std::cerr << "[getCurrentPose] linkaname = " << linkname
            << ", frame_name = " << (frame_name ? frame_name : "(null)")
            << std::endl;
...
::CORBA::Boolean ForwardKinematics::getRelativeCurrentPosition(
...
void ForwardKinematicsInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<ForwardKinematics>,
