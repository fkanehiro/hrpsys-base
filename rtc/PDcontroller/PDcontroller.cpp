...
static const char* PDcontroller_spec[] = {
    "implementation_id", "PDcontroller", "type_name", "PDcontroller",
    "description", "PDcontroller component", "version", HRPSYS_PACKAGE_VERSION,
    "vendor", "AIST", "category", "example", "activity_type",
    "DataFlowComponent", "max_instance", "10", "language", "C++", "lang_type",
    "compile",
    // Configuration variables
    "conf.default.pdgains_sim_file_name", "", "conf.default.debugLevel", "0",
    ""};
// </rtc-template>
...
RTC::ReturnCode_t PDcontroller::onExecute(RTC::UniqueId ec_id) {
  ...
  for (size_t i = 0; i < dof; i++) {
    ...
    double q_ref = step > 0
                       ? qold_ref[i] + (m_angleRef.data[i] - qold_ref[i]) / step
                       : qold_ref[i];
    ...
    if (m_robot && m_robot->numJoints() == dof) {
      tlimit = m_robot->joint(i)->climit * m_robot->joint(i)->gearRatio *
               m_robot->joint(i)->torqueConst * tlimit_ratio[i];
    } else {
      tlimit = (std::numeric_limits<double>::max)() * tlimit_ratio[i];
      if (i == 0 && loop % 500 == 0) {
        std::cerr
            << "[" << m_profile.instance_name
            << "] m_robot is not set properly!! Maybe ModelLoader is missing?"
            << std::endl;
      }
    }
    if (loop % 100 == 0 && m_debugLevel == 1) {
      std::cerr << "[" << m_profile.instance_name << "] joint = " << i
                << ", tq = " << m_torque.data[i] << ", q,qref = (" << q << ", "
                << q_ref << "), dq,dqref = (" << dq << ", " << dq_ref
                << "), pd = (" << Pgain[i] << ", " << Dgain[i]
...
void PDcontroller::readGainFile() {
  ...
  if (gain.is_open()) {
    ...
    for (unsigned int i = 0; i < dof; i++) {
      if (gain >> tmp) {
        ...
        std::cerr << "[" << m_profile.instance_name << "] Gain file ["
                  << gain_fname << "] is too short" << std::endl;
      }
      if (gain >> tmp) {
        Dgain[i] = tmp;
      } else {
    ...
    gain.close();
    std::cerr << "[" << m_profile.instance_name << "] Gain file [" << gain_fname
              << "] opened" << std::endl;
  } else {
    std::cerr << "[" << m_profile.instance_name << "] Gain file [" << gain_fname
              << "] not opened" << std::endl;
  ...
  // tlimit_ratio initialize
  {
    ...
    if (prop["pdcontrol_tlimit_ratio"] != "") {
      coil::vstring tlimit_ratio_str =
          coil::split(prop["pdcontrol_tlimit_ratio"], ",");
      if (tlimit_ratio_str.size() == dof) {
        for (size_t i = 0; i < dof; i++) {
          coil::stringTo(tlimit_ratio[i], tlimit_ratio_str[i].c_str());
        }
        std::cerr << "[" << m_profile.instance_name
                  << "] tlimit_ratio is set to "
                  << prop["pdcontrol_tlimit_ratio"] << std::endl;
      } else {
        for (size_t i = 0; i < dof; i++) {
          tlimit_ratio[i] = 1.0;
        }
        std::cerr << "[" << m_profile.instance_name
                  << "] pdcontrol_tlimit_ratio found, but invalid length ("
                  << tlimit_ratio_str.size() << " != " << dof << ")."
                  << std::endl;
        std::cerr << "[" << m_profile.instance_name
  ...
  for (unsigned int i = 0; i < m_robot->numJoints(); i++) {
    std::cerr << "[" << m_profile.instance_name << "]   "
              << m_robot->joint(i)->name << ", pgain = " << Pgain[i]
              << ", dgain = " << Dgain[i] << std::endl;
  }
}
...
void PDcontrollerInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<PDcontroller>,
