...
static const char *virtualcamera_spec[] = {
    "implementation_id", "VirtualCamera", "type_name", "VirtualCamera",
    "description", "virtual camera component", "version",
    HRPSYS_PACKAGE_VERSION, "vendor", "AIST", "category", "example",
    "activity_type", "DataFlowComponent", "max_instance", "10", "language",
    "C++", "lang_type", "compile",
    // Configuration variables
    "conf.default.rangerMaxAngle", "0.25", "conf.default.rangerMinAngle",
    "-0.25", "conf.default.rangerAngularRes", "0.01",
    "conf.default.rangerMaxRange", "5.0", "conf.default.rangerMinRange", "0.5",
    "conf.default.generateRange", "1", "conf.default.generatePointCloud", "0",
    "conf.default.generatePointCloudStep", "1", "conf.default.pcFormat", "xyz",
    "conf.default.generateMovie", "0", "conf.default.debugLevel", "0",
    "conf.default.project", "", "conf.default.camera", "",
...
RTC::ReturnCode_t VirtualCamera::onActivated(RTC::UniqueId ec_id) {
  ...
  int w = 0, h = 0;
  OpenHRP::ModelLoader_var ml = hrp::getModelLoader(
  ...
  if (!m_camera) {
    std::cerr << "VirtualCamera: can't find camera(" << cameraName << ")"
              << std::endl;
    return RTC::RTC_ERROR;
  }
  ...
  m_image.data.image.raw_data.length(m_image.data.image.width *
                                     m_image.data.image.height * 3);
...
RTC::ReturnCode_t VirtualCamera::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_sceneStateIn.isNew()) {
    ...
    for (unsigned int i = 0; i < m_sceneState.states.length(); i++) {
      ...
      if (rtcglb) {
        ...
        body->setPosition(state.basePose.position.x, state.basePose.position.y,
  ...
  capture(m_camera->width(), m_camera->height(),
  ...
  if (m_generateMovie) {
    if (!m_isGeneratingMovie) {
      ...
      m_videoWriter = cvCreateVideoWriter(
          fname.c_str(), CV_FOURCC('D', 'I', 'V', 'X'), 10,
    ...
    for (int i = 0; i < m_image.data.image.width * m_image.data.image.height;
         i++) {
  ...
  if (m_debugLevel > 0) {
    ...
    dt = t7 - t6;
    std::cout << ", render:" << dt.sec() * 1e3 + dt.usec() / 1e3;
    ...
    while (idx >= 0) {
      ...
      if (th > angle) {
        ...
        if (r != 0 && r_old != 0 && alpha > 0.01) {
          m_range.ranges[i] =
              ((th - angle) * r_old + (angle - th_old) * r) / dth;
...
void VirtualCameraInit(RTC::Manager *manager) {
  ...
  manager->registerFactory(profile, RTC::Create<VirtualCamera>,
