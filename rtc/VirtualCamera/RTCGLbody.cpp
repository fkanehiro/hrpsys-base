...
void RTCGLbody::input() {
  ...
  if (m_posIn.isNew()) {
    ...
    m_body->setPosition(m_pos.data.x, m_pos.data.y, m_pos.data.z);
  }
  if (m_rpyIn.isNew()) {
    while (m_rpyIn.isNew()) m_rpyIn.read();
    m_body->setRotation(m_rpy.data.r, m_rpy.data.p, m_rpy.data.y);
  }
  if (m_poseBaseIn.isNew()) {
    while (m_poseBaseIn.isNew()) m_poseBaseIn.read();
    m_body->setPosition(m_poseBase.data.position.x, m_poseBase.data.position.y,
