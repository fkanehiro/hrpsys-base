...
static const char* softerrorlimiter_spec[] = {
    "implementation_id", "GraspController", "type_name", "GraspController",
...
RTC::ReturnCode_t GraspController::onInitialize() {
  ...
  m_GraspControllerServicePort.registerProvider(
      "service0", "GraspControllerService", m_service0);
  ...
  // <name> : <joint1>, <direction1>, <joint2>, <direction2>, <joint1>,
  // <direction2>, <name> : <joint1>, <direction1>
  // check num of grasp
  coil::vstring grasp_joint_params =
      coil::split(prop["grasp_joint_groups"], ",");
  ...
  for (unsigned int i = 0, f = 0; i < grasp_joint_params.size(); i++) {
    coil::vstring grasp_joint_group_names =
        coil::split(grasp_joint_params[i], ":");
    if (grasp_joint_group_names.size() > 1) {
      ...
      if (!!m_robot->link(grasp_joint_group_names[1])) {
        grasp_joint.id =
            m_robot->link(std::string(grasp_joint_group_names[1].c_str()))
                ->jointId;
      }
      f = 0;
      i++;
  ...
  if (m_debugLevel) {
    ...
    while (it != m_grasp_param.end()) {
      ...
      for (unsigned int i = 0; i < it->second.joints.size(); i++) {
        std::cerr << "id = " << it->second.joints[i].id
                  << ", dir = " << it->second.joints[i].dir << ", ";
      }
      std::cerr << std::endl;
      it++;
    }
...
RTC::ReturnCode_t GraspController::onDeactivated(RTC::UniqueId ec_id) {
  std::cout << m_profile.instance_name << ": onDeactivated(" << ec_id << ")"
            << std::endl;
  for (std::map<std::string, GraspParam>::iterator it = m_grasp_param.begin();
       it != m_grasp_param.end(); it++) {
      ...
      if (grasp_param.time < 0) {  // staring
        ...
        for (unsigned int j = 0; j < grasp_param.joints.size(); j++) {
          ...
          if (0 <= i && (unsigned int)i < m_qRef.data.length()) {
            double error = (m_qCurrent.data[i] - m_qRef.data[i]) +
                           grasp_param.target_error * grasp_param.joints[j].dir;
            double diff = fabs(error);
            if (error > 0) m_q.data[i] = m_qRef.data[i] + diff;
            if (error < 0) m_q.data[i] = m_qRef.data[i] - diff;
            // std::cerr << "id = " << i << ", ref = " << m_qRef.data[i] << ",
            // cur =" << m_qCurrent.data[i] << " error = " << error << ", diff =
            // " << diff << ", q = " << m_q.data[i] << " (target = " <<
            // grasp_param.target_error << ", dir=" << grasp_param.joints[j].dir
            // << ")" << std::endl;
          } else {
            if (m_debugLevel == 1)
              std::cerr << "GraspController is not working..., id = " << i
                        << std::endl;
          }
        }
        ...
        for (unsigned int j = 0; j < grasp_param.joints.size(); j++) {
          ...
          if (0 <= i && (unsigned int)i < m_qRef.data.length()) {
            m_qRef.data[i] =
                (m_qRef.data[i] - m_q.data[i]) * grasp_param.time / 1000 +
                m_q.data[i];
            ...
            // std::cerr << "id = " << i << ", ref = " << m_qRef.data[i] << ",
            // cur =" << m_qCurrent.data[i] << " q = " << m_q.data[i] <<
            // std::endl;
          }
        }
...
bool GraspController::startGrasp(const char* name, double target_error) {
  ...
  std::cerr << "[" << m_profile.instance_name << "] Start Grasp " << name
            << std::endl;
...
bool GraspController::stopGrasp(const char* name) {
  ...
  std::cerr << "[" << m_profile.instance_name << "] Stop Grasp " << name
            << std::endl;
...
void GraspControllerInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<GraspController>,
