...
class testObjectContactTurnaroundDetectorBase {
 protected:
  ...
  void gen_pattern_and_plot(const std::vector<double>& force_vec,
    ...
    for (size_t i = 0; i < time_vec.size(); i++) {
      ...
      fprintf(fp, "%f %f %f %f\n", time_vec[i], force_vec[i],
              otd.getFilteredWrench(), otd.getFilteredDwrench(), detected);
    }
    fclose(fp);
    if (use_gnuplot) {
      // plot
      ...
      fprintf(gp, "set arrow from %f,%f to %f,%f\n", detect_time,
              force_vec.front(), detect_time, force_vec.back());
      fprintf(gp,
              "plot '/tmp/plot-otd.dat' using 1:2 with lines title 'Wrench' lw "
              "4, '/tmp/plot-otd.dat' using 1:3 with lines title "
              "'FilteredWrench' lw 4\n");
      fprintf(gp, "set xlabel 'Time [s]'\n");
      fprintf(gp, "set ylabel 'Dwrench'\n");
      fprintf(gp,
              "plot '/tmp/plot-otd.dat' using 1:4 with lines title 'Dwrench' "
              "lw 4\n");
      fflush(gp);
      double tmp;
      std::cin >> tmp;
 ...
 public:
  ...
  testObjectContactTurnaroundDetectorBase(const double _dt = 0.004)
      : dt(_dt), otd(_dt), use_gnuplot(true){};
  void test0() {
...
void print_usage() {
  std::cerr << "Usage : testObjectContactTurnaroundDetectorBase [option]"
            << std::endl;
