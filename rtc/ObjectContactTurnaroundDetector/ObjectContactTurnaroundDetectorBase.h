...
class ObjectContactTurnaroundDetectorBase {
 ...
 private:
  ...
  double detect_ratio_thre, start_ratio_thre, ref_dwrench, max_time,
      current_time, current_wrench;
  size_t count;
  // detect_count_thre*dt and start_ratio_thre*dt are threshould for time.
  //   detect_count_thre*dt : Threshould for time [s] after the first object
  //   contact turnaround detection (Wait detect_time_thre [s] after first
  //   object contact turnaround detection).
  //   start_count_thre*dt  : Threshould for time [s] after the first starting
  //   detection (Wait start_time_thre [s] after first start detection).
 ...
 public:
  ObjectContactTurnaroundDetectorBase(const double _dt)
      : axis(-1 * hrp::Vector3::UnitZ()),
        moment_center(hrp::Vector3::Zero()),
        prev_wrench(0.0),
        dt(_dt),
        detect_ratio_thre(0.01),
        start_ratio_thre(0.5),
        count(0),
        detect_count_thre(5),
        start_count_thre(5),
        pmode(MODE_IDLE),
    ...
    wrench_filter = boost::shared_ptr<FirstOrderLowPassFilter<double> >(
        new FirstOrderLowPassFilter<double>(default_cutoff_freq, _dt, 0));
    dwrench_filter = boost::shared_ptr<FirstOrderLowPassFilter<double> >(
        new FirstOrderLowPassFilter<double>(default_cutoff_freq, _dt, 0));
  };
  ...
  void startDetection(const double _ref_diff_wrench, const double _max_time) {
    ...
    count = 0;
    std::cerr << "[" << print_str
              << "] Start Object Turnaround Detection (ref_dwrench = "
              << ref_dwrench
  ...
  double calcTotalMoment(const std::vector<hrp::Vector3>& fmv,
  ...
  bool checkDetection(const double wrench_value) {
    ...
    switch (pmode) {
      case MODE_IDLE:
        if (tmp_dwr > ref_dwrench * start_ratio_thre) {
          ...
          if (count > start_count_thre) {
            ...
            count = 0;
            std::cerr << "[" << print_str
                      << "] Object Turnaround Detection Started. ("
                      << start_count_thre * dt
      ...
      case MODE_STARTED:
        ...
        // std::cerr << "[" << print_str << "] " << tmp_wr << " " << tmp_dwr <<
        // " " << count << std::endl;
        break;
      case MODE_DETECTED:
        break;
    ...
    if (max_time <= current_time && (pmode != MODE_DETECTED)) {
      if (pmode != MODE_MAX_TIME)
        std::cerr << "[" << print_str
                  << "] Object Turnaround Detection max time reached."
                  << std::endl;
      pmode = MODE_MAX_TIME;
    }
    current_time += dt;
  ...
  void printParams() const {
    std::cerr << "[" << print_str
              << "]   ObjectContactTurnaroundDetectorBase params ("
              << (dtw == TOTAL_FORCE ? "TOTAL_FORCE" : "TOTAL_MOMENT") << ")"
              << std::endl;
    std::cerr << "[" << print_str
              << "]    wrench_cutoff_freq = " << wrench_filter->getCutOffFreq()
              << "[Hz], dwrench_cutoff_freq = "
              << dwrench_filter->getCutOffFreq() << "[Hz]" << std::endl;
    std::cerr << "[" << print_str
              << "]    detect_ratio_thre = " << detect_ratio_thre
              << ", start_ratio_thre = " << start_ratio_thre
              << ", start_time_thre = " << start_count_thre * dt
              << "[s], detect_time_thre = " << detect_count_thre * dt << "[s]"
              << std::endl;
    std::cerr << "[" << print_str << "]    axis = [" << axis(0) << ", "
              << axis(1) << ", " << axis(2)
              << "], moment_center = " << moment_center(0) << ", "
              << moment_center(1) << ", " << moment_center(2) << "][m]"
              << std::endl;
  };
  ...
  double getDwrenchCutoffFreq() const {
    return dwrench_filter->getCutOffFreq();
  };
  ...
  double getFilteredDwrench() const {
    return dwrench_filter->getCurrentValue();
  };
