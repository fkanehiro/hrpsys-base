...
class ServoSerial {
 public:
  ...
  ~ServoSerial() { close(fd); }

  int setReset(int id) { sendPacket(0xFAAF, id, 0x20, 0xFF, 0, 0, NULL); }
  ...
  int setPosition(int id, double rad, double sec) {  // #32
    ...
    printf("[ServoSerial] setPosition %f %f, %04x, %04x\n", 180 / M_PI * rad,
           sec, angle, msec);
    unsigned char data[4] = {0xff & angle, 0xff & (angle >> 8), 0xff & msec,
                             0xff & (msec >> 8)};
  ...
  int setPositions(int len, int *id, double *rad, double *sec) {  // #32
    ...
    for (int i = 0; i < len; i++) {
      ...
      printf("[ServoSerial] setPositions %d: %f %f, %04x, %04x\n", id[i],
             180 / M_PI * rad[i], sec[i], angle, msec);
      data[i * 5 + 0] = id[i];
  ...
  int receivePacket(int id, int address, int length, unsigned char data[]) {
    ...
    if (flags & 0x0002) {  // 0b00000010
      fprintf(stderr,
              "[ServoSerial] Failed to receive packet from servo(id:%d) Fail "
              "to process received packet\n",
              id);
      ret = -1;
    }
  ...
  int sendPacket(int header, int id, int flag, int address, int length,
                 int count, void *data) {
