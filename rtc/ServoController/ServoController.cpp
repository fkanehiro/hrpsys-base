...
static const char *nullcomponent_spec[] = {
    "implementation_id", "ServoController", "type_name", "ServoController",
...
RTC::ReturnCode_t ServoController::onInitialize() {
  ...
  m_ServoControllerServicePort.registerProvider(
      "service0", "ServoControllerService", m_service0);
  ...
  if (devname == "") {
    std::cerr << "\e[1;31m[WARNING] " << m_profile.instance_name
  ...
  if (servo_ids.size() == 0) {
    std::cerr << "\e[1;31m[ERROR] " << m_profile.instance_name
  ...
  if (servo_ids.size() != servo_offset.size()) {
    std::cerr
        << "\e[1;31m[ERROR] " << m_profile.instance_name
        << ": servo.id and servo.offset property must have same length\e[0m"
        << std::endl;
    return RTC::RTC_ERROR;
  }
  ...
  if (servo_ids.size() != servo_dir.size()) {
    std::cerr << "\e[1;31m[ERROR] " << m_profile.instance_name
              << ": servo.id and servo.dir property must have same length\e[0m"
              << std::endl;
    return RTC::RTC_ERROR;
  }
...
bool ServoController::setJointAngles(
  ...
  if (angles.length() != servo_id.size()) {
    std::cerr << "[ERROR] " << m_profile.instance_name << ": size of servo.id("
              << angles.length() << ") is not correct, expected"
...
bool ServoController::getJointAngles(
...
bool ServoController::addJointGroup(
...
bool ServoController::setJointAnglesOfGroup(
  ...
  if (joint_groups.find(gname) != joint_groups.end()) {
    ...
    if (angles.length() != len) {
      std::cerr << "[ERROR] " << m_profile.instance_name
...
bool ServoController::servoOn() {
  ...
  for (vector<int>::iterator it = servo_id.begin(); it != servo_id.end();
       it++) {
...
void ServoControllerInit(RTC::Manager *manager) {
  ...
  manager->registerFactory(profile, RTC::Create<ServoController>,
