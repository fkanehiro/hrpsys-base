...
static const char* accelerationfilter_spec[] = {
    "implementation_id", "AccelerationFilter", "type_name",
    "AccelerationFilter", "description", "Acceleration Filter component",
    "version", "1.0", "vendor", "JSK", "category", "example", "activity_type",
    "SPORADIC", "kind", "DataFlowComponent", "max_instance", "10", "language",
    "C++", "lang_type", "compile",
...
RTC::ReturnCode_t AccelerationFilter::onInitialize() {
  ...
  m_AccelerationFilterServicePort.registerProvider(
      "service0", "AccelerationFilterService", m_service0);
  ...
  std::cerr << "[" << m_profile.instance_name << "] gravity : " << m_gravity
            << std::endl;
...
RTC::ReturnCode_t AccelerationFilter::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_accInIn.isNew()) {
    ...
    hrp::Matrix33 imuR =
        hrp::rotFromRpy(m_rpyIn.data.r, m_rpyIn.data.p, m_rpyIn.data.y);
    ...
    hrp::Vector3 _result_vel =
        imuR.inverse() *
        m_global_vel;  // result should be described in sensor coords
...
bool AccelerationFilter::resetFilter(
...
bool AccelerationFilter::setParam(
    const ::OpenHRP::AccelerationFilterService::AccelerationFilterParam&
        i_param) {
  Guard guard(m_mutex);
...
bool AccelerationFilter::getParam(
...
void AccelerationFilterInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<AccelerationFilter>,
