...
RTC::ReturnCode_t ReferenceForceUpdater::onInitialize()
{
  ...
  if (end_effectors_str.size() > 0) {
      ...
      for (size_t i = 0; i < num; i++) {
          ...
          //set rfu param
          rfu_param.m_dt=m_dt;
          rfu_param.update_freq = 50; // Hz
          rfu_param.p_gain = 0.02;
          rfu_param.d_gain = 0;
          rfu_param.i_gain = 0;
          rfu_param.arm = ee_name;
          rfu_param.is_active = false;
          rfu_param.update_time_ratio = 0.5;
          rfu_param.motion_dir = hrp::Vector3::UnitZ();
          rfu_param.is_stopping = false;
    ...
    if ( (ee_map.find("rleg") != ee_map.end() && ee_map.find("lleg") != ee_map.end()) // if legged robot
      ...
      for ( std::map<std::string, ReferenceForceUpdaterParam>::iterator itr = m_RFUParam.begin(); itr != m_RFUParam.end(); itr++ ) {
        ...
        if (!transition_interpolator_isEmpty) {
          ...
          if (transition_interpolator[arm]->isEmpty() && itr->second.is_active && itr->second.is_stopping) {
            std::cerr << "[" << m_profile.instance_name << "] ReferenceForceUpdater active => inactive." << std::endl;
            itr->second.is_active = false;
            itr->second.is_stopping = false;
          }
        }
        ...
        if (itr->second.is_active && loop % m_RFUParam[arm].update_count() == 0) {
...
bool ReferenceForceUpdater::stopReferenceForceUpdater(const std::string& i_name_)
{
    std::cerr << "[" << m_profile.instance_name << "] stopReferenceForceUpdater" << std::endl;
    {
        ...
        if ( m_RFUParam[i_name_].is_active == false ){
          if (m_RFUParam[i_name_].is_stopping == true)
            return false;
          else
            return true;
        }
