...
static const char* ReferenceForceUpdater_spec[] = {
    "implementation_id", "ReferenceForceUpdater", "type_name",
    "ReferenceForceUpdater", "description", "update reference force", "version",
...
ReferenceForceUpdater::ReferenceForceUpdater(RTC::Manager* manager)
    : RTC::DataFlowComponentBase(manager),
      ...
      m_rpyIn("rpy", m_rpy),
      m_diffFootOriginExtMomentIn("diffFootOriginExtMoment",
                                  m_diffFootOriginExtMoment),
      m_refFootOriginExtMomentOut("refFootOriginExtMoment",
...
RTC::ReturnCode_t ReferenceForceUpdater::onInitialize() {
  ...
  m_ReferenceForceUpdaterServicePort.registerProvider(
      "service0", "ReferenceForceUpdaterService",
  ...
  // Get dt
  RTC::Properties& prop =
      getProperties();  // get properties information from .wrl file
  ...
  //   add ports for all force sensors (real force, input and output of
  //   ref_force)
  ...
  std::cerr << "[" << m_profile.instance_name << "] create force sensor ports"
            << std::endl;
  for (unsigned int i = 0; i < nforce; i++) {
    ...
    m_ref_forceIn[i] = new InPort<TimedDoubleSeq>(
        std::string("ref_" + fsensor_names[i] + "In").c_str(),
        m_ref_force_in[i]);
    ...
    m_ref_forceOut[i] = new OutPort<TimedDoubleSeq>(
        std::string("ref_" + fsensor_names[i] + "Out").c_str(),
        m_ref_force_out[i]);
    registerOutPort(std::string("ref_" + fsensor_names[i] + "Out").c_str(),
                    *m_ref_forceOut[i]);
    std::cerr << "[" << m_profile.instance_name
              << "]   name = " << fsensor_names[i] << std::endl;
  }
  ...
  if (end_effectors_str.size() > 0) {
    ...
    for (size_t i = 0; i < num; i++) {
      ...
      if ((ee_name != "rleg") && (ee_name != "lleg"))
        m_RFUParam.insert(std::pair<std::string, ReferenceForceUpdaterParam>(
            ee_name, rfu_param));

      ee_index_map.insert(std::pair<std::string, size_t>(ee_name, i));
      ref_force.push_back(hrp::Vector3::Zero());
      // ref_force_interpolator.insert(std::pair<std::string,
      // interpolator*>(ee_name, new interpolator(3, m_dt)));
      ref_force_interpolator.insert(std::pair<std::string, interpolator*>(
          ee_name, new interpolator(3, m_dt, interpolator::LINEAR)));
      if ((ee_name != "lleg") && (ee_name != "rleg"))
        transition_interpolator.insert(std::pair<std::string, interpolator*>(
            ee_name, new interpolator(1, m_dt)));
      std::cerr << "[" << m_profile.instance_name << "] End Effector ["
                << ee_name << "]" << ee_target << " " << ee_base << std::endl;
      std::cerr << "[" << m_profile.instance_name
                << "]   target = " << ee_target << ", base = " << ee_base
                << ", sensor_name = " << eet.sensor_name << std::endl;
    ...
    // For FootOriginExtMoment
    {
      ...
      ee_map.insert(std::pair<std::string, ee_trans>(ee_name, eet));
      ee_index_map.insert(
          std::pair<std::string, size_t>(ee_name, ref_force.size()));
      ref_force.push_back(hrp::Vector3::Zero());
      ref_force_interpolator.insert(std::pair<std::string, interpolator*>(
          ee_name, new interpolator(3, m_dt, interpolator::LINEAR)));
      transition_interpolator.insert(std::pair<std::string, interpolator*>(
  ...
  for (unsigned int i = 0; i < transition_interpolator.size(); i++)
    transition_interpolator_ratio[i] = 0;
...
RTC::ReturnCode_t ReferenceForceUpdater::onFinalize() {
  ...
  for (std::map<std::string, interpolator*>::iterator it =
           ref_force_interpolator.begin();
       it != ref_force_interpolator.end(); it++) {
    delete it->second;
  }
  for (std::map<std::string, interpolator*>::iterator it =
           transition_interpolator.begin();
       it != transition_interpolator.end(); it++) {
    delete it->second;
  }
...
RTC::ReturnCode_t ReferenceForceUpdater::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_qRef.data.length() == m_robot->numJoints()) {
    ...
    // Interpolator
    for (std::map<std::string, ReferenceForceUpdaterParam>::iterator itr =
             m_RFUParam.begin();
         itr != m_RFUParam.end(); itr++) {
      std::string arm = itr->first;
      size_t arm_idx = ee_index_map[arm];
      bool transition_interpolator_isEmpty =
          transition_interpolator[arm]->isEmpty();
      if (!transition_interpolator_isEmpty) {
        transition_interpolator[arm]->get(
            &transition_interpolator_ratio[arm_idx], true);
        if (transition_interpolator[arm]->isEmpty() &&
            m_RFUParam[arm].is_active && m_RFUParam[arm].is_stopping) {
    ...
    // If RFU is active
    {
      ...
      if ((ee_map.find("rleg") != ee_map.end() &&
        ...
        //  This will be removed after seq outputs adequate waistRPY discussed
        //  in https://github.com/fkanehiro/hrpsys-base/issues/272
        ...
        for (size_t i = 0; i < leg_names.size(); i++) {
          hrp::Link* target_link =
              m_robot->link(ee_map[leg_names[i]].target_name);
    ...
    for (std::map<std::string, ReferenceForceUpdaterParam>::iterator itr =
      ...
      std::string arm = itr->first;
      if (m_RFUParam[arm].is_active &&
          loop % m_RFUParam[arm].update_count == 0) {
        if (isFootOriginExtMoment(arm))
          updateRefFootOriginExtMoment(arm);
        else
          updateRefForces(arm);
      }
      if (!ref_force_interpolator[arm]->isEmpty()) {
        ref_force_interpolator[arm]->get(ref_force[ee_index_map[arm]].data(),
                                         true);
      }
    }
  }
  ...
  for (unsigned int i = 0; i < m_ref_force_in.size(); i++) {
    ...
    for (unsigned int j = 0; j < 3; j++) {
      m_ref_force_out[i].data[j] =
          ref_force[i](j)*transition_interpolator_ratio[i] +
          m_ref_force_in[i].data[j] * (1 - transition_interpolator_ratio[i]);
    }
    m_ref_force_out[i].tm = m_ref_force_in[i].tm;
    m_ref_forceOut[i]->write();
  }
...
void ReferenceForceUpdater::calcFootOriginCoords(
    hrp::Vector3& foot_origin_pos, hrp::Matrix33& foot_origin_rot) {
  rats::coordinates leg_c[2], tmpc;
  hrp::Vector3 ez = hrp::Vector3::UnitZ();
  hrp::Vector3 ex = hrp::Vector3::UnitX();
  size_t i = 0;
  for (std::map<std::string, ee_trans>::iterator itr = ee_map.begin();
       itr != ee_map.end(); itr++) {
    if (itr->first.find("leg") == std::string::npos) continue;
    hrp::Link* target =
        m_robot->sensor<hrp::ForceSensor>(itr->second.sensor_name)->link;
...
void ReferenceForceUpdater::updateRefFootOriginExtMoment(
  ...
  hrp::Vector3 df =
      foot_origin_rot *
      (-1 *
       hrp::Vector3(m_diffFootOriginExtMoment.data.x,
                    m_diffFootOriginExtMoment.data.y,
                    m_diffFootOriginExtMoment.data.z));  // diff = ref - act;
  if (!m_RFUParam[arm].is_hold_value)
    ref_force[arm_idx] =
        ref_force[arm_idx] +
        (m_RFUParam[arm].p_gain * transition_interpolator_ratio[arm_idx]) * df;
  interpolation_time =
      (1 / m_RFUParam[arm].update_freq) * m_RFUParam[arm].update_time_ratio;
  if (ref_force_interpolator[arm]->isEmpty()) {
    ref_force_interpolator[arm]->setGoal(ref_force[arm_idx].data(),
                                         interpolation_time, true);
  }
  ...
  if (DEBUGP) {
    std::cerr << "[" << m_profile.instance_name
              << "] Updating reference moment [" << arm << "]" << std::endl;
    std::cerr << "[" << m_profile.instance_name
              << "]   diff foot origin ext moment = "
              << df.format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ",
                                           ", ", "", "", "    [", "]"))
              << "[Nm], interpolation_time = " << interpolation_time << "[s]"
              << std::endl;
    std::cerr << "[" << m_profile.instance_name
              << "]   new foot origin ext moment = "
              << ref_force[arm_idx].format(
...
void ReferenceForceUpdater::updateRefForces(const std::string& arm) {
  ...
  if (!std::fabs((abs_motion_dir.norm() - 0.0)) < 1e-5) {
    ...
    if (!(inner_product < 0 && ref_force[arm_idx].dot(abs_motion_dir) < 0.0)) {
      ...
      if (!m_RFUParam[arm].is_hold_value)
        ref_force[arm_idx] =
            ref_force[arm_idx].dot(abs_motion_dir) * abs_motion_dir + in_f +
            (m_RFUParam[arm].p_gain * inner_product *
  ...
  if (DEBUGP) {
    std::cerr << "[" << m_profile.instance_name
              << "] Updating reference force [" << arm << "]" << std::endl;
    std::cerr << "[" << m_profile.instance_name
              << "]   inner_product = " << inner_product
              << "[N], ref_force = " << ref_force[arm_idx].dot(abs_motion_dir)
              << "[N], interpolation_time = " << interpolation_time << "[s]"
              << std::endl;
    std::cerr << "[" << m_profile.instance_name << "]   new ref_force = "
              << ref_force[arm_idx].format(
                     Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "",
                                     "", "    [", "]"))
              << "[N]" << std::endl;
    std::cerr << "[" << m_profile.instance_name << "]   act_force = "
              << tmp_act_force.format(Eigen::IOFormat(Eigen::StreamPrecision, 0,
                                                      ", ", ", ", "", "",
                                                      "    [", "]"))
              << "[N]" << std::endl;
    std::cerr << "[" << m_profile.instance_name << "]   df = "
              << df.format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ",
                                           ", ", "", "", "    [", "]"))
              << "[N]" << std::endl;
  }
...
bool ReferenceForceUpdater::setReferenceForceUpdaterParam(
  ...
  if (std::string(i_param.frame) != "local" &&
      std::string(i_param.frame) != "world") {
    std::cerr << "[" << m_profile.instance_name
              << "] \"frame\" parameter must be local/world. could not set \""
              << std::string(i_param.frame) << "\"" << std::endl;
    return false;
  }
  // Parameters which cannot be changed when active
  if (m_RFUParam[arm].is_active) {  // When active, check parameter changing,
                                    // and if changed, do not change the value.
    if (!eps_eq(m_RFUParam[arm].update_freq, i_param.update_freq)) {
      std::cerr << "[" << m_profile.instance_name
                << "] Could not set update_freq because rfu [" << i_name_
                << "] is active (current = " << m_RFUParam[arm].update_freq
                << ", new = " << i_param.update_freq << ")" << std::endl;
      return false;
    } else {
    ...
    if (!eps_eq(m_RFUParam[arm].update_time_ratio, i_param.update_time_ratio)) {
      std::cerr << "[" << m_profile.instance_name
                ...
                << ", new = " << i_param.update_time_ratio << ")" << std::endl;
      return false;
    } else {
    ...
    if (m_RFUParam[arm].frame != std::string(i_param.frame)) {
      std::cerr << "[" << m_profile.instance_name
                ...
                << "] is active (current = " << m_RFUParam[arm].frame
                << ", new = " << i_param.frame << ")" << std::endl;
      return false;
    } else {
    ...
    m_RFUParam[arm].update_freq = i_param.update_freq;
    m_RFUParam[arm].update_time_ratio = i_param.update_time_ratio;
    m_RFUParam[arm].update_count =
        round((1 / m_RFUParam[arm].update_freq) / m_dt);
  ...
  for (size_t i = 0; i < 3; i++)
    m_RFUParam[arm].motion_dir(i) = i_param.motion_dir[i];

  // Print values
  std::cerr << "[" << m_profile.instance_name
            << "]   p_gain = " << m_RFUParam[arm].p_gain
            << ", d_gain = " << m_RFUParam[arm].d_gain
            << ", i_gain = " << m_RFUParam[arm].i_gain << std::endl;
  std::cerr << "[" << m_profile.instance_name
            << "]   update_freq = " << m_RFUParam[arm].update_freq
            << "[Hz], update_time_ratio = " << m_RFUParam[arm].update_time_ratio
            << std::endl;
  std::cerr << "[" << m_profile.instance_name << "]   motion_dir = "
            << m_RFUParam[arm].motion_dir.format(Eigen::IOFormat(
                   Eigen::StreamPrecision, 0, ", ", ", ", "", "", "    [", "]"))
            << std::endl;
  std::cerr << "[" << m_profile.instance_name
            << "]   frame = " << m_RFUParam[arm].frame << ", is_hold_value = "
            << (m_RFUParam[arm].is_hold_value ? "true" : "false") << std::endl;
  return true;
};

bool ReferenceForceUpdater::getReferenceForceUpdaterParam(
    const std::string& i_name_,
  ...
  for (size_t i = 0; i < 3; i++)
    i_param->motion_dir[i] = m_RFUParam[arm].motion_dir(i);
  return true;
};
...
bool ReferenceForceUpdater::startReferenceForceUpdater(
    const std::string& i_name_) {
  std::cerr << "[" << m_profile.instance_name
            << "] startReferenceForceUpdater [" << i_name_ << "]" << std::endl;
  {
    Guard guard(m_mutex);
    if (m_RFUParam.find(i_name_) == m_RFUParam.end()) {
      std::cerr << "[" << m_profile.instance_name
                << "] Could not found reference force updater param ["
                << i_name_ << "]" << std::endl;
      return false;
    }
    if (m_RFUParam[i_name_].is_active == true) return true;
    if (transition_interpolator[i_name_]->isEmpty()) {
      ...
      if (isFootOriginExtMoment(std::string(i_name_))) {
        currentRefForce = hrp::Vector3(m_diffFootOriginExtMoment.data.x,
                                       m_diffFootOriginExtMoment.data.y,
                                       m_diffFootOriginExtMoment.data.z);
      } else {
        currentRefForce = hrp::Vector3(m_ref_force_in[arm_idx].data[0],
                                       m_ref_force_in[arm_idx].data[1],
                                       m_ref_force_in[arm_idx].data[2]);
      }
...
bool ReferenceForceUpdater::stopReferenceForceUpdater(
    const std::string& i_name_) {
  std::cerr << "[" << m_profile.instance_name << "] stopReferenceForceUpdater ["
            << i_name_ << "]" << std::endl;
  {
    Guard guard(m_mutex);
    if (m_RFUParam.find(i_name_) == m_RFUParam.end()) {
...
void ReferenceForceUpdaterInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<ReferenceForceUpdater>,
