...
static const char* joystick2velocity_spec[] = {
    "implementation_id", "Joystick2Velocity3D", "type_name",
    "Joystick2Velocity3D", "description",
    "joystick output to velocity converter", "version", HRPSYS_PACKAGE_VERSION,
    "vendor", "AIST", "category", "example", "activity_type",
    "DataFlowComponent", "max_instance", "10", "language", "C++", "lang_type",
    "compile",
    // Configuration variables
    "conf.default.debugLevel", "0", "conf.default.axesIds", "0,1,2",
...
RTC::ReturnCode_t Joystick2Velocity3D::onDeactivated(RTC::UniqueId ec_id) {
  std::cout << "Joystick2Velocity3D::onDeactivated(" << ec_id << ")"
            << std::endl;
  return RTC::RTC_OK;
}
...
RTC::ReturnCode_t Joystick2Velocity3D::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_debugLevel > 0) {
    printf("velocity command: %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f\n",
           m_vel.data.vx, m_vel.data.vy, m_vel.data.vz, m_vel.data.vr,
           m_vel.data.vp, m_vel.data.va);
  }
  m_velOut.write();
...
void Joystick2Velocity3DInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<Joystick2Velocity3D>,
