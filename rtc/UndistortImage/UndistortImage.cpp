...
static const char *cameraimageviewercomponent_spec[] = {
    "implementation_id", "UndistortImage", "type_name", "UndistortImage",
    "description", "camera image undistortion component", "version",
...
RTC::ReturnCode_t UndistortImage::onActivated(RTC::UniqueId ec_id) {
  ...
  if (!fs) {
    std::cerr << m_profile.instance_name << ": can't open " << m_calibFile
              << std::endl;
...
RTC::ReturnCode_t UndistortImage::onExecute(RTC::UniqueId ec_id) {
  ...
  if (!m_cvImage) {
    switch (m_image.data.image.format) {
      ...
      case Img::CF_GRAY:
        ...
        break;
      default:
        std::cerr << "unsupported color format(" << m_image.data.image.format
                  << ")" << std::endl;
        return RTC::RTC_ERROR;
    }
  }
  ...
  switch (m_image.data.image.format) {
    ...
    case Img::CF_GRAY:
      memcpy(m_image.data.image.raw_data.get_buffer(), dst_img->imageData,
             m_image.data.image.raw_data.length());
      break;
    default:
      break;
...
void UndistortImageInit(RTC::Manager *manager) {
  ...
  manager->registerFactory(profile, RTC::Create<UndistortImage>,
