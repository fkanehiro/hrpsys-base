...
static const char* spec[] = {
    "implementation_id", "RotateImage", "type_name", "RotateImage",
    "description", "rotate image component", "version", HRPSYS_PACKAGE_VERSION,
...
RTC::ReturnCode_t RotateImage::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_originalIn.isNew()) {
    ...
    if (m_src && (m_src->width != idat.width || m_src->height != idat.height)) {
      cvReleaseImage(&m_src);
      cvReleaseImage(&m_dst);
      m_src = m_dst = NULL;
    }
    if (!m_src) {
      m_src = cvCreateImage(cvSize(idat.width, idat.height), IPL_DEPTH_8U,
                            nchannels);
      m_dst = cvCreateImage(cvSize(idat.width, idat.height), IPL_DEPTH_8U,
                            nchannels);
      m_rotated.data.image.width = idat.width;
      m_rotated.data.image.height = idat.height;
      m_rotated.data.image.format = idat.format;
      m_rotated.data.image.raw_data.length(idat.width * idat.height *
                                           nchannels);
    }
    ...
    memcpy(m_rotated.data.image.raw_data.get_buffer(), m_dst->imageData,
           m_rotated.data.image.raw_data.length());
...
void RotateImageInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<RotateImage>,
