...
class CMapSceneNode : public irr::scene::ISceneNode {
 public:
  CMapSceneNode(ISceneNode *i_parent, ISceneManager *i_mgr, s32 i_id,
      ...
      : ISceneNode(i_parent, i_mgr, i_id), m_map(NULL) {
    // bounding box
    m_vertices[0] = vector3df(i_origin[0], -i_origin[1], i_origin[2]);
    m_vertices[1] =
        vector3df(i_origin[0] + i_size[0], -i_origin[1], i_origin[2]);
    m_vertices[2] = vector3df(i_origin[0] + i_size[0],
                              -(i_origin[1] + i_size[1]), i_origin[2]);
    m_vertices[3] =
        vector3df(i_origin[0], -(i_origin[1] + i_size[1]), i_origin[2]);

    m_vertices[4] = m_vertices[0];
    m_vertices[4].Z += i_size[2];
    m_vertices[5] = m_vertices[1];
    m_vertices[5].Z += i_size[2];
    m_vertices[6] = m_vertices[2];
    m_vertices[6].Z += i_size[2];
    m_vertices[7] = m_vertices[3];
    m_vertices[7].Z += i_size[2];
  ...
  void setupCubeVertices(double i_res) {
    ...
    // +z
    m_cubeVerts[0] =
        S3DVertex(-i_res / 2, -i_res / 2, i_res / 2, 0, 0, 1, white, 0, 0);
    m_cubeVerts[1] =
        S3DVertex(i_res / 2, -i_res / 2, i_res / 2, 0, 0, 1, white, 0, 0);
    m_cubeVerts[2] =
        S3DVertex(i_res / 2, i_res / 2, i_res / 2, 0, 0, 1, white, 0, 0);
    ...
    // +x
    m_cubeVerts[8] =
        S3DVertex(i_res / 2, -i_res / 2, i_res / 2, 1, 0, 0, white, 0, 0);
    m_cubeVerts[9] =
        S3DVertex(i_res / 2, -i_res / 2, -i_res / 2, 1, 0, 0, white, 0, 0);
    m_cubeVerts[10] =
        S3DVertex(i_res / 2, i_res / 2, -i_res / 2, 1, 0, 0, white, 0, 0);
    ...
    // +y
    m_cubeVerts[16] =
        S3DVertex(i_res / 2, i_res / 2, i_res / 2, 0, 1, 0, white, 0, 0);
    m_cubeVerts[17] =
        S3DVertex(i_res / 2, i_res / 2, -i_res / 2, 0, 1, 0, white, 0, 0);
    m_cubeVerts[18] =
        S3DVertex(-i_res / 2, i_res / 2, -i_res / 2, 0, 1, 0, white, 0, 0);
    ...
    for (int i = 0; i < m_map->nx; i++) {
      ...
      for (int j = 0; j < m_map->ny; j++) {
        ...
        for (int k = 0; k < m_map->nz; k++) {
          ...
          if (p != OpenHRP::gridUnknown && p != OpenHRP::gridEmpty) {
            driver->setTransform(ETS_WORLD, m);
            driver->drawIndexedTriangleList(m_cubeVerts, 24, m_cubeIndices, 12);
          }
        }
      }
    }
...
static const char *nullcomponent_spec[] = {
    "implementation_id", "OGMap3DViewer", "type_name", "OGMap3DViewer",
    ...
    "conf.default.generateImageSequence", "0", "conf.default.generateMovie",
    "0", "conf.default.xSize", "4", "conf.default.ySize", "4",
    "conf.default.zSize", "4", "conf.default.xOrigin", "0",
    "conf.default.yOrigin", "-2", "conf.default.zOrigin", "0",
...
RTC::ReturnCode_t OGMap3DViewer::onInitialize() {
  ...
  m_OGMap3DServicePort.registerConsumer("service1", "OGMap3DService",
                                        m_OGMap3DService);
...
RTC::ReturnCode_t OGMap3DViewer::onExecute(RTC::UniqueId ec_id) {
  ...
  if (!m_isopen) {
    ...
    m_mapNode =
        new CMapSceneNode(smgr->getRootSceneNode(), smgr, -1, origin, size);
  ...
  if (m_generateMovie) {
    if (!m_isGeneratingMovie) {
      ...
      fname += ".avi";
      m_videoWriter =
          cvCreateVideoWriter(fname.c_str(), CV_FOURCC('D', 'I', 'V', 'X'), 10,
                              cvSize(camera->width(), camera->height()));
      m_cvImage = cvCreateImage(cvSize(camera->width(), camera->height()),
                                IPL_DEPTH_8U, 3);
...
void OGMap3DViewerInit(RTC::Manager *manager) {
  ...
  manager->registerFactory(profile, RTC::Create<OGMap3DViewer>,
