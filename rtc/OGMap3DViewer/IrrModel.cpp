...
class MyEventReceiver : public IEventReceiver {
 public:
  MyEventReceiver(ICameraSceneNode *i_camera, double i_radius = 3.0)
      : m_camera(i_camera), m_radius(i_radius), m_pan(-M_PI / 6), m_tilt(0) {
    updateEye();
  }
...
class GLlink : public ISceneNode {
 public:
  GLlink(ISceneNode *i_parent, ISceneManager *i_mgr, s32 i_id,
      ...
      : ISceneNode(i_parent, i_mgr, i_id), m_jointId(i_li.jointId) {
    setAutomaticCulling(scene::EAC_OFF);

    setPosition(vector3df(i_li.translation[0], -i_li.translation[1],
                          i_li.translation[2]));
    Vector3 axis(i_li.rotation[0], i_li.rotation[1], i_li.rotation[2]);
    Matrix33 R;
    hrp::calcRodrigues(R, axis, i_li.rotation[3]);
    Vector3 rpy(rpyFromRot(R));
    // std::cout << "rpy:" << rpy << std::endl;
    setRotation(vector3df(-180 / M_PI * rpy[0], 180 / M_PI * rpy[1],
                          -180 / M_PI * rpy[2]));
    ...
    for (unsigned int l = 0; l < tsis.length(); l++) {
      ...
      // std::cout << "length of normals = " << ai.normals.length() <<
      // std::endl;
      const LongSequence &normalIndices = ai.normalIndices;
      // std::cout << "length of normalIndices = " << normalIndices.length() <<
      // std::endl;
      ...
      if (ai.colors.length()) {
        color.set(0xff, 0xff * ai.colors[0], 0xff * ai.colors[1],
                  0xff * ai.colors[2]);
      } else if (ai.materialIndex >= 0) {
        const MaterialInfo &mi = mis[ai.materialIndex];
        color.set(0xff, 0xff * mi.diffuseColor[0], 0xff * mi.diffuseColor[1],
                  0xff * mi.diffuseColor[2]);
      } else {
        std::cout << "no material" << std::endl;
      }

      SMeshBuffer *mb = reinterpret_cast<SMeshBuffer *>(
      ...
      if (ai.textureIndex >= 0) {
        ...
        // std::cout << "length of textureCoordinate:" <<
        // ai.textureCoordinate.length() << std::endl;
        // std::cout << "length of vertices:" << si.vertices.length() <<
        // std::endl;
      }
      ...
      for (int j = 0; j < numTriangles; ++j) {
        ...
        for (int k = 0; k < 3; ++k) {
          ...
          // std::cout << vertices[p] <<"," << vertices[p+1] << "," <<
          // vertices[p+2] << std::endl;
          vector2df texc;
          if (textureCoordinate) {
            texc.X = textureCoordinate[ai.textureCoordIndices[j * 3 + k] * 2];
            texc.Y =
                textureCoordinate[ai.textureCoordIndices[j * 3 + k] * 2 + 1];
          }
          // redundant vertices
      ...
      IMeshSceneNode *node =
          i_mgr->addMeshSceneNode(mesh, this, -1, pos, rpy, noscale);
    ...
    for (unsigned int i = 0; i < sensors.length(); i++) {
      ...
      if (type == "Vision") {
        ...
        camera->setPosition(vector3df(si.translation[0], -si.translation[1],
                                      si.translation[2]));
        Vector3 axis(si.rotation[0], si.rotation[1], si.rotation[2]);
        Matrix33 R;
        hrp::calcRodrigues(R, axis, si.rotation[3]);
        Vector3 rpy(rpyFromRot(R));
        camera->setRotation(vector3df(-180 / M_PI * rpy[0], 180 / M_PI * rpy[1],
                                      -180 / M_PI * rpy[2]));
  ...
  GLcamera *findCamera(const char *i_name) {
    for (unsigned int i = 0; i < m_cameraInfos.size(); i++) {
      if (strcmp(i_name, m_cameraInfos[i]->name()) == 0)
        return m_cameraInfos[i];
    }
    return NULL;
  }
...
GLbody::GLbody(ISceneNode *i_parent, ISceneManager *i_mgr, s32 i_id,
  ...
  for (unsigned int i = 0; i < lis->length(); i++) {
    m_links.push_back(
        new GLlink(i_mgr->getRootSceneNode(), i_mgr, -1, lis[i], i_binfo));
  }
...
void GLbody::setPosture(double *i_angles, double *i_pos, double *i_rpy) {
  ...
  m_root->setRotation(vector3df(-180 / M_PI * i_rpy[0], 180 / M_PI * i_rpy[1],
                                -180 / M_PI * i_rpy[2]));
  for (unsigned int i = 0; i < m_links.size(); i++) {
...
const char *GLcamera::name() { return m_node->getName(); }

int GLcamera::width() { return m_width; }

int GLcamera::height() { return m_height; }
...
void GLcamera::getAbsTransform(double *o_T) {
  ...
  Matrix33 R(
      rotFromRpy(-rpy.X * M_PI / 180, rpy.Y * M_PI / 180, -rpy.Z * M_PI / 180));
  o_T[0] = R(0, 0);
  o_T[4] = R(0, 1);
  o_T[8] = R(0, 2);
  o_T[12] = pos.X;
...
float GLcamera::near() { return m_near; }

float GLcamera::far() { return m_far; }

float GLcamera::fovy() { return m_fovy; }
...
bool GLscene::init(int w, int h) {
  m_device = createDevice(video::EDT_OPENGL, dimension2d<u32>(w, h), 32, false,
                          false, false, 0);
  ...
  smgr->addLightSceneNode(0, vector3df(18, -12, 6), SColorf(1.0, 1.0, 1.0),
                          30.0f);
  smgr->addLightSceneNode(0, vector3df(-18, 12, 6), SColorf(1.0, 1.0, 1.0),
                          30.0f);
...
GLcamera *GLscene::getCamera() { return m_camera; }

GLcamera::GLcamera(ISceneNode *i_node)
    : m_node(i_node),
      m_near(0.1),
      m_far(100.0),
      m_fovy(M_PI / 4),
...
IVideoDriver *GLscene::getVideoDriver() { return m_device->getVideoDriver(); }
