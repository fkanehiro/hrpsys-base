...
robot::robot(double dt)
    : m_fzLimitRatio(0),
      m_maxZmpError(DEFAULT_MAX_ZMP_ERROR),
      m_accLimit(0),
      m_calibRequested(false),
      m_pdgainsFilename("PDgains.sav"),
      m_reportedEmergency(true),
      m_dt(dt),
      m_enable_poweroff_check(false) {
...
bool robot::init() {
  ...
  if ((number_of_joints() != (int)numJoints()) ||
    ...
    std::cerr << "  joints:" << numJoints() << "(VRML), " << number_of_joints()
              << "(IOB)" << std::endl;
    std::cerr << "  force sensor:" << numSensors(Sensor::FORCE) << "(VRML), "
              << number_of_force_sensors() << "(IOB)" << std::endl;
    std::cerr << "  gyro sensor:" << numSensors(Sensor::RATE_GYRO) << "(VRML), "
              << number_of_gyro_sensors() << "(IOB)" << std::endl;
    std::cerr << "  accelerometer:" << numSensors(Sensor::ACCELERATION)
              << "(VRML), " << number_of_accelerometers() << "(IOB)"
              << std::endl;
    return false;
  }
  G << 0, 0, 9.8;
...
bool robot::loadGain() {
  ...
  for (unsigned int i = 0; i < numJoints(); i++) {
    std::cerr << "[RobotHardware]   " << joint(i)->name
              ...
              << ", dgain = " << default_dgain[i] << std::endl;
  }
  return true;
}
...
void robot::startInertiaSensorCalibration() {
  ...
  sem_wait(&wait_sem);
  std::cerr << "[RobotHardware] startInertiaSensorCalibration...done."
            << std::endl;
}
...
void robot::calibrateInertiaSensorOneStep() {
  if (inertia_calib_counter > 0) {
    for (unsigned int j = 0; j < numSensors(Sensor::RATE_GYRO); j++) {
      ...
      for (int i = 0; i < 3; i++) gyro_sum[j][i] += rate[i];
    }

    for (unsigned int j = 0; j < numSensors(Sensor::ACCELERATION); j++) {
      double acc[3];
      read_accelerometer(j, acc);
      for (int i = 0; i < 3; i++) accel_sum[j][i] += acc[i];
    }
...
void robot::calibrateForceSensorOneStep() {
  if (force_calib_counter > 0) {
    for (unsigned int j = 0; j < numSensors(Sensor::FORCE); j++) {
      ...
      for (int i = 0; i < 6; i++) force_sum[j][i] += force[i];
    }
    force_calib_counter--;
    if (force_calib_counter == 0) {
      for (unsigned int j = 0; j < numSensors(Sensor::FORCE); j++) {
...
void robot::gain_control(int i) {
  ...
  if (gain_counter[i] < GAIN_COUNT) {
    gain_counter[i]++;
    new_pgain =
        (pgain[i] - old_pgain[i]) * gain_counter[i] / GAIN_COUNT + old_pgain[i];
    new_dgain =
        (dgain[i] - old_dgain[i]) * gain_counter[i] / GAIN_COUNT + old_dgain[i];
    ...
    if (state == ON) {
      ...
      if (m_accLimit && fabs(a) > m_accLimit) {
        std::cerr << time_string()
                  ...
                  << joint(i)->name << ", alimit = " << m_accLimit / M_PI * 180
                  << "[deg/s^2], v = " << a / M_PI * 180 << "[deg/s^2]"
                  << std::endl;
        return true;
      }
    }
...
bool robot::checkEmergency(emg_reason &o_reason, int &o_id) {
  ...
  if (m_rLegForceSensorId >= 0) {
    ...
    if (force[FZ] > totalMass() * G(2) * m_fzLimitRatio) {
      std::cerr << time_string() << ": right Fz limit over: Fz = " << force[FZ]
                << std::endl;
      o_reason = EMG_FZ;
  ...
  if (m_lLegForceSensorId >= 0) {
    ...
    if (force[FZ] > totalMass() * G(2) * m_fzLimitRatio) {
      std::cerr << time_string() << ": left Fz limit over: Fz = " << force[FZ]
                << std::endl;
      o_reason = EMG_FZ;
  ...
  if (m_enable_poweroff_check) {
    ...
    for (unsigned int i = 0; i < numJoints(); i++) {
      ...
      if (!m_reportedEmergency && (pstate == OFF) && (sstate == ON)) {
        ...
        o_id = i;
        std::cerr << time_string()
                  << ": power off detected : joint = " << joint(i)->name
                  << std::endl;
        return true;
      }
    }
...
bool robot::setServoGainPercentage(const char *i_jname, double i_percentage) {
  if (i_percentage < 0 || 100 < i_percentage) {
    std::cerr << "[RobotHardware] Invalid percentage " << i_percentage
              << "[%] for setServoGainPercentage. Percentage should be in (0, "
                 "100)[%]."
              << std::endl;
    return false;
  }
  ...
  if (strcmp(i_jname, "all") == 0 || strcmp(i_jname, "ALL") == 0) {
    ...
    std::cerr << "[RobotHardware] setServoGainPercentage " << i_percentage
              << "[%] for all joints" << std::endl;
  } else if ((l = link(i_jname))) {
    if (!read_pgain(l->jointId, &old_pgain[l->jointId]))
      old_pgain[l->jointId] = pgain[l->jointId];
    pgain[l->jointId] = default_pgain[l->jointId] * i_percentage / 100.0;
    if (!read_dgain(l->jointId, &old_dgain[l->jointId]))
      old_dgain[l->jointId] = dgain[l->jointId];
    ...
    if (jgroup.size() == 0) return false;
    for (unsigned int i = 0; i < jgroup.size(); i++) {
      if (!read_pgain(jgroup[i], &old_pgain[jgroup[i]]))
        old_pgain[jgroup[i]] = pgain[jgroup[i]];
      pgain[jgroup[i]] = default_pgain[jgroup[i]] * i_percentage / 100.0;
      if (!read_dgain(jgroup[i], &old_dgain[jgroup[i]]))
        old_dgain[jgroup[i]] = dgain[jgroup[i]];
