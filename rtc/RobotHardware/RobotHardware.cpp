...
static const char* robothardware_spec[] = {
    "implementation_id", "RobotHardware", "type_name", "RobotHardware",
    "description", "RobotHardware", "version", HRPSYS_PACKAGE_VERSION, "vendor",
    "AIST", "category", "example", "activity_type", "DataFlowComponent",
    "max_instance", "1", "language", "C++", "lang_type", "compile",
    // Configuration variables
    "conf.default.isDemoMode", "0", "conf.default.fzLimitRatio", "2.0",
    "conf.default.servoErrorLimit", ",", "conf.default.jointAccelerationLimit",
    "0",
...
RTC::ReturnCode_t RobotHardware::onInitialize() {
  ...
  if (!loadBodyFromModelLoader(
    ...
    if (prop["model"] == "") {
      std::cerr << "[" << m_profile.instance_name
                << "] path to the model file is not defined. Please check the "
                   "configuration file"
                << std::endl;
    } else {
  ...
  for (unsigned int i = 0; i < m_rate.size(); i++) {
    ...
    m_rateOut[i] =
        new OutPort<TimedAngularVelocity3D>(s->name.c_str(), m_rate[i]);
...
RTC::ReturnCode_t RobotHardware::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_qRefIn.isNew()) {
    m_qRefIn.read();
    // std::cout << "RobotHardware: qRef[21] = " << m_qRef.data[21] <<
    // std::endl;
    if (!m_isDemoMode &&
  ...
  if (m_dqRefIn.isNew()) {
    m_dqRefIn.read();
    // std::cout << "RobotHardware: dqRef[21] = " << m_dqRef.data[21] <<
    // std::endl;
    // output to iob
  ...
  if (m_tauRefIn.isNew()) {
    m_tauRefIn.read();
    // std::cout << "RobotHardware: tauRef[21] = " << m_tauRef.data[21] <<
    // std::endl;
    // output to iob
  ...
  for (unsigned int i = 0; i < m_servoState.data.length(); i++) {
    ...
    m_robot->readExtraServoState(i,
                                 (int*)(m_servoState.data[i].get_buffer() + 1));
  }
  m_servoState.tm = tm;
...
void RobotHardwareInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<RobotHardware>,
