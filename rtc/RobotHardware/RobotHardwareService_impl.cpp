...
RobotHardwareService_impl::RobotHardwareService_impl()
    : m_robot(boost::shared_ptr<robot>()) {}
...
#define GetStatus                                                             \
  ...
  for (unsigned int i = 0; i < rs->servoState.length(); ++i) {                \
    ...
    m_robot->readExtraServoState(i,                                           \
                                 (int*)(rs->servoState[i].get_buffer() + 1)); \
  }                                                                           \
                                                                              \
...
void RobotHardwareService_impl::getStatus(
...
void RobotHardwareService_impl::getStatus2(
  ...
  for (unsigned int i = 0; i < rs->batteries.length(); i++) {
    m_robot->readBatteryState(i, rs->batteries[i].voltage,
                              rs->batteries[i].current, rs->batteries[i].soc);
  }
...
CORBA::Boolean RobotHardwareService_impl::power(
...
CORBA::Boolean RobotHardwareService_impl::servo(
    const char* jname, OpenHRP::RobotHardwareService::SwitchStatus ss) {
...
CORBA::Boolean RobotHardwareService_impl::addJointGroup(
    const char* gname,
...
CORBA::Boolean RobotHardwareService_impl::readDigitalInput(
...
CORBA::Boolean RobotHardwareService_impl::writeDigitalOutput(
    const ::OpenHRP::RobotHardwareService::OctSequence& dout) {
  return m_robot->writeDigitalOutput((const char*)(dout.get_buffer()));
}

CORBA::Boolean RobotHardwareService_impl::writeDigitalOutputWithMask(
    const ::OpenHRP::RobotHardwareService::OctSequence& dout,
    const ::OpenHRP::RobotHardwareService::OctSequence& mask) {
  return m_robot->writeDigitalOutputWithMask((const char*)(dout.get_buffer()),
...
CORBA::Boolean RobotHardwareService_impl::readDigitalOutput(
