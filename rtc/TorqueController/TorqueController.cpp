...
static const char* torquecontroller_spec[] = {
    "implementation_id", "TorqueController", "type_name", "TorqueController",
    "description", "Component for joint torque control", "version",
...
RTC::ReturnCode_t TorqueController::onInitialize() {
  ...
  m_TorqueControllerServicePort.registerProvider(
      "service0", "TorqueControllerService", m_service0);
  ...
  coil::vstring motorTorqueControllerParamsFromConf =
      coil::split(prop["torque_controller_params"], ",");
  // make controlle type map
  MotorTorqueController::motor_model_t model_type;
  std::map<int, MotorTorqueController::motor_model_t>
      param_num_to_motor_model_type;
  int tdc_params_num =
      TwoDofController::TwoDofControllerParam::getControllerParamNum() *
      m_robot->numJoints();
  int tdc_pd_model_params_num =
      TwoDofControllerPDModel::TwoDofControllerPDModelParam::
          getControllerParamNum() *
      m_robot->numJoints();
  int tdc_dynamics_model_params_num =
      TwoDofControllerDynamicsModel::TwoDofControllerDynamicsModelParam::
          getControllerParamNum() *
      m_robot->numJoints();
  param_num_to_motor_model_type[tdc_params_num] =
      MotorTorqueController::TWO_DOF_CONTROLLER;
  param_num_to_motor_model_type[tdc_pd_model_params_num] =
      MotorTorqueController::TWO_DOF_CONTROLLER_PD_MODEL;
  param_num_to_motor_model_type[tdc_dynamics_model_params_num] =
      MotorTorqueController::TWO_DOF_CONTROLLER_DYNAMICS_MODEL;
  if (param_num_to_motor_model_type.find(
          motorTorqueControllerParamsFromConf.size()) ==
      param_num_to_motor_model_type.end()) {
    std::cerr << "[" << m_profile.instance_name << "]"
              << "torque_controller_params is not correct number, "
              << motorTorqueControllerParamsFromConf.size()
              << ". Use default controller." << std::endl;
    model_type = MotorTorqueController::TWO_DOF_CONTROLLER;  // default
  } else {
    model_type =
        param_num_to_motor_model_type[motorTorqueControllerParamsFromConf
                                          .size()];
  }
  // define controller paramters
  switch (model_type) {
    case MotorTorqueController::
        TWO_DOF_CONTROLLER_DYNAMICS_MODEL:  // use TwoDofControllerDynamicsModel
    {  // limit scope of tdc_dynamics_model_params
      std::cerr << "[" << m_profile.instance_name << "]"
                << "use TwoDofControllerDynamicsModel" << std::endl;
      std::vector<
          TwoDofControllerDynamicsModel::TwoDofControllerDynamicsModelParam>
          tdc_dynamics_model_params(m_robot->numJoints());
      for (unsigned int i = 0; i < m_robot->numJoints(); i++) {
        if (motorTorqueControllerParamsFromConf.size() ==
            (unsigned int)tdc_dynamics_model_params_num) {  // use conf params
                                                            // if parameter num
                                                            // is correct
          coil::stringTo(tdc_dynamics_model_params[i].alpha,
                         motorTorqueControllerParamsFromConf[4 * i].c_str());
          coil::stringTo(
              tdc_dynamics_model_params[i].beta,
              motorTorqueControllerParamsFromConf[4 * i + 1].c_str());
          coil::stringTo(
              tdc_dynamics_model_params[i].ki,
              motorTorqueControllerParamsFromConf[4 * i + 2].c_str());
          coil::stringTo(
              tdc_dynamics_model_params[i].tc,
              motorTorqueControllerParamsFromConf[4 * i + 3].c_str());
        }
        tdc_dynamics_model_params[i].dt = m_dt;
        m_motorTorqueControllers.push_back(MotorTorqueController(
            m_robot->joint(i)->name, tdc_dynamics_model_params[i]));
      }
      if (m_debugLevel > 0) {
        std::cerr << "[" << m_profile.instance_name << "]"
        ...
        for (unsigned int i = 0; i < m_robot->numJoints(); i++) {
          std::cerr << "[" << m_profile.instance_name << "]"
                    ...
                    << tdc_dynamics_model_params[i].alpha << " "
                    << tdc_dynamics_model_params[i].beta << " "
                    << tdc_dynamics_model_params[i].ki << " "
                    << tdc_dynamics_model_params[i].tc << " "
                    << tdc_dynamics_model_params[i].dt << std::endl;
        }
      }
      break;
    }
    case MotorTorqueController::
        TWO_DOF_CONTROLLER_PD_MODEL:  // use TwoDofControllerPDModel
    {                                 // limit scope of tdc_pd_model_params
      std::cerr << "[" << m_profile.instance_name << "]"
                << "use TwoDofControllerPDModel" << std::endl;
      std::vector<TwoDofControllerPDModel::TwoDofControllerPDModelParam>
          tdc_pd_model_params(m_robot->numJoints());
      for (unsigned int i = 0; i < m_robot->numJoints(); i++) {
        if (motorTorqueControllerParamsFromConf.size() ==
            (unsigned int)tdc_pd_model_params_num) {  // use conf params if
                                                      // parameter num is
                                                      // correct
          coil::stringTo(tdc_pd_model_params[i].ke,
                         motorTorqueControllerParamsFromConf[3 * i].c_str());
          coil::stringTo(
              tdc_pd_model_params[i].kd,
              motorTorqueControllerParamsFromConf[3 * i + 1].c_str());
          coil::stringTo(
              tdc_pd_model_params[i].tc,
              motorTorqueControllerParamsFromConf[3 * i + 2].c_str());
        }
        tdc_pd_model_params[i].dt = m_dt;
        m_motorTorqueControllers.push_back(MotorTorqueController(
            m_robot->joint(i)->name, tdc_pd_model_params[i]));
      }
      if (m_debugLevel > 0) {
        std::cerr << "[" << m_profile.instance_name << "]"
                  << "torque controller parames:" << std::endl;
        for (unsigned int i = 0; i < m_robot->numJoints(); i++) {
          std::cerr << "[" << m_profile.instance_name << "]"
                    << m_robot->joint(i)->name << ":"
                    << tdc_pd_model_params[i].ke << " "
                    << tdc_pd_model_params[i].kd << " "
                    << tdc_pd_model_params[i].tc << " "
                    << tdc_pd_model_params[i].dt << std::endl;
        }
      }
      break;
    }
    case MotorTorqueController::TWO_DOF_CONTROLLER:  // use TwoDofController
    default: {  // limit scope of tdc_params
      std::cerr << "[" << m_profile.instance_name << "]"
                << "use TwoDofController" << std::endl;
      std::vector<TwoDofController::TwoDofControllerParam> tdc_params(
          m_robot->numJoints());
      for (unsigned int i = 0; i < m_robot->numJoints(); i++) {
        if (motorTorqueControllerParamsFromConf.size() ==
            (unsigned int)tdc_params_num) {  // use conf params if parameter num
                                             // is correct
          coil::stringTo(tdc_params[i].ke,
                         motorTorqueControllerParamsFromConf[2 * i].c_str());
          coil::stringTo(
              tdc_params[i].tc,
              motorTorqueControllerParamsFromConf[2 * i + 1].c_str());
        }
        tdc_params[i].dt = m_dt;
        m_motorTorqueControllers.push_back(
            MotorTorqueController(m_robot->joint(i)->name, tdc_params[i]));
      }
      if (m_debugLevel > 0) {
        std::cerr << "[" << m_profile.instance_name << "]"
                  << "torque controller parames:" << std::endl;
        for (unsigned int i = 0; i < m_robot->numJoints(); i++) {
          std::cerr << "[" << m_profile.instance_name << "]"
                    << m_robot->joint(i)->name << ":" << tdc_params[i].ke << " "
                    << tdc_params[i].tc << " " << tdc_params[i].dt << std::endl;
        }
      }
      break;
    }
  ...
  coil::vstring dqMinMaxFromConf =
      coil::split(prop["torque_controler_min_max_dq"], ",");
  if (dqMinMaxFromConf.size() == 2 * m_robot->numJoints()) {
    dq_min_max_from_conf_is_valid = true;
  } else {
    std::cerr << "[" << m_profile.instance_name << "]"
              << "size of torque_controller_min_max_dq "
              << dqMinMaxFromConf.size() << " is not correct number "
              << 2 * m_robot->numJoints() << ". Use default values."
              << std::endl;
    dq_min_max_from_conf_is_valid = false;
  }
  for (unsigned int i = 0; i < m_robot->numJoints(); i++) {
    m_motorTorqueControllers[i].setErrorPrefix(
        std::string(m_profile.instance_name));
    m_motorTorqueControllers[i].setupMotorControllerTransitionMinMaxDq(
        m_robot->joint(i)->lvlimit * m_dt, m_robot->joint(i)->uvlimit * m_dt);
    if (dq_min_max_from_conf_is_valid) {
      ...
      m_motorTorqueControllers[i].setupMotorControllerControlMinMaxDq(
          tmp_dq_min, tmp_dq_max);
    }
  }
  ...
  if (m_qRefIn.data.length() == m_robot->numJoints()) {
    if (m_tauCurrentIn.data.length() == m_robot->numJoints() &&
      ...
      for (unsigned int i = 0; i < m_robot->numJoints(); i++) {
        if (m_motorTorqueControllers[i].isEnabled()) {
          m_qRefOut.data[i] = std::min(
              std::max(m_qRefIn.data[i] + dq[i], m_robot->joint(i)->llimit),
              m_robot->joint(i)->ulimit);
        } else {
          m_qRefOut.data[i] =
              m_qRefIn.data[i];  // pass joint angle when controller is disabled
        }
      }
      ...
      if (isDebug()) {
        ...
        std::cerr << "[" << m_profile.instance_name << "]"
                  << " numJoints: " << m_robot->numJoints() << std::endl;
        std::cerr << "[" << m_profile.instance_name << "]"
                  << "  qCurrent: " << m_qCurrentIn.data.length() << std::endl;
        std::cerr << "[" << m_profile.instance_name << "]"
                  << "tauCurrent: " << m_tauCurrentIn.data.length()
                  << std::endl;
        std::cerr << std::endl;
      }
    ...
    if (isDebug()) {
      ...
      std::cerr << "[" << m_profile.instance_name << "]"
                << " numJoints: " << m_robot->numJoints() << std::endl;
      std::cerr << "[" << m_profile.instance_name << "]"
                << "    qRefIn: " << m_qRefIn.data.length() << std::endl;
      std::cerr << std::endl;
    }
  }
...
void TorqueController::executeTorqueControl(hrp::dvector& dq) {
  ...
  for (unsigned int i = 0; i < numJoints; i++) {
    double tauMaxFromModel = m_robot->joint(i)->climit *
                             m_robot->joint(i)->gearRatio *
  ...
  Guard guard(m_mutex);
  for (unsigned int i = 0; i < numJoints; i++) {
    dq[i] = m_motorTorqueControllers[i].execute(
        m_tauCurrentIn.data[i],
        tauMax[i]);  // twoDofController: tau = -K(q - qRef)
    // output debug message
    if (isDebug() &&
        m_motorTorqueControllers[i].getMotorControllerState() !=
            MotorTorqueController::INACTIVE) {
...
bool TorqueController::enableTorqueController(std::string jname) {
  bool succeed = false;
  for (std::vector<MotorTorqueController>::iterator it =
           m_motorTorqueControllers.begin();
       it != m_motorTorqueControllers.end(); ++it) {
...
bool TorqueController::enableMultipleTorqueControllers(
...
bool TorqueController::disableMultipleTorqueControllers(
    const OpenHRP::TorqueControllerService::StrSequence& jnames) {
  bool succeed = true;
  bool retval;
  for (unsigned int i = 0; i < jnames.length(); i++) {
    ...
    if ((*it).getJointName() == jname) {
      ...
      if (!(*it).isEnabled()) {
        ...
        if (!succeed) {
          if (m_debugLevel > 0) {
            std::cerr << "[" << m_profile.instance_name << "]"
                      << "Failed to enable torque control in " << jname
                      << std::endl;
          }
          return succeed;
        }
...
bool TorqueController::startMultipleTorqueControls(
    const OpenHRP::TorqueControllerService::StrSequence& jnames) {
  bool succeed = true;
  bool retval;
  for (unsigned int i = 0; i < jnames.length(); i++) {
...
bool TorqueController::stopMultipleTorqueControls(
    const OpenHRP::TorqueControllerService::StrSequence& jnames) {
  bool succeed = true;
  bool retval;
  for (unsigned int i = 0; i < jnames.length(); i++) {
...
bool TorqueController::setReferenceTorque(std::string jname, double tauRef) {
  ...
  for (std::vector<MotorTorqueController>::iterator it =
    ...
    if ((*it).getJointName() == jname) {
      if (m_debugLevel > 0) {
        std::cerr << "[" << m_profile.instance_name << "]"
                  << "Set " << jname << " reference torque to " << tauRef
                  << std::endl;
      }
...
bool TorqueController::setMultipleReferenceTorques(
    const OpenHRP::TorqueControllerService::StrSequence& jnames,
...
bool TorqueController::setTorqueControllerParam(
  ...
  if (tgt_controller == NULL) {
    std::cerr << "[" << m_profile.instance_name << "]" << jname
  ...
  bool retval;
  MotorTorqueController::motor_model_t model_type =
      tgt_controller->getMotorModelType();
  switch (model_type) {  // dt is defined by controller cycle
    case MotorTorqueController::TWO_DOF_CONTROLLER: {  // limit scope for param
      std::cerr << "[" << m_profile.instance_name << "]"
                << "new param:" << i_param.ke << " " << i_param.tc << " "
                << std::endl;
    ...
    case MotorTorqueController::TWO_DOF_CONTROLLER_PD_MODEL: {  // limit scope
                                                                // for param
      std::cerr << "[" << m_profile.instance_name << "]"
                << "new param:" << i_param.ke << " " << i_param.kd << " "
                << i_param.tc << " " << std::endl;
    ...
    case MotorTorqueController::TWO_DOF_CONTROLLER_DYNAMICS_MODEL: {  // limit
                                                                      // scope
                                                                      // for
                                                                      // param
      std::cerr << "[" << m_profile.instance_name << "]"
                << "new param:" << i_param.alpha << " " << i_param.beta << " "
                << i_param.ki << " " << i_param.tc << " " << std::endl;
      TwoDofControllerDynamicsModel::TwoDofControllerDynamicsModelParam param;
      param.alpha = i_param.alpha;
      param.beta = i_param.beta;
      param.ki = i_param.ki;
      param.tc = i_param.tc;
      param.dt = m_dt;
      retval = tgt_controller->updateControllerParam(param);
...
bool TorqueController::getTorqueControllerParam(
    const std::string jname,
...
void TorqueControllerInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<TorqueController>,
