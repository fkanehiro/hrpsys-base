...
TwoDofControllerPDModel::TwoDofControllerPDModel() {
  param =
      TwoDofControllerPDModel::TwoDofControllerPDModelParam();  // use default
                                                                // constructor
  current_time = 0;
  convolutions.clear();
...
bool TwoDofControllerPDModel::getParameter() { return false; }

bool TwoDofControllerPDModel::getParameter(
    TwoDofControllerPDModel::TwoDofControllerPDModelParam &_p) {
  _p.ke = param.ke;
  _p.kd = param.kd;
  _p.tc = param.tc;
  _p.dt = param.dt;
...
double TwoDofControllerPDModel::update(double _x, double _xd) {
  ...
  if (!param.ke || !param.kd || !param.tc || !param.dt) {
    std::cerr << "[" << error_prefix << "]"
              << "TwoDofControllerPDModel parameters are not set." << std::endl;
    std::cerr << "[" << error_prefix << "]"
              << "ke: " << param.ke << ", kd: " << param.kd
  ...
  convolutions[1].update(std::exp((param.ke / param.kd) * current_time),
                         _xd - _x);
  convolutions[2].update(1 - std::exp((param.ke / param.kd) * current_time),
                         _xd - _x);
