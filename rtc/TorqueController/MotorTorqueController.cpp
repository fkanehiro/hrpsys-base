...
#define DEFAULT_MIN_MAX_TRANSITION_DQ \
  (0.17 * m_dt)  // default min/max is 10[deg/sec] = 0.17[rad/sec]
...
MotorTorqueController::MotorTorqueController(
  ...
  setupMotorControllerTransitionMinMaxDq(-DEFAULT_MIN_MAX_TRANSITION_DQ,
                                         DEFAULT_MIN_MAX_TRANSITION_DQ);
}

void MotorTorqueController::setupController(
...
bool MotorTorqueController::getControllerParam(
    TwoDofController::TwoDofControllerParam &_param) {
  if (m_motor_model_type == TWO_DOF_CONTROLLER) {
    bool retval;
    retval = m_normalController.getTwoDofControllerParam(
        _param);  // assuming normalController and emergencyController has same
                  // parameters
    return retval;
  } else {
...
bool MotorTorqueController::updateControllerParam(
    TwoDofController::TwoDofControllerParam &_param) {
  if (m_motor_model_type == TWO_DOF_CONTROLLER) {
    bool retval;
    retval = m_normalController.updateTwoDofControllerParam(_param);
    retval =
        m_emergencyController.updateTwoDofControllerParam(_param) && retval;
    return retval;
  } else {
    std::cerr << "motor model type is not TwoDofController" << std::endl;
    return false;
...
bool MotorTorqueController::getControllerParam(
  ...
  if (m_motor_model_type == TWO_DOF_CONTROLLER_PD_MODEL) {
    bool retval;
    retval = m_normalController.updateTwoDofControllerPDModelParam(
        _param);  // assuming normalController and emergencyController has same
                  // parameters
    return retval;
  } else {
    std::cerr << "[" << m_error_prefix << "]"
...
bool MotorTorqueController::updateControllerParam(
  ...
  if (m_motor_model_type == TWO_DOF_CONTROLLER_PD_MODEL) {
    ...
    retval = m_emergencyController.updateTwoDofControllerPDModelParam(_param) &&
             retval;
    return retval;
  } else {
    std::cerr << "[" << m_error_prefix << "]"
...
bool MotorTorqueController::getControllerParam(
  ...
  if (m_motor_model_type == TWO_DOF_CONTROLLER_DYNAMICS_MODEL) {
    bool retval;
    retval = m_normalController.getTwoDofControllerDynamiccsModelParam(
        _param);  // assuming normalController and emergencyController has same
                  // parameters
    return retval;
  } else {
    std::cerr << "[" << m_error_prefix << "]"
              << "motor model type is not TwoDofControllerDynamicsModel"
              << std::endl;
    return false;
  }
}
...
bool MotorTorqueController::updateControllerParam(
  ...
  if (m_motor_model_type == TWO_DOF_CONTROLLER_DYNAMICS_MODEL) {
    bool retval;
    retval =
        m_normalController.updateTwoDofControllerDynamiccsModelParam(_param);
    retval = m_emergencyController.updateTwoDofControllerDynamiccsModelParam(
                 _param) &&
             retval;
    return retval;
  } else {
...
bool MotorTorqueController::disable(void) {
  ...
  if (m_normalController.state != INACTIVE) {
    std::cerr << "[" << m_error_prefix << "]"
              << "Normal torque control in " << m_joint_name << " is active"
              << std::endl;
    retval = false;
  } else if (m_emergencyController.state != INACTIVE) {
    std::cerr << "[" << m_error_prefix << "]"
              << "Emergency torque control in " << m_joint_name << " is active"
              << std::endl;
    retval = false;
  } else {
    m_enable_flag = false;
...
void MotorTorqueController::setupMotorControllerControlMinMaxDq(
...
void MotorTorqueController::setupMotorControllerTransitionMinMaxDq(
...
bool MotorTorqueController::activate(void) {
  ...
  if (m_normalController.state == INACTIVE) {
    ...
    std::cerr << "[ERROR] Torque control in " << m_joint_name
...
double MotorTorqueController::execute(double _tau, double _tauMax) {
  ...
  if (std::abs(_tau) > std::abs(_tauMax)) {
    if (m_emergencyController.state != ACTIVE) {
      ...
      if (m_emergencyController.state != INACTIVE) {
        m_emergencyController.transition_dq =
            m_emergencyController.getMotorControllerDq();
      } else if (m_normalController.state != INACTIVE) {
        m_emergencyController.transition_dq =
            m_normalController.getMotorControllerDq();
      }
    ...
    if (m_emergencyController.state == ACTIVE &&
      ...
      if (m_normalController.state !=
          INACTIVE) {  // take control over normal process
        m_normalController.transition_dq =
            m_emergencyController.getMotorControllerDq();
  ...
  if (m_emergencyController.state !=
      INACTIVE) {  // overwrite by tauMax control when emergency mode
#if BOOST_VERSION >= 103500
...
std::string MotorTorqueController::getJointName(void) { return m_joint_name; }

MotorTorqueController::controller_state_t
MotorTorqueController::getMotorControllerState(void) {
  if (m_emergencyController.state == INACTIVE) {
...
bool MotorTorqueController::isEnabled(void) { return m_enable_flag; }
...
void MotorTorqueController::printMotorControllerVariables(void) {
  ...
  prefix += m_joint_name + ".";
  std::cerr << prefix << "normalController.state:" << m_normalController.state
            << std::endl;
  std::cerr << prefix << "normalController.dq:"
            << m_normalController.getMotorControllerDq() << std::endl;
  std::cerr << prefix
            << "emergencyController.state:" << m_emergencyController.state
            << std::endl;
  std::cerr << prefix << "emergencyController.dq:"
            << m_emergencyController.getMotorControllerDq() << std::endl;
...
MotorTorqueController::motor_model_t MotorTorqueController::getMotorModelType(
    void) {
...
void MotorTorqueController::resetMotorControllerVariables(
...
void MotorTorqueController::prepareStop(
    MotorTorqueController::MotorController &_mc) {
  // angle difference to be recoverd
  _mc.transition_dq = _mc.getMotorControllerDq();

  // determine transition in 1 cycle
  _mc.recovery_dq = std::min(
      std::max(_mc.transition_dq / MAX_TRANSITION_COUNT, _mc.min_transition_dq),
      _mc.max_transition_dq);  // transition in 1 cycle
  std::cerr << _mc.recovery_dq << std::endl;

  _mc.dq = 0;  // dq must be reseted after recovery_dq setting(used in
               // getMotoroControllerDq)
  _mc.state = STOP;
  return;
}

void MotorTorqueController::updateController(
    double _tau, double _tauRef, MotorTorqueController::MotorController &_mc) {
  switch (_mc.state) {
    case ACTIVE:
...
void MotorTorqueController::MotorController::setupTwoDofController(
    TwoDofController::TwoDofControllerParam &_param) {
  controller.reset(new TwoDofController(_param));
  controller->reset();
}

bool MotorTorqueController::MotorController::getTwoDofControllerParam(
    TwoDofController::TwoDofControllerParam &_param) {
  if (typeid(*controller) != typeid(TwoDofController) ||
      boost::dynamic_pointer_cast<TwoDofController>(controller) == NULL) {
    std::cerr << "[" << error_prefix << "]"
              << "incorrect controller type: TwoDofController" << std::endl;
    return false;
  }
  TwoDofController::TwoDofControllerParam param;
  (boost::dynamic_pointer_cast<TwoDofController>(controller))
      ->getParameter(param);
...
bool MotorTorqueController::MotorController::updateTwoDofControllerParam(
    TwoDofController::TwoDofControllerParam &_param) {
  if (typeid(*controller) != typeid(TwoDofController) ||
      boost::dynamic_pointer_cast<TwoDofController>(controller) == NULL) {
    std::cerr << "[" << error_prefix << "]"
  ...
  // update parameters which are not 0 using updateParam (parameter is not
  // updated when _param is 0)
  TwoDofController::TwoDofControllerParam param;
  (boost::dynamic_pointer_cast<TwoDofController>(controller))
      ->getParameter(param);
...
void MotorTorqueController::MotorController::setupTwoDofControllerPDModel(
    TwoDofControllerPDModel::TwoDofControllerPDModelParam &_param) {
  controller.reset(new TwoDofControllerPDModel(_param));
  controller->reset();
}

bool MotorTorqueController::MotorController::getTwoDofControllerPDModelParam(
    TwoDofControllerPDModel::TwoDofControllerPDModelParam &_param) {
  if (typeid(*controller) != typeid(TwoDofControllerPDModel) ||
      boost::dynamic_pointer_cast<TwoDofControllerPDModel>(controller) ==
          NULL) {
    std::cerr << "[" << error_prefix << "]"
              << "incorrect controller type: TwoDofControllerPDModel"
              << std::endl;
    return false;
  }
  // update parameters which are not 0 using updateParam (parameter is not
...
bool MotorTorqueController::MotorController::updateTwoDofControllerPDModelParam(
    TwoDofControllerPDModel::TwoDofControllerPDModelParam &_param) {
  if (typeid(*controller) != typeid(TwoDofControllerPDModel) ||
      boost::dynamic_pointer_cast<TwoDofControllerPDModel>(controller) ==
          NULL) {
...
void MotorTorqueController::MotorController::setupTwoDofControllerDynamicsModel(
    TwoDofControllerDynamicsModel::TwoDofControllerDynamicsModelParam &_param) {
  controller.reset(new TwoDofControllerDynamicsModel(_param));
  controller->reset();
}

bool MotorTorqueController::MotorController::
    getTwoDofControllerDynamiccsModelParam(
        TwoDofControllerDynamicsModel::TwoDofControllerDynamicsModelParam
            &_param) {
  if (typeid(*controller) != typeid(TwoDofControllerDynamicsModel) ||
      boost::dynamic_pointer_cast<TwoDofControllerDynamicsModel>(controller) ==
          NULL) {
    std::cerr << "[" << error_prefix << "]"
              << "incorrect controller type: TwoDofControllerDynamicsModel"
              << std::endl;
    return false;
  }
...
bool MotorTorqueController::MotorController::updateParam(
    double &_param, const double &_new_value) {
  if (_new_value != 0) {  // update parameter if given value is not 0 (new_value
                          // = 0 express holding existent parameter)
    _param = _new_value;
    return true;
  }
  return false;
...
void MotorTorqueController::MotorController::setErrorPrefix(
...
double MotorTorqueController::MotorController::getMotorControllerDq(void) {
  ...
  switch (state) {
    case ACTIVE:
      ret_dq = dq + transition_dq;  // if contorller interrupt its transition,
                                    // base joint angle is not qRef, qRef +
                                    // transition_dq
      break;
    case STOP:
