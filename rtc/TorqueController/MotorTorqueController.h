...
class MotorTorqueController {
 public:
  ...
  MotorTorqueController(std::string _jname,
  ...
  void setupController(
      TwoDofControllerPDModel::TwoDofControllerPDModelParam &_param);
  bool getControllerParam(
      TwoDofControllerPDModel::TwoDofControllerPDModelParam &_param);
  bool updateControllerParam(
      TwoDofControllerPDModel::TwoDofControllerPDModelParam &_param);
  ...
  bool enable(void);   // enable torque controller (normal controller is not
                       // activated but emergency toruqe control may be
                       // activated)
  bool disable(void);  // disable torque controller (emergency controller is
                       // also ignored)

  // for normal torque controller
  void setupMotorControllerControlMinMaxDq(
      double _min_dq, double _max_dq);  // set min/max dq for control
  void setupMotorControllerTransitionMinMaxDq(
      double _min_transition_dq,
      double _max_transition_dq);  // set min/max dq for transition
  ...
  bool setReferenceTorque(
      double _tauRef);  // set reference torque (does not activate controller)

  double execute(double _tau, double _tauMax);  // determine final state and
                                                // tauRef, then throw tau,
                                                // tauRef and state to
                                                // executeControl
 ...
 private:
  class MotorController {
   public:
    ...
    double dq;  // difference of joint angle from base(qRef) from tdc. it is
                // calcurated by dq = integrate(qd * dt), dq*dt is output of tdc
    double transition_dq;  // for transition. first value is last difference of
                           // joint angle from qRef (dq + transition_dq) when
                           // state was changed to STOP
    ...
    bool getTwoDofControllerParam(
        TwoDofController::TwoDofControllerParam &_param);
    bool updateTwoDofControllerParam(
        TwoDofController::TwoDofControllerParam &_param);
    // for TwoDofControllerPDModel
    void setupTwoDofControllerPDModel(
        TwoDofControllerPDModel::TwoDofControllerPDModelParam &_param);
    bool getTwoDofControllerPDModelParam(
        TwoDofControllerPDModel::TwoDofControllerPDModelParam &_param);
    bool updateTwoDofControllerPDModelParam(
        TwoDofControllerPDModel::TwoDofControllerPDModelParam &_param);
    // for TwoDofControllerDynamicsModel
    void setupTwoDofControllerDynamicsModel(
        TwoDofControllerDynamicsModel::TwoDofControllerDynamicsModelParam
            &_param);
    bool getTwoDofControllerDynamiccsModelParam(
        TwoDofControllerDynamicsModel::TwoDofControllerDynamicsModelParam
            &_param);
    bool updateTwoDofControllerDynamiccsModelParam(
        TwoDofControllerDynamicsModel::TwoDofControllerDynamicsModelParam
            &_param);
   ...
   private:
    bool updateParam(
        double &_param,
        const double &_new_value);  // update param if new_value is acceptable
    std::string error_prefix;
  };

  // internal functions
  void setupControllerCommon(std::string _jname, double _dt);
  void resetMotorControllerVariables(
      MotorController &_mc);  // reset internal torque control parameter
  void prepareStop(MotorController &_mc);
  void updateController(double _tau, double _tauRef,
                        MotorController &_mc);  // execute control and update
                                                // controller member valiables
  ...
  double m_actual_tauRef;  // reference tau which is limited or overwritten by
                           // emergency (mainly for debug message)
  MotorController m_normalController;  // substance of two dof controller
  MotorController
      m_emergencyController;   // overwrite normal controller when emergency
