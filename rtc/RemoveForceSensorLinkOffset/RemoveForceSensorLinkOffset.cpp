...
static const char* removeforcesensorlinkoffset_spec[] = {
    "implementation_id", "RemoveForceSensorLinkOffset", "type_name",
    "RemoveForceSensorLinkOffset", "description", "null component", "version",
...
RTC::ReturnCode_t RemoveForceSensorLinkOffset::onInitialize() {
  ...
  m_RemoveForceSensorLinkOffsetServicePort.registerProvider(
      "service0", "RemoveForceSensorLinkOffsetService", m_service0);
  ...
  for (unsigned int i = 0; i < nforce; i++) {
    ...
    m_forceOut[i] = new OutPort<TimedDoubleSeq>(
        std::string("off_" + s->name).c_str(), m_force[i]);
    ...
    m_forcemoment_offset_param.insert(
        std::pair<std::string, ForceMomentOffsetParam>(
            s->name, ForceMomentOffsetParam()));
  }
  max_sensor_offset_calib_counter =
      static_cast<int>(8.0 / m_dt);  // 8.0[s] by default
  return RTC::RTC_OK;
}
...
RTC::ReturnCode_t RemoveForceSensorLinkOffset::onActivated(
    RTC::UniqueId ec_id) {
...
RTC::ReturnCode_t RemoveForceSensorLinkOffset::onDeactivated(
    RTC::UniqueId ec_id) {
    ...
    for (unsigned int i = 0; i < m_forceIn.size(); i++) {
      if (m_force[i].data.length() == 6) {
        ...
        hrp::ForceSensor* sensor =
            m_robot->sensor<hrp::ForceSensor>(sensor_name);
        hrp::Vector3 data_p(m_force[i].data[0], m_force[i].data[1],
                            m_force[i].data[2]);
        hrp::Vector3 data_r(m_force[i].data[3], m_force[i].data[4],
                            m_force[i].data[5]);
        if (DEBUGP) {
          std::cerr << "[" << m_profile.instance_name << "] wrench ["
                    << m_forceIn[i]->name() << "]" << std::endl;
          ;
          std::cerr << "[" << m_profile.instance_name << "]   raw force = "
                    << data_p.format(Eigen::IOFormat(Eigen::StreamPrecision, 0,
                                                     ", ", ", ", "", "", "[",
                                                     "]"))
                    << std::endl;
          std::cerr << "[" << m_profile.instance_name << "]   raw moment = "
        ...
        if (sensor) {
          ...
          hrp::Vector3 mg =
              hrp::Vector3(0, 0, fmp.link_offset_mass * grav * -1);
          hrp::Vector3 cxmg =
              hrp::Vector3(sensorR * fmp.link_offset_centroid).cross(mg);
          // Sensor offset calib
          if (fmp.sensor_offset_calib_counter > 0) {  // while calibrating
            ...
            if (fmp.sensor_offset_calib_counter == 0) {
              fmp.force_offset =
                  fmp.force_offset_sum / max_sensor_offset_calib_counter;
              fmp.moment_offset =
                  fmp.moment_offset_sum / max_sensor_offset_calib_counter;
          ...
          if (DEBUGP) {
            std::cerr << "[" << m_profile.instance_name << "]   off force = "
                      << fmp.off_force.format(
                             Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ",
                                             ", ", "", "", "[", "]"))
                      << std::endl;
            std::cerr << "[" << m_profile.instance_name << "]   off moment = "
                      << fmp.off_moment.format(
                             Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ",
                                             ", ", "", "", "[", "]"))
                      << std::endl;
          }
...
void RemoveForceSensorLinkOffset::updateRootLinkPosRot(
  ...
  if (m_robot->numSensors(hrp::Sensor::ACCELERATION) > 0) {
    ...
    rats::rotm3times(
        ...
        m_robot->rootLink()->R);
    rats::rotm3times(m_robot->rootLink()->R,
                     hrp::rotFromRpy(rpy(0), rpy(1), rpy(2)), tmpr);
  }
}

void RemoveForceSensorLinkOffset::printForceMomentOffsetParam(
    const std::string& i_name_) {
  std::cerr << "[" << m_profile.instance_name << "]   force_offset = "
            << m_forcemoment_offset_param[i_name_].force_offset.format(
                   Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "",
                                   "", "[", "]"))
            << "[N]" << std::endl;
  std::cerr << "[" << m_profile.instance_name << "]   moment_offset = "
            << m_forcemoment_offset_param[i_name_].moment_offset.format(
                   Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "",
                                   "", "[", "]"))
            << "[Nm]" << std::endl;
  std::cerr << "[" << m_profile.instance_name << "]   link_offset_centroid = "
            << m_forcemoment_offset_param[i_name_].link_offset_centroid.format(
                   Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ", "",
                                   "", "[", "]"))
            << "[m]" << std::endl;
  std::cerr << "[" << m_profile.instance_name << "]   link_offset_mass = "
            << m_forcemoment_offset_param[i_name_].link_offset_mass << "[kg]"
            << std::endl;
};

bool RemoveForceSensorLinkOffset::setForceMomentOffsetParam(
    const std::string& i_name_,
    const RemoveForceSensorLinkOffsetService::forcemomentOffsetParam&
        i_param_) {
  std::cerr << "[" << m_profile.instance_name << "] setForceMomentOffsetParam ["
            << i_name_ << "]" << std::endl;
  if (m_forcemoment_offset_param.find(i_name_) !=
      m_forcemoment_offset_param.end()) {
    memcpy(m_forcemoment_offset_param[i_name_].force_offset.data(),
           i_param_.force_offset.get_buffer(), sizeof(double) * 3);
    memcpy(m_forcemoment_offset_param[i_name_].moment_offset.data(),
           i_param_.moment_offset.get_buffer(), sizeof(double) * 3);
    ...
    m_forcemoment_offset_param[i_name_].link_offset_mass =
        i_param_.link_offset_mass;
    printForceMomentOffsetParam(i_name_);
    return true;
  } else {
    std::cerr << "[" << m_profile.instance_name << "]   No such limb"
              << std::endl;
    return false;
  }
}

bool RemoveForceSensorLinkOffset::getForceMomentOffsetParam(
    const std::string& i_name_,
    RemoveForceSensorLinkOffsetService::forcemomentOffsetParam& i_param_) {
  if (m_forcemoment_offset_param.find(i_name_) !=
      m_forcemoment_offset_param.end()) {
    // std::cerr << "OK " << i_name_ << " in getForceMomentOffsetParam" <<
    // std::endl;
    memcpy(i_param_.force_offset.get_buffer(),
           m_forcemoment_offset_param[i_name_].force_offset.data(),
           sizeof(double) * 3);
    memcpy(i_param_.moment_offset.get_buffer(),
           m_forcemoment_offset_param[i_name_].moment_offset.data(),
           sizeof(double) * 3);
    ...
    std::cerr << "[" << m_profile.instance_name << "] No such limb " << i_name_
              << " in getForceMomentOffsetParam" << std::endl;
    return false;
  }
}

bool RemoveForceSensorLinkOffset::loadForceMomentOffsetParams(
    const std::string& filename) {
  std::cerr << "[" << m_profile.instance_name << "] loadForceMomentOffsetParams"
            << std::endl;
  ...
  if (ifs.is_open()) {
    while (ifs.eof() == 0) {
      ...
      if (ifs >> tmps) {
        if (m_forcemoment_offset_param.find(tmps) !=
            m_forcemoment_offset_param.end()) {
          ...
          std::cerr << "[" << m_profile.instance_name << "]   " << tmps << ""
                    << std::endl;
          printForceMomentOffsetParam(tmps);
        } else {
          std::cerr << "[" << m_profile.instance_name << "] no such (" << tmps
                    << ")" << std::endl;
          return false;
        }
      }
    ...
    std::cerr << "[" << m_profile.instance_name << "] failed to open("
              << filename << ")" << std::endl;
    return false;
  }
  return true;
...
bool RemoveForceSensorLinkOffset::dumpForceMomentOffsetParams(
    const std::string& filename) {
  std::cerr << "[" << m_profile.instance_name << "] dumpForceMomentOffsetParams"
            << std::endl;
  std::ofstream ofs(filename.c_str());
  if (ofs.is_open()) {
    for (std::map<std::string, ForceMomentOffsetParam>::iterator it =
             m_forcemoment_offset_param.begin();
         it != m_forcemoment_offset_param.end(); it++) {
      ofs << it->first << " ";
      ofs << it->second.force_offset[0] << " " << it->second.force_offset[1]
          << " " << it->second.force_offset[2] << " ";
      ofs << it->second.moment_offset[0] << " " << it->second.moment_offset[1]
          << " " << it->second.moment_offset[2] << " ";
      ofs << it->second.link_offset_centroid[0] << " "
...
bool RemoveForceSensorLinkOffset::removeForceSensorOffset(
    const ::OpenHRP::RemoveForceSensorLinkOffsetService::StrSequence& names,
    const double tm) {
  std::cerr << "[" << m_profile.instance_name << "] removeForceSensorOffset..."
            << std::endl;
  ...
  bool is_valid_argument = true;
  {
    Guard guard(m_mutex);
    if (names.length() ==
        0) {  // If no sensor names are specified, calibrate all sensors.
      std::cerr
      ...
      for (size_t i = 0; i < names.length(); i++) {
        ...
        if (m_forcemoment_offset_param.find(name) !=
            m_forcemoment_offset_param.end()) {
          if (m_forcemoment_offset_param[name].sensor_offset_calib_counter ==
              0) {
  ...
  std::cerr << "]" << std::endl;
  {
    ...
    for (size_t i = 0; i < valid_names.size(); i++) {
      std::cerr << "[" << m_profile.instance_name
                << "]     Offset-removed force before calib [" << valid_names[i]
                << "], ";
      std::cerr << "force = "
    ...
    for (size_t i = 0; i < valid_names.size(); i++) {
      m_forcemoment_offset_param[valid_names[i]].force_offset_sum =
          hrp::Vector3::Zero();
      m_forcemoment_offset_param[valid_names[i]].moment_offset_sum =
          hrp::Vector3::Zero();
      m_forcemoment_offset_param[valid_names[i]].sensor_offset_calib_counter =
          max_sensor_offset_calib_counter;
    }
  }
  //   Wait
  ...
  //   Print output force and offset after calib
  {
    Guard guard(m_mutex);
    std::cerr << "[" << m_profile.instance_name
              << "]   Calibrate done (calib time = " << tm << "[s])"
              << std::endl;
    for (size_t i = 0; i < valid_names.size(); i++) {
      std::cerr << "[" << m_profile.instance_name << "]     Calibrated offset ["
                << valid_names[i] << "], ";
      std::cerr
          << "force_offset = "
      ...
      std::cerr
          ...
          << std::endl;
      std::cerr << "[" << m_profile.instance_name
                << "]     Offset-removed force after calib [" << valid_names[i]
                << "], ";
      std::cerr << "force = "
                << m_forcemoment_offset_param[valid_names[i]].off_force.format(
                       Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", ", ",
...
/*
RTC::ReturnCode_t RemoveForceSensorLinkOffset::onStateUpdate(RTC::UniqueId
ec_id)
{
  return RTC::RTC_OK;
}
...
/*
RTC::ReturnCode_t RemoveForceSensorLinkOffset::onRateChanged(RTC::UniqueId
ec_id)
{
  return RTC::RTC_OK;
}
...
void RemoveForceSensorLinkOffsetInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<RemoveForceSensorLinkOffset>,
