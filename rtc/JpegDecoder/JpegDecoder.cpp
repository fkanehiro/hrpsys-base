...
static const char* jpegdecoder_spec[] = {
    "implementation_id", "JpegDecoder", "type_name", "JpegDecoder",
...
RTC::ReturnCode_t JpegDecoder::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_encodedIn.isNew()) {
    ...
    int flags = (idat.format == Img::CF_GRAY_JPEG) ? CV_LOAD_IMAGE_GRAYSCALE
                                                   : CV_LOAD_IMAGE_COLOR;
...
void JpegDecoderInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<JpegDecoder>,
