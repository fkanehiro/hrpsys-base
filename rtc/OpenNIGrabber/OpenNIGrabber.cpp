...
static const char *spec[] = {
    "implementation_id", "OpenNIGrabber", "type_name", "OpenNIGrabber",
    "description", "OpenNI grabber", "version", HRPSYS_PACKAGE_VERSION,
    "vendor", "AIST", "category", "example", "activity_type",
    "DataFlowComponent", "max_instance", "10", "language", "C++", "lang_type",
    "compile",
    // Configuration variables
    "conf.default.outputColorImage", "0", "conf.default.outputDepthImage", "0",
    "conf.default.outputPointCloud", "0", "conf.default.outputPointCloudRGBA",
    "0", "conf.default.debugLevel", "0",
...
void OpenNIGrabber::grabberCallbackColorImage(
...
void OpenNIGrabber::outputColorImage(
    const boost::shared_ptr<pcl::io::Image> &image) {
...
void OpenNIGrabber::grabberCallbackDepthImage(
...
void OpenNIGrabber::outputDepthImage(
    const boost::shared_ptr<pcl::io::DepthImage> &image) {
...
void OpenNIGrabber::grabberCallbackColorAndDepthImage(
    const boost::shared_ptr<pcl::io::Image> &image,
...
void OpenNIGrabber::grabberCallbackPointCloudRGBA(
...
void OpenNIGrabber::grabberCallbackPointCloud(
  ...
  try {
    ...
    if (m_outputColorImage && !m_outputDepthImage) {
      boost::function<void(const boost::shared_ptr<pcl::io::Image> &)> f =
          boost::bind(&OpenNIGrabber::grabberCallbackColorImage, this, _1);
      m_interface->registerCallback(f);
    } else if (!m_outputColorImage && m_outputDepthImage) {
      boost::function<void(const boost::shared_ptr<pcl::io::DepthImage> &)> f =
          boost::bind(&OpenNIGrabber::grabberCallbackDepthImage, this, _1);
      m_interface->registerCallback(f);
    } else if (m_outputColorImage && m_outputDepthImage) {
      boost::function<void(const boost::shared_ptr<pcl::io::Image> &,
                           const boost::shared_ptr<pcl::io::DepthImage> &,
                           float)> f =
          boost::bind(&OpenNIGrabber::grabberCallbackColorAndDepthImage, this,
                      _1, _2, _3);
      m_interface->registerCallback(f);
    }
    if (m_outputPointCloud) {
      ...
      boost::function<void(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &)>
          f = boost::bind(&OpenNIGrabber::grabberCallbackPointCloud, this, _1);
      m_interface->registerCallback(f);
      ...
      boost::function<void(
          ...
          boost::bind(&OpenNIGrabber::grabberCallbackPointCloudRGBA, this, _1);
      m_interface->registerCallback(f);
    }

    m_interface->start();
  } catch (pcl::IOException &ex) {
    std::cerr << "[" << m_profile.instance_name << "] Error: " << ex.what()
              << std::endl;
    return RTC::RTC_ERROR;
  } catch (...) {
    std::cerr << "[" << m_profile.instance_name
              << "] Error: An exception occurred while starting grabber"
              << std::endl;
    return RTC::RTC_ERROR;
  }
...
RTC::ReturnCode_t OpenNIGrabber::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_debugLevel > 0) {
    if (m_interface->isRunning()) {
      ...
      std::cerr << "[" << m_profile.instance_name << "] grabber is not running"
                << std::endl;
    }
  }
...
void OpenNIGrabberInit(RTC::Manager *manager) {
  ...
  manager->registerFactory(profile, RTC::Create<OpenNIGrabber>,
