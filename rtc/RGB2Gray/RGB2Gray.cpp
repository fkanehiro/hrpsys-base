...
static const char* jpegdecoder_spec[] = {
    "implementation_id", "RGB2Gray", "type_name", "RGB2Gray", "description",
    "rgb2gray component", "version", HRPSYS_PACKAGE_VERSION, "vendor", "AIST",
...
RTC::ReturnCode_t RGB2Gray::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_rgbIn.isNew()) {
    ...
    cv::Mat src(idat.height, idat.width, CV_8UC3, idat.raw_data.get_buffer());
    cv::Mat dst;
    ...
    memcpy(m_gray.data.image.raw_data.get_buffer(), dst.data,
           idat.width * idat.height);
...
void RGB2GrayInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<RGB2Gray>,
                           RTC::Delete<RGB2Gray>);
}
};
