...
void RTCBody::createPorts(RTC::DataFlowComponentBase *comp) {
  ...
  for (unsigned int i = 0; i < m_rate.size(); i++) {
    ...
    m_rateOut[i] = new RTC::OutPort<RTC::TimedAngularVelocity3D>(
        s->name.c_str(), m_rate[i]);
  ...
  for (unsigned int i = 0; i < m_acc.size(); i++) {
    ...
    m_accOut[i] =
        new RTC::OutPort<RTC::TimedAcceleration3D>(s->name.c_str(), m_acc[i]);
  ...
  for (unsigned int i = 0; i < m_force.size(); i++) {
    ...
    m_forceOut[i] =
        new RTC::OutPort<RTC::TimedDoubleSeq>(s->name.c_str(), m_force[i]);
