...
static const char* component_spec[] = {
    "implementation_id", "Simulator", "type_name", "Simulator", "description",
    "dynamics simulator component", "version", HRPSYS_PACKAGE_VERSION, "vendor",
    "AIST", "category", "example", "activity_type", "DataFlowComponent",
    "max_instance", "10", "language", "C++", "lang_type", "compile",
    // Configuration variables
    "conf.default.project", "", "conf.default.kinematics_only", "0",
...
RTC::ReturnCode_t Simulator::onActivated(RTC::UniqueId ec_id) {
        ...
        for (size_t k = 0; k < links2.size(); ++k) {
          ...
          if (link1 && link2 && link1 != link2) {
            m_world.constraintForceSolver.addCollisionCheckLinkPair(
                ...
                cpi.slidingFriction, 0.01, 0.0, 0.0);
          }
        }
      }
    }
  ...
  for (unsigned int i = 0; i < m_bodies.size(); i++) {
    m_sceneState.states[i].name =
        CORBA::string_dup(m_bodies[i]->name().c_str());
...
void SimulatorInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<Simulator>,
