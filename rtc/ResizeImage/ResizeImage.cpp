...
static const char* jpegdecoder_spec[] = {
    "implementation_id", "ResizeImage", "type_name", "ResizeImage",
    "description", "resize image component", "version", HRPSYS_PACKAGE_VERSION,
...
RTC::ReturnCode_t ResizeImage::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_originalIn.isNew()) {
    ...
    memcpy(m_resized.data.image.raw_data.get_buffer(), m_dst->imageData,
           m_resized.data.image.raw_data.length());
...
void ResizeImageInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<ResizeImage>,
