...
static const char* spec[] = {
    "implementation_id", "AccelerationChecker", "type_name",
    "AccelerationChecker", "description", "Joint acceleration checker",
    "version", HRPSYS_PACKAGE_VERSION, "vendor", "AIST", "category", "example",
    "activity_type", "DataFlowComponent", "max_instance", "10", "language",
    "C++", "lang_type", "compile",
    // Configuration variables
    "conf.default.thd", "1000", "conf.default.print", "0",
    ...
    for (unsigned int i = 0; i < m_q.data.length(); i++) {
      ...
      if (fabs(ddq) > m_thd) {
        std::cout << std::fixed << std::setprecision(3) << "["
                  ...
                  << "] Warning: too big joint acceleration for " << i
                  << "th joint(" << ddq << "[rad/m^2])" << std::endl;
      }
    }
    m_qOld = m_q;
    m_dqOld = m_dq;
...
void AccelerationCheckerInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<AccelerationChecker>,
