...
TwoDofController::TwoDofController(
    TwoDofController::TwoDofControllerParam &_param, unsigned int _range) {
  param.ke = _param.ke;
  param.tc = _param.tc;
  param.dt = _param.dt;
...
void TwoDofController::reset() { integrator.reset(); }

bool TwoDofController::getParameter() { return false; }

bool TwoDofController::getParameter(
    TwoDofController::TwoDofControllerParam &_p) {
  _p.ke = param.ke;
  _p.tc = param.tc;
  _p.dt = param.dt;
  return true;
...
double TwoDofController::update(double _x, double _xd) {
  ...
  // 2 dof controller
  velocity = (-_x + (_xd - _x) + (integrator.calculate() / param.tc)) /
             (-param.ke * param.tc);
