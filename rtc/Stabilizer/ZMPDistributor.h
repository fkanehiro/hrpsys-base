...
class FootSupportPolygon {
 ...
 public:
  FootSupportPolygon(){};
  bool inside_foot(size_t idx) { return true; };
...
class SimpleZMPDistributor {
  ...
  double leg_inside_margin, leg_outside_margin, leg_front_margin,
      leg_rear_margin, wrench_alpha_blending;
 ...
 public:
  ...
  SimpleZMPDistributor(const double _dt) : wrench_alpha_blending(0.5) {
    alpha_filter = boost::shared_ptr<FirstOrderLowPassFilter<double> >(
        new FirstOrderLowPassFilter<double>(
            1e7, _dt, 0.5));  // [Hz], Almost no filter by default
  };

  inline bool is_inside_foot(const hrp::Vector3& leg_pos, const bool is_lleg,
                             const double margin = 0.0) {
    if (is_lleg)
      return (leg_pos(1) >= (-1 * leg_inside_margin + margin)) &&
             (leg_pos(1) <= (leg_outside_margin - margin));
    else
      return (leg_pos(1) <= (leg_inside_margin - margin)) &&
             (leg_pos(1) >= (-1 * leg_outside_margin + margin));
  };
  ...
  inline bool is_cp_inside_foot(const hrp::Vector3& cp,
                                const leg_type support_leg,
                                const double margin = 0.0,
                                const double offset = 0.0) {
    if (support_leg == RLEG)
      return (cp(1) <= (leg_inside_margin - margin)) &&
             (cp(1) >= (-1 * leg_outside_margin + margin)) &&
             (cp(0) <= (leg_front_margin - margin)) &&
             (cp(0) >= (-1 * leg_rear_margin + margin));
    else if (support_leg == LLEG)
      return (cp(1) >= (-1 * leg_inside_margin + margin)) &&
             (cp(1) <= (leg_outside_margin - margin)) &&
             (cp(0) <= (leg_front_margin - margin)) &&
             (cp(0) >= (-1 * leg_rear_margin + margin));
    else if (support_leg == BOTH)
      return (cp(1) <= (leg_outside_margin + offset - margin)) &&
             (cp(1) >= (-1 * (leg_outside_margin + offset) + margin)) &&
             (cp(0) <= (leg_front_margin - margin)) &&
             (cp(0) >= (-1 * leg_rear_margin + margin));
    else
      return true;
  };
  inline bool is_inside_support_polygon(
      Eigen::Vector2d& p, const std::vector<hrp::Vector3>& ee_pos,
      const std::vector<hrp::Matrix33>& ee_rot,
      const std::vector<std::string>& ee_name, const leg_type& support_leg,
      const std::vector<double>& tmp_margin = std::vector<double>(),
      const hrp::Vector3& offset = hrp::Vector3(0.0, 0.0, 0.0)) {
    if (ee_pos.size() == 0 || ee_rot.size() == 0 || ee_name.size() == 0)
      return true;
    size_t l_idx, r_idx;
    for (size_t i = 0; i < ee_name.size(); i++) {
      if (ee_name[i] == "rleg")
        r_idx = i;
      else if (ee_name[i] == "lleg")
        l_idx = i;
    }
    ...
    // RLEG
    rleg_vertices.push_back(Eigen::Vector2d(
        ee_pos[r_idx](0) + leg_front_margin - margin[0] + offset(0),
        ee_pos[r_idx](1) + leg_inside_margin - margin[2] + offset(1)));
    rleg_vertices.push_back(Eigen::Vector2d(
        ee_pos[r_idx](0) + leg_front_margin - margin[0] + offset(0),
        ee_pos[r_idx](1) + -1 * (leg_outside_margin - margin[3]) + offset(1)));
    rleg_vertices.push_back(Eigen::Vector2d(
        ee_pos[r_idx](0) + -1 * (leg_rear_margin - margin[1]) + offset(0),
        ee_pos[r_idx](1) + -1 * (leg_outside_margin - margin[3]) + offset(1)));
    rleg_vertices.push_back(Eigen::Vector2d(
        ee_pos[r_idx](0) + -1 * (leg_rear_margin - margin[1]) + offset(0),
        ee_pos[r_idx](1) + leg_inside_margin - margin[2] + offset(1)));
    // LLEG
    lleg_vertices.push_back(Eigen::Vector2d(
        ee_pos[l_idx](0) + leg_front_margin - margin[0] + offset(0),
        ee_pos[l_idx](1) + leg_outside_margin - margin[3] + offset(1)));
    lleg_vertices.push_back(Eigen::Vector2d(
        ee_pos[l_idx](0) + leg_front_margin - margin[0] + offset(0),
        ee_pos[l_idx](1) + -1 * (leg_inside_margin - margin[2]) + offset(1)));
    lleg_vertices.push_back(Eigen::Vector2d(
        ee_pos[l_idx](0) + -1 * (leg_rear_margin - margin[1]) + offset(0),
        ee_pos[l_idx](1) + -1 * (leg_inside_margin - margin[2]) + offset(1)));
    lleg_vertices.push_back(Eigen::Vector2d(
        ee_pos[l_idx](0) + -1 * (leg_rear_margin - margin[1]) + offset(0),
        ee_pos[l_idx](1) + leg_outside_margin - margin[3] + offset(1)));
    ...
    if (support_leg == BOTH) {
      ...
      std::copy(rleg_vertices.begin(), rleg_vertices.end(),
                std::back_inserter(convex_vertices));
    ...
    if (calcCrossProduct(p, convex_vertices.front(), convex_vertices.back()) <
        0)
      return false;
    return true;
  };
  void print_params(const std::string& str) {
    std::cerr << "[" << str << "]   leg_inside_margin = " << leg_inside_margin
              << "[m], leg_outside_margin = " << leg_outside_margin
              << "[m], leg_front_margin = " << leg_front_margin
              << "[m], leg_rear_margin = " << leg_rear_margin << "[m]"
              << std::endl;
    std::cerr << "[" << str
  ...
  void print_vertices(const std::string& str) { fs.print_vertices(str); };
  // setter
  ...
  void set_vertices_from_margin_params() {
    ...
    // RLEG
    {
      ...
      tvec.push_back(
          Eigen::Vector2d(leg_front_margin, -1 * leg_outside_margin));
      tvec.push_back(
          Eigen::Vector2d(-1 * leg_rear_margin, -1 * leg_outside_margin));
    ...
    // LLEG
    {
      ...
      tvec.push_back(
          Eigen::Vector2d(-1 * leg_rear_margin, -1 * leg_inside_margin));
    ...
    //     std::vector<Eigen::Vector2d> tvec;
    //     tvec.push_back(Eigen::Vector2d(leg_front_margin, leg_inside_margin));
    //     tvec.push_back(Eigen::Vector2d(leg_front_margin,
    //     -1*leg_outside_margin));
    //     tvec.push_back(Eigen::Vector2d(-1*leg_rear_margin,
    //     -1*leg_outside_margin));
    //     tvec.push_back(Eigen::Vector2d(-1*leg_rear_margin,
    //     leg_inside_margin));
    //     vec.push_back(tvec);
  ...
  double calcAlpha(const hrp::Vector3& tmprefzmp,
    ...
    hrp::Vector3 l_local_zmp =
        ee_rot[l_idx].transpose() * (tmprefzmp - ee_pos[l_idx]);
    hrp::Vector3 r_local_zmp =
        ee_rot[r_idx].transpose() * (tmprefzmp - ee_pos[r_idx]);

    // std::cerr << "a " << l_local_zmp(0) << " " << l_local_zmp(1) <<
    // std::endl;
    // std::cerr << "b " << r_local_zmp(0) << " " << r_local_zmp(1) <<
    // std::endl;
    if (is_inside_foot(l_local_zmp, true) && !is_front_of_foot(l_local_zmp) &&
        !is_rear_of_foot(l_local_zmp)) {  // new_refzmp is inside lfoot
      alpha = 0.0;
    } else if (is_inside_foot(r_local_zmp, false) &&
               !is_front_of_foot(r_local_zmp) &&
               !is_rear_of_foot(r_local_zmp)) {  // new_refzmp is inside rfoot
      alpha = 1.0;
    } else {
      ...
      } else if (!is_inside_foot(l_local_zmp, true) &&
                 is_front_of_foot(l_local_zmp)) {
        ledge_foot =
            hrp::Vector3(leg_front_margin, -1 * leg_inside_margin, 0.0);
      } else if (!is_inside_foot(l_local_zmp, true) &&
                 !is_front_of_foot(l_local_zmp) &&
                 !is_rear_of_foot(l_local_zmp)) {
  ...
  void calcAlphaVector(std::vector<double>& alpha_vector,
    ...
    // Blending
    fz_alpha =
        wrench_alpha_blending * fz_alpha + (1 - wrench_alpha_blending) * alpha;
    for (size_t i = 0; i < ee_name.size(); i++) {
  ...
  void calcAlphaVectorFromCOPDistanceCommon(
      std::vector<double>& alpha_vector,
      const std::vector<hrp::Vector3>& cop_pos,
      const std::vector<std::string>& ee_name, const hrp::Vector3& ref_zmp) {
  ...
  void calcAlphaVectorFromCOPDistance(std::vector<double>& alpha_vector,
                                      ...
                                      const hrp::Vector3& new_refzmp,
                                      const hrp::Vector3& ref_zmp) {
    calcAlphaVectorFromCOPDistanceCommon(alpha_vector, cop_pos, ee_name,
                                         new_refzmp);
    calcAlphaVectorFromCOPDistanceCommon(fz_alpha_vector, cop_pos, ee_name,
                                         ref_zmp);
    for (size_t i = 0; i < ee_name.size(); i++) {
      fz_alpha_vector[i] = wrench_alpha_blending * fz_alpha_vector[i] +
                           (1 - wrench_alpha_blending) * alpha_vector[i];
    }
  };

  void distributeZMPToForceMoments(std::vector<hrp::Vector3>& ref_foot_force,
                                   ...
                                   const hrp::Vector3& new_refzmp,
                                   const hrp::Vector3& ref_zmp,
                                   const double total_fz, const double dt,
                                   const bool printp = true,
                                   const std::string& print_str = "") {
    std::vector<double> alpha_vector(2), fz_alpha_vector(2);
    calcAlphaVector(alpha_vector, fz_alpha_vector, ee_pos, ee_rot, ee_name,
                    new_refzmp, ref_zmp);
    ...
    }
    {
      ...
      //             hrp::Vector3 foot_dist_coords_y = (cop_pos[1] -
      //             cop_pos[0]); // e_y'
      //             foot_dist_coords_y(2) = 0.0;
      //             foot_dist_coords_y.normalize();
      //             hrp::Vector3 foot_dist_coords_x =
      //             hrp::Vector3(foot_dist_coords_y.cross(hrp::Vector3::UnitZ()));
      //             // e_x'
      ...
      //             hrp::Vector3 tau_0_f = foot_dist_coords_rot.transpose() *
      //             tau_0; // tau_0'
      //             // x
      //             //         // right
      //             //         if (tau_0_f(0) > 0) ref_foot_moment[0](0) =
      //             tau_0_f(0);
      ...
      //             // <= Foot-distribution-coords frame
      //             // Convert foot-distribution-coords frame => actual world
      //             frame
      //             ref_foot_moment[0] = foot_dist_coords_rot *
      //             ref_foot_moment[0];
      //             ref_foot_moment[1] = foot_dist_coords_rot *
      //             ref_foot_moment[1];
      //
    ...
    if (printp) {
      // std::cerr << "[" << print_str << "]   alpha = " << alpha << ", fz_alpha
      // = " << fz_alpha << std::endl;
      std::cerr << "[" << print_str << "]   "
                << "total_tau    = "
                << hrp::Vector3(tau_0).format(Eigen::IOFormat(
                       Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]"))
                << "[Nm]" << std::endl;
      std::cerr << "[" << print_str << "]   "
                << "ref_force_R  = "
                << hrp::Vector3(ref_foot_force[0])
                       .format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ",
                                               ", ", "", "", "[", "]"))
                << "[N]" << std::endl;
      std::cerr << "[" << print_str << "]   "
                << "ref_force_L  = "
                << hrp::Vector3(ref_foot_force[1])
                       .format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ",
                                               ", ", "", "", "[", "]"))
                << "[N]" << std::endl;
      std::cerr << "[" << print_str << "]   "
                << "ref_moment_R = "
                << hrp::Vector3(ref_foot_moment[0])
                       .format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ",
                                               ", ", "", "", "[", "]"))
                << "[Nm]" << std::endl;
      std::cerr << "[" << print_str << "]   "
                << "ref_moment_L = "
                << hrp::Vector3(ref_foot_moment[1])
                       .format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ",
                                               ", ", "", "", "[", "]"))
                << "[Nm]" << std::endl;
    }
  ...
  void solveForceMomentQPOASES(std::vector<hrp::dvector>& fret,
                               const size_t state_dim, const size_t ee_num,
  ...
  void distributeZMPToForceMomentsQP(std::vector<hrp::Vector3>& ref_foot_force,
                                     std::vector<hrp::Vector3>& ref_foot_moment,
                                     const std::vector<hrp::Vector3>& ee_pos,
                                     const std::vector<hrp::Vector3>& cop_pos,
                                     const std::vector<hrp::Matrix33>& ee_rot,
    ...
    if (use_cop_distribution) {
      // calcAlphaVectorFromCOP(alpha_vector, fz_alpha_vector, cop_pos, ee_name,
      // new_refzmp, ref_zmp);
      calcAlphaVectorFromCOPDistance(alpha_vector, fz_alpha_vector, cop_pos,
                                     ee_name, new_refzmp, ref_zmp);
    } else {
      calcAlphaVector(alpha_vector, fz_alpha_vector, ee_pos, ee_rot, ee_name,
                      new_refzmp, ref_zmp);
    }
    ...
    for (size_t i = 0; i < fz_alpha_vector.size(); i++) {
      ...
      fz_alpha_vector[i] = (fz_alpha_vector[i] < alpha_thre)
                               ? 1 / alpha_thre
                               : 1 / fz_alpha_vector[i];
    }
    for (size_t j = 0; j < fz_alpha_vector.size(); j++) {
      for (size_t i = 0; i < state_dim_one; i++) {
        Hmat(i + j * state_dim_one, i + j * state_dim_one) =
            norm_weight * fz_alpha_vector[j];
      }
    }
    ...
    for (size_t fidx = 0; fidx < ee_num; fidx++) {
      for (size_t i = 0; i < state_dim_one; i++) {
        hrp::Vector3 fpos =
            ee_rot[fidx] * hrp::Vector3(fs.get_foot_vertex(fidx, i)(0),
                                        fs.get_foot_vertex(fidx, i)(1), 0) +
            ee_pos[fidx];
        mm[fidx](0, i) = 1.0;
    ...
    }
    {
      ...
      for (size_t j = 0; j < ee_num; j++) {
        for (size_t i = 0; i < state_dim_one; i++) {
          fpos = ee_rot[j] * hrp::Vector3(fs.get_foot_vertex(j, i)(0),
                                          fs.get_foot_vertex(j, i)(1), 0) +
                 ee_pos[j];
    ...
    if (printp) {
      std::cerr << "[" << print_str << "] force moment distribution "
                << (use_cop_distribution ? "(QP COP)" : "(QP)") << std::endl;
      // std::cerr << "[" << print_str << "]   alpha = " << alpha << ", fz_alpha
      // = " << fz_alpha << std::endl;
      // std::cerr << "[" << print_str << "]   "
      //           << "total_tau    = " <<
      //           hrp::Vector3(tau_0).format(Eigen::IOFormat(Eigen::StreamPrecision,
      //           0, ", ", ", ", "", "", "[", "]")) << "[Nm]" << std::endl;
      for (size_t i = 0; i < ee_num; i++) {
        std::cerr << "[" << print_str << "]   "
                  << "ref_force  [" << ee_name[i] << "] "
                  << hrp::Vector3(ref_foot_force[i])
                         .format(Eigen::IOFormat(Eigen::StreamPrecision, 0,
                                                 ", ", ", ", "", "", "[", "]"))
                  << "[N]" << std::endl;
        std::cerr << "[" << print_str << "]   "
                  << "ref_moment [" << ee_name[i] << "] "
                  << hrp::Vector3(ref_foot_moment[i])
                         .format(Eigen::IOFormat(Eigen::StreamPrecision, 0,
                                                 ", ", ", ", "", "", "[", "]"))
                  << "[Nm]" << std::endl;
      }
  ...
  void distributeZMPToForceMomentsQP(std::vector<hrp::Vector3>& ref_foot_force,
                                     ...
                                     const std::string& print_str = "",
                                     const bool use_cop_distribution = false) {
    distributeZMPToForceMoments(ref_foot_force, ref_foot_moment, ee_pos,
                                cop_pos, ee_rot, ee_name, limb_gains,
                                toeheel_ratio, new_refzmp, ref_zmp, total_fz,
                                dt, printp, print_str);
  };
#endif  // USE_QPOASES
  ...
  void distributeZMPToForceMomentsPseudoInverse(
      std::vector<hrp::Vector3>& ref_foot_force,
      std::vector<hrp::Vector3>& ref_foot_moment,
      const std::vector<hrp::Vector3>& ee_pos,
      const std::vector<hrp::Vector3>& cop_pos,
      const std::vector<hrp::Matrix33>& ee_rot,
      const std::vector<std::string>& ee_name,
      const std::vector<double>& limb_gains,
      const std::vector<double>& toeheel_ratio, const hrp::Vector3& new_refzmp,
      const hrp::Vector3& ref_zmp, const double total_fz, const double dt,
    ...
    calcAlphaVectorFromCOPDistance(alpha_vector, fz_alpha_vector, cop_pos,
                                   ee_name, new_refzmp, ref_zmp);
    if (printp) {
      std::cerr << "[" << print_str << "] force moment distribution (Pinv)"
                << std::endl;
    }
    ...
    // Temporarily set ref_foot_force and ref_foot_moment based on reference
    // values
    {
      size_t total_wrench_dim = 5;
    ...
    for (size_t fidx = 0; fidx < ee_num; fidx++) {
      double tmp_tau_x =
          -(cop_pos[fidx](2) - ref_zmp(2)) * ref_foot_force[fidx](1) +
          (cop_pos[fidx](1) - ref_zmp(1)) * ref_foot_force[fidx](2) +
          ref_foot_moment[fidx](0);
      total_wrench(3) -= tmp_tau_x;
      double tmp_tau_y =
          (cop_pos[fidx](2) - ref_zmp(2)) * ref_foot_force[fidx](0) -
    ...
    total_wrench(3) -= -(ref_zmp(2) - new_refzmp(2)) * ref_total_force(1) +
                       (ref_zmp(1) - new_refzmp(1)) * ref_total_force(2);
    total_wrench(4) -= (ref_zmp(2) - new_refzmp(2)) * ref_total_force(0) -
                       (ref_zmp(0) - new_refzmp(0)) * ref_total_force(2);
    //   2. Calculate G (Gmat)
    ...
    for (size_t j = 0; j < ee_num; j++) {
      for (size_t i = 0; i < 3; i++) {
        ...
        Wmat(i + j * 6 + 3, i + j * 6 + 3) =
            (1.0 / norm_moment_weight) * fz_alpha_vector[j] * limb_gains[j];
        // Set local Y moment
        //   If toeheel_ratio is 0, toe and heel contact and local Y moment
        //   should be 0.
        if (i == 1) {
          Wmat(i + j * 6 + 3, i + j * 6 + 3) =
              toeheel_ratio[j] * Wmat(i + j * 6 + 3, i + j * 6 + 3);
        }
    ...
    // Extract force and moment with converting local frame -> absolute frame
    // (ret is local frame)
    for (size_t fidx = 0; fidx < ee_num; fidx++) {
      ref_foot_force[fidx] +=
          ee_rot[fidx] *
          hrp::Vector3(ret(6 * fidx), ret(6 * fidx + 1), ret(6 * fidx + 2));
      ref_foot_moment[fidx] +=
          ee_rot[fidx] *
  ...
  void distributeZMPToForceMomentsPseudoInverse2(
      ...
      const std::vector<double>& limb_gains,
      const std::vector<double>& toeheel_ratio, const hrp::Vector3& new_refzmp,
      const hrp::Vector3& ref_zmp, const hrp::Vector3& total_force,
      const hrp::Vector3& total_moment,
      const std::vector<hrp::dvector6>& ee_forcemoment_distribution_weight,
      const double total_fz, const double dt, const bool printp = true,
      const std::string& print_str = "") {
#define FORCE_MOMENT_DIFF_CONTROL
    ...
    for (size_t fidx = 0; fidx < ee_num; fidx++) {
      double tmp_tau_x =
          -(cop_pos[fidx](2) - new_refzmp(2)) * ref_foot_force[fidx](1) +
          (cop_pos[fidx](1) - new_refzmp(1)) * ref_foot_force[fidx](2);
      total_wrench(3) -= tmp_tau_x;
      double tmp_tau_y =
          (cop_pos[fidx](2) - new_refzmp(2)) * ref_foot_force[fidx](0) -
          (cop_pos[fidx](0) - new_refzmp(0)) * ref_foot_force[fidx](2);
      total_wrench(4) -= tmp_tau_y;
    }
#endif
    ...
    for (size_t j = 0; j < ee_num; j++) {
      for (size_t i = 0; i < 3; i++) {
        Wmat(i + j * 6, i + j * 6) = ee_forcemoment_distribution_weight[j][i] *
                                     fz_alpha_vector[j] * limb_gains[j];
        // double norm_moment_weight = 1e2;
        // Wmat(i+j*6+3, i+j*6+3) = ee_forcemoment_distribution_weight[j][i+3] *
        // (1.0/norm_moment_weight) * fz_alpha_vector[j] * limb_gains[j];
        Wmat(i + j * 6 + 3, i + j * 6 + 3) =
            ee_forcemoment_distribution_weight[j][i + 3] * fz_alpha_vector[j] *
            limb_gains[j];
      }
    }
    if (printp) {
    ...
    {
      hrp::dvector selected_total_wrench =
          hrp::dvector::Zero(selection_matrix.rows());
      hrp::dmatrix selected_Gmat =
          hrp::dmatrix::Zero(selection_matrix.rows(), Gmat.cols());
      selected_total_wrench = selection_matrix * total_wrench;
      selected_Gmat = selection_matrix * Gmat;
      calcWeightedLinearEquation(ret, selected_Gmat, Wmat,
                                 selected_total_wrench);
    }
    ...
    if (printp) {
      ...
      std::cerr << "[" << print_str << "]   "
                << "test_diff "
                << tmpretv.format(Eigen::IOFormat(Eigen::StreamPrecision, 0,
                                                  ", ", ", ", "", "", "[", "]"))
                << "[N][Nm]" << std::endl;
      std::cerr << "[" << print_str << "]   "
                << "total_wrench "
...
#ifdef FORCE_MOMENT_DIFF_CONTROL
        std::cerr << "[" << print_str << "]   "
                  << "dif_ref_force  [" << ee_name[i] << "] "
                  << hrp::Vector3(hrp::Vector3(ret(6 * i), ret(6 * i + 1),
                                               ret(6 * i + 2)))
                         .format(Eigen::IOFormat(Eigen::StreamPrecision, 0,
                                                 ", ", ", ", "", "", "[", "]"))
                  << "[N], "
                  << "dif_ref_moment [" << ee_name[i] << "] "
                  << hrp::Vector3(hrp::Vector3(ret(6 * i + 3), ret(6 * i + 4),
                                               ret(6 * i + 5)))
                         .format(Eigen::IOFormat(Eigen::StreamPrecision, 0,
                                                 ", ", ", ", "", "", "[", "]"))
                  << "[Nm]" << std::endl;
  ...
  double calcCrossProduct(Eigen::Vector2d& a, Eigen::Vector2d& b,
                          Eigen::Vector2d& o) {
    return (a(0) - o(0)) * (b(1) - o(1)) - (a(1) - o(1)) * (b(0) - o(0));
  };

  // assume that vertices are listed in clockwise order
  std::vector<Eigen::Vector2d> calcConvexHull(
      std::vector<Eigen::Vector2d> vertices) {
    std::vector<Eigen::Vector2d> convex_vertices;

    convex_vertices.push_back(vertices.front());
    for (size_t i = 1; i < vertices.size() - 1; i++) {
      if (calcCrossProduct(vertices[i + 1], vertices[i - 1], vertices[i]) < 0)
        convex_vertices.push_back(vertices[i]);
    }
