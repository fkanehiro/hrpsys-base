...
class Integrator {
 public:
  // if range = 0, integrate from 0 to t. Otherwise, integrate from t - (range -
  // 1) * dt to t.
 ...
 private:
  ...
  bool init_integration_flag;  // integration inited flag (true when first value
                               // is updated)
  unsigned int
      range;  // integration range (from t_now - range * dt to t_now [sec])
