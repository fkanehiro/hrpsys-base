...
class testZMPDistributor {
 ...
 private:
  void gen_and_plot() {
    ...
    if (distribution_algorithm == EEFM)
      std::cerr << "EEFM";
    else if (distribution_algorithm == EEFMQP)
      std::cerr << "EEFMQP";
    else if (distribution_algorithm == EEFMQP2)
      std::cerr << "EEFMQP(cop_distribution)";
    ...
    std::vector<hrp::Vector3> ref_foot_force(names.size(),
                                             hrp::Vector3::Zero()),
        ref_foot_moment(names.size(), hrp::Vector3::Zero());
    std::vector<std::vector<Eigen::Vector2d> > fs;
    szd->get_vertices(fs);
    //
    std::ostringstream oss_foot_params("");
    oss_foot_params
        << "'/tmp/plotrleg.dat' using 1:2:3 with lines title 'rleg', "
    ...
    for (size_t i = 0; i < refzmp_vec.size(); i++) {
      ...
      if (distribution_algorithm == EEFMQP) {
        szd->distributeZMPToForceMoments(
            ref_foot_force, ref_foot_moment, ee_pos, cop_pos, ee_rot, names,
            limb_gains, toeheel_ratio, refzmp_vec[i], refzmp_vec[i], total_fz,
            dt);
      } else if (distribution_algorithm == EEFMQP ||
        ...
        szd->distributeZMPToForceMomentsQP(
            ref_foot_force, ref_foot_moment, ee_pos, cop_pos, ee_rot, names,
            limb_gains, toeheel_ratio, refzmp_vec[i], refzmp_vec[i], total_fz,
            dt, true, "", (distribution_algorithm == EEFMQP2));
      } else if (distribution_algorithm == EEFMQPCOP) {
        ...
        // std::vector<double> ee_forcemoment_distribution_weight(names.size(),
        // 1.0);
        // szd->distributeZMPToForceMomentsPseudoInverse2(ref_foot_force,
        // ref_foot_moment,
        //                                                ee_pos, cop_pos,
        //                                                ee_rot, names,
        //                                                limb_gains,
      ...
      for (size_t j = 0; j < fs.size(); j++) {
        ...
        for (size_t i = 0; i < fs[j].size(); i++) {
          hrp::Vector3 tmpf(hrp::Vector3(
              ee_rot[j] * hrp::Vector3(fs[j][i](0), fs[j][i](1), 0) +
              ee_pos[j]));
          fprintf(fp, "%f %f %f\n", tmpf(0), tmpf(1), tmpf(2));
        }
        hrp::Vector3 tmpf(hrp::Vector3(
            ee_rot[j] * hrp::Vector3(fs[j][0](0), fs[j][0](1), 0) + ee_pos[j]));
        fprintf(fp, "%f %f %f\n", tmpf(0), tmpf(1), tmpf(2));
        fclose(fp);
      }
      for (size_t j = 0; j < fs.size(); j++) {
      ...
      fprintf(fp_fm, "%f %f %f %f %f %f %f %f %f\n", refzmp_vec[i](0),
              refzmp_vec[i](1), ref_foot_force[0](2), ref_foot_force[1](2),
              ref_foot_moment[0](0), ref_foot_moment[1](0),
              ref_foot_moment[0](1), ref_foot_moment[1](1), alpha);
      {
      ...
      if (use_gnuplot) {
        ...
        oss << "splot [-0.5:0.5][-0.5:0.5][-1:1000] " << foot_params_str
            << "'/tmp/plotrlegfm.dat' using 1:2:3 with lines title 'rleg fm' "
               "lw 5, "
            << "'/tmp/plotllegfm.dat' using 1:2:3 with lines title 'lleg fm' "
               "lw 5, "
    ...
    if (use_gnuplot) {
      ...
      fflush(gp_m);
      oss.str("");
      oss << "splot [-0.5:0.5][-0.5:0.5][-50:" << total_fz * 1.1 << "] "
          << foot_params_str
...
void print_usage() {
  std::cerr << "Usage : testZMPDistributor [robot-name] [test-type] [option]"
            << std::endl;
