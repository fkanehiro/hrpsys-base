...
static const char* stateholder_spec[] = {
    "implementation_id", "StateHolder", "type_name", "StateHolder",
    "description", "state holder", "version", HRPSYS_PACKAGE_VERSION, "vendor",
...
RTC::ReturnCode_t StateHolder::onInitialize() {
  ...
  m_StateHolderServicePort.registerProvider("service0", "StateHolderService",
                                            m_service0);
  m_TimeKeeperServicePort.registerProvider("service1", "TimeKeeperService",
                                           m_service1);
...
RTC::ReturnCode_t StateHolder::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_requestGoActual ||
      (m_q.data.length() == 0 && m_currentQ.data.length() > 0)) {
    m_q = m_currentQ;
    ...
    for (unsigned int i = 0; i < m_wrenchesIn.size(); i++) {
      m_wrenches[i].data[0] = m_wrenches[i].data[1] = m_wrenches[i].data[2] =
          0.0;
      m_wrenches[i].data[3] = m_wrenches[i].data[4] = m_wrenches[i].data[5] =
          0.0;
    }
  }
...
void StateHolder::getCommand(StateHolderService::Command& com) {
  ...
  memcpy(com.jointRefs.get_buffer(), m_q.data.get_buffer(),
         sizeof(double) * m_q.data.length());
...
void StateHolderInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<StateHolder>,
