...
bool robot::setServoErrorLimit(const char *i_jname, double i_limit) {
  ...
  if (strcmp(i_jname, "all") == 0 || strcmp(i_jname, "ALL") == 0) {
    ...
    std::cerr << "[el] setServoErrorLimit " << i_limit << "[rad] for all joints"
              << std::endl;
  } else if ((l = link(i_jname))) {
    m_servoErrorLimit[l->jointId] = i_limit;
    std::cerr << "[el] setServoErrorLimit " << i_limit << "[rad] for "
              << i_jname << std::endl;
  } else {
    std::cerr << "[el] Invalid joint name of setServoErrorLimit " << i_jname
              << "!" << std::endl;
    return false;
  }
  return true;
