...
static const char* softerrorlimiter_spec[] = {
    "implementation_id", "SoftErrorLimiter", "type_name", "SoftErrorLimiter",
    "description", "soft error limiter", "version", HRPSYS_PACKAGE_VERSION,
...
SoftErrorLimiter::~SoftErrorLimiter() { quit_beep(); }
...
RTC::ReturnCode_t SoftErrorLimiter::onInitialize() {
  ...
  m_SoftErrorLimiterServicePort.registerProvider(
      "service0", "SoftErrorLimiterService", m_service0);
  ...
  std::cout << "[" << m_profile.instance_name
            << "] dof = " << m_robot->numJoints() << std::endl;
  ...
  soft_limit_error_beep_freq =
      static_cast<int>(1.0 / (4.0 * dt));  // soft limit error => 4 times / 1[s]
  position_limit_error_beep_freq = static_cast<int>(
      1.0 / (2.0 * dt));  // position limit error => 2 times / 1[s]
  ...
  hrp::readJointLimitTableFromProperties(joint_limit_tables, m_robot,
                                         prop["joint_limit_table"],
  ...
  if (m_qRef.data.length() == m_qCurrent.data.length() &&
    ...
    for (unsigned int i = 0; i < m_qRef.data.length(); i++) {
      servo_state[i] =
          ...
          OpenHRP::RobotHardwareService::SERVO_STATE_SHIFT;  // enum
                                                             // SwitchStatus
                                                             // {SWITCH_ON,
                                                             // SWITCH_OFF};
    }
    ...
    /*
      ...
      inline Link* joint(int id) const
       ...
       inline Link* link(int index) const
         This function returns the link of a given index in the whole link
      sequence.
         The order of the sequence corresponds to a link-tree traverse from the
      root link.
    ...
    for (unsigned int i = 0; i < m_qRef.data.length(); i++) {
      // Determin total upper-lower limit considering velocity, position, and
      // error limits.
      // e.g.,
      //  total lower limit = max (vel, pos, err) <= severest lower limit
      //  total upper limit = min (vel, pos, err) <= severest upper limit
      double total_upper_limit = std::numeric_limits<double>::max(),
             total_lower_limit = -std::numeric_limits<double>::max();
      {
        double qvel = (m_qRef.data[i] - prev_angle[i]) / dt;
        double lvlimit =
            m_robot->joint(i)->lvlimit + 0.000175;  // 0.01 deg / sec
        double uvlimit = m_robot->joint(i)->uvlimit - 0.000175;
        // fixed joint has ulimit = vlimit
        if (servo_state[i] == 1 && (lvlimit < uvlimit) &&
            ((lvlimit > qvel) || (uvlimit < qvel))) {
          if (loop % debug_print_freq == 0 || debug_print_velocity_first) {
            std::cerr << "[" << m_profile.instance_name << "] [" << m_qRef.tm
                      << "] velocity limit over " << m_robot->joint(i)->name
                      << "(" << i << "), qvel=" << qvel
                      << ", lvlimit =" << lvlimit << ", uvlimit =" << uvlimit
          ...
          if (lvlimit > qvel) {
            total_lower_limit =
                std::max(prev_angle[i] + lvlimit * dt, total_lower_limit);
          }
          if (uvlimit < qvel) {
            total_upper_limit =
                std::min(prev_angle[i] + uvlimit * dt, total_upper_limit);
          }
      ...
      // Position limitation for reference joint angles
      {
        ...
        if (joint_limit_tables.find(m_robot->joint(i)->name) !=
            joint_limit_tables.end()) {
          std::map<std::string, hrp::JointLimitTable>::iterator it =
              joint_limit_tables.find(m_robot->joint(i)->name);
          llimit =
              it->second.getLlimit(m_qRef.data[it->second.getTargetJointId()]);
          ulimit =
              it->second.getUlimit(m_qRef.data[it->second.getTargetJointId()]);
        }
        // fixed joint have vlimit = ulimit
        bool servo_limit_state =
            (llimit < ulimit) &&
            ((llimit > m_qRef.data[i]) || (ulimit < m_qRef.data[i]));
        if (servo_state[i] == 1 && servo_limit_state) {
          if (loop % debug_print_freq == 0 || debug_print_position_first) {
            std::cerr << "[" << m_profile.instance_name << "] [" << m_qRef.tm
                      << "] position limit over " << m_robot->joint(i)->name
                      << "(" << i << "), qRef=" << m_qRef.data[i]
                      << ", llimit =" << llimit << ", ulimit =" << ulimit
                      << ", servo_state = " << (servo_state[i] ? "ON" : "OFF")
                      << ", prev_angle = " << prev_angle[i] << std::endl;
          }
          // fix joint angle
          if (llimit > m_qRef.data[i] &&
              prev_angle[i] >
                  m_qRef.data[i]) {  // ref < llimit and prev < ref -> OK
            total_lower_limit = std::max(llimit, total_lower_limit);
          }
          if (ulimit < m_qRef.data[i] &&
              prev_angle[i] <
                  m_qRef.data[i]) {  // ulimit < ref and ref < prev -> OK
            total_upper_limit = std::min(ulimit, total_upper_limit);
          }
          m_servoState.data[i][0] |=
              (0x200 << OpenHRP::RobotHardwareService::SERVO_ALARM_SHIFT);
          position_limit_error = true;
        }
      }

      // Servo error limitation between reference joint angles and actual joint
      // angles
      //   pos_vel_limited_angle is current output joint angle which arleady
      //   finished position limit and velocity limit.
      //   Check and limit error between pos_vel_limited_angle and current
      //   actual joint angle.
      {
        double pos_vel_limited_angle = std::min(
            total_upper_limit, std::max(total_lower_limit, m_qRef.data[i]));
        ...
        if (servo_state[i] == 1 && fabs(error) > limit) {
          if (loop % debug_print_freq == 0 || debug_print_error_first) {
            std::cerr << "[" << m_profile.instance_name << "] [" << m_qRef.tm
                      << "] error limit over " << m_robot->joint(i)->name << "("
                      << i << "), qRef=" << pos_vel_limited_angle
          ...
          if (error > limit) {
            total_upper_limit =
                std::min(m_qCurrent.data[i] + limit, total_upper_limit);
          } else {
            total_lower_limit =
                std::max(m_qCurrent.data[i] - limit, total_lower_limit);
          }
          if (loop % debug_print_freq == 0 || debug_print_error_first) {
            std::cerr << ", q=" << m_qRef.data[i] << std::endl;
          }
          m_servoState.data[i][0] |=
              (0x040 << OpenHRP::RobotHardwareService::SERVO_ALARM_SHIFT);
          soft_limit_error = true;
        }
      }

      // Limitation of current output considering total upper and lower limits
      prev_angle[i] = m_qRef.data[i] = std::min(
          total_upper_limit, std::max(total_lower_limit, m_qRef.data[i]));
    }
    ...
    if (soft_limit_error) {  // play beep
      if (is_beep_port_connected) {
        if (loop % soft_limit_error_beep_freq == 0)
          bc.startBeep(3136, soft_limit_error_beep_freq * 0.8);
        else
          bc.stopBeep();
      } else {
        if (loop % soft_limit_error_beep_freq == 0)
          start_beep(3136, soft_limit_error_beep_freq * 0.8);
      }
    } else if (position_limit_error || velocity_limit_error) {  // play beep
      if (is_beep_port_connected) {
        if (loop % position_limit_error_beep_freq == 0)
          bc.startBeep(3520, position_limit_error_beep_freq * 0.8);
        else
          bc.stopBeep();
      } else {
        if (loop % position_limit_error_beep_freq == 0)
          start_beep(3520, position_limit_error_beep_freq * 0.8);
      }
    } else {
      if (is_beep_port_connected) {
        bc.stopBeep();
    ...
    if (loop % 100 == 1) {
      ...
      std::cerr << "   m_servoState " << m_servoState.data.length()
                << std::endl;
    }
  }
  if (is_beep_port_connected) {
...
void SoftErrorLimiterInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<SoftErrorLimiter>,
