...
static const char* capturecontroller_spec[] = {
    "implementation_id", "CaptureController", "type_name", "CaptureController",
    "description", "capture controller", "version", HRPSYS_PACKAGE_VERSION,
    "vendor", "AIST", "category", "example", "activity_type",
    "DataFlowComponent", "max_instance", "10", "language", "C++", "lang_type",
    "compile",
    // Configuration variables
    "conf.default.frameRate", "1", "conf.default.initialMode", "sleep",
...
void CaptureController::take_one_frame() { m_mode = ONESHOT; }

void CaptureController::start_continuous() { m_mode = CONTINUOUS; }

void CaptureController::stop_continuous() { m_mode = SLEEP; }

extern "C" {
void CaptureControllerInit(RTC::Manager* manager) {
  RTC::Properties profile(capturecontroller_spec);
  manager->registerFactory(profile, RTC::Create<CaptureController>,
