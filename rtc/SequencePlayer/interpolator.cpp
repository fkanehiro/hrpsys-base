...
// 1dof interpolator
void interpolator::hoffarbib(double &remain_t_, double a0, double a1, double a2,
                             double a3, double a4, double a5, double &xx,
                             double &vv, double &aa) {
#define EPS 1e-6
  if (remain_t_ > dt + EPS) {
    remain_t_ -= dt;
  ...
  xx = a0 + a1 * t + a2 * t * t + a3 * t * t * t + a4 * t * t * t * t +
       a5 * t * t * t * t * t;
  vv = a1 + 2 * a2 * t + 3 * a3 * t * t + 4 * a4 * t * t * t +
       5 * a5 * t * t * t * t;
  aa = 2 * a2 + 6 * a3 * t + 12 * a4 * t * t + 20 * a5 * t * t * t;
}

void interpolator::linear_interpolation(double &remain_t_, double gx,
...
#define MIN_INTERPOLATION_TIME (1.0)
  if (remain_t_ < MIN_INTERPOLATION_TIME) {
    std::cerr << "[interpolator][" << name
              << "] MIN_INTERPOLATION_TIME violated!! Limit remain_t ("
              << remain_t << ") by MIN_INTERPOLATION_TIME ("
              << MIN_INTERPOLATION_TIME << ")."
              << "(max_diff = " << max_diff
              << ", default_avg_vel = " << default_avg_vel << ")" << std::endl;
    ;
...
bool interpolator::setInterpolationMode(interpolation_mode i_mode_) {
  if (i_mode_ != LINEAR && i_mode_ != HOFFARBIB && i_mode_ != QUINTICSPLINE &&
      i_mode_ != CUBICSPLINE)
    return false;
  imode = i_mode_;
  return true;
...
void interpolator::setGoal(const double *newg, double time, bool online) {
...
void interpolator::setGoal(const double *newg, const double *newv, double time,
  ...
  if (newv != NULL)
    memcpy(gv, newv, sizeof(double) * dim);
  else {
    for (int i = 0; i < dim; i++) {
      gv[i] = 0;
    }
  ...
  for (int i = 0; i < dim; i++) {
    switch (imode) {
      case HOFFARBIB:
        A = (gx[i] -
             (x[i] + v[i] * target_t + (a[i] / 2.0) * target_t * target_t)) /
      ...
      case QUINTICSPLINE:
        ...
        a1[i] = v[i];
        a2[i] = 0.5 * a[i];
        a3[i] = (-20 * x[i] + 20 * gx[i] - 3 * a[i] * target_t * target_t +
                 ga[i] * target_t * target_t - 12 * v[i] * target_t -
                 8 * gv[i] * target_t) /
                (2 * target_t * target_t * target_t);
        a4[i] = (30 * x[i] - 30 * gx[i] + 3 * a[i] * target_t * target_t -
                 2 * ga[i] * target_t * target_t + 16 * v[i] * target_t +
                 14 * gv[i] * target_t) /
                (2 * target_t * target_t * target_t * target_t);
        a5[i] = (-12 * x[i] + 12 * gx[i] - a[i] * target_t * target_t +
                 ga[i] * target_t * target_t - 6 * v[i] * target_t -
                 6 * gv[i] * target_t) /
      ...
      case CUBICSPLINE:
        ...
        a1[i] = v[i];
        a2[i] =
            (-3 * x[i] + 3 * gx[i] - 2 * v[i] * target_t - gv[i] * target_t) /
            (target_t * target_t);
        a3[i] = (2 * x[i] - 2 * gx[i] + v[i] * target_t + gv[i] * target_t) /
                (target_t * target_t * target_t);
        a4[i] = a[5] = 0;
        break;
    }
...
void interpolator::interpolate(double &remain_t_) {
  ...
  for (int i = 0; i < dim; i++) {
    ...
    switch (imode) {
      ...
      case QUINTICSPLINE:
      case CUBICSPLINE:
        hoffarbib(tm, a0[i], a1[i], a2[i], a3[i], a4[i], a5[i], x[i], v[i],
                  a[i]);
        break;
    }
  }
...
void interpolator::go(const double *newg, const double *newv, double time,
                      bool immediate) {
...
void interpolator::load(const char *fname, double time_to_start, double scale,
  ...
  ifstream strm(fname);
  if (!strm.is_open()) {
    cerr << "[interpolator " << name << "] file not found(" << fname << ")"
         << endl;
    return;
  }
...
void interpolator::push(const double *x_, const double *v_, const double *a_,
                        bool immediate) {
