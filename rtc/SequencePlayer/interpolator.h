...
class interpolator {
  // interpolator class is to interpolate from current value to goal value
  // considering position, velocities, and accelerations.
  //   Two status : empty or not
  //                Interpolator interpolates based on remaining time (remain_t)
  //                and pushes value to queue (q, dq, ddq).
  //                Users can get interpolated results from queue (q, dq, ddq).
  //                If remain_t <= 0 and queue is empty, interpolator is
  //                "empty", otherwise "not empty".
 ...
 public:
  ...
  ~interpolator();
  void push(const double *x_, const double *v_, const double *a_,
            bool immediate = true);
  double *front();
  // Getter function.
  ...
  void go(const double *gx, const double *gv, double time,
          bool immediate = true);
  ...
  // Set goal
  //   If online=true, user can get and interpolate value through get()
  //   function.
 ...
 private:
  ...
  // Queue of positions, velocities, and accelerations ([q_t, q_t+1, ....,
  // q_t+n]).
  ...
  void hoffarbib(double &remain_t_, double a0, double a1, double a2, double a3,
                 double a4, double a5, double &xx, double &vv, double &aa);
  void linear_interpolation(double &remain_t_, double gx, double &xx,
                            double &vv, double &aa);
