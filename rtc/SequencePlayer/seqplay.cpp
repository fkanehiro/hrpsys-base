...
seqplay::seqplay(unsigned int i_dof, double i_dt, unsigned int i_fnum,
                 unsigned int optional_data_dim)
    : m_dof(i_dof) {
  ...
  interpolators[WRENCHES] =
      new interpolator(6 * i_fnum, i_dt, interpolator::HOFFARBIB,
...
void seqplay::playPattern(std::vector<const double *> pos,
                          ...
                          std::vector<double> tm, const double *qInit,
                          unsigned int len) {
  const double *q = NULL, *z = NULL, *a = NULL, *p = NULL, *e = NULL,
               *tq = NULL, *wr = NULL, *od = NULL;
  double t = 0;
  ...
  for (unsigned int i = 0; i < pos.size(); i++) {
    ...
    if (i < tm.size()) t = tm[i];
    go(q, z, a, p, e, tq, wr, od, v, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
       t, false);
  }
  sync();
  delete[] v;
...
void seqplay::go(const double *i_q, const double *i_zmp, const double *i_acc,
                 ...
                 const double *i_wrenches, const double *i_optional_data,
                 double i_time, bool immediate) {
  go(i_q, i_zmp, i_acc, i_p, i_rpy, i_tq, i_wrenches, i_optional_data, NULL,
     NULL, NULL, NULL, NULL, NULL, NULL, NULL, i_time, immediate);
}
...
void seqplay::go(const double *i_q, const double *i_zmp, const double *i_acc,
                 ...
                 const double *ii_wrenches, const double *ii_optional_data,
                 double i_time, bool immediate) {
  ...
  if (i_wrenches)
    interpolators[WRENCHES]->go(i_wrenches, ii_wrenches, i_time, false);
  if (i_optional_data)
    interpolators[OPTIONAL_DATA]->go(i_optional_data, ii_optional_data, i_time,
                                     false);
  if (immediate) sync();
}
...
bool seqplay::setInterpolationMode(interpolator::interpolation_mode i_mode_) {
  if (i_mode_ != interpolator::LINEAR && i_mode_ != interpolator::HOFFARBIB &&
      i_mode_ != interpolator::QUINTICSPLINE &&
      i_mode_ != interpolator::CUBICSPLINE)
    return false;
...
bool seqplay::addJointGroup(const char *gname,
  ...
  groupInterpolator *i = groupInterpolators[gname];
  if (i) {
    std::cerr << "[addJointGroup] group name " << gname
...
bool seqplay::getJointGroup(const char *gname, std::vector<int> &indices) {
  ...
  if (i) {
    ...
    std::cerr << "[getJointGroup] group name " << gname << " is not installed"
              << std::endl;
    return false;
  }
}
...
bool seqplay::removeJointGroup(const char *gname, double time) {
  ...
  if (i) {
    ...
    std::cerr << "[removeJointGroup] group name " << gname
  ...
  if (i) {
    ...
    for (it = groupInterpolators.begin(); it != groupInterpolators.end();
      ...
      if (it->first != std::string(gname)) {  // other
        groupInterpolator *gi = it->second;
        if (gi && (gi->state == groupInterpolator::created ||
                   gi->state == groupInterpolator::working) &&
            gi->inter->isEmpty()) {
          gi->set(full);
        }
      }
    ...
    std::cerr << "[resetJointGroup] group name " << gname << " is not installed"
              << std::endl;
    return false;
  }
}

bool seqplay::setJointAnglesOfGroup(const char *gname, const double *i_qRef,
                                    const size_t i_qsize, double i_tm) {
  char *s = (char *)gname;
  while (*s) {
    *s = toupper(*s);
    s++;
...
bool seqplay::playPatternOfGroup(const char *gname,
  ...
  if (i) {
    if (len != i->indices.size()) {
      std::cerr << "[playPatternOfGroup] group name " << gname
...
bool seqplay::setJointAnglesSequenceFull(
    std::vector<const double *> i_pos, std::vector<const double *> i_vel,
  ...
  int fnum = interpolators[WRENCHES]->dimension() / 6,
      optional_data_dim = interpolators[OPTIONAL_DATA]->dimension();
  double zmp[3], wrench[6 * fnum], dummy_fnum[6 * fnum],
      optional[optional_data_dim], dummy_optional[optional_data_dim];
  ...
  interpolators[OPTIONAL_DATA]->push(optional, dummy_optional, dummy_optional,
                                     true);
...
bool seqplay::clearJointAnglesOfGroup(const char *gname) {
  ...
  if (!i) {
    std::cerr << "[clearJointAnglesOfGroup] group name " << gname
              << " is not installed" << std::endl;
    return false;
  }
  ...
  if (i->state == groupInterpolator::removing ||
      i->state == groupInterpolator::removed) {
    std::cerr << "[clearJointAnglesOfGroup] group name " << gname
