...
class seqplay {
 public:
  ...
  void playPattern(std::vector<const double *> pos,
                   std::vector<const double *> zmp,
                   std::vector<const double *> rpy, std::vector<double> tm,
  ...
  bool setJointAnglesOfGroup(const char *gname, const double *i_qRef,
                             const size_t i_qsize, double i_tm = 0.0);
  void clearOfGroup(const char *gname, double i_timeLimit);
  bool playPatternOfGroup(const char *gname, std::vector<const double *> pos,
                          std::vector<double> tm, const double *qInit,
                          unsigned int len);

  bool resetJointGroup(const char *gname, const double *full);
  //
  bool setJointAnglesSequence(std::vector<const double *> pos,
  ...
  bool setJointAnglesSequenceFull(
      std::vector<const double *> pos, std::vector<const double *> vel,
  ...
  void get(double *o_q, double *o_zmp, double *o_accel, double *o_basePos,
           double *o_baseRpy, double *o_tq, double *o_wrenches,
 ...
 private:
  class groupInterpolator {
   public:
    ...
    ~groupInterpolator() { delete inter; }
