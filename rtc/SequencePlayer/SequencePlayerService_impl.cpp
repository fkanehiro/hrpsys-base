...
CORBA::Boolean SequencePlayerService_impl::waitInterpolationOfGroup(
    const char* gname) {
...
CORBA::Boolean SequencePlayerService_impl::setJointAngles(const dSequence& jvs,
  ...
  if (jvs.length() != (unsigned int)(m_player->robot()->numJoints())) {
    std::cerr << __PRETTY_FUNCTION__
              ...
              << ", robot:" << (unsigned int)(m_player->robot()->numJoints())
              << std::endl;
    return false;
  }
...
CORBA::Boolean SequencePlayerService_impl::setJointAnglesSequence(
    const dSequenceSequence& jvss, const dSequence& tms) {
...
CORBA::Boolean SequencePlayerService_impl::setJointAnglesSequenceWithMask(
    const dSequenceSequence& jvss, const bSequence& mask,
    const dSequence& tms) {
  if (jvss.length() <= 0) {
    std::cerr << __PRETTY_FUNCTION__
  ...
  if (jvss.length() != tms.length()) {
    std::cerr << __PRETTY_FUNCTION__
              << " length of joint angles sequence and time sequence differ, "
                 "joint angle:"
              << jvss.length() << ", time:" << tms.length() << std::endl;
    return false;
  }
...
CORBA::Boolean SequencePlayerService_impl::setJointAnglesSequenceFull(
    const dSequenceSequence& jvss, const dSequenceSequence& vels,
  ...
  if (vels.length() > 0) {
    ...
    if (vels.length() != tms.length()) {
      std::cerr << __PRETTY_FUNCTION__
                << " length of joint velocitys sequence and time sequence "
                   "differ, joint velocity:"
  ...
  if (torques.length() > 0) {
    ...
    if (torques.length() != tms.length()) {
      std::cerr << __PRETTY_FUNCTION__
                << " length of joint torque sequence and time sequence differ, "
                   "joint torque:"
                << torques.length() << ", time:" << tms.length() << std::endl;
      return false;
    }
    if (torque.length() != (unsigned int)(m_player->robot()->numJoints())) {
      std::cerr << __PRETTY_FUNCTION__
                << " num of joint is differ, input:" << jvs.length()
                << ", torque:" << torque.length() << ", robot"
                << (unsigned int)(m_player->robot()->numJoints()) << std::endl;
      return false;
    }
  }
  ...
  if (poss.length() > 0) {
    ...
    if (poss.length() != tms.length()) {
      std::cerr << __PRETTY_FUNCTION__
                << " length of base pos sequence and time sequence differ, pos:"
                << poss.length() << ", time:" << tms.length() << std::endl;
      return false;
    }
    if (pos.length() != 3) {
      std::cerr << __PRETTY_FUNCTION__
                << " num of base pos is differ, pos:" << pos.length()
                << std::endl;
      return false;
    }
  }
  ...
  if (rpys.length() > 0) {
    ...
    if (rpys.length() != tms.length()) {
      std::cerr << __PRETTY_FUNCTION__
                << " length of base rpy sequence and time sequence differ, rpy:"
                << rpys.length() << ", time:" << tms.length() << std::endl;
      return false;
    }
    if (rpy.length() != 3) {
      std::cerr << __PRETTY_FUNCTION__
                << " num of base rpy is differ, rpy:" << rpy.length()
                << std::endl;
      return false;
    }
  }
  ...
  if (accs.length() > 0) {
    ...
    if (accs.length() != tms.length()) {
      std::cerr << __PRETTY_FUNCTION__
                << " length of joint accocitys sequence and time sequence "
                   "differ, joint accocity:"
  ...
  if (zmps.length() > 0) {
    ...
    if (zmps.length() != tms.length()) {
      std::cerr << __PRETTY_FUNCTION__
                << " length of zmp sequence and time sequence differ, zmp:"
                << zmps.length() << ", time:" << tms.length() << std::endl;
      return false;
    }
    if (zmp.length() != 3) {
      std::cerr << __PRETTY_FUNCTION__
  ...
  if (wrenches.length() > 0) {
    if (wrenches.length() != tms.length()) {
      std::cerr
          << __PRETTY_FUNCTION__
          << " length of wrench sequence and time sequence differ, wrench:"
          << wrenches.length() << ", time:" << tms.length() << std::endl;
      return false;
    }
  ...
  if (optionals.length() > 0) {
    if (optionals.length() != tms.length()) {
      std::cerr
          << __PRETTY_FUNCTION__
          << " length of optional sequence and time sequence differ, optional:"
          << optionals.length() << ", time:" << tms.length() << std::endl;
      return false;
    }
  ...
  return m_player->setJointAnglesSequenceFull(
      jvss, vels, torques, poss, rpys, accs, zmps, wrenches, optionals, tms);
}
...
CORBA::Boolean SequencePlayerService_impl::setWrenches(
...
CORBA::Boolean SequencePlayerService_impl::setTargetPose(const char* gname,
  ...
  if (frame_name) {
    ((char*)gname)[frame_name - gname] =
        '\0';      // cut frame_name, gname[strpos(':')] = 0x00
    frame_name++;  // skip ":"
  }
...
CORBA::Boolean SequencePlayerService_impl::clearOfGroup(
    const char* gname, CORBA::Double i_limitation) {
  m_player->player()->clearOfGroup(gname, i_limitation);
}

void SequencePlayerService_impl::clearNoWait() { m_player->setClearFlag(); }

CORBA::Boolean SequencePlayerService_impl::setInterpolationMode(
...
CORBA::Boolean SequencePlayerService_impl::addJointGroup(
    const char* gname,
...
CORBA::Boolean SequencePlayerService_impl::setJointAnglesOfGroup(
    const char* gname, const dSequence& jvs, CORBA::Double tm) {
  return m_player->setJointAnglesOfGroup(gname, jvs, tm);
}

CORBA::Boolean SequencePlayerService_impl::setJointAnglesSequenceOfGroup(
    const char* gname, const dSequenceSequence& jvss, const dSequence& tms) {
  if (jvss.length() != tms.length()) {
    std::cerr << __PRETTY_FUNCTION__
              << " length of joint angles sequence and time sequence differ, "
                 "joint angle:"
...
CORBA::Boolean SequencePlayerService_impl::clearJointAnglesOfGroup(
    const char* gname) {
  return m_player->clearJointAnglesOfGroup(gname);
}

CORBA::Boolean SequencePlayerService_impl::playPatternOfGroup(
