...
static const char* spec[] = {
    "implementation_id", "PointCloudViewer", "type_name", "PointCloudViewer",
    "description", "Point Cloud Viewer", "version", HRPSYS_PACKAGE_VERSION,
...
RTC::ReturnCode_t PointCloudViewer::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_cloudIn.isNew()) {
    ...
    for (int i = 0; i < m_cloud.fields.length(); i++) {
      ...
      if (tmp_name.find("r") != std::string::npos ||
          tmp_name.find("g") != std::string::npos ||
          tmp_name.find("b") != std::string::npos) {
    ...
    if (is_color_points) {
      ...
      cloud->is_dense = m_cloud.is_dense;  // need to handle pointcloud which
                                           // has invalid points (is_dense =
                                           // false)
...
void PointCloudViewerInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<PointCloudViewer>,
