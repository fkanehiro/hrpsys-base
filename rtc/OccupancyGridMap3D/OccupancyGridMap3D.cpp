...
static const char *occupancygridmap3d_spec[] = {
    "implementation_id", "OccupancyGridMap3D", "type_name",
    "OccupancyGridMap3D", "description", "null component", "version",
    HRPSYS_PACKAGE_VERSION, "vendor", "AIST", "category", "example",
    "activity_type", "DataFlowComponent", "max_instance", "10", "language",
    "C++", "lang_type", "compile",
    // Configuration variables
    "conf.default.occupiedThd", "0.5", "conf.default.resolution", "0.1",
    "conf.default.initialMap", "", "conf.default.knownMap", "",
...
RTC::ReturnCode_t OccupancyGridMap3D::onInitialize() {
  ...
  m_OGMap3DServicePort.registerProvider("service1", "OGMap3DService",
                                        m_service0);
...
RTC::ReturnCode_t OccupancyGridMap3D::onActivated(RTC::UniqueId ec_id) {
  ...
  if (m_initialMap != "") {
    // Working directories of threads which calls onInitialize() and
    // onActivate() are different on MacOS
    // Assume path of initial map is given by a relative path to working
    // directory of the thread which calls onInitialize()
  ...
  if (KDEBUG) {
    std::cout << m_profile.instance_name
              << ": initial tree depth = " << m_map->getTreeDepth()
              << std::endl;
    std::cout << m_profile.instance_name
              << ": initial tree size = " << m_map->size() << std::endl;
    point3d p;
    //    p.x() = 0.1; //0.05;
    //    p.y() = 2.2; //2.15;
    //    p.z() = 1.4; //1.45;
    ...
    if (result) {
      ...
      std::cout << m_profile.instance_name << ": initial " << p << " " << prob
                << std::endl;
      std::cout << m_profile.instance_name
                << ": second tree depth = " << m_map->getTreeDepth()
                << std::endl;
      std::cout << m_profile.instance_name
                << ": second tree size = " << m_map->size() << std::endl;
    }
  }
...
RTC::ReturnCode_t OccupancyGridMap3D::onExecute(RTC::UniqueId ec_id) {
  ...
  while (m_rangeIn.isNew()) {
    ...
    pose6d frame(pose.position.x, pose.position.y, pose.position.z,
  ...
  if (m_cloudIn.isNew()) {
    ...
    if (strcmp(m_cloud.type, "xyz") == 0 ||
      ...
      point3d sensor(m_sensorPos.data.x, m_sensorPos.data.y,
                     m_sensorPos.data.z);
      pose6d frame(m_pose.data.position.x, m_pose.data.position.y,
                   m_pose.data.position.z, m_pose.data.orientation.r,
                   m_pose.data.orientation.p, m_pose.data.orientation.y);
      m_map->insertPointCloud(cloud, sensor, frame);
      ...
      R = hrp::rotFromRpy(m_pose.data.orientation.r, m_pose.data.orientation.p,
      ...
      if (result1) {
        ...
        std::cout << m_profile.instance_name << ": " << pp1
                  << " original prob = " << prob1 << std::endl;
        std::cout << m_profile.instance_name << ": " << pp1
                  << " key = " << result1_key[0] << " " << result1_key[1] << " "
      ...
      if (result2) {
        ...
        std::cout << m_profile.instance_name << ": " << pp2
                  << " original prob = " << prob2 << std::endl;
        std::cout << m_profile.instance_name << ": " << pp2
                  << " key = " << result2_key[0] << " " << result2_key[1] << " "
      ...
      if (result3) {
        ...
        std::cout << m_profile.instance_name << ": " << pp3
                  << " original prob = " << prob3 << std::endl;
        std::cout << m_profile.instance_name << ": " << pp3
                  << " key = " << result3_key[0] << " " << result3_key[1] << " "
      ...
      if (result4) {
        ...
        std::cout << m_profile.instance_name << ": " << pp4
                  << " original prob = " << prob4 << std::endl;
        std::cout << m_profile.instance_name << ": " << pp4
                  << " key = " << result4_key[0] << " " << result4_key[1] << " "
      ...
      if (result5) {
        ...
        std::cout << m_profile.instance_name << ": " << pp5
                  << " original prob = " << prob5 << std::endl;
        std::cout << m_profile.instance_name << ": " << pp5
                  << " key = " << result5_key[0] << " " << result5_key[1] << " "
      ...
      for (int i = 0; i < 5; i++) {
        ...
        for (int j = 0; j < 5; j++) {
          ...
          if (result) {
            ...
            std::cout << m_profile.instance_name << ": " << pp
                      << " original prob = " << prob << std::endl;
            std::cout << m_profile.instance_name << ": " << pp
                      << " key = " << result_key[0] << " " << result_key[1]
      ...
      for (unsigned int i = 0; i < m_cloud.data.length() / 16; i++, ptr += 4) {
        ...
        if ((pworld[0] > (pp1.x() - 0.1) && pworld[0] < pp1.x() &&
             pworld[1] > pp1.y() && pworld[1] < (pp1.y() + 0.1) &&
             pworld[2] > pp1.z() && pworld[2] < (pp1.z() + 0.1)) ||
            (pworld[0] > (pp2.x() - 0.1) && pworld[0] < pp2.x() &&
             pworld[1] > pp2.y() && pworld[1] < (pp2.y() + 0.1) &&
             pworld[2] > pp2.z() && pworld[2] < (pp2.z() + 0.1)) ||
            (pworld[0] > (pp3.x() - 0.1) && pworld[0] < pp3.x() &&
             pworld[1] > pp3.y() && pworld[1] < (pp3.y() + 0.1) &&
             pworld[2] > pp3.z() && pworld[2] < (pp3.z() + 0.1))) {
          target_node = m_map->search(pog);  // 121023
          m_map->genKey(pog, target_key);    // 121023
          if (target_node) {
            double beforeprob = target_node->getOccupancy();
            std::cout << m_profile.instance_name << ": " << pog
                      << " before prob = " << beforeprob << std::endl;
            std::cout << m_profile.instance_name << ": " << pog
                      << " key = " << target_key[0] << " " << target_key[1]
        ...
        OcTreeNode *updated_node = m_map->updateNode(
            pog, ptr[3] > 0.0 ? true : false, false);  // 121023
#if KDEBUG
#if 0
        ...
        if ((pworld[0] > (pp1.x() - 0.1) && pworld[0] < pp1.x() &&
          ...
          if (result2) {
            ...
            //		  if(!(fabs(newprob - prob2) < 0.0001)){ // correct
            // 121023
            std::cout << m_profile.instance_name << ": " << pp2
                      << " updated prob = " << newprob << " by " << pog
                      << " prob = " << prob2 << std::endl;
            std::cout << m_profile.instance_name << ": " << pp2
                      << " key = " << result2_key[0] << " " << result2_key[1]
                      << " " << result2_key[2] << std::endl;
            std::cout << m_profile.instance_name << ": " << pp2
                      << " hasChildren = " << result2->hasChildren()
                      << std::endl;
            prob2 = newprob;
            //		  }
          } else
      ...
      if (KDEBUG)
        std::cout << m_profile.instance_name << ": " << ocnum << " " << emnum
                  << " " << p << std::endl;
    } else {
      std::cout << "point type(" << m_cloud.type << ") is not supported"
                << std::endl;
        ...
        for (int k = 0; k < map->nz; k++) {
          ...
          if (result && !(result->hasChildren())) {  // 121023
            ...
            if (prob >= m_occupiedThd) {
              ...
              no++;
              //			    if(KDEBUG) std::cout <<
              // m_profile.instance_name << ": output " << p << " " << prob <<
              // std::endl;
            } else {
...
void OccupancyGridMap3DInit(RTC::Manager *manager) {
  ...
  manager->registerFactory(profile, RTC::Create<OccupancyGridMap3D>,
