...
class testIIRFilter {
 protected:
  ...
  bool use_gnuplot;
  void fprintf_value(FILE* fp, const double _time, const T& _input,
                     const T& _output);
  void fprintf_plot(FILE* gp_pos);
  void gen_pattern_and_plot(const std::vector<double>& time_vec,
                            const std::vector<T>& input_vec) {
    std::string fname("/tmp/plot-iirfilter.dat");
    FILE* fp = fopen(fname.c_str(), "w");
    for (size_t i = 0; i < time_vec.size(); i++) {
      fprintf_value(fp, time_vec[i], input_vec[i],
                    filter->passFilter(input_vec[i]));
    }
    fclose(fp);
    if (use_gnuplot) {
      //   plot pos
  ...
  double calc_sin_value(const size_t i) {
    return std::sin(2 * M_PI * i * dt * input_freq);
  };

 public:
  std::vector<std::string> arg_strs;
  testIIRFilter(const double _dt = 0.004)
      : dt(_dt), input_freq(1.0), use_gnuplot(true) {
    initialize();
  };
  void initialize() {
  ...
  void parse_params() {
    for (int i = 0; i < arg_strs.size(); ++i) {
      if (arg_strs[i] == "--use-gnuplot") {
        ...
        if (++i < arg_strs.size())
          filter->setCutOffFreq(atof(arg_strs[i].c_str()));
      } else if (arg_strs[i] == "--input-freq") {
        if (++i < arg_strs.size()) input_freq = atof(arg_strs[i].c_str());
      }
    }
    std::cerr << "[testIIRFilter] params" << std::endl;
    std::cerr << "[testIIRFilter]   dt = " << dt
              << "[s], cutoff-freq = " << filter->getCutOffFreq()
              << "[Hz], input-freq = " << input_freq << "[Hz]" << std::endl;
  };
};

// Specialization for double
template <>
void testIIRFilter<double, FirstOrderLowPassFilter<double> >::fprintf_value(
...
template <>
double
testIIRFilter<double, FirstOrderLowPassFilter<double> >::test0_input_value(
    const size_t i) {
  return calc_sin_value(i);
};
template <>
void testIIRFilter<double, FirstOrderLowPassFilter<double> >::fprintf_plot(
    FILE* gp_pos) {
  fprintf(gp_pos,
          "plot '/tmp/plot-iirfilter.dat' using 1:2 with lines title 'input' "
          "lw 4, '/tmp/plot-iirfilter.dat' using 1:3 with lines title "
          "'filtered' lw 3\n");
};

// Specialization for hrp::Vector3
template <>
void testIIRFilter<hrp::Vector3, FirstOrderLowPassFilter<hrp::Vector3> >::
    fprintf_value(FILE* fp, const double _time, const hrp::Vector3& _input,
                  const hrp::Vector3& _output) {
  fprintf(fp, "%f %f %f %f %f %f %f\n", _time, _input[0], _output[0], _input[1],
          _output[1], _input[2], _output[2]);
};
template <>
hrp::Vector3 testIIRFilter<
    hrp::Vector3, FirstOrderLowPassFilter<hrp::Vector3> >::init_value() {
  return hrp::Vector3::Zero();
};
template <>
hrp::Vector3 testIIRFilter<
    hrp::Vector3,
    FirstOrderLowPassFilter<hrp::Vector3> >::test0_input_value(const size_t i) {
  double tmp = calc_sin_value(i);
  return hrp::Vector3(tmp, 2 * tmp, -0.5 * tmp);
};
template <>
void testIIRFilter<hrp::Vector3, FirstOrderLowPassFilter<hrp::Vector3> >::
    fprintf_plot(FILE* gp_pos) {
  fprintf(gp_pos,
          "plot '/tmp/plot-iirfilter.dat' using 1:2 with lines title 'input "
          "(0)' lw 4, '/tmp/plot-iirfilter.dat' using 1:3 with lines title "
          "'filtered (0)' lw 3,");
  fprintf(gp_pos,
          "'/tmp/plot-iirfilter.dat' using 1:4 with lines title 'input (1)' lw "
          "4, '/tmp/plot-iirfilter.dat' using 1:5 with lines title 'filtered "
          "(1)' lw 3,");
  fprintf(gp_pos,
          "'/tmp/plot-iirfilter.dat' using 1:6 with lines title 'input (2)' lw "
          "4, '/tmp/plot-iirfilter.dat' using 1:7 with lines title 'filtered "
          "(2)' lw 3\n");
};
///
template <>
void testIIRFilter<double, IIRFilter>::initialize() {
...
void testIIRFilter<double, IIRFilter>::parse_params() {
  ...
  std::cerr << "[testIIRFilter] params" << std::endl;
  std::cerr << "[testIIRFilter]   dt = " << dt << "[s], cutoff-freq = " << 8
            << "[Hz], input-freq = " << input_freq << "[Hz]" << std::endl;
};
template <>
