...
IIRFilter::IIRFilter(unsigned int dim, std::vector<double>& fb_coeffs,
  ...
  if (fb_coeffs.size() != dim + 1 || ff_coeffs.size() != dim + 1) {
    std::cout << "[" << m_error_prefix << "]"
              << "IIRFilter coefficients size error" << std::endl;
    return;
  }
  for (std::vector<double>::iterator it = fb_coeffs.begin();
       it != fb_coeffs.end(); it++) {
    m_fb_coefficients.push_back(*it);
  }
  for (std::vector<double>::iterator it = ff_coeffs.begin();
       it != ff_coeffs.end(); it++) {
...
IIRFilter::IIRFilter(const std::string& error_prefix) : m_initialized(false) {
  m_error_prefix = error_prefix;
}

bool IIRFilter::setParameter(int dim, std::vector<double>& A,
                             std::vector<double>& B) {
  m_dimension = dim;
...
bool IIRFilter::setParameterAsBiquad(const double f_cutoff, const double Q,
                                     const double hz) {
...
void IIRFilter::getParameter(int& dim, std::vector<double>& A,
                             std::vector<double>& B) {
  dim = m_dimension;
...
void IIRFilter::reset(double initial_input) {
  ...
  double sum_ff_coeffs =
      std::accumulate(m_ff_coefficients.begin(), m_ff_coefficients.end(), 0.0);
