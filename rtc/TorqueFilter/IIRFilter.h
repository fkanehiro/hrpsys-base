...
/**
   ...
   y[n] = sum(0, dim, ff_coeffs[i] * x[n - i]) + sum(1, dim, fb_coeffs[i] * y[n
   - i])
 */
class IIRFilter {
 public:
  ...
  */
  IIRFilter(unsigned int dim, std::vector<double>& fb_coeffs,
  ...
  */
  ...
  /**
     ...
     Y[n] = B[0] * X[n] + B[1] * X[n-1] + ... + B[dim] * X[n-dim] - A[1] *
     Y[n-1] ... - A[dim] * Y[n-dim]
     A[0] would be 1.0
  ...
  */
  bool setParameterAsBiquad(const double f_cutoff, const double Q,
                            const double hz);
 ...
 private:
  ...
  int m_dimension;
  std::vector<double> m_fb_coefficients;  // fb parameters (dim must be
                                          // m_dimension + 1,
                                          // m_fb_coefficients[0] would be 1.0)
  std::vector<double>
      m_ff_coefficients;  // ff parameters (dim must be m_dimension + 1)
...
class FirstOrderLowPassFilter {
 ...
 public:
  FirstOrderLowPassFilter(const double _cutoff_freq, const double _dt,
                          const T init_value)
      : prev_value(init_value), dt(_dt) {
    setCutOffFreq(_cutoff_freq);
  };
  ~FirstOrderLowPassFilter(){};
  T passFilter(T value) {
    prev_value = 1.0 / (1 + const_param) * prev_value +
                 const_param / (1 + const_param) * value;
    return prev_value;
  };
