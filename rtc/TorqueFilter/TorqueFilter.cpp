...
static const char* torquefilter_spec[] = {
    "implementation_id", "TorqueFilter", "type_name", "TorqueFilter",
...
RTC::ReturnCode_t TorqueFilter::onInitialize() {
  ...
  coil::stringTo(m_is_gravity_compensation,
                 prop["gravity_compensation"].c_str());
  if (m_debugLevel > 0) {
    std::cerr << "[" << m_profile.instance_name
              << "] : gravity compensation flag: " << m_is_gravity_compensation
              << std::endl;
  }
  ...
  if (m_torque_offset.size() == torque_offset.size() && m_debugLevel > 0) {
    for (unsigned int i = 0; i < m_robot->numJoints(); i++) {
      ...
      std::cerr << "[" << m_profile.instance_name << "]"
                << "offset[" << m_robot->joint(i)->name
                << "]: " << m_torque_offset[i] << std::endl;
    }
  } else {
    if (m_debugLevel > 0) {
      std::cerr << "[" << m_profile.instance_name << "]"
      ...
      std::cerr << "[" << m_profile.instance_name << "]"
                << "joints: " << m_robot->numJoints() << std::endl;
      std::cerr << "[" << m_profile.instance_name << "]"
  ...
  // make filter
  // filter_dim, fb_coeffs[0], ..., fb_coeffs[filter_dim], ff_coeffs[0], ...,
  // ff_coeffs[filter_dim]
  coil::vstring torque_filter_params =
      coil::split(prop["torque_filter_params"], ",");  // filter values
  int filter_dim = 0;
  ...
  if (torque_filter_params.size() > 0) {
    ...
    if (m_debugLevel > 0) {
      ...
      std::cerr << "[" << m_profile.instance_name << "]"
                << "torque filter param size: " << torque_filter_params.size()
                << std::endl;
    }
  } else {
    ...
    if (m_debugLevel > 0) {
      std::cerr << "[" << m_profile.instance_name << "]"
                << "There is no torque_filter_params. Use default values."
                << std::endl;
    }
  }
  if (!use_default_flag &&
      ((filter_dim + 1) * 2 + 1 != (int)torque_filter_params.size())) {
    if (m_debugLevel > 0) {
      std::cerr
          << "[" << m_profile.instance_name << "]"
          << "Size of torque_filter_params is not correct. Use default values."
          << std::endl;
    }
    use_default_flag = true;
  }
  ...
  if (use_default_flag) {
    ...
    for (int i = 0; i < filter_dim + 1; i++) {
      ...
      coil::stringTo(ff_coeffs[i],
                     torque_filter_params[i + (filter_dim + 2)].c_str());
    }
  }
  ...
  if (m_debugLevel > 0) {
    for (int i = 0; i < filter_dim + 1; i++) {
      std::cerr << "[" << m_profile.instance_name << "]"
                << "fb[" << i << "]: " << fb_coeffs[i] << std::endl;
      std::cerr << "[" << m_profile.instance_name << "]"
                << "ff[" << i << "]: " << ff_coeffs[i] << std::endl;
    }
  }
  ...
  if (m_tauIn.data.length() == m_robot->numJoints()) {
    ...
    if (m_qCurrent.data.length() == m_robot->numJoints()) {
      ...
      for (int i = 0; i < num_joints; i++) {
        ...
        g_joint_torque[i] =
            (m_robot->joint(i)->subm * g)
                .cross(m_robot->joint(i)->submwc / m_robot->joint(i)->subm -
                       m_robot->joint(i)->p)
                .dot(m_robot->joint(i)->R * m_robot->joint(i)->a);
      }
    } else {
      if (DEBUGP) {
        std::cerr << "[" << m_profile.instance_name << "]"
        ...
        std::cerr << "[" << m_profile.instance_name << "]"
                  << " qCurrent: " << m_qCurrent.data.length() << std::endl;
        std::cerr << std::endl;
      }
      for (int i = 0; i < num_joints; i++) {
    ...
    for (int i = 0; i < num_joints; i++) {
      ...
      // torque[j] =
      // m_filters[path->joint(j)->jointId].executeFilter(m_tauIn.data[path->joint(j)->jointId])
      // - joint_torque(j); // use filtered tau
      torque[i] =
          m_filters[i].executeFilter(m_tauIn.data[i]) - m_torque_offset[i];

      // torque calclation from error of joint angle
      // if ( m_error_to_torque_gain[path->joint(j)->jointId] == 0.0
      //      || fabs(joint_error(j)) <
      //      m_error_dead_zone[path->joint(j)->jointId] ) {
      //   torque[j] = 0;
      // } else {
      //   torque[j] = error2torque(j,
      //   fabs(m_error_dead_zone[path->joint(j)->jointId]));
      // }
    ...
    if (DEBUGP) {
      ...
      std::cerr << "[" << m_profile.instance_name << "]"
                << " numJoints: " << m_robot->numJoints() << std::endl;
      std::cerr << "[" << m_profile.instance_name << "]"
                << " tauIn: " << m_tauIn.data.length() << std::endl;
      std::cerr << std::endl;
    }
  }
...
void TorqueFilterInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<TorqueFilter>,
