...
static const char* spec[] = {
    "implementation_id", "PlaneRemover", "type_name", "PlaneRemover",
    "description", "Plane Remover", "version", HRPSYS_PACKAGE_VERSION, "vendor",
    "AIST", "category", "example", "activity_type", "DataFlowComponent",
    "max_instance", "10", "language", "C++", "lang_type", "compile",
    // Configuration variables
    "conf.default.distanceThd", "0.02", "conf.default.pointNumThd", "500",
...
RTC::ReturnCode_t PlaneRemover::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_originalIn.isNew()) {
    ...
    // std::cout << "PLaneRemover: original = " << original->points.size() << ",
    // filtered = " << cloud->points.size() << ", thd=" << m_distThd <<
    // std::endl;
...
void PlaneRemoverInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<PlaneRemover>,
