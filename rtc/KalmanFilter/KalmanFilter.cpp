...
static const char* kalmanfilter_spec[] = {
    "implementation_id", "KalmanFilter", "type_name", "KalmanFilter",
    "description", "kalman filter", "version", HRPSYS_PACKAGE_VERSION, "vendor",
...
RTC::ReturnCode_t KalmanFilter::onInitialize() {
  ...
  m_KalmanFilterServicePort.registerProvider("service0", "KalmanFilterService",
                                             m_service0);
  ...
  if (!coil::stringTo(m_dt, prop["dt"].c_str())) {
    std::cerr << "[" << m_profile.instance_name << "]failed to get dt"
              << std::endl;
...
RTC::ReturnCode_t KalmanFilter::onExecute(RTC::UniqueId ec_id) {
  ...
  // std::cerr << m_profile.instance_name<< ": onExecute(" << ec_id << ") " <<
  // std::endl;
  if (m_rpyIn.isNew()) {
    m_rpyIn.read();
  ...
  if (m_accRefIn.isNew()) {
    m_accRefIn.read();
    sx_ref = m_accRef.data.ax, sy_ref = m_accRef.data.ay,
    sz_ref = m_accRef.data.az;
  }
  if (m_accIn.isNew()) {
    m_accIn.read();
    ...
    Eigen::Vector3d acc =
        m_sensorR *
        hrp::Vector3(m_acc.data.ax - sx_ref + acc_offset(0),
                     m_acc.data.ay - sy_ref + acc_offset(1),
                     m_acc.data.az - sz_ref +
                         acc_offset(2));  // transform to imaginary acc data
    acc = sensorR_offset * acc;
    Eigen::Vector3d gyro =
        m_sensorR *
        hrp::Vector3(m_rate.data.avx, m_rate.data.avy,
                     m_rate.data.avz);  // transform to imaginary rate data
    gyro = sensorR_offset * gyro;
    if (DEBUGP) {
      std::cerr << "[" << m_profile.instance_name
                << "] raw data acc : " << std::endl
                << acc << std::endl;
      std::cerr << "[" << m_profile.instance_name
                << "] raw data gyro : " << std::endl
                << gyro << std::endl;
    }
    hrp::Vector3 rpy, rpyRaw, baseRpyCurrent;
    if (kf_algorithm ==
        OpenHRP::KalmanFilterService::QuaternionExtendedKalmanFilter) {
...
bool KalmanFilter::setKalmanFilterParam(
    const OpenHRP::KalmanFilterService::KalmanFilterParam& i_param) {
  std::cerr << "[" << m_profile.instance_name << "] setKalmanFilterParam"
            << std::endl;
  rpy_kf.setParam(m_dt, i_param.Q_angle, i_param.Q_rate, i_param.R_angle,
                  std::string(m_profile.instance_name));
  ...
  std::cerr << "[" << m_profile.instance_name << "]   kf_algorithm="
            << (kf_algorithm == OpenHRP::KalmanFilterService::RPYKalmanFilter
                    ? "RPYKalmanFilter"
                    : "QuaternionExtendedKalmanFilter")
            << std::endl;
  std::cerr << "[" << m_profile.instance_name << "]   acc_offset = "
            << acc_offset.format(Eigen::IOFormat(
                   Eigen::StreamPrecision, 0, ", ", ", ", "", "", "    [", "]"))
            << std::endl;
  std::cerr << "[" << m_profile.instance_name << "]   sensorRPY_offset = "
...
bool KalmanFilter::getKalmanFilterParam(
...
void KalmanFilterInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<KalmanFilter>,
