...
class EKFilter {
 public:
  EKFilter()
      : P(hrp::Matrix77::Identity() * 0.1),
        ...
        min_mag_thre_acc(0.005),
        max_mag_thre_acc(0.05),
        min_mag_thre_gyro(0.0075),
        max_mag_thre_gyro(0.035) {
    x << 1, 0, 0, 0, 0, 0, 0;
  }
  ...
  void calcOmega(Eigen::Matrix4d& omega, const Eigen::Vector3d& w) const {
    ...
    omega << 0, -w[0], -w[1], -w[2], w[0], 0, w[2], -w[1], w[1], -w[2], 0, w[0],
        w[2], w[1], -w[0], 0;
  }

  void calcPredictedState(hrp::Vector7& _x_a_priori, const Eigen::Vector4d& q,
  ...
  void calcF(hrp::Matrix77& F, const Eigen::Vector4d& q,
             const Eigen::Vector3d& gyro, const Eigen::Vector3d& drift) const {
    F = hrp::Matrix77::Identity();
    Eigen::Vector3d gyro_compensated = gyro - drift;
    Eigen::Matrix4d omega;
    calcOmega(omega, gyro_compensated);
    F.block<4, 4>(0, 0) += dt / 2 * omega;
    F.block<4, 3>(0, 4) << +q[1], +q[2], +q[3], -q[0], +q[3], -q[2], -q[3],
        -q[0], +q[1], +q[2], -q[1], -q[0];
  ...
  void calcPredictedCovariance(hrp::Matrix77& _P_a_priori,
    ...
    V_upper << -q[1], -q[2], -q[3], +q[0], -q[3], +q[2], +q[3], +q[0], -q[1],
        -q[2], +q[1], +q[0];
    V_upper *= dt / 2;
  ...
  void calcH(Eigen::Matrix<double, 3, 7>& H, const Eigen::Vector4d& q) const {
    ...
    H << -y, +z, -w, +x, 0, 0, 0, +x, +w, +z, +y, 0, 0, 0, +w, -x, -y, +z, 0, 0,
        0;
    H *= 2 * g_vec[2];
  }
  ...
  void printAll() const {
    ...
    std::cerr << "P_a_priori" << std::endl
              << P_a_priori << std::endl
              << std::endl;
  }

  // Basically Equation (23), (24) and (25) in the paper [1]
  // [1] Chul Woo Kang and Chan Gook Park. Attitude estimation with
  // accelerometers and gyros using fuzzy tuned Kalman filter.
  //     In European Control Conference, 2009.
  void calcRWithFuzzyRule(Eigen::Matrix3d& fuzzyR, const hrp::Vector3& acc,
                          const hrp::Vector3& gyro) const {
    double alpha =
        std::min(std::fabs(acc.norm() - g_vec.norm()) / g_vec.norm(), 0.1);
    double beta = std::min(gyro.norm(), 0.05);
    double large_mu_acc =
        std::max(std::min((alpha - min_mag_thre_acc) /
                              (max_mag_thre_acc - min_mag_thre_acc),
                          1.0),
                 0.0);
    double large_mu_gyro =
        std::max(std::min((beta - min_mag_thre_gyro) /
                              (max_mag_thre_gyro - min_mag_thre_gyro),
                          1.0),
                 0.0);
    double w1, w2, w3, w4;
    ...
    double z = (w1 * 0.0 + w2 * (3.5 * alpha + 8.0 * beta + 0.5) +
                w3 * (3.5 * alpha + 8.0 * beta + 0.5) + w4 * 1.0) /
               (w1 + w2 + w3 + w4);
    double k1 = 400;
  ...
  void main_one(hrp::Vector3& rpy, hrp::Vector3& rpyRaw,
                const hrp::Vector3& acc, const hrp::Vector3& gyro) {
 ...
 private:
  ...
  double dt;
  double min_mag_thre_acc, max_mag_thre_acc, min_mag_thre_gyro,
      max_mag_thre_gyro;
};
