...
// Usage
// testKalmanFilterEstimation --acc-file /tmp/kftest/test.acc --rate-file
// /tmp/kftest/test.rate --pose-file /tmp/kftest/test.pose --Q-angle 1e-3
// --Q-rate 1e-10 --dt 0.005 --R-angle 1 --use-gnuplot true
...
int main(int argc, char* argv[]) {
  ...
  double R_angle = 10;
  double dt = 0.002;
  std::string pose_file("test.pose"), acc_file("test.acc"),
      rate_file("test.rate");
  ...
  for (int i = 0; i < argc; ++i) {
    ...
    if (arg == "--Q-angle") {  // KF parameters
      ...
      if (++i < argc)
        use_gnuplot = (std::string(argv[i]) == "true" ? true : false);
    }
  }

  // Setup input and output files
  std::ifstream ratef(rate_file.c_str()), accf(acc_file.c_str()),
      posef(pose_file.c_str());
  std::cerr << "File : " << rate_file << " " << acc_file << " " << pose_file
            << std::endl;
  if (!ratef.is_open() || !accf.is_open() || !posef.is_open()) {
    std::cerr << "No such " << rate_file << " " << acc_file << " " << pose_file
              << std::endl;
    return -1;
  }
  ...
  while (!ratef.eof()) {
    posef >> time >> time >> time >> time >> rpyAct[0] >> rpyAct[1] >>
        rpyAct[2];  // Neglect translation in .pose file
    ratef >> time >> rate[0] >> rate[1] >> rate[2];
    accf >> time >> acc[0] >> acc[1] >> acc[2];
    rpy_kf.main_one(rpy, rpyRaw, baseRpyCurrent, acc, rate, 0.0,
                    hrp::Matrix33::Identity());
    // rad->deg
    rpy *= 180 / 3.14159;
    rpyAct *= 180 / 3.14159;
    if (use_gnuplot) {
      ofs << time2 << " " << rpy[0] << " " << rpy[1] << " " << rpy[2] << " "
          << rpyAct[0] << " " << rpyAct[1] << " " << rpyAct[2] << std::endl;
    } else {
      std::cout << rpy[0] << " " << rpy[1] << " " << rpy[2] << " " << rpyAct[0]
                << " " << rpyAct[1] << " " << rpyAct[2] << std::endl;
    }
    time2 += dt;
  }
  // gnuplot
