...
class KFilter {
 public:
  ...
  void setF(const double _f0, const double _f1, const double _f2,
            const double _f3) {
    F << _f0, _f1, _f2, _f3;
  };
  void setP(const double _p0, const double _p1, const double _p2,
            const double _p3) {
    P << _p0, _p1, _p2, _p3;
  };
  void setQ(const double _q0, const double _q1, const double _q2,
            const double _q3) {
    Q << _q0, _q1, _q2, _q3;
  };
...
class RPYKalmanFilter {
 public:
  ...
  void main_one(hrp::Vector3& rpy, hrp::Vector3& rpyRaw,
                hrp::Vector3& baseRpyCurrent, const hrp::Vector3& acc,
                const hrp::Vector3& gyro, const double& sl_y,
    ...
    // hrp::RateGyroSensor* sensor =
    // m_robot->sensor<hrp::RateGyroSensor>("gyrometer");
    double g = sqrt(acc(0) * acc(0) + acc(1) * acc(1) + acc(2) * acc(2));
    // atan2(y, x) = atan(y/x)
    double a, b;
    b = atan2(-acc(0) / g,
              sqrt(acc(1) / g * acc(1) / g + acc(2) / g * acc(2) / g));
    a = atan2((acc(1) / g), (acc(2) / g));
    rpyRaw = hrp::Vector3(a, b, sl_y);
    // #if 0
    //       // complementary filter
    //       m_rpy.data.r = 0.98 *(m_rpy.data.r+m_rate.data.avx*m_dt) +
    //       0.02*m_rpyRaw.data.r;
    //       m_rpy.data.p = 0.98 *(m_rpy.data.p+m_rate.data.avy*m_dt) +
    //       0.02*m_rpyRaw.data.p;
    //       m_rpy.data.y = 0.98 *(m_rpy.data.y+m_rate.data.avz*m_dt) +
    //       0.02*m_rpyRaw.data.y;
    // #endif
    // kalman filter
    // x[0] = m_rpyRaw.data.r; // angle (from m/sec Acceleration3D, unstable, no
    // drift )
    // x[1] = m_rate.data.avx; // rate ( rad/sec, AngularVelocity, gyro,
    // stable/drift )
    ...
    hrp::Matrix33 tmpMat;
    tmpMat << 1.0, sin(roll) * sin(pitch) / cos(pitch),
        cos(roll) * sin(pitch) / cos(pitch), 0, cos(roll), -sin(roll), 0,
        sin(roll) / cos(pitch), cos(roll) / cos(pitch);
    gyro2 = tmpMat * gyro;
#endif
    ...
    hrp::Matrix33 imaginaryRotationMatrix =
        hrp::rotFromRpy(r_filter.getx()[0], p_filter.getx()[0],
                        y_filter.getx()[0]);  // the imaginary frame relative to
                                              // a world reference frame
    hrp::Matrix33 realRotationMatrix =
        imaginaryRotationMatrix *
        m_sensorR;  // the imu frame relative to a world reference frame
    ...
    hrp::Matrix33 WtoB =
        realRotationMatrix *
        BtoS.transpose();  // the body frame relative to a world reference frame
    hrp::Vector3 euler2 = hrp::rpyFromRot(WtoB);
    baseRpyCurrent(0) = euler2(0);
    baseRpyCurrent(1) = euler2(1);
    baseRpyCurrent(2) = euler2(2);
  };
  void setParam(const double _dt, const double _Q_angle, const double _Q_rate,
                const double _R_angle, const std::string print_str = "") {
    Q_angle = _Q_angle;
    Q_rate = _Q_rate;
    R_angle = _R_angle;
    ...
    y_filter.setB(_dt, 0);
    std::cerr << "[" << print_str << "]   Q_angle=" << Q_angle
              << ", Q_rate=" << Q_rate << ", R_angle=" << R_angle << std::endl;
  };
