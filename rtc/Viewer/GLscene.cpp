...
void GLscene::updateScene() {
  ...
  for (unsigned int i = 0; i < ss.states.length(); i++) {
    ...
    glbody->setPosition(rs.basePose.position.x, rs.basePose.position.y,
                        rs.basePose.position.z);
    glbody->setRotation(rs.basePose.orientation.r, rs.basePose.orientation.p,
...
#define HEIGHT_STEP 12
    ...
    for (unsigned int i = 0; i < glbody->numJoints(); i++) {
      ...
      if (l) {
        ...
        sprintf(buf, "%13s %8.3f", l->name.c_str(), rstate->q[i] * 180 / M_PI);
        glRasterPos2f(x, height);
        drawString2(buf);
        x += 8 * (14 + 9);
