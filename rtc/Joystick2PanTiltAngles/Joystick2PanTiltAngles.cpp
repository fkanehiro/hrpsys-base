...
static const char* joystick2velocity_spec[] = {
    "implementation_id", "Joystick2PanTiltAngles", "type_name",
    "Joystick2PanTiltAngles", "description",
    ...
    "conf.default.scales", "1.0,1.0,1.0", "conf.default.neutrals",
    "0.0,0.0,0.0", ""};
// </rtc-template>
...
RTC::ReturnCode_t Joystick2PanTiltAngles::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_debugLevel > 0) {
    printf("pan/tilt command: %5.2f %5.2f", m_angles.pan, m_angles.tilt);
  }
  m_anglesOut.write();
...
void Joystick2PanTiltAnglesInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<Joystick2PanTiltAngles>,
