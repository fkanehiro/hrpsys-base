...
static const char* emergencystopper_spec[] = {
    "implementation_id", "EmergencyStopper", "type_name", "EmergencyStopper",
    "description", "emergency stopper", "version", HRPSYS_PACKAGE_VERSION,
...
RTC::ReturnCode_t EmergencyStopper::onInitialize() {
  ...
  m_EmergencyStopperServicePort.registerProvider(
      "service0", "EmergencyStopperService", m_service0);
  ...
  for (unsigned int i = 0; i < nforce; i++) {
    m_wrenchesIn[i] = new InPort<TimedDoubleSeq>(
        std::string(fsensor_names[i] + "In").c_str(), m_wrenchesRef[i]);
    m_wrenchesOut[i] = new OutPort<TimedDoubleSeq>(
        std::string(fsensor_names[i] + "Out").c_str(), m_wrenches[i]);
    m_wrenchesRef[i].data.length(6);
    m_wrenchesRef[i].data[0] = m_wrenchesRef[i].data[1] =
        m_wrenchesRef[i].data[2] = 0.0;
    m_wrenchesRef[i].data[3] = m_wrenchesRef[i].data[4] =
        m_wrenchesRef[i].data[5] = 0.0;
    ...
    registerInPort(std::string(fsensor_names[i] + "In").c_str(),
                   *m_wrenchesIn[i]);
    registerOutPort(std::string(fsensor_names[i] + "Out").c_str(),
                    *m_wrenchesOut[i]);
  }
  ...
  m_wrenches_interpolator->setName(std::string(m_profile.instance_name) +
                                   " interpolator wrenches");
  ...
  emergency_stopper_beep_freq =
      static_cast<int>(1.0 / (2.0 * m_dt));  // 2 times / 1[s]
...
RTC::ReturnCode_t EmergencyStopper::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_qRefIn.isNew()) {
    ...
    if (!is_stop_mode) {
      for (unsigned int i = 0; i < m_qRef.data.length(); i++) {
        if (recover_time > 0) {  // Until releasing is finished, do not use
                                 // m_stop_posture in input queue because too
                                 // large error.
      ...
      for (unsigned int i = 0; i < m_wrenchesRef.size(); i++) {
        for (int j = 0; j < 6; j++) {
          if (recover_time > 0) {
            ...
            m_stop_wrenches[i * 6 + j] =
                m_input_wrenches_queue.front()[i * 6 + j];
          }
        }
      }
    }
  ...
  if (DEBUGP) {
    std::cerr << "[" << m_profile.instance_name
              << "] is_stop_mode : " << is_stop_mode
              << " recover_time : " << recover_time
              << "[s] retrieve_time : " << retrieve_time << "[s]" << std::endl;
  }
  ...
  if (is_stop_mode && has_servoOn) {  // If stop mode and some joint is servoOn
    if (emergency_stopper_beep_count % emergency_stopper_beep_freq == 0 &&
        emergency_stopper_beep_count % (emergency_stopper_beep_freq * 3) != 0) {
...
bool EmergencyStopper::stopMotion() {
  ...
  if (!is_stop_mode) {
    is_stop_mode = true;
    std::cerr << "[" << m_profile.instance_name << "] stopMotion is called"
              << std::endl;
  }
  return true;
}
...
bool EmergencyStopper::releaseMotion() {
  ...
  if (is_stop_mode) {
    is_stop_mode = false;
    std::cerr << "[" << m_profile.instance_name << "] releaseMotion is called"
              << std::endl;
  }
  return true;
}

bool EmergencyStopper::getEmergencyStopperParam(
    OpenHRP::EmergencyStopperService::EmergencyStopperParam& i_param) {
  std::cerr << "[" << m_profile.instance_name << "] getEmergencyStopperParam"
            << std::endl;
...
bool EmergencyStopper::setEmergencyStopperParam(
    const OpenHRP::EmergencyStopperService::EmergencyStopperParam& i_param) {
  std::cerr << "[" << m_profile.instance_name << "] setEmergencyStopperParam"
            << std::endl;
  default_recover_time = i_param.default_recover_time / m_dt;
  default_retrieve_time = i_param.default_retrieve_time / m_dt;
  std::cerr << "[" << m_profile.instance_name
            << "]   default_recover_time = " << default_recover_time * m_dt
            << "[s], default_retrieve_time = " << default_retrieve_time * m_dt
            << "[s]" << std::endl;
  return true;
};
...
void EmergencyStopperInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<EmergencyStopper>,
