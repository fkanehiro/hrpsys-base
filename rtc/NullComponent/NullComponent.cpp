...
static const char* nullcomponent_spec[] = {
    "implementation_id", "NullComponent", "type_name", "NullComponent",
    ...
    "conf.default.string", "test", "conf.default.intvec", "1,2,3",
...
RTC::ReturnCode_t NullComponent::onExecute(RTC::UniqueId ec_id) {
  ...
  while (m_dataIn.isNew()) {
    m_dataIn.read();
    std::cout << m_profile.instance_name << ": read(), data = " << m_data.data
              << std::endl;
  }
  m_data.data += 1;
...
void NullComponentInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<NullComponent>,
