...
static const char* spec[] = {
    "implementation_id", "VoxelGridFilter", "type_name", "VoxelGridFilter",
    "description", "Voxel Grid Filter", "version", HRPSYS_PACKAGE_VERSION,
...
RTC::ReturnCode_t VoxelGridFilter::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_originalIn.isNew()) {
    ...
    if (m_debugLevel > 0) {
      std::cout << "original:" << cloud->points.size()
                << ", filterd:" << cloud_filtered->points.size() << std::endl;
    }
...
void VoxelGridFilterInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<VoxelGridFilter>,
