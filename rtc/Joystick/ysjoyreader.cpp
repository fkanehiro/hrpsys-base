...
void YsJoyReaderAxis::CaptureCenter(void) { calibCenter = value; }
...
YsJoyReader::YsJoyReader() { hidDev = NULL; }
  ...
  if (hidDev != NULL) {
    ...
    for (j = 0; j < nElem; j++) {
      IOHIDElementRef elem =
          (IOHIDElementRef)CFArrayGetValueAtIndex(elemAry, j);
          ...
          switch (usagePage) {
            case kHIDPage_GenericDesktop:
              switch (usage) {
                ...
                case kHIDUsage_GD_X:
                  printf(
                      "    This element is for X-Axis (%d->%d) "
                      "Scaled(%d->%d)\n",
                ...
                case kHIDUsage_GD_Y:
                  printf(
                      "    This element is for Y-Axis (%d->%d) "
                      "Scaled(%d->%d)\n",
                      min, max, scaledMin, scaledMax);
                  AddAxis(nAxis++, elem, min, max, scaledMin, scaledMax);
                  break;
                case kHIDUsage_GD_Z:
                  printf(
                      "    This element is for Z-Axis (%d->%d) "
                      "Scaled(%d->%d)\n",
                      min, max, scaledMin, scaledMax);
                  AddAxis(nAxis++, elem, min, max, scaledMin, scaledMax);
                  break;
                case kHIDUsage_GD_Rx:
                  printf(
                      "    This element is for Rx-Axis (%d->%d) "
                      "Scaled(%d->%d)\n",
                      min, max, scaledMin, scaledMax);
                  AddAxis(nAxis++, elem, min, max, scaledMin, scaledMax);
                  break;
                case kHIDUsage_GD_Ry:
                  printf(
                      "    This element is for Ry-Axis (%d->%d) "
                      "Scaled(%d->%d)\n",
                      min, max, scaledMin, scaledMax);
                  AddAxis(nAxis++, elem, min, max, scaledMin, scaledMax);
                  break;
                case kHIDUsage_GD_Rz:
                  printf(
                      "    This element is for Rz-Axis (%d->%d) "
                      "Scaled(%d->%d)\n",
                      min, max, scaledMin, scaledMax);
                  AddAxis(nAxis++, elem, min, max, scaledMin, scaledMax);
                  break;
                case kHIDUsage_GD_Slider:
                  printf(
                      "    This element is for Slider (%d->%d) "
                      "Scaled(%d->%d)\n",
                      min, max, scaledMin, scaledMax);
                  AddAxis(nAxis++, elem, min, max, scaledMin, scaledMax);
                  break;
                ...
                case kHIDUsage_GD_Hatswitch:
                  printf(
                      "    This element is for Hatswitch (%d->%d) "
                      "Scaled(%d->%d)\n",
                      min, max, scaledMin, scaledMax);
...
void YsJoyReader::AddAxis(int axisId, IOHIDElementRef elem, int min, int max,
...
int YsJoyReader::SetUpJoystick(int &nJoystick, YsJoyReader joystick[],
                               int maxNumJoystick) {
  nJoystick = 0;
  ...
  if (NULL != hidManager) {
    IOHIDManagerSetDeviceMatching(hidManager,
                                  NULL);  // Just enumrate all devices
    ...
    for (i = 0; i < nDev && nJoystick < maxNumJoystick; i++) {
      IOHIDDeviceRef hidDev =
          (IOHIDDeviceRef)CFArrayGetValueAtIndex(devArray, i);
...
int YsJoyReader::WriteCalibInfoFile(FILE *fp) const {
  ...
  for (i = 0; i < YsJoyReaderMaxNumAxis; i++) {
    if (0 != axis[i].exist) {
      fprintf(fp, "AXSINF %d %d %d %d\n", i, axis[i].calibCenter,
              axis[i].calibMin, axis[i].calibMax);
    }
  }
#ifdef YSJOYREADER_USE_HAT_CALIBRATION
  for (i = 0; i < YsJoyReaderMaxNumHatSwitch; i++) {
    if (0 != hatSwitch[i].exist) {
      fprintf(fp, "HATINF %d %d %d %d %d %d\n", i, hatSwitch[i].valueNeutral,
              hatSwitch[i].value0Deg, hatSwitch[i].value90Deg,
              hatSwitch[i].value180Deg, hatSwitch[i].value270Deg);
    }
  }
#endif
  fprintf(fp, "ENDJOY\n");
...
int YsJoyReader::ReadCalibInfoFile(FILE *fp) {
  ...
  while (fgets(str, 255, fp) != NULL) {
    if (strncmp(str, "AXSINF", 6) == 0) {
      ...
      int hatId;
      int valueNeutral = 0, value0Deg = 1, value90Deg = 3, value180Deg = 5,
          value270Deg = 7;
      sscanf(str, "%*s %d %d %d %d %d %d", &hatId, &valueNeutral, &value0Deg,
             &value90Deg, &value180Deg, &value270Deg);
