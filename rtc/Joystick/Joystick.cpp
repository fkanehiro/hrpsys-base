...
static const char* joystick_spec[] = {
    "implementation_id", "Joystick", "type_name", "Joystick", "description",
    "Access a joystick control device.", "version", HRPSYS_PACKAGE_VERSION,
    "vendor", "AIST", "category", "Human input", "activity_type", "PERIODIC",
    "kind", "DataFlowComponent", "max_instance", "1", "language", "C++",
    "lang_type", "compile",
    // Configuration variables
    "conf.default.device", "/dev/input/js1", "conf.default.debugLevel", "0",
    ""};
// </rtc-template>
...
RTC::ReturnCode_t Joystick::onActivated(RTC::UniqueId ec_id) {
  ...
  if (m_js->is_open()) {
    ...
    std::cerr << "Joystick device(" << m_device << ") is not opened"
              << std::endl;
...
void JoystickInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<Joystick>,
                           RTC::Delete<Joystick>);
}
};
