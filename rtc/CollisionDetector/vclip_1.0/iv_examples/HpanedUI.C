...
String HpanedUI::_defaultHpanedUIResources[] = {
    "*autoToggle.labelString:  auto move", "*speedScale.titleString:  speed",
...
void HpanedUI::create(Widget parent) {
  ...
  _baseWidget = _hpaned = XtVaCreateWidget(_name, sgHorzPanedWindowWidgetClass,
                                           parent, (XtPointer)NULL);
  ...
  _bulletinBoard = XtVaCreateManagedWidget(
      "bulletinBoard", xmBulletinBoardWidgetClass, _baseWidget, XmNresizePolicy,
      XmRESIZE_GROW, XmNwidth, 132, XmNheight, 566, (XtPointer)NULL);

  _speedScale = XtVaCreateManagedWidget(
      "speedScale", xmScaleWidgetClass, _bulletinBoard, XmNsliderVisual,
      XmSHADOWED, XmNslidingMode, XmSLIDER, XmNvalue, 50, XmNorientation,
      XmVERTICAL, XmNshowValue, True, XmNx, 12, XmNy, 55, XmNwidth, 110,
      XmNheight, 330, (XtPointer)NULL);

  XtAddCallback(_speedScale, XmNvalueChangedCallback,
                &HpanedUI::speedScaleCBCallback, (XtPointer) this);

  XtAddCallback(_speedScale, XmNdragCallback, &HpanedUI::speedScaleCBCallback,
                (XtPointer) this);

  _scale = XtVaCreateManagedWidget("scale", xmScaleWidgetClass, _bulletinBoard,
                                   XmNorientation, XmHORIZONTAL, XmNshowValue,
                                   True, XmNx, 630, XmNy, 85, XmNwidth, 40,
                                   XmNheight, 34, (XtPointer)NULL);

  _autoToggle = XtVaCreateManagedWidget(
      "autoToggle", xmToggleButtonWidgetClass, _bulletinBoard, XmNalignment,
      XmALIGNMENT_BEGINNING, XmNlabelType, XmSTRING, XmNx, 10, XmNy, 470,
      XmNwidth, 105, XmNheight, 26, (XtPointer)NULL);

  XtAddCallback(_autoToggle, XmNvalueChangedCallback,
                &HpanedUI::autoToggleCBCallback, (XtPointer) this);

  XtVaSetValues(_viewer->getWidget(), XmNwidth, 588, XmNheight, 566,
                (XtPointer)NULL);
...
const char* HpanedUI::className() { return ("HpanedUI"); }  // End className()
...
void HpanedUI::autoToggleCBCallback(Widget w, XtPointer clientData,
                                    XtPointer callData) {
  HpanedUI* obj = (HpanedUI*)clientData;
  obj->autoToggleCB(w, callData);
}

void HpanedUI::speedScaleCBCallback(Widget w, XtPointer clientData,
                                    XtPointer callData) {
  HpanedUI* obj = (HpanedUI*)clientData;
