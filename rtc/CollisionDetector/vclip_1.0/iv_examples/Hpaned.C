...
Hpaned::Hpaned(const char *name, Widget parent) : HpanedUI(name, parent) {
...
Hpaned::Hpaned(const char *name) : HpanedUI(name) {
...
void Hpaned::autoToggleCB(Widget w, XtPointer callData) {
  ...
  if ((int)EZ(_autoToggle))
    autoMotionSensor->schedule();
  else
    autoMotionSensor->unschedule();
