...
int random(int n) { return (int)floor(((double)n) * rand() / (RAND_MAX + 1)); }

// return random real in interval [0,1)
double randomReal() { return ((double)rand()) / (RAND_MAX + 1); }
...
void updateCpSegs(int only = -1) {
  ...
  for (i = 0; i < numPtrees; i++)
    graph[i]->replaceChild(1, matChoices->getChild(numPens[i] < numMatChoices
                                                       ? numPens[i]
                                                       : numMatChoices - 1));
}
...
int pathToPtreeIndex(SoPath *selection) {
  ...
  if (!selection->getTail()->isOfType(SoShapeKit::getClassTypeId())) return -1;

  sa.setType(SoTransform::getClassTypeId(),
             TRUE);  // catch TransformManips also
  ...
  for (i = 0; i < numPtrees; i++)
    if (XFORM(i) == selectionXform) break;
...
void detachManip(void *, SoPath *deselectionPath) {
  ...
  phase[i].x =
      fmod(acos(pose[i].trans().x / amp[i].x) - freq[i].x * time_, 2 * M_PI);
  phase[i].y =
      fmod(acos(pose[i].trans().y / amp[i].y) - freq[i].y * time_, 2 * M_PI);
  phase[i].z =
      fmod(acos(pose[i].trans().z / amp[i].z) - freq[i].z * time_, 2 * M_PI);
...
void setup() {
  ...
  timer->speed = 2.0;
  SoSFTime *theTime = (SoSFTime *)SoDB::createGlobalField(
      "theTime", SoType::fromName("SoSFTime"));
