...
int main(int argc, char* argv[]) {
  if (argc < 2) {
    std::cerr << "Usage:" << argv[0] << " project.xml [-co CollisionDetector "
                                        "Component] [-size size] [-bg r g b]"
              << std::endl;
    return 1;
  }
  ...
  while (window.oneStep()) {
    ...
    if (CORBA::is_nil(coService)) {
      try {
        ...
        coService = OpenHRP::CollisionDetectorService::_narrow(
            orb->string_to_object(ior));
      } catch (...) {
        std::cerr << "could not found collision detector component " << coname
                  << std::endl;
        return 1;
      }
    }
