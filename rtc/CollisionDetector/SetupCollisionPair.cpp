...
RTC::ReturnCode_t setupCollisionModel(hrp::BodyPtr m_robot, const char *url,
  ...
  for (unsigned int i = 0; i < m_robot->numLinks(); i++) {
    ...
    Rs44 << Rs33(0, 0), Rs33(0, 1), Rs33(0, 2), 0, Rs33(1, 0), Rs33(1, 1),
        Rs33(1, 2), 0, Rs33(2, 0), Rs33(2, 1), Rs33(2, 2), 0, 0.0, 0.0, 0.0,
        1.0;
    for (unsigned int l = 0; l < tsis.length(); l++) {
      const OpenHRP::DblArray12 &M = tsis[l].transformMatrix;
      hrp::Matrix44 T0;
      T0 << M[0], M[1], M[2], M[3], M[4], M[5], M[6], M[7], M[8], M[9], M[10],
          M[11], 0.0, 0.0, 0.0, 1.0;
      ...
      for (unsigned int j = 0; j < triangles.length() / 3; ++j) {
        for (int k = 0; k < 3; ++k) {
          ...
          hrp::Vector4 v(T * hrp::Vector4(vertices[p + 0], vertices[p + 1],
                                          vertices[p + 2], 1.0));
          points[points_i++] = v[0];
          points[points_i++] = v[1];
          points[points_i++] = v[2];
    ...
    if (!qh_new_qhull(3, numTriangles, points, ismalloc, flags, NULL, stderr)) {
      ...
      FORALLvertices {
        ...
        index[p] = vertexIndex;
        coldetModel->setVertex(vertexIndex++, points[p * 3 + 0],
                               points[p * 3 + 1], points[p * 3 + 2]);
      }
      facetT *facet;
      num = qh num_facets;
      ;
    ...
    //
    std::cerr << std::setw(16) << i_li.name << " reduce triangles from "
              << std::setw(5) << numTriangles << " to " << std::setw(4) << num
              << std::endl;
  }
...
bool checkCollisionForAllJointRange(
    int i, hrp::JointPathPtr jointPath,
  ...
  if (l->jointType == hrp::Link::FIXED_JOINT) {
    ...
    for (float angle = l->llimit; angle <= l->ulimit; angle += step) {
      ...
      // std::cerr << "    " << collisionPair->link(0)->name << ":" <<
      // collisionPair->link(1)->name << " ";
      // for(int j = 0; j < jointPath->numJoints(); j++) std::cerr <<
      // rad2deg(jointPath->joint(j)->q) << " ";
      // std::cerr << "id = " << i << "/" << jointPath->numJoints() << ", " <<
      // l->name << " " << rad2deg(angle) << " (" << rad2deg(l->llimit) << ","
      // << rad2deg(l->ulimit) << ") " << collisionPair->detectIntersection() <<
      // " " << collisionPair->link(0)->name << ":" <<
      // collisionPair->link(1)->name << std::endl;
      for (std::vector<hrp::ColdetLinkPairPtr>::iterator it =
               collisionPairs.begin();
           it != collisionPairs.end();) {
        if ((*it)->detectIntersection()) {
          std::cerr << "Find collision for " << (*it)->link(0)->name << " "
...
bool compare_joint_path_length(const hrp::ColdetLinkPairPtr &p1,
  ...
  return (m_robot->getJointPath(p1->link(0), p1->link(1))->numJoints()) >
         (m_robot->getJointPath(p2->link(0), p2->link(1))->numJoints());
}
...
void setupCollisionLinkPair() {
  ...
  std::cerr << "step 0: Remove collision pair if they are adjacent pair"
            << std::endl;
  it = tmp_pair.begin();
  while (it != tmp_pair.end()) {
    hrp::JointPathPtr jointPath =
        m_robot->getJointPath((*it)->link(0), (*it)->link(1));
    if (jointPath->numJoints() == 1) {
      ...
      l->llimit = -M_PI;
      l->ulimit = M_PI;
      std::cerr << l->name << " ulimit - " << l->llimit << " " << l->ulimit
                << std::endl;
      for (float angle = l->llimit; angle <= l->ulimit; angle += deg2rad(5)) {
        ...
        // std::cerr << l->name << " " << deg2rad(l->q) << " intersect:" <<
        // (*it)->detectIntersection() <<  " check:" << (*it)->checkCollision()
        // << std::endl;
        (*it)->clearCollisions();
      }
      std::cerr << "  pair (" << jointPath->numJoints() << ") "
                << (*it)->link(0)->name << "/" << (*it)->link(1)->name
                << (always_collide ? " always collide" : "")
                << (never_collide ? " never collide" : "") << std::endl;
  ...
  //
  std::cerr << "step 1: Remove complex (2 or 3 distance) collision pair if "
               "they never/always collide"
            << std::endl;
  it = tmp_pair.begin();
  while (it != tmp_pair.end()) {
    hrp::JointPathPtr jointPath =
    ...
    if (jointPath->numJoints() == 2 || jointPath->numJoints() == 3) {
      ...
      std::cerr << "  pair (" << jointPath->numJoints() << ") "
                << (*it)->link(0)->name << "/" << (*it)->link(1)->name
                << " always:" << always_collide << ", never:" << never_collide
                << " "
                << ((always_collide || never_collide) ? "remove from list" : "")
                << std::endl;
  ...
  std::cerr << "  Remove always/never collide pair for a length of 1,2,3 ... "
            << tmp_pair.size() << std::endl;

  // setup data structure for check collision pair
  // pair_tree[<pair:L1,L2,L3,L4>, [ <pair:L1,L2,L3,L4>, <pair:L1,L2,L3>,
  // <pair:L1,L2> ]]
  // pair_tree[<pair:L2,L3,L4,L5>, [ <pair:L2,L3,L4,L5>, <pair:L2,L3,L4>,
  // <pair:L2,L3> ]]
  std::map<hrp::ColdetLinkPairPtr, std::vector<hrp::ColdetLinkPairPtr> >
      pair_tree;
  std::sort(tmp_pair.begin(), tmp_pair.end(), compare_joint_path_length);
  int i = 0;
  it = tmp_pair.begin();
  while (it != tmp_pair.end()) {
    hrp::JointPathPtr jointPath1 =
        m_robot->getJointPath((*it)->link(0), (*it)->link(1));
    // std::cerr << "  " << i << "/" << tmp_pair.size() << "  pair (" <<
    // jointPath1->numJoints() << ") " << (*it)->link(0)->name << " " <<
    // (*it)->link(1)->name <<std::endl;
    // check if JointPath1 is included in some of the pair_tree (jointPath2)
    bool is_new_key = true;
    for (std::map<hrp::ColdetLinkPairPtr,
                  std::vector<hrp::ColdetLinkPairPtr> >::iterator ii =
             pair_tree.begin();
         ii != pair_tree.end(); ++ii) {
      hrp::JointPathPtr jointPath2 =
          m_robot->getJointPath(((*ii).first)->link(0), ((*ii).first)->link(1));
  ...
  std::cerr << "step 2: Remove collision pair if they never collide"
            << std::endl;
  i = 0;
  for (std::map<hrp::ColdetLinkPairPtr,
                std::vector<hrp::ColdetLinkPairPtr> >::iterator ii =
    ...
    hrp::JointPathPtr jointPath =
        m_robot->getJointPath(key_pair->link(0), key_pair->link(1));
    std::sort(sub_pairs.begin(), sub_pairs.end(), compare_joint_path_length);
    //
    std::cerr << "  " << i << "/" << tmp_pair.size() << "  pair ("
              << jointPath->numJoints() << ") " << (*ii).first->link(0)->name
              << " " << (*ii).first->link(1)->name << std::endl;
    i += (*ii).second.size();
    for (std::vector<hrp::ColdetLinkPairPtr>::iterator it = sub_pairs.begin();
         it != sub_pairs.end(); it++) {
      hrp::JointPathPtr jointPath =
          m_robot->getJointPath((*it)->link(0), (*it)->link(1));
    ...
    for (std::vector<hrp::ColdetLinkPairPtr>::iterator p = sub_pairs.begin();
         p != sub_pairs.end(); p++) {
      for (std::vector<hrp::ColdetLinkPairPtr>::iterator it = tmp_pair.begin();
           it != tmp_pair.end();) {
        if ((*p)->link(0) == (*it)->link(0) &&
            (*p)->link(1) == (*it)->link(1)) {
          tmp_pair.erase(it);
          continue;
        }
...
int main(int argc, char **argv) {
  ...
  if (argc < 2) {
    std::cerr << "usage: " << argv[0]
              << " --model <model file> <blacklist : R_HAND_J0 L_HAND_J0 "
                 "EYEBROW_P EYELID_P EYE_Y EYE_P MOUTH_P UPPERLIP_P LOWERLIP_P "
                 "CHEEK_P>"
              << std::endl;
    exit(1);
  }
  ...
  ofs << "collision_pair:";
  for (std::vector<hrp::ColdetLinkPairPtr>::iterator it = m_pair.begin();
       it != m_pair.end(); it++) {
    ofs << " " << (*it)->link(0)->name << ":" << (*it)->link(1)->name;
    std::cerr << (*it)->link(0)->name << ":" << (*it)->link(1)->name
              << std::endl;
  }
  ofs << std::endl;
  ofs.close();
