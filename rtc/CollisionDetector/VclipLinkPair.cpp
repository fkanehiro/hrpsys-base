...
VclipLinkPair::VclipLinkPair(hrp::Link* link0, Vclip::Polyhedron* vclip_model0,
                             hrp::Link* link1, Vclip::Polyhedron* vclip_model1,
                             double tolerance) {
  links_[0] = link0;
  links_[1] = link1;
  ...
  Feature_Pair.first =
      (const Vclip::Feature*)new Vclip::Vertex(Vclip_Model1->verts().front());
  Feature_Pair.second =
      (const Vclip::Feature*)new Vclip::Vertex(Vclip_Model2->verts().front());
}
...
double VclipLinkPair::computeDistance(double* q1, double* q2) {
  ...
  X21.invert(X12);
  Vclip::Vect3 cp1, cp2;
  double len = Vclip::Polyhedron::vclip(Vclip_Model1, Vclip_Model2, X12, X21,
