...
static const char* component_spec[] = {
    "implementation_id", "CollisionDetector", "type_name", "CollisionDetector",
    "description", "collisoin detector component", "version",
...
CollisionDetector::~CollisionDetector() { quit_beep(); }
...
RTC::ReturnCode_t CollisionDetector::onInitialize() {
  ...
  m_CollisionDetectorServicePort.registerProvider(
      "service0", "CollisionDetectorService", m_service0);
  ...
  if (prop["collision_pair"] != "") {
    std::cerr << "[" << m_profile.instance_name << "] prop[collision_pair] ->"
              << prop["collision_pair"] << std::endl;
    ...
    while (getline(iss, tmp, ' ')) {
      ...
      std::cerr << "[" << m_profile.instance_name
                << "] check collisions between " << m_robot->link(name1)->name
                << " and " << m_robot->link(name2)->name << std::endl;
      m_pair[tmp] = new CollisionLinkPair(new VclipLinkPair(
          m_robot->link(name1), m_VclipLinks[m_robot->link(name1)->index],
  ...
  if (prop["collision_loop"] != "") {
    ...
    std::cerr << "[" << m_profile.instance_name
              << "] set collision_loop: " << m_collision_loop << std::endl;
  }
#ifdef USE_HRPSYSUTIL
  if (m_use_viewer) {
  ...
  m_curr_collision_mask.resize(m_robot->numJoints());  // collision_mask used to
                                                       // select output
                                                       // 0: passthough
                                                       // reference data, 1
                                                       // output safe data
  std::fill(m_curr_collision_mask.begin(), m_curr_collision_mask.end(), 0);
  m_init_collision_mask.resize(m_robot->numJoints());  // collision_mask defined
                                                       // in .conf as
                                                       // [collisoin_mask] 0:
                                                       // move even if collide,
                                                       // 1: do not move when
                                                       // collide
  std::fill(m_init_collision_mask.begin(), m_init_collision_mask.end(), 1);
  if (prop["collision_mask"] != "") {
    std::cerr << "[" << m_profile.instance_name << "] prop[collision_mask] ->"
              << prop["collision_mask"] << std::endl;
    coil::vstring mask_str = coil::split(prop["collision_mask"], ",");
    if (mask_str.size() == m_robot->numJoints()) {
      for (size_t i = 0; i < m_robot->numJoints(); i++) {
        coil::stringTo(m_init_collision_mask[i], mask_str[i].c_str());
      }
      for (size_t i = 0; i < m_robot->numJoints(); i++) {
        if (m_init_collision_mask[i] == 0) {
          std::cerr << "[" << m_profile.instance_name
                    << "] CollisionDetector will not control "
                    << m_robot->joint(i)->name << std::endl;
        }
      }
    } else {
      std::cerr << "[" << m_profile.instance_name
                << "] ERROR size of collision_mask is differ from robot joint "
                   "number .. "
                << mask_str.size() << ", " << m_robot->numJoints() << std::endl;
    }
  }

  if (prop["use_limb_collision"] != "") {
    std::cerr << "[" << m_profile.instance_name
              << "] prop[use_limb_collision] -> " << prop["use_limb_collision"]
              << std::endl;
  ...
  m_interpolator->setName(std::string(m_profile.instance_name) +
                          " interpolator");
...
RTC::ReturnCode_t CollisionDetector::onExecute(RTC::UniqueId ec_id) {
  ...
  //   If m_beepCommand is not connected to BeeperRTC and sometimes, check
  //   connection.
  ...
  if (!is_beep_port_connected && (loop % 500 == 0)) {
    if (m_beepCommandOut.connectors().size() > 0) {
      ...
      quit_beep();
      std::cerr << "[" << m_profile.instance_name
                << "] beepCommand data port connection found! Use BeeperRTC."
                << std::endl;
    }
  }
  ...
  if (!m_enable) {
    if (DEBUGP || loop % 100 == 1) {
      std::cerr << "[" << m_profile.instance_name
                << "] CAUTION!! The robot is moving without checking self "
                   "collision detection!!! please send "
                   "enableCollisionDetection to CollisoinDetection RTC"
                << std::endl;
    }
    if (m_qRefIn.isNew()) {
      m_qRefIn.read();
  ...
  if (m_enable && m_qRefIn.isNew()) {
    ...
    for (unsigned int i = 0; i < m_robot->numJoints(); i++) {
      int servo_state = (m_servoState.data[i][0] &
                         OpenHRP::RobotHardwareService::SERVO_STATE_MASK) >>
                        OpenHRP::RobotHardwareService::SERVO_STATE_SHIFT;
    ...
    //  2. recovery                               .. check based on
    //  q'(m_recover_jointdata)
    if (m_recover_time == 0 ||
        m_recover_time ==
            default_recover_time) {  // 1. current safe angle or collision angle
      if (m_loop_for_check ==
          0) {  // update robot posutre for each m_loop_for_check timing
        for (unsigned int i = 0; i < m_robot->numJoints(); i++) {
      ...
      for (unsigned int i = 0; i < m_robot->numJoints(); i++) {
        if (m_loop_for_check ==
            0) {  // update robot posutre for each m_loop_for_check timing
          if (m_curr_collision_mask[i] ==
              1) {  // joint with 1 (do not move when collide :default), need to
                    // be updated using recover(safe) data
            m_robot->joint(i)->q = m_recover_jointdata[i];
          } else {  // joint with 0 (move even if collide), need to be updated
                    // using reference(dangerous) data
            m_robot->joint(i)->q = m_qRef.data[i];
          }
        }
      }
    ...
    for (int i = 0; it != m_pair.end(); it++, i++) {
      int sub_size = (m_pair.size() + m_collision_loop - 1) /
                     m_collision_loop;  // 10 / 3 = 3  / floor
      // 0 : 0 .. sub_size-1                            // 0 .. 2
      // 1 : sub_size ... sub_size*2-1                  // 3 .. 5
      // k : sub_size*k ... sub_size*(k+1)-1            // 6 .. 8
      // n : sub_size*n ... m_pair.size()               // 9 .. 10
      if (sub_size * m_loop_for_check <= i &&
          i < sub_size * (m_loop_for_check + 1)) {
        CollisionLinkPair* c = it->second;
        c->distance =
            c->pair->computeDistance(c->point0.data(), c->point1.data());
        ...
        if (c->distance <= c->pair->getTolerance()) {
          ...
          if (loop % 200 == 0 || last_safe_posture) {
            hrp::JointPathPtr jointPath =
                m_robot->getJointPath(p->link(0), p->link(1));
            std::cerr << "[" << m_profile.instance_name << "] " << i << "/"
                      << m_pair.size() << " pair: " << p->link(0)->name << "/"
                      << p->link(1)->name << "(" << jointPath->numJoints()
          ...
          if (m_use_limb_collision) {
            hrp::JointPathPtr jointPath =
                m_robot->getJointPath(p->link(0), p->link(1));
            bool stop_all = true;
            // if all joint is within false(0:move even if collide) in initial
            // mask ( for example leg to leg ) we stop them
            // if some joint is not within true(1:do not move within collide) on
            // initial mask, stop only true joint (for exmple leg to arm)
            ...
            for (unsigned int i = 0; i < jointPath->numJoints(); i++) {
              ...
              // collision_mask used to select output                0:
              // passthough reference data, 1 output safe data
              if (stop_all) {
                m_curr_collision_mask[id] =
                    1;  // true (1: output safe data) do not move when collide,
              } else if (m_init_collision_mask[id] ==
                         1) {  // skip false (0: move even if collide)
                m_curr_collision_mask[id] = 1;
              }
            }
          } else {
            std::copy(
                m_init_collision_mask.begin(), m_init_collision_mask.end(),
                m_curr_collision_mask.begin());  // copy init_collision_mask to
                                                 // curr_collision_mask
          }
#ifdef USE_HRPSYSUTIL
          if (m_use_viewer) {
      ...
      if (m_safe_posture) {
        if (has_servoOn) {
          if (!m_have_safe_posture) {
            ...
            m_interpolator->set(m_lastsafe_jointdata);  // Set current angles as
                                                        // initial angle for
                                                        // recover
          }
        ...
        for (unsigned int i = 0; i < m_q.data.length(); i++) {
          if (m_curr_collision_mask[i] ==
              0) {  // if collisoin_mask is 0 (move even if collide), we update
                    // lastsafe_joint_data from input data
            m_lastsafe_jointdata[i] = m_robot->joint(i)->q;
          }
        }
      }
    ...
    if (m_safe_posture && m_recover_time == 0) {  // safe mode
      ...
      // collision_mask used to select output                0: passthough
      // reference data, 1 output safe data
      std::fill(m_curr_collision_mask.begin(), m_curr_collision_mask.end(),
                0);  // false(0) clear output data
    } else {
      ...
      if (m_safe_posture) {  // recover
      ...
      } else if (m_collision_loop > 1 &&
                 (m_recover_time !=
                  default_recover_time)) {  // collision with collision_loop
        // std::cerr << "collision_loop-------------- " << std::endl;
        collision_loop_recover = m_collision_loop;
        m_recover_time = default_recover_time;  // m_recover_time should be set
        ...
        m_recover_time = default_recover_time;  // m_recover_time should be set
                                                ...
                                                // qRef and q
        m_interpolator->set(
            m_lastsafe_jointdata);  // Set last safe joint data as initial angle
        // m_interpolator->set(m_q.data.get_buffer()); //Set initial angle
      }
      for (unsigned int i = 0; i < m_q.data.length(); i++) {
        if (m_curr_collision_mask[i] == 0) {  // 0: passthough reference data, 1
                                              // output safe data, stop joints
                                              // only joint with 1
          m_q.data[i] = m_qRef.data[i];
        }
      }
// calc q'
      ...
      if (collision_loop_recover != 0) {
        ...
        for (unsigned int i = 0; i < m_q.data.length(); i++) {
          m_q.data[i] =
              m_lastsafe_jointdata[i] +
              collision_loop_recover *
                  ((m_q.data[i] - m_lastsafe_jointdata[i]) / m_collision_loop);
        }
      } else {
    ...
    if (DEBUGP) {
      std::cerr << "[" << m_profile.instance_name << "] check collisions for "
                << m_pair.size() << " pairs in "
                << (tm2.sec() - tm1.sec()) * 1000 +
                       (tm2.usec() - tm1.usec()) / 1000.0
                << " [msec], safe = " << m_safe_posture
                << ", time = " << m_recover_time * m_dt
                << "[s], loop = " << m_loop_for_check << "/" << m_collision_loop
                << std::endl;
    }
    if (m_pair.size() == 0 && (DEBUGP || (loop % ((int)(5 / m_dt))) == 1)) {
      std::cerr << "[" << m_profile.instance_name
                << "] CAUTION!! The robot is moving without checking self "
                   "collision detection!!! please define collision_pair in "
                   "configuration file"
                << std::endl;
    }
    if (!m_have_safe_posture && !m_safe_posture) {
      if (DEBUGP || (loop % ((int)(5 / m_dt))) == 1) {
        std::cerr << "[" << m_profile.instance_name
                  << "] CAUTION!! The robot is moving while collision "
                     "detection!!!, since we do not get safe_posture yet"
                  << std::endl;
      }
      for (unsigned int i = 0; i < m_q.data.length(); i++) {
        m_lastsafe_jointdata[i] = m_recover_jointdata[i] = m_q.data[i] =
            m_qRef.data[i];
      }
    }
    //
    ...
    if (!m_safe_posture &&
        has_servoOn) {  // If collided and some joint is servoOn
      if (is_beep_port_connected) {
        if (collision_beep_count % collision_beep_freq == 0 &&
            collision_beep_count % (collision_beep_freq * 3) != 0)
          bc.startBeep(2352, collision_beep_freq * 0.7);
        else
          bc.stopBeep();
      } else {
        if (collision_beep_count % collision_beep_freq == 0 &&
            collision_beep_count % (collision_beep_freq * 3) != 0)
          start_beep(2352, collision_beep_freq * 0.7);
        else
          stop_beep();
      }
...
bool CollisionDetector::setTolerance(const char* i_link_pair_name,
                                     double i_tolerance) {
  if (strcmp(i_link_pair_name, "all") == 0 ||
      strcmp(i_link_pair_name, "ALL") == 0) {
    for (std::map<std::string, CollisionLinkPair*>::iterator it =
             m_pair.begin();
         it != m_pair.end(); it++) {
...
bool CollisionDetector::getCollisionStatus(
...
bool CollisionDetector::checkIsSafeTransition(void) {
  for (unsigned int i = 0; i < m_q.data.length(); i++) {
    // If servoOn, check too large joint angle gap. Otherwise (servoOff),
    // neglect too large joint angle gap.
    int servo_state =
        (m_servoState.data[i][0] &
         OpenHRP::RobotHardwareService::SERVO_STATE_MASK) >>
        OpenHRP::RobotHardwareService::SERVO_STATE_SHIFT;  // enum SwitchStatus
                                                           // {SWITCH_ON,
                                                           // SWITCH_OFF};
    if (servo_state == 1 && abs(m_q.data[i] - m_qRef.data[i]) > 0.017)
      return false;
  }
  return true;
}
...
bool CollisionDetector::enable(void) {
  ...
  if (!checkIsSafeTransition()) {
    std::cerr << "[" << m_profile.instance_name
              << "] CollisionDetector cannot be enabled because of different "
  ...
  for (unsigned int i = 0; it != m_pair.end(); it++, i++) {
    ...
    if (c->distance <= c->pair->getTolerance()) {
      hrp::JointPathPtr jointPath =
          m_robot->getJointPath(p->link(0), p->link(1));
      std::cerr << "[" << m_profile.instance_name
                << "] CollisionDetector cannot be enabled because of collision"
                << std::endl;
      std::cerr << "[" << m_profile.instance_name << "] " << i << "/"
                << m_pair.size() << " pair: " << p->link(0)->name << "/"
                << p->link(1)->name << "(" << jointPath->numJoints()
...
bool CollisionDetector::disable(void) {
  if (!checkIsSafeTransition()) {
    std::cerr << "[" << m_profile.instance_name
              << "] CollisionDetector cannot be disabled because of different "
                 "reference joint angle"
              << std::endl;
    return false;
  }
...
hrp::Link* hrplinkFactory() { return new hrp::Link(); }
#endif  // USE_HRPSYSUTIL

extern "C" {
void CollisionDetectorInit(RTC::Manager* manager) {
  RTC::Properties profile(component_spec);
  manager->registerFactory(profile, RTC::Create<CollisionDetector>,
