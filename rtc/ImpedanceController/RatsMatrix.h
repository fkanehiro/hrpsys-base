...
void rotm3times(hrp::Matrix33& m12, const hrp::Matrix33& m1,
                const hrp::Matrix33& m2);
void difference_rotation(hrp::Vector3& ret_dif_rot,
...
struct coordinates {
  ...
  void rotate_with_matrix(const hrp::Matrix33& mat,
  ...
  void rotate(const double theta, const hrp::Vector3& axis,
              const std::string& wrt = ":local") {
  ...
  /* } */
  void difference(hrp::Vector3& dif_pos, hrp::Vector3& dif_rot,
  ...
  void transformation(coordinates& tc, coordinates c,
                      const std::string& wrt = ":local") const {
    tc = *this;
...
void mid_rot(hrp::Matrix33& mid_rot, const double p, const hrp::Matrix33& rot1,
             const hrp::Matrix33& rot2, const double eps = 0.001);
void mid_coords(coordinates& mid_coords, const double p, const coordinates& c1,
                const coordinates& c2, const double eps = 0.001);
};
#endif /* RATSMATRIX_H */
