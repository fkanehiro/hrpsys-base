...
namespace hrp {
int calcSRInverse(const dmatrix& _a, dmatrix& _a_sr, double _sr_ratio = 1.0,
                  dmatrix _w = dmatrix::Identity(0, 0));
};
...
class JointPathEx : public JointPath {
 public:
  ...
  bool calcInverseKinematics2Loop(const Vector3& dp, const Vector3& omega,
                                  const double LAMBDA,
                                  const double avoid_gain = 0.0,
                                  const double reference_gain = 0.0,
                                  const dvector* reference_q = NULL);
  bool calcInverseKinematics2Loop(
      const Vector3& end_effector_p, const Matrix33& end_effector_R,
      const double LAMBDA, const double avoid_gain = 0.0,
      const double reference_gain = 0.0, const hrp::dvector* reference_q = NULL,
 ...
 protected:
  ...
  //  pair = [index of joint1, index of joint2], index is considered as index
  //  for "joints[index]"
...
// set current Body q,base_p,base_R into InvDynStateBuffer and update vel and
// acc
void calcAccelerationsForInverseDynamics(const hrp::BodyPtr _m_robot,
                                         InvDynStateBuffer& _idsb);
// set all vel and acc into Body, and call Body::calcInverseDynamics()
void calcRootLinkWrenchFromInverseDynamics(hrp::BodyPtr _m_robot,
                                           InvDynStateBuffer& _idsb,
                                           hrp::Vector3& _f_ans,
                                           hrp::Vector3& _t_ans);
// call calcRootLinkWrenchFromInverseDynamics() and convert f,tau into ZMP
void calcWorldZMPFromInverseDynamics(hrp::BodyPtr _m_robot,
                                     InvDynStateBuffer& _idsb,
