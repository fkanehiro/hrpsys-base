 ...
 protected:
  ...
  void gen_pattern_and_plot(const std::vector<hrp::Vector3>& force_diff_vec,
    ...
    for (size_t i = 0; i < time_vec.size(); i++) {
      ...
      imp.calcTargetVelocity(vel_p, vel_r, eeR, force_diff_vec[i],
                             moment_diff_vec[i], dt);
      hrp::Vector3 output_rot, target_rot;
      rats::difference_rotation(output_rot, hrp::Matrix33::Identity(),
                                imp.output_r1);
      rats::difference_rotation(target_rot, hrp::Matrix33::Identity(),
                                imp.target_r1);
      fprintf(fp, "%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n",
              time_vec[i], imp.output_p1(0), imp.output_p1(1), imp.output_p1(2),
              imp.target_p1(0), imp.target_p1(1), imp.target_p1(2),
              force_diff_vec[i](0) / imp.K_p, force_diff_vec[i](1) / imp.K_p,
              force_diff_vec[i](2) / imp.K_p, rad2deg(output_rot(0)),
              rad2deg(output_rot(1)), rad2deg(output_rot(2)),
              rad2deg(target_rot(0)), rad2deg(target_rot(1)),
              rad2deg(target_rot(2)), rad2deg(moment_diff_vec[i](0) / imp.K_r),
              rad2deg(moment_diff_vec[i](1) / imp.K_r),
              rad2deg(moment_diff_vec[i](2) / imp.K_r));
    }
    fclose(fp);
    if (use_gnuplot) {
      // plot
      ...
      for (size_t ii = 0; ii < 3; ii++) {
        ...
        fprintf(gp_pos,
                "plot '/tmp/plot-imp.dat' using 1:%d with lines title 'cur "
                "pos(%s)' lw 4, '/tmp/plot-imp.dat' using 1:%d with lines "
                "title 'tgt pos(%s)' lw 3, '/tmp/plot-imp.dat' using 1:%d with "
                "lines title 'force_diff/K(%s)' lw 2\n",
                ii + 2, titles[ii].c_str(), ii + 2 + 3, titles[ii].c_str(),
                ii + 2 + 3 * 2, titles[ii].c_str());
      }
      fflush(gp_pos);
      //   plot rot
      ...
      for (size_t ii = 0; ii < 3; ii++) {
        ...
        fprintf(gp_rot,
                "plot '/tmp/plot-imp.dat' using 1:%d with lines title 'cur "
                "rot(%s)' lw 4, '/tmp/plot-imp.dat' using 1:%d with lines "
                "title 'tgt rot(%s)' lw 3, '/tmp/plot-imp.dat' using 1:%d with "
                "lines title 'moment_diff/K(%s)' lw 2\n",
                ii + 2 + 9, titles[ii].c_str(), ii + 2 + 3 + 9,
                titles[ii].c_str(), ii + 2 + 3 * 2 + 9, titles[ii].c_str());
      }
      fflush(gp_rot);
      double tmp;
      std::cin >> tmp;
 ...
 public:
  ...
  void test0() {
    ...
    for (size_t i = 0; i < static_cast<size_t>(total_tm / dt); i++) {
      ...
      force_diff_vec.push_back((i * dt < total_tm * 0.2
                                    ? hrp::Vector3::Zero()
                                    : hrp::Vector3(10, -20, 30)));
      moment_diff_vec.push_back((i * dt < total_tm * 0.3
                                     ? hrp::Vector3::Zero()
                                     : hrp::Vector3(5, -10, 15)));
    ...
    gen_pattern_and_plot(force_diff_vec, moment_diff_vec, target_p0_vec,
                         target_r0_vec, time_vec);
  };
  void test1() {
    ...
    for (size_t i = 0; i < static_cast<size_t>(total_tm / dt); i++) {
      ...
      double ratio =
          (i * dt < total_tm * 0.3 ? i * dt / (total_tm * 0.3) : 1.0);
      target_p0_vec.push_back(
          hrp::Vector3((1 - ratio) * hrp::Vector3::Zero() +
                       ratio * hrp::Vector3(0.01, -0.02, 0.03)));
      hrp::Vector3 tmpv(hrp::Vector3((1 - ratio) * hrp::Vector3::Zero() +
                                     ratio * hrp::Vector3(0.1, -0.2, 0.3)));
