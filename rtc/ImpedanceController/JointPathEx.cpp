...
std::ostream& operator<<(std::ostream& out, hrp::dmatrix& a) {
  ...
  for (int i = 0; i < c; i++) {
    out << "      :";
    for (int j = 0; j < n; j++) {
      out << " " << std::setw(7) << std::setiosflags(std::ios::fixed)
          << std::setprecision(4) << (a)(i, j);
    }
    out << std::endl;
  }
}
...
using namespace std;
using namespace hrp;
int hrp::calcSRInverse(const dmatrix& _a, dmatrix& _a_sr, double _sr_ratio,
                       dmatrix _w) {
  ...
  // if (DEBUG) { dmatrix aat = _a * at; std::cerr << " a*at :" << std::endl <<
  // aat; }

  _a_sr = _w * at * a1;
  // if (DEBUG) { dmatrix ii = _a * _a_sr; std::cerr << "    i :" << std::endl
  // << ii; }
}
...
Vector3 omegaFromRotEx(const Matrix33& r) {
  ...
  if (fabs(alpha - 1.0) < 1.0e-12) {  // th=0,2PI;
    ...
    if (s < numeric_limits<double>::epsilon()) {  // th=PI
      return Vector3(sqrt((r(0, 0) + 1) * 0.5) * th,
                     sqrt((r(1, 1) + 1) * 0.5) * th,
                     sqrt((r(2, 2) + 1) * 0.5) * th);
    }
    ...
    return Vector3((r(1, 2) - r(2, 1)) * k, (r(2, 0) - r(0, 2)) * k,
                   (r(0, 1) - r(1, 0)) * k);
  }
}
...
JointPathEx::JointPathEx(BodyPtr& robot, Link* base, Link* end,
    ...
    : JointPath(base, end),
      ...
      dt(control_cycle),
      debug_print_prefix(_debug_print_prefix + ",JointPathEx"),
      joint_limit_debug_print_counts(numJoints(), 0),
      debug_print_freq_count(
          static_cast<size_t>(0.25 / dt)),  // once per 0.25[s]
...
void JointPathEx::setMaxIKError(double e) { maxIKErrorSqr = e * e; }

void JointPathEx::setMaxIKIteration(int iter) { maxIKIteration = iter; }

bool JointPathEx::setInterlockingJointPairIndices(
    const std::vector<std::pair<Link*, Link*> >& pairs,
  ...
  if (tmp_vec.size() > 0) {
    ...
    for (size_t j = 0; j < tmp_vec.size(); j++) {
      std::cerr << "[" << joints[tmp_vec[j].first]->name << ", "
                << joints[tmp_vec[j].second]->name << "] ";
    }
    std::cerr << std::endl;
...
void JointPathEx::getInterlockingJointPairIndices(
    std::vector<std::pair<size_t, size_t> >& pairs) {
  pairs = interlocking_joint_pair_indices;
};

bool JointPathEx::calcJacobianInverseNullspace(dmatrix& J, dmatrix& Jinv,
                                               dmatrix& Jnull) {
  ...
  for (int j = 0; j < n; j++) {
    ...
    // If use_inside_joint_weight_retrieval = true (true by default), use T. F.
    // Chang and R.-V. Dubeby weight retrieval inward.
    // Otherwise, joint weight is always calculated from limit value to resolve
  ...
  if (DEBUG) {
    std::cerr << " cost :";
    for (int j = 0; j < n; j++) {
      std::cerr << std::setw(8) << std::setiosflags(std::ios::fixed)
                << std::setprecision(4) << avoid_weight_gain[j];
    }
    std::cerr << std::endl;
    std::cerr << " optw :";
    for (int j = 0; j < n; j++) {
      std::cerr << std::setw(8) << std::setiosflags(std::ios::fixed)
                << std::setprecision(4) << optional_weight_vector[j];
    }
    std::cerr << std::endl;
    std::cerr << "    w :";
    for (int j = 0; j < n; j++) {
      std::cerr << std::setw(8) << std::setiosflags(std::ios::fixed)
                << std::setprecision(4) << w(j, j);
    }
    std::cerr << std::endl;
  }
  ...
  if (manipulability < manipulability_limit) {
    k = manipulability_gain *
        pow((1 - (manipulability / manipulability_limit)), 2);
  }
  if (DEBUG) {
    std::cerr << " manipulability = " << manipulability << " < "
              << manipulability_limit << ", k = " << k << " -> " << sr_gain * k
              << std::endl;
  }
...
bool JointPathEx::calcInverseKinematics2Loop(const Vector3& dp,
  ...
  if (DEBUG) {
    ...
    for (int j = 0; j < n; ++j) {
      std::cerr << std::setw(8) << std::setiosflags(std::ios::fixed)
                << std::setprecision(4) << rad2deg(joints[j]->q);
    }
    std::cerr << endl;
  }

  size_t ee_workspace_dim =
      6;  // End-effector workspace dimensions including pos(3) and rot(3)
  size_t ij_workspace_dim =
      interlocking_joint_pair_indices.size();  // Dimensions for interlocking
                                               // joints constraint. 0 by
                                               // default.
  ...
  if (avoid_gain > 0.0) {
    ...
    if (DEBUG) {
      ...
      for (int j = 0; j < n; ++j) {
        std::cerr << std::setw(8) << std::setiosflags(std::ios::fixed)
                  << std::setprecision(4) << rad2deg(u(j));
      }
      std::cerr << std::endl;
      std::cerr << " JN*u :";
      ...
      for (int j = 0; j < n; ++j) {
        std::cerr << std::setw(8) << std::setiosflags(std::ios::fixed)
                  << std::setprecision(4) << rad2deg(Jnullu(j));
      }
      std::cerr << std::endl;
    }
    dq = dq + Jnull * u;
  }
  // If reference_gain and reference_q are set, add following to reference_q by
  // null space vector
  if (reference_gain > 0.0 && reference_q != NULL) {
    ...
    for (unsigned int j = 0; j < numJoints(); j++) {
      u[j] = optional_weight_vector[j] * reference_gain *
             ((*reference_q)[joint(j)->jointId] - joint(j)->q);
    }
    if (DEBUG) {
      std::cerr << "u(ref):";
      for (int j = 0; j < n; ++j) {
      ...
      for (int j = 0; j < n; ++j) {
        std::cerr << std::setw(8) << std::setiosflags(std::ios::fixed)
                  << std::setprecision(4) << rad2deg(nullu(j));
      }
      std::cerr << std::endl;
    }
    dq = dq + Jnull * u;
  ...
  if (DEBUG) {
    ...
    for (int j = 0; j < n; ++j) {
      std::cerr << std::setw(8) << std::setiosflags(std::ios::fixed)
                << std::setprecision(4) << rad2deg(dq(j));
    }
    std::cerr << std::endl;
  }
  ...
  if (!solve_linear_equation) {
    std::cerr << "[" << debug_print_prefix << "] ERROR nan/inf is found"
              << std::endl;
    return false;
  }
  ...
  for (int j = 0; j < n; ++j) {
    ...
    if (joints[j]->q > joints[j]->ulimit) {
      ...
      if (joint_limit_debug_print_counts[j] % debug_print_freq_count == 0) {
        std::cerr << "[" << debug_print_prefix << "] Upper joint limit over "
                  << joints[j]->name << " (ja=" << joints[j]->q
                  << "[rad], limit=" << joints[j]->ulimit
                  << "[rad], count=" << joint_limit_debug_print_counts[j]
                  << ", debug_print_freq_count=" << debug_print_freq_count
                  << ")" << std::endl;
      }
    ...
    if (joints[j]->q < joints[j]->llimit) {
      is_limit_over = true;
      if (joint_limit_debug_print_counts[j] % debug_print_freq_count == 0) {
        std::cerr << "[" << debug_print_prefix << "] Lower joint limit over "
                  << joints[j]->name << " (ja=" << joints[j]->q
                  << "[rad], limit=" << joints[j]->llimit
                  << "[rad], count=" << joint_limit_debug_print_counts[j]
                  << ", debug_print_freq_count=" << debug_print_freq_count
                  << ")" << std::endl;
      }
...
// TODO : matrix_logEx should be omegaFromRotEx after checking on real robot
// testing.
...
bool JointPathEx::calcInverseKinematics2Loop(
    const Vector3& end_effector_p, const Matrix33& end_effector_R,
    const double LAMBDA, const double avoid_gain, const double reference_gain,
    const hrp::dvector* reference_q, const double vel_gain,
  ...
  // TODO : matrix_logEx should be omegaFromRotEx after checking on real robot
  // testing.
  // hrp::Vector3 vel_r(endLink()->R * omegaFromRotEx(endLink()->R.transpose() *
  // target_link_R));
  hrp::Vector3 vel_r(endLink()->R *
                     matrix_logEx(endLink()->R.transpose() * target_link_R));
  vel_p *= vel_gain;
  vel_r *= vel_gain;
  return calcInverseKinematics2Loop(vel_p, vel_r, LAMBDA, avoid_gain,
...
bool JointPathEx::calcInverseKinematics2(const Vector3& end_p,
  ...
  for (iter = 0; iter < MAX_IK_ITERATION; iter++) {
    if (DEBUG) {
      std::cerr << " iter : " << iter << " / " << MAX_IK_ITERATION
                << ", n = " << n << std::endl;
    }
    ...
    if (DEBUG) {
      std::cerr << "   dp : " << dp[0] << " " << dp[1] << " " << dp[2]
                << std::endl;
      std::cerr << "omega : " << omega[0] << " " << omega[1] << " " << omega[2]
                << std::endl;
      // std::cerr << "    J :" << std::endl << J;
      // std::cerr << "  det : " << det(J) << std::endl;
      std::cerr << "  err : dp = " << dp.dot(dp)
    ...
    if (isBestEffortIKMode) {
      ...
      if (DEBUG)
        std::cerr << "  err : fabs(" << std::setw(18)
                  << std::setiosflags(std::ios::fixed) << std::setprecision(14)
                  << errsqr << " - " << errsqr0
                  << ") = " << fabs(errsqr - errsqr0) << " < " << maxIKErrorSqr
      ...
      if (DEBUG)
        std::cerr << "  err : " << std::setw(18)
                  << std::setiosflags(std::ios::fixed) << std::setprecision(14)
                  << sqrt(dp.dot(dp)) << " < " << sqrt(maxIKPosErrorSqr) << ", "
                  << std::setw(18) << std::setiosflags(std::ios::fixed)
                  << std::setprecision(14) << sqrt(omega.dot(omega)) << " < "
                  << sqrt(maxIKRotErrorSqr) << std::endl;
      if ((dp.dot(dp) < maxIKPosErrorSqr) &&
          (omega.dot(omega) < maxIKRotErrorSqr)) {
        converged = true;
        break;
      }
    }

    if (!calcInverseKinematics2Loop(dp, omega, LAMBDA, avoid_gain,
                                    reference_gain, reference_q))
      return false;
  }
  ...
  if (!converged) {
    ...
    if (isBestEffortIKMode) {
      std::cerr << "  err : fabs(" << errsqr << " - " << errsqr0
                << ") = " << fabs(errsqr - errsqr0) << " < " << maxIKErrorSqr
                << " BestEffortIKMode" << std::endl;
    } else {
      std::cerr << "  err : " << dp.dot(dp) << " ( " << dp[0] << " " << dp[1]
                << " " << dp[2] << ") < " << maxIKPosErrorSqr << std::endl;
      std::cerr << "      : " << omega.dot(omega) << " ( " << omega[0] << " "
                << omega[1] << " " << omega[2] << ") < " << maxIKRotErrorSqr
                << std::endl;
    }
    for (int i = 0; i < n; ++i) {
      joints[i]->q = qorg[i];
...
void hrp::readVirtualForceSensorParamFromProperties(
    std::map<std::string, hrp::VirtualForceSensorParam>& vfs,
  ...
  for (unsigned int i = 0; i < nvforce; i++) {
    ...
    vfs.insert(std::pair<std::string, VirtualForceSensorParam>(
        name, VirtualForceSensorParam()));
    VirtualForceSensorParam& p = vfs[name];
    p.localPos = hrp::Vector3(tr[0], tr[1], tr[2]);
    p.localR =
        Eigen::AngleAxis<double>(tr[6], hrp::Vector3(tr[3], tr[4], tr[5]))
    ...
    std::cerr << "[" << instance_name << "]   name = " << name
              << ", parent = " << p.link->name << ", id = " << p.id
              << std::endl;
    std::cerr << "[" << instance_name << "]   localP = "
              << p.localPos.format(Eigen::IOFormat(
                     Eigen::StreamPrecision, 0, ", ", ", ", "", "", "[", "]"))
              << "[m]" << std::endl;
    std::cerr << "[" << instance_name << "]   localR = "
              << p.localR.format(Eigen::IOFormat(Eigen::StreamPrecision, 0,
                                                 ", ", "\n", "    [", "]"))
              << std::endl;
  }
};
...
void hrp::readInterlockingJointsParamFromProperties(
    std::vector<std::pair<Link*, Link*> >& pairs, hrp::BodyPtr m_robot,
    const std::string& prop_string, const std::string& instance_name) {
  ...
  if (interlocking_joints_str.size() > 0) {
    ...
    for (size_t i = 0; i < num; i++) {
      // std::cerr << "[" << instance_name << "] Interlocking Joints [" <<
      // interlocking_joints_str[i*ij_prop_num] << "], [" <<
      // interlocking_joints_str[i*ij_prop_num+1] << "]" << std::endl;
      hrp::Link* link1 =
          m_robot->link(interlocking_joints_str[i * ij_prop_num]);
      hrp::Link* link2 =
          m_robot->link(interlocking_joints_str[i * ij_prop_num + 1]);
      if (link1 == NULL || link2 == NULL) {
        std::cerr << "[" << instance_name << "] No such interlocking joints ["
                  << interlocking_joints_str[i * ij_prop_num] << "], ["
                  << interlocking_joints_str[i * ij_prop_num + 1] << "]"
                  << std::endl;
        continue;
      }
...
void hrp::calcAccelerationsForInverseDynamics(const hrp::BodyPtr _m_robot,
  ...
  _idsb.base_w = hrp::Vector3(_idsb.base_w_hat(2, 1), -_idsb.base_w_hat(0, 2),
                              _idsb.base_w_hat(1, 0));
...
void hrp::calcRootLinkWrenchFromInverseDynamics(hrp::BodyPtr _m_robot,
  ...
  _m_robot->rootLink()->dvo =
      _idsb.base_dv - _idsb.base_dw.cross(_idsb.base_p) -
      _idsb.base_w.cross(_idsb.base_v);  // calc in differential way
  ...
  _m_robot->calcInverseDynamics(_m_robot->rootLink(), _f_ans,
                                _t_ans);  // this returns f,t at the coordinate
                                          // origin (not at base link pos)
};
