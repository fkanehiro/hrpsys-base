...
void difference_rotation(hrp::Vector3& ret_dif_rot,
  ...
  // ret_dif_rot = self_rot * hrp::omegaFromRot(self_rot.transpose() *
  // target_rot);
  ret_dif_rot = self_rot * hrp::Vector3(rats::matrix_log(hrp::Matrix33(
                               self_rot.transpose() * target_rot)));
}

void mid_rot(hrp::Matrix33& mid_rot, const double p, const hrp::Matrix33& rot1,
             const hrp::Matrix33& rot2, const double eps) {
...
void mid_coords(coordinates& mid_coords, const double p, const coordinates& c1,
                const coordinates& c2, const double eps) {
