...
static const char* nullcomponent_spec[] = {
    "implementation_id", "DataLogger", "type_name", "DataLogger", "description",
    "data logger component", "version", HRPSYS_PACKAGE_VERSION, "vendor",
...
void printData(std::ostream& os, const RTC::Pose3D& data,
               unsigned int precision = 0) {
  LOG_SET_PRECISION(os);
  os << data.position.x << " " << data.position.y << " " << data.position.z
     << " " << data.orientation.r << " " << data.orientation.p << " "
     << data.orientation.y << " ";
  LOG_UNSET_PRECISION(os);
}
...
class LoggerPortForPointCloud : public LoggerPort<PointCloudTypes::PointCloud> {
 public:
  LoggerPortForPointCloud(const char* name)
      : LoggerPort<PointCloudTypes::PointCloud>(name) {}
...
RTC::ReturnCode_t DataLogger::onInitialize() {
  ...
  m_DataLoggerServicePort.registerProvider("service0", "DataLoggerService",
                                           m_service0);
...
RTC::ReturnCode_t DataLogger::onExecute(RTC::UniqueId ec_id) {
  if (ec_id == 0) {
    if (m_emergencySignalIn.isNew()) {
      ...
      std::cout << "received emergency signal. saving log files(" << basename
                << ")" << std::endl;
      save(basename);
...
bool DataLogger::add(const char* i_type, const char* i_name) {
  ...
  if (strcmp(i_type, "TimedDoubleSeq") == 0) {
    ...
    LoggerPort<OpenHRP::TimedLongSeqSeq>* lp =
        new LoggerPort<OpenHRP::TimedLongSeqSeq>(i_name);
    new_port = lp;
    if (!addInPort(i_name, lp->port())) {
      resumeLogging();
      return false;
    ...
    LoggerPort<TimedOrientation3D>* lp =
        new LoggerPort<TimedOrientation3D>(i_name);
    new_port = lp;
    if (!addInPort(i_name, lp->port())) {
      resumeLogging();
      return false;
    ...
    LoggerPort<TimedAcceleration3D>* lp =
        new LoggerPort<TimedAcceleration3D>(i_name);
    new_port = lp;
    if (!addInPort(i_name, lp->port())) {
      resumeLogging();
      return false;
    ...
    LoggerPort<TimedAngularVelocity3D>* lp =
        new LoggerPort<TimedAngularVelocity3D>(i_name);
    new_port = lp;
    if (!addInPort(i_name, lp->port())) {
      resumeLogging();
      return false;
    ...
    LoggerPort<PointCloudTypes::PointCloud>* lp =
        new LoggerPortForPointCloud(i_name);
    new_port = lp;
    if (!addInPort(i_name, lp->port())) {
      resumeLogging();
      return false;
...
bool DataLogger::save(const char* i_basename) {
  ...
  if (ret)
    std::cerr << "[" << m_profile.instance_name << "] Save log to "
              << i_basename << ".*" << std::endl;
  resumeLogging();
  return ret;
}
...
void DataLogger::maxLength(unsigned int len) {
  ...
  std::cerr << "[" << m_profile.instance_name << "] Log max length is set to "
            << len << std::endl;
  resumeLogging();
}
...
void DataLoggerInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<DataLogger>,
