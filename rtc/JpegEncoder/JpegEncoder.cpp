...
static const char* jpegdecoder_spec[] = {
    "implementation_id", "JpegEncoder", "type_name", "JpegEncoder",
...
RTC::ReturnCode_t JpegEncoder::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_decodedIn.isNew()) {
    ...
    switch (idat.format) {
      ...
      case Img::CF_GRAY: {
        cv::Mat src(idat.height, idat.width, CV_8U, idat.raw_data.get_buffer());
        cv::imencode(".jpg", src, buf, param);
...
void JpegEncoderInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<JpegEncoder>,
