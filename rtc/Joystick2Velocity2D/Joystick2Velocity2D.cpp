...
static const char* joystick2velocity_spec[] = {
    "implementation_id", "Joystick2Velocity2D", "type_name",
    "Joystick2Velocity2D", "description",
...
RTC::ReturnCode_t Joystick2Velocity2D::onExecute(RTC::UniqueId ec_id) {
  ...
  if (m_debugLevel > 0) {
    printf("velocity command: %5.2f %5.2f %5.2f", m_vel.data.vx, m_vel.data.vy,
           m_vel.data.va);
  }
  m_velOut.write();
...
void Joystick2Velocity2DInit(RTC::Manager* manager) {
  ...
  manager->registerFactory(profile, RTC::Create<Joystick2Velocity2D>,
