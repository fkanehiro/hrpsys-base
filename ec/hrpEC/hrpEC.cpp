...
bool hrpExecutionContext::enterRT() {
  ...
  if (sched_setscheduler(0, SCHED_FIFO, &param) == -1) {
    ...
    std::cerr << "If you are running this program on normal linux kernel for "
                 "debug purpose, you can ignore the error message displayed "
                 "above. If not, this program must have superuser privilege."
              << std::endl;
    // return -1;
  } else {
    /* Lock memory */
...
bool hrpExecutionContext::exitRT() { return true; }
};
...
#else
  RTC::ExecutionContextFactory::instance().addFactory(
      "hrpExecutionContext",
      ::coil::Creator< ::RTC::ExecutionContextBase, RTC::hrpExecutionContext>,
