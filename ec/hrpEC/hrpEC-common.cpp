...
int hrpExecutionContext::svc(void) {
  ...
  int loop = 0;
  int debug_count =
      5000.0 / (get_signal_period() * nsubstep /
                1e6);  // Loop count for debug print. Once per 5000.0 [ms].
...
#define DELTA_SEC(start, end) \
  (end.tv_sec - start.tv_sec + (end.tv_usec - start.tv_usec) / 1e6)
      ...
      m_profile.avg_period =
          (m_profile.avg_period * m_profile.count + dt) / (m_profile.count + 1);
    }
    m_profile.count++;
    m_tv = tv;
    if (loop % debug_count == 0 && ENABLE_DEBUG_PRINT)
      gettimeofday(&debug_tv2, NULL);
    ...
    for (unsigned int i = 0; i < m_profile.profiles.length(); i++) {
      ...
      if (lcs == ACTIVE_STATE) {
        prof.avg_process =
            (prof.avg_process * prof.count + dt) / (++prof.count);
      }
...
#endif
          rtc_names.push_back(
              std::string(rtc->get_component_profile()->instance_name));
        }
      }
      printRTCProcessingTime(processes);
#endif
    ...
    if (loop % debug_count == 0 && ENABLE_DEBUG_PRINT) {
      ...
      fprintf(stderr,
              "[hrpEC] [%d.%6.6d] Processing time breakdown : "
              "waitForNextPeriod %f[ms], warker (onExecute) %f[ms], "
              "ExecutionProfile %f[ms], time from prev cicle %f[ms]\n",
              tv.tv_sec, tv.tv_usec, DELTA_SEC(debug_tv1, debug_tv2) * 1e3,
              DELTA_SEC(debug_tv2, debug_tv3) * 1e3,
              DELTA_SEC(debug_tv3, debug_tv4) * 1e3,
              DELTA_SEC(debug_tv5, debug_tv1) * 1e3);
    }
    if (loop % debug_count == (debug_count - 1) && ENABLE_DEBUG_PRINT)
      gettimeofday(&debug_tv5, NULL);
  } while (m_running);
#else
  } while (isRunning());
#endif
...
hrpExecutionContext::getComponentProfile(RTC::LightweightRTObject_ptr obj) {
#ifndef OPENRTM_VERSION_TRUNK
  ...
  throw OpenHRP::ExecutionProfileService::ExecutionProfileServiceException(
      "no such component");
}
