...
void initWorld(Project &prj, BodyFactory &factory,
  ...
  for (unsigned int i = 0; i < prj.collisionPairs().size(); i++) {
    ...
    if (bodyIndex1 < 0) {
      ...
      if (prj.models().find(cpi.objectName1) != prj.models().end()) {
        bodyIndex1 = world.bodyIndex(prj.models()[cpi.objectName1].rtcName);
      }
    }
    ...
    if (bodyIndex2 < 0) {
      ...
      if (prj.models().find(cpi.objectName2) != prj.models().end()) {
        bodyIndex2 = world.bodyIndex(prj.models()[cpi.objectName2].rtcName);
      }
    }
    ...
    if (bodyIndex1 >= 0 && bodyIndex2 >= 0) {
      ...
      for (size_t j = 0; j < links1.size(); ++j) {
        for (size_t k = 0; k < links2.size(); ++k) {
          ...
          if (link1 && link2 && link1 != link2 && link1->parent != link2 &&
              link1 != link2->parent) {
            world.constraintForceSolver.addCollisionCheckLinkPair(
                bodyIndex1, link1, bodyIndex2, link2, cpi.staticFriction,
                cpi.slidingFriction, cpi.cullingThresh, cpi.restitution, 0.0);
  ...
  for (unsigned int i = 0; i < prj.extraJoints().size(); i++) {
    ...
    if (bodyIndex1 < 0) {
      ...
      if (prj.models().find(ej.object1Name) != prj.models().end()) {
        bodyIndex1 = world.bodyIndex(prj.models()[ej.object1Name].rtcName);
      }
    }
    ...
    if (bodyIndex2 < 0) {
      ...
      if (prj.models().find(ej.object2Name) != prj.models().end()) {
        bodyIndex2 = world.bodyIndex(prj.models()[ej.object2Name].rtcName);
      }
    }
    ...
    if (bodyIndex1 >= 0 && bodyIndex2 >= 0) {
      ...
      if (link1 && link2) {
        world.constraintForceSolver.addExtraJoint(
            bodyIndex1, link1, bodyIndex2, link2, ej.link1LocalPos.data(),
            ej.link2LocalPos.data(), jtype, ej.jointAxis.data());
      }
    }
  }
  ...
  for (std::map<std::string, ModelItem>::iterator it = prj.models().begin();
    ...
    for (std::map<std::string, JointItem>::iterator it2 =
             it->second.joint.begin();
    ...
    body->setDefaultRootPosition(body->rootLink()->p,
                                 body->rootLink()->attitude());
  }
  world.initialize();
#if 0
...
void initRTS(Project &prj, std::vector<ClockReceiver> &receivers) {
  ...
  for (std::map<std::string, RTSItem::rtc>::iterator it =
           rts.components.begin();
       it != rts.components.end(); it++) {
  ...
  for (std::map<std::string, RTSItem::rtc>::iterator it =
    ...
    if (!rtc) {
      if (it->second.name == "") {
        std::cerr << "factory name for " << it->first << " is not defined"
                  << std::endl;
        continue;
      }
  ...
  for (std::vector<std::pair<std::string, std::string> >::iterator it =
           rts.connections.begin();
       it != rts.connections.end(); it++) {
    std::cout << "making a connection between " << it->first << " and "
              << it->second << std::endl;
