...
void convertToAABB(hrp::Link *i_link) {
  ...
  int triangles[] = {0, 2, 3, 0, 1, 2, 4, 3, 7, 4, 0, 3, 0, 4, 5, 5, 1, 0,
                     1, 5, 6, 1, 6, 2, 2, 6, 7, 2, 7, 3, 7, 6, 4, 5, 4, 6};
...
void convertToConvexHull(hrp::Link *i_link) {
  ...
  FORALLvertices {
    ...
    index[p] = vertexIndex;
    coldetModel->setVertex(vertexIndex++, points[p * 3 + 0], points[p * 3 + 1],
                           points[p * 3 + 2]);
  }
  facetT *facet;
  ...
  //
  // std::cerr << i_link->name << " reduce triangles from " << numTriangles << "
  // to " << num << std::endl;
}
