...
const char *BodyRTC::bodyrtc_spec[] = {
    "implementation_id", "BodyRTC", "type_name", "BodyRTC", "description",
    "BodyRTC component", "version", "0.1", "vendor", "AIST", "category",
    "Generic", "activity_type", "DataFlowComponent", "max_instance", "10",
...
RTC::ReturnCode_t BodyRTC::setup() {
  ...
  for (unsigned int i = 0; i < numJoints(); i++) {
    calib_status[i] = servo_status[i] = power_status[i] =
...
void parsePortConfig(const std::string &config, std::string &name,
                     std::string &type, std::vector<std::string> &elements) {
  ...
  if (elements.size() == 0) {
    ...
    for (size_t i = 0; i < elements.size(); i++) {
      ...
      if (j) {
        ...
        std::cerr << "can't find a joint(" << elements[i] << ")" << std::endl;
        return false;
      }
    }
  }
...
void BodyRTC::createInPort(const std::string &config) {
  ...
  if (type == "JOINT_VALUE") {
    ...
    if (getJointList(this, elements, joints)) {
      m_inports.push_back(new JointValueInPortHandler(this, name.c_str(),
                                                      joints, &servo_status));
    }
  } else if (type == "JOINT_VELOCITY") {
    std::vector<hrp::Link *> joints;
    if (getJointList(this, elements, joints)) {
      m_inports.push_back(new JointVelocityInPortHandler(
          this, name.c_str(), joints, &servo_status));
    }
  } else if (type == "JOINT_ACCELERATION") {
    std::vector<hrp::Link *> joints;
    if (getJointList(this, elements, joints)) {
      m_inports.push_back(new JointAccelerationInPortHandler(
          this, name.c_str(), joints, &servo_status));
    }
  } else if (type == "JOINT_TORQUE") {
    std::vector<hrp::Link *> joints;
    if (getJointList(this, elements, joints)) {
      m_inports.push_back(new JointTorqueInPortHandler(this, name.c_str(),
                                                       joints, &servo_status));
    }
    ...
    if (!s) {
      std::cerr << "can't find a sensor(" << elements[0] << ")" << std::endl;
      return;
    }
    ...
    if (!l) {
      std::cerr << "can't find a light(" << elements[0] << ")" << std::endl;
      return;
    }
...
void BodyRTC::createOutPort(const std::string &config) {
  ...
  if (type == "JOINT_VALUE") {
    ...
    if (elements.size() != 1) {
      std::cerr << "link name is not specified for port " << name << std::endl;
      return;
    }
    hrp::Link *l = this->link(elements[0]);
    if (l) {
    ...
    if (l) {
      ...
      std::cerr << "can't find a link(" << elements[0] << ")" << std::endl;
    }
  } else if (type == "ABS_ACCELERATION") {
    if (elements.size() != 1) {
      std::cerr << "link name is not specified for port " << name << std::endl;
    ...
    if (elements.size() != 1) {
      std::cerr << "sensor name is not specified for port" << name << std::endl;
      return;
    }
    ...
    m_outports.push_back(new RateGyroSensorPortHandler(this, name.c_str(), s));
  ...
  m_outports.push_back(
      new EmergencySignalPortHandler(this, "emergencySignal", this));
  m_outports.push_back(new ServoStatePortHandler(this, "servoState", this));

  m_service0.setRobot(this);
  m_RobotHardwareServicePort.registerProvider(
      "service0", "RobotHardwareService", m_service0);
...
char *time_string() {
  ...
  static char time[20];
  sprintf(time, "%02d:%02d:%02d.%06d", tm_->tm_hour, tm_->tm_min, tm_->tm_sec,
          (int)tv.tv_usec);
  return time;
}
    ...
    if (state == ON && m_servoErrorLimit[i] != 0) {
      ...
      if (fabs(angle - command) > m_servoErrorLimit[i]) {
        std::cerr << time_string()
                  ...
                  << ", qRef = " << command / M_PI * 180
                  << "[deg], q = " << angle / M_PI * 180 << "[deg]"
                  << std::endl;
...
bool BodyRTC::preOneStep() {
  ...
  bool emulate_highgain_servo_off_mode =
      (numJoints() > 0);  // If no joints, do not use servo off emulation
  for (unsigned int i = 0; i < numJoints(); ++i) {
    Link *j = joint(i);
    commands[i] = j->q;
    ...
    switch (j->jointType) {
      case Link::ROTATIONAL_JOINT: {
        j->q +=
            (j->subm * g).cross(j->submwc / j->subm - j->p).dot(j->R * j->a) *
            0.005 * 0.01;
        if (j->q < j->llimit) {
          j->q = j->llimit;
      ...
      default:
        std::cerr << "calcCMJacobian() : unsupported jointType(" << j->jointType
                  << std::endl;
    }
  }
  if (m_resetPosition) {
...
void RobotHardwareServicePort::initializeJointAngle(const char *name,
                                                    const char *option) {}
CORBA::Boolean RobotHardwareServicePort::addJointGroup(
    const char *gname,
...
CORBA::Boolean RobotHardwareServicePort::readDigitalInput(
    ::OpenHRP::RobotHardwareService::OctSequence_out din) {
  return false;
}
CORBA::Long RobotHardwareServicePort::lengthDigitalInput() { return 0; }
CORBA::Boolean RobotHardwareServicePort::writeDigitalOutput(
    const ::OpenHRP::RobotHardwareService::OctSequence &dout) {
  return false;
}
CORBA::Boolean RobotHardwareServicePort::writeDigitalOutputWithMask(
    const ::OpenHRP::RobotHardwareService::OctSequence &dout,
    const ::OpenHRP::RobotHardwareService::OctSequence &mask) {
  return false;
}
CORBA::Long RobotHardwareServicePort::lengthDigitalOutput() { return 0; }
CORBA::Boolean RobotHardwareServicePort::readDigitalOutput(
...
void BodyRTC::moduleInit(RTC::Manager *manager) {
  ...
  manager->registerFactory(profile, RTC::Create<BodyRTC>, DummyDelete<BodyRTC>
                           // RTC::Delete<BodyRTC>
                           );
}
