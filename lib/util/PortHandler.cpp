...
JointInPortHandler::JointInPortHandler(
    RTC::DataFlowComponentBase *i_rtc, const char *i_portName,
...
void JointValueInPortHandler::update() {
  if (m_port.isNew()) {
    ...
    for (size_t i = 0; i < m_joints.size(); i++) {
      if (m_joints[i] && m_servo[i] == OpenHRP::RobotHardwareService::SWITCH_ON)
        m_joints[i]->q = m_data.data[i];
    }
  }
}
...
void JointTorqueInPortHandler::update() {
  if (m_port.isNew()) {
    ...
    if (m_data.data.length() != m_joints.size()) {
      std::cerr << "JointTorqueInPortHandler: data length mismatch(length of "
                   "input data:"
                << m_data.data.length()
                << "<->the number of joints:" << m_joints.size() << ")"
                << std::endl;
    }
    for (size_t i = 0; i < m_joints.size(); i++) {
      if (m_joints[i] && m_servo[i] == OpenHRP::RobotHardwareService::SWITCH_ON)
        m_joints[i]->u = m_data.data[i];
    }
  }
}
...
ForceSensorPortHandler::ForceSensorPortHandler(
    ...
    : SensorPortHandler<ForceSensor, RTC::TimedDoubleSeq>(i_rtc, i_portName,
                                                          i_sensor) {
...
RateGyroSensorPortHandler::RateGyroSensorPortHandler(
    ...
    : SensorPortHandler<RateGyroSensor, RTC::TimedAngularVelocity3D>(
          i_rtc, i_portName, i_sensor) {}
...
AccelSensorPortHandler::AccelSensorPortHandler(
    ...
    : SensorPortHandler<AccelSensor, RTC::TimedAcceleration3D>(
          i_rtc, i_portName, i_sensor) {}
...
RangeSensorPortHandler::RangeSensorPortHandler(
    ...
    : SensorPortHandler<RangeSensor, RTC::RangeData>(i_rtc, i_portName,
                                                     i_sensor) {
...
VisionSensorPortHandler::VisionSensorPortHandler(
    ...
    : SensorPortHandler<VisionSensor, Img::TimedCameraImage>(i_rtc, i_portName,
                                                             i_sensor) {
  i_sensor->isEnabled = true;
  if (m_sensor->imageType == VisionSensor::COLOR ||
    ...
    m_data.data.intrinsic.matrix_element[0] =
        0.5 * m_sensor->width / tan(fovx / 2.0);
    m_data.data.intrinsic.matrix_element[1] = 0.0;
    m_data.data.intrinsic.matrix_element[2] = m_sensor->width / 2;
    m_data.data.intrinsic.matrix_element[3] =
        0.5 * m_sensor->height / tan(m_sensor->fovy / 2.0);
  ...
  if (m_sensor->isUpdated) {
    if (m_sensor->imageType == VisionSensor::COLOR ||
      ...
      if (m_data.data.image.raw_data.length() != m_sensor->image.size()) {
        ...
        memcpy(m_data.data.image.raw_data.get_buffer(), &m_sensor->image[0],
               m_sensor->image.size());
        write(time);
#if 0
                char filename[20];
...
PointCloudPortHandler::PointCloudPortHandler(RTC::DataFlowComponentBase *i_rtc,
                                             const char *i_portName,
                                             VisionSensor *i_sensor)
    : SensorPortHandler<VisionSensor, PointCloudTypes::PointCloud>(
          i_rtc, i_portName, i_sensor) {
  i_sensor->isEnabled = true;
  ...
  if (m_pcFormat == "xyz") {
    ...
    std::cerr << "unknown point cloud format:[" << m_pcFormat << "]"
              << std::endl;
  }
...
AbsTransformInPortHandler::AbsTransformInPortHandler(
    RTC::DataFlowComponentBase *i_rtc, const char *i_portName,
    hrp::Link *i_link)
    : InPortHandler<RTC::TimedPose3D>(i_rtc, i_portName), m_link(i_link) {}
...
void AbsTransformInPortHandler::update() {
  if (m_port.isNew()) {
    ...
    m_link->p << m_data.data.position.x, m_data.data.position.y,
        m_data.data.position.z;
    hrp::Matrix33 R =
        hrp::rotFromRpy(m_data.data.orientation.r, m_data.data.orientation.p,
...
AbsVelocityInPortHandler::AbsVelocityInPortHandler(
    RTC::DataFlowComponentBase *i_rtc, const char *i_portName,
    hrp::Link *i_link)
    : InPortHandler<RTC::TimedDoubleSeq>(i_rtc, i_portName), m_link(i_link) {}
...
FrameRateInPortHandler::FrameRateInPortHandler(
    ...
    : InPortHandler<RTC::TimedDouble>(i_rtc, i_portName), m_sensor(i_sensor) {}
...
LightSwitchInPortHandler::LightSwitchInPortHandler(
    RTC::DataFlowComponentBase *i_rtc, const char *i_portName,
    hrp::Light *i_light)
    : InPortHandler<RTC::TimedBoolean>(i_rtc, i_portName), m_light(i_light) {}
...
AbsVelocityOutPortHandler::AbsVelocityOutPortHandler(
    RTC::DataFlowComponentBase *i_rtc, const char *i_portName,
    hrp::Link *i_link)
    : OutPortHandler<RTC::TimedDoubleSeq>(i_rtc, i_portName), m_link(i_link) {
  m_data.data.length(6);
}
...
EmergencySignalPortHandler::EmergencySignalPortHandler(
    ...
    : OutPortHandler<RTC::TimedLong>(i_rtc, i_portName), m_body(i_body) {}
