...
void GLshape::setVertices(unsigned int nvertices, const float *vertices) {
  ...
  for (size_t i = 0; i < nvertices; i++) {
    m_vertices[i] = Eigen::Vector3f(vertices[i * 3], vertices[i * 3 + 1],
                                    vertices[i * 3 + 2]);
  }
}
...
void GLshape::setTriangles(unsigned int ntriangles, const int *vertexIndices) {
  ...
  for (size_t i = 0; i < ntriangles; i++) {
    m_triangles[i] =
        Eigen::Vector3i(vertexIndices[i * 3], vertexIndices[i * 3 + 1],
                        vertexIndices[i * 3 + 2]);
  }
}
...
void GLshape::setNormals(unsigned int nnormal, const float *normals) {
  ...
  for (size_t i = 0; i < nnormal; i++) {
    m_normals[i] =
        Eigen::Vector3f(normals[i * 3], normals[i * 3 + 1], normals[i * 3 + 2]);
  }
}
...
void GLshape::setColors(unsigned int ncolors, const float *colors) {
  ...
  for (size_t i = 0; i < ncolors; i++) {
    m_colors[i] =
        Eigen::Vector3f(colors[i * 3], colors[i * 3 + 1], colors[i * 3 + 2]);
  }
}

void GLshape::normalPerVertex(bool flag) { m_normalPerVertex = flag; }

void GLshape::solid(bool flag) { m_solid = flag; }
...
void GLshape::setTextureCoordinates(unsigned int ncoords,
                                    const float *coordinates) {
  m_textureCoordinates.resize(ncoords);
  for (size_t i = 0; i < ncoords; i++) {
    m_textureCoordinates[i] =
        Eigen::Vector2f(coordinates[i * 2], coordinates[i * 2 + 1]);
  }
}

void GLshape::setTextureCoordIndices(unsigned int len,
                                     const int *coordIndices) {
...
void GLshape::setTexture(GLtexture *texture) { m_texture = texture; }

void GLshape::compile() { m_requestCompile = true; }
...
int GLshape::doCompile(bool isWireFrameMode) {
  ...
  double scale[3];
  for (int i = 0; i < 3; i++) {
    scale[i] = sqrt(m_trans[i] * m_trans[i] + m_trans[i + 4] * m_trans[i + 4] +
  ...
  if (!isWireFrameMode && m_texture && !m_highlight) {
    ...
    gluBuild2DMipmaps(GL_TEXTURE_2D, 3, m_texture->width, m_texture->height,
                      format, GL_UNSIGNED_BYTE, &m_texture->image[0]);
  ...
  if (!m_triangles.size() && m_vertices.size()) {
    ...
    for (size_t i = 0; i < m_vertices.size(); i++) {
      if (m_colors.size() >= m_vertices.size()) glColor3fv(m_colors[i].data());
...
void GLshape::setShininess(float s) { m_shininess = s; }
...
void GLshape::divideLargeTriangles(double maxEdgeLen) {
  ...
  for (size_t i = 0; i < m_triangles.size(); i++) {
    ...
    if (m_texture) {
      dq_textureCoordIndices.push_back(Eigen::Vector3i(
          m_textureCoordIndices[i * 3], m_textureCoordIndices[i * 3 + 1],
    ...
    while (dq_triangles.size()) {
      ...
      if (l[maxidx] <= maxEdgeLen) {
        ...
        dq_triangles.push_back(Eigen::Vector3i(vnew, tri[(maxidx + 1) % 3],
                                               tri[(maxidx + 2) % 3]));
        if (m_normalPerVertex) {
