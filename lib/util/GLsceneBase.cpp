...
GLsceneBase::GLsceneBase(LogManagerBase *i_log)
    : m_showingStatus(false),
      ...
      m_cvImage(NULL),
      m_log(i_log),
      m_showFloorGrid(true),
      m_showInfo(true),
      m_defaultLights(true),
      m_request(REQ_NONE),
      m_maxEdgeLen(0),
      m_targetObject(-1),
      m_isCapturing(false) {
  m_default_camera =
      new GLcamera(DEFAULT_W, DEFAULT_H, 0.1, 100.0, 30 * M_PI / 180);
...
GLcamera *GLsceneBase::getCamera() { return m_camera; }

GLcamera *GLsceneBase::getDefaultCamera() { return m_default_camera; }
...
bool GLsceneBase::defaultLights() { return m_defaultLights; }
...
void GLsceneBase::showFloorGrid(bool flag) { m_showFloorGrid = flag; }

bool GLsceneBase::showFloorGrid() { return m_showFloorGrid; }
...
void GLsceneBase::showInfo(bool flag) { m_showInfo = flag; }
...
void GLsceneBase::drawInfo(double fps, size_t ntri) {
  ...
  if (m_camera != m_default_camera) {
    sprintf(buf, "Camera: %s.%s", m_camera->link()->body->name().c_str(),
            m_camera->name().c_str());
    h -= 15;
    glRasterPos2f(10, h);
    drawString(buf);
...
void GLsceneBase::draw() {
  ...
  double fps = 1.0 / ((tv.tv_sec - m_lastDraw.tv_sec) +
                      (tv.tv_usec - m_lastDraw.tv_usec) / 1e6);
  m_lastDraw = tv;
  ...
  if (m_log->isRecording() && !m_isCapturing && !m_videoWriter) {
    m_videoWriter =
        cvCreateVideoWriter("olv.avi", CV_FOURCC('D', 'I', 'V', 'X'),
                            m_log->fps(), cvSize(m_width, m_height));
    m_cvImage = cvCreateImage(cvSize(m_width, m_height), IPL_DEPTH_8U, 3);
  }
  if (m_videoWriter) {
...
void GLsceneBase::maxEdgeLen(double i_len) { m_maxEdgeLen = i_len; }
