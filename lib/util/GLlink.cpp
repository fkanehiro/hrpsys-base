...
void GLlink::useAbsTransformToDraw() { m_useAbsTransformToDraw = true; }
...
void GLlink::computeAbsTransform() { computeAbsTransform(m_absTrans); }
...
void GLlink::addShape(GLshape *shape) { m_shapes.push_back(shape); }

void GLlink::addCamera(GLcamera *camera) { m_cameras.push_back(camera); }

void GLlink::showAxes(bool flag) { m_showAxes = flag; }

hrp::Link *GLlinkFactory() { return new GLlink(); }

const std::vector<GLcamera *> &GLlink::cameras() { return m_cameras; }

int GLlink::drawMode() { return m_drawMode; }

void GLlink::drawMode(int i_mode) { m_drawMode = i_mode; }
