...
GLcamera::GLcamera(int i_width, int i_height, double i_near, double i_far,
                   double i_fovy, GLlink *i_link, int i_id)
    : m_link(i_link),
      m_near(i_near),
      m_far(i_far),
      m_fovy(i_fovy),
...
void GLcamera::name(const std::string &i_name) { m_name = i_name; }
...
void GLcamera::setView() { setView(width(), height()); }
void GLcamera::setView(int w, int h) {
  glMatrixMode(GL_PROJECTION);
  glLoadIdentity();
  gluPerspective(fovy() * 180 / M_PI, (double)w / (double)h, near(), far());
  if (m_link) {
    ...
    gluLookAt(m_viewPoint[0], m_viewPoint[1], m_viewPoint[2], m_viewTarget[0],
              m_viewTarget[1], m_viewTarget[2], 0, 0, 1);
  }
}
...
double *GLcamera::getAbsTransform() { return m_absTrans; }

GLlink *GLcamera::link() { return m_link; }
...
void GLcamera::render(GLsceneBase *i_scene) {
  ...
  if (m_sensor->imageType == VisionSensor::DEPTH ||
    ...
    glReadPixels(0, 0, m_width, m_height, GL_DEPTH_COMPONENT, GL_FLOAT, depth);
    // depth -> point cloud
...
void GLcamera::initRenderbuffer(void) {
  ...
  glRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL_DEPTH_COMPONENT, m_width,
                           m_height);
}

VisionSensor *GLcamera::sensor() { return m_sensor; }

void GLcamera::addShape(GLshape *i_shape) { m_shapes.push_back(i_shape); }
