...
int connectPorts(RTC::PortService_ptr outPort, RTC::PortService_ptr inPort) {
  RTC::ConnectorProfileList_var connectorProfiles =
      inPort->get_connector_profiles();
  ...
  CORBA_SeqUtil::push_back(
      cprof.properties, NVUtil::newNV("dataport.interface_type", "corba_cdr"));
  CORBA_SeqUtil::push_back(
      cprof.properties, NVUtil::newNV("dataport.subscription_type", "flush"));
...
void setConfiguration(RTC::RTObject_var rtc, const std::string& name,
                      const std::string& value) {
  SDOPackage::Configuration_ptr cfg = rtc->get_configuration();
  SDOPackage::ConfigurationSetList_var cfgsets = cfg->get_configuration_sets();
  if (cfgsets->length() == 0) {
