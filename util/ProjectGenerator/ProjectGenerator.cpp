...
void xmlTextWriterWriteProperty(const xmlTextWriterPtr writer,
...
int main(int argc, char **argv) {
  ...
  std::string conf_file_option, robothardware_conf_file_option,
      integrate("true"), dt("0.005"), timeStep(dt), joint_properties;
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--output") {
      ...
      if (++i < argc)
        robothardware_conf_file_option += std::string("\n") + argv[i];
    } else if (arg == "--joint-properties") {
      if (++i < argc) joint_properties = argv[i];
    } else if (arg == "--use-highgain-mode") {
      if (++i < argc)
        use_highgain_mode =
            (std::string(argv[i]) == std::string("true") ? true : false);
    } else if (arg.find("--gtest_output") == 0 || arg.find("--text") == 0 ||
               arg.find("__log") == 0 || arg.find("__name") == 0) {  // skip
    } else if (arg[0] == '-') {
  ...
  xmlTextWriterStartElement(writer, BAD_CAST "grxui");
  {
    ...
    xmlTextWriterWriteAttribute(writer, BAD_CAST "name", BAD_CAST "Simulation");
    {
      ...
      xmlTextWriterWriteAttribute(
          writer, BAD_CAST "class",
          BAD_CAST "com.generalrobotix.ui.item.GrxSimulationItem");
      xmlTextWriterWriteAttribute(writer, BAD_CAST "name",
                                  BAD_CAST "simulationItem");
      {
      ...
      Eigen::AngleAxis<double> WAIST_offset_rot = Eigen::AngleAxis<double>(
          0, hrp::Vector3(
                 0, 0, 1));  // rotation in VRML is represented by axis + angle
      for (std::vector<std::string>::iterator it = inputs.begin();
        ...
        //    WAIST offset: for example, "0,0,0,0,0,1,0" -> position offset
        //    (3dof) + axis for rotation offset (3dof) + angle for rotation
        //    offset (1dof)
        ...
        if (filename_arg_str.size() > 1) {  // if WAIST offset is specified
          for (size_t i = 0; i < 3; i++) {
            coil::stringTo(WAIST_offset_pos(i),
                           filename_arg_str[i + 1].c_str());
            coil::stringTo(WAIST_offset_rot.axis()(i),
                           filename_arg_str[i + 1 + 3].c_str());
          }
          coil::stringTo(WAIST_offset_rot.angle(),
                         filename_arg_str[1 + 3 + 3].c_str());
        }
        ...
        xmlTextWriterStartElement(writer, BAD_CAST "item");
        xmlTextWriterWriteAttribute(writer, BAD_CAST "class", BAD_CAST
        ...
        if (use_highgain_mode) {
          ...
          xmlTextWriterWriteProperty(writer, "HGcontroller0.factory",
                                     "HGcontroller");
          if (it == inputs.begin()) {
            xmlTextWriterWriteProperty(
                writer, "connection",
                "HGcontroller0.qOut:" + name + "(Robot)0.qRef");
            xmlTextWriterWriteProperty(
                writer, "connection",
                "HGcontroller0.dqOut:" + name + "(Robot)0.dqRef");
            xmlTextWriterWriteProperty(
                writer, "connection",
                "HGcontroller0.ddqOut:" + name + "(Robot)0.ddqRef");
          }
        } else {
          xmlTextWriterWriteProperty(writer, "PDcontroller0.period", dt);
          xmlTextWriterWriteProperty(writer, "PDcontroller0.factory",
                                     "PDcontroller");
          if (it == inputs.begin()) {
            xmlTextWriterWriteProperty(
                writer, "connection",
                "PDcontroller0.torque:" + name + "(Robot)0.tauRef");
            xmlTextWriterWriteProperty(
                writer, "connection",
                "" + name + "(Robot)0.q:PDcontroller0.angle");
          }
        }
        xmlTextWriterEndElement(writer);  // item
        ...
        xmlTextWriterWriteAttribute(writer, BAD_CAST "name",
                                    BAD_CAST(basename(*it).c_str()));
        xmlTextWriterWriteAttribute(writer, BAD_CAST "url",
                                    BAD_CAST filename.c_str());
        ...
        if (use_highgain_mode) {
          ...
          xmlTextWriterWriteProperty(writer, "inport",
                                     "ddqRef:JOINT_ACCELERATION");
          if (integrate == "false") {  // For kinematics only simultion
            xmlTextWriterWriteProperty(
                writer, "inport",
                "basePoseRef:" + body->rootLink()->name + ":ABS_TRANSFORM");
          }
        } else {
        ...
        for (unsigned int i = 0; i < nforce; i++) {
          ...
          xmlTextWriterWriteProperty(writer, "outport",
                                     s->name + ":" + s->name + ":FORCE_SENSOR");
          std::cerr << " " << s->name;
        }
        ...
        if (ngyro == 1) {
          ...
          xmlTextWriterWriteProperty(
              writer, "outport", s->name + ":" + s->name + ":RATE_GYRO_SENSOR");
          std::cerr << " " << s->name;
        } else {
        ...
        // write waist pos and rot by considering both VRML original WAIST and
        // WAIST_offset_pos and WAIST_offset_rot from arguments
        ...
        xmlTextWriterWriteProperty(writer, root_name + ".translation",
                                   os.str());
        os.str("");  // reset ostringstream
        Eigen::AngleAxis<double> tmpAA = Eigen::AngleAxis<double>(hrp::Matrix33(
            body->rootLink()->R * WAIST_offset_rot.toRotationMatrix()));
        os << tmpAA.axis()(0) << " " << tmpAA.axis()(1) << " "
           << tmpAA.axis()(2) << " " << tmpAA.angle();
        ...
        //   [property 1],[value 1],....
        //   ex. --joint-properties
        //   RARM_JOINT0.angle,0.0,RARM_JOINT2.mode,HighGain,...
        coil::vstring joint_properties_arg_str =
            coil::split(joint_properties, ",");
        std::map<std::string, std::string> joint_properties_map;
        for (size_t i = 0; i < joint_properties_arg_str.size() / 2; i++) {
          joint_properties_map.insert(std::pair<std::string, std::string>(
              joint_properties_arg_str[i * 2],
              joint_properties_arg_str[i * 2 + 1]));
        }
        ...
        for (unsigned int i = 0; i < body->numJoints(); i++) {
          if (body->joint(i)->index > 0) {
            ...
            xmlTextWriterWriteProperty(writer, j_property,
                                       (joint_properties_map.find(j_property) !=
                                        joint_properties_map.end())
                                           ? joint_properties_map[j_property]
                                           : "0.0");
            ...
            xmlTextWriterWriteProperty(
                ...
                (joint_properties_map.find(j_property) !=
                 joint_properties_map.end())
                    ? joint_properties_map[j_property]
                    : (use_highgain_mode ? "HighGain" : "Torque"));
        ...
        //
        // comment out self collision settings according to issues at
        // https://github.com/fkanehiro/hrpsys-base/issues/122
        // xmlTextWriterStartElement(writer, BAD_CAST "item");
        // xmlTextWriterWriteAttribute(writer, BAD_CAST "class", BAD_CAST
        ...
        // xmlTextWriterWriteAttribute(writer, BAD_CAST "select", BAD_CAST
        // "true");
        // {
        //   xmlTextWriterWriteProperty(writer, "springConstant", "0 0 0 0 0
        //   0");
        ...
        //   xmlTextWriterWriteProperty(writer, "damperConstant", "0 0 0 0 0
        //   0");
        ...
        xmlTextWriterWriteAttribute(
            writer, BAD_CAST "class",
            BAD_CAST "com.generalrobotix.ui.view.GrxRobotHardwareClientView");
        xmlTextWriterWriteAttribute(writer, BAD_CAST "name",
                                    BAD_CAST "RobotHardware RTC Client");
        ...
        xmlTextWriterWriteProperty(writer, "RobotHardwareServiceRTC",
                                   "RobotHardware0");
        ...
        xmlTextWriterWriteProperty(
            writer, "eyeHomePosition",
            "-0.70711 -0 0.70711 2 0.70711 -0 0.70711 2 0 1 0 0.8 0 0 0 1 ");
      ...
      {
        for (std::vector<std::string>::iterator it1 = inputs.begin();
             it1 != inputs.end(); it1++) {
          std::string name1 = basename(*it1);
          for (std::vector<std::string>::iterator it2 = it1 + 1;
               it2 != inputs.end(); it2++) {
            ...
            xmlTextWriterWriteAttribute(
                writer, BAD_CAST "class",
                BAD_CAST "com.generalrobotix.ui.item.GrxCollisionPairItem");
            xmlTextWriterWriteAttribute(
                writer, BAD_CAST "name",
                BAD_CAST std::string("CP#" + name2 + "_#" + name1 + "_")
                    .c_str());
            {
              xmlTextWriterWriteProperty(writer, "springConstant",
                                         "0 0 0 0 0 0");
              xmlTextWriterWriteProperty(writer, "slidingFriction", "0.5");
              xmlTextWriterWriteProperty(writer, "jointName2", "");
              xmlTextWriterWriteProperty(writer, "jointName1", "");
              xmlTextWriterWriteProperty(writer, "damperConstant",
                                         "0 0 0 0 0 0");
...

  {
    std::string conf_file =
        output.substr(0, output.find_last_of('.')) + ".conf";
  ...
  {
    std::string conf_file =
        output.substr(0, output.find_last_of('.')) + ".RobotHardware.conf";
    std::fstream s(conf_file.c_str(), std::ios::out);

    s << "model: file://" << filenames[0] << std::endl;
    s << "exec_cxt.periodic.type: hrpExecutionContext" << std::endl;
    s << "exec_cxt.periodic.rate: "
      << static_cast<size_t>(1 / atof(dt.c_str()) + 0.5)
      << std::endl;  // rounding to specify integer rate value
    s << robothardware_conf_file_option << std::endl;
  }
  std::cerr << "\033[1;31mProjectGenerator in hrpsys-base is old, so please "
               "use projectGenerator in openhrp3.\033[0m"
            << std::endl;
  return 0;
}
