...
void GLscene::updateScene() {
  ...
  for (unsigned int i = 0; i < state.characterPositions.length(); i++) {
    ...
    for (unsigned int j = 0; j < cpos.linkPositions.length(); j++) {
      ...
      double T[] = {lp.R[0], lp.R[3], lp.R[6], 0, lp.R[1], lp.R[4], lp.R[7], 0,
                    lp.R[2], lp.R[5], lp.R[8], 0, lp.p[0], lp.p[1], lp.p[2], 1};
#if 0
            for (int i=0; i<4; i++){
                for (int j=0; j<4; j++){
                    printf("%6.3f ", T[i*4+j]);
...
void GLscene::drawAdditionalLines() {
  ...
  for (unsigned int i = 0; i < cs.length(); i++) {
    ...
    for (unsigned int j = 0; j < cps.length(); j++) {
      ...
      for (int k = 0; k < 3; k++) {
        e[k] =
            cps[j].position[k] + cps[j].normal[k] * (cps[j].idepth * 10 + 0.1);
      }
      glVertex3dv(e);
    }
  }
...
GLscene::GLscene(LogManagerBase *i_log) : GLsceneBase(i_log) {}
