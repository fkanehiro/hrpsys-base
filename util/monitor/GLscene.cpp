...
void white() { glColor3d(1.0, 1.0, 1.0); }

void red() { glColor3d(1.0, 0.0, 0.0); }

void yellow() { glColor3d(1.0, 1.0, 0.0); }

void green() { glColor3d(0.0, 1.0, 0.0); }

void blue() { glColor3d(0.0, 0.0, 1.0); }

void black() { glColor3d(0.0, 0.0, 0.0); }
...
int servoAlarm(int s) {
  return (s & OpenHRP::RobotHardwareService::SERVO_ALARM_MASK) >>
         OpenHRP::RobotHardwareService::SERVO_ALARM_SHIFT;
}
int temperature(int s) {
  return (s & OpenHRP::RobotHardwareService::DRIVER_TEMP_MASK) >>
...
void GLscene::updateScene() {
  ...
  if (com.baseTransform.length() == 12) {
    ...
    root->R << tform[3], tform[4], tform[5], tform[6], tform[7], tform[8],
  ...
  for (size_t i = 0; i < m_pairs.size(); i++) {
    if (m_pairs[i]->checkCollision()) {
      ...
      std::cout << m_pairs[i]->link(0)->name << "<->"
                << m_pairs[i]->link(1)->name << std::endl;
    }
  }
}
...
#define HEIGHT_STEP 12
    ...
    for (unsigned int i = 0; i < glbody->numJoints(); i++) {
      ...
      if (l) {
        ...
        sprintf(
            buf, "%13s %8.3f %8.3f %6.1f", l->name.c_str(),
            (i < rstate.angle.length()) ? (rstate.angle[i] * 180 / M_PI) : 0,
            (i < rstate.command.length()) ? (rstate.command[i] * 180 / M_PI)
                                          : 0,
    ...
    if (rstate.accel.length()) {
      ...
      for (unsigned int i = 0; i < rstate.accel.length(); i++) {
        sprintf(buf, "  %8.4f %8.4f %8.4f", rstate.accel[i][0],
    ...
    if (rstate.rateGyro.length()) {
      ...
      for (unsigned int i = 0; i < rstate.rateGyro.length(); i++) {
        sprintf(buf, "  %8.4f %8.4f %8.4f", rstate.rateGyro[i][0],
...
void GLscene::setCollisionCheckPairs(
    const std::vector<hrp::ColdetLinkPairPtr> &i_pairs) {
  m_pairs = i_pairs;
}

void GLscene::showCoMonFloor(bool flag) { m_showCoMonFloor = flag; }
