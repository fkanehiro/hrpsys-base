...
int main(int argc, char* argv[]) {
  if (argc < 2) {
    std::cerr << "Usage:" << argv[0]
              << " project.xml [-rh RobotHardwareComponent] [-sh StateHolder "
                 "component] [-size size] [-bg r g b] [-host localhost] [-port "
                 "2809] [-interval 100] [-nogui]"
              << std::endl;
    return 1;
  }
  ...
  if (orbinitref == false) {
    ...
    char buf2[256];
    sprintf(buf2, "NameService=corbaloc:iiop:%s:%d/NameService",
            hostname == NULL ? "localhost" : hostname, port == 0 ? 2809 : port);
    nargv[nargc++] = buf1;
    nargv[nargc++] = buf2;
  }
  ...
  for (int i = 0; i < nargc - 1; i++) {
    if (strcmp(nargv[i], "-ORBInitRef") == 0) {
      std::cerr << "[monitor] Starting CORBA Client with " << nargv[i + 1]
                << std::endl;
    }
  }
  CORBA::ORB_var orb;
  ...
  std::cerr << "[monitor] Monitor service with " << rhview.hostname << "@"
            << rhview.port << ", " << 1000 / rhview.interval << "Hz"
            << std::endl;
  Monitor monitor(orb, rhview.hostname, rhview.port, rhview.interval, &log);
  if (rhname) {
    ...
    for (std::map<std::string, ModelItem>::iterator it = prj.models().begin();
      ...
      if (tmp_body) {
        ...
        if (tmp_body->numJoints() > 0)
          ...
          std::cerr << "[monitor] Skipping non-robot model (" << name << ")"
                    << std::endl;
      }
    }
    if (!body) {
