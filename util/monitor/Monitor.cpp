...
Monitor::Monitor(CORBA::ORB_var orb, const std::string &i_hostname, int i_port,
                 int i_interval, LogManager<TimedRobotState> *i_log)
    : m_orb(orb),
  ...
  try {
    ...
    std::cerr << "[monitor] Failed to initialize CORBA " << std::endl
              << ex._rep_id() << " with " << buf << std::endl;
  } catch (const std::string &error) {
    std::cerr << "[monitor] Failed to initialize CORBA " << std::endl
              << error << " with " << buf << std::endl;
  } catch (...) {
    std::cerr << "[monitor] Failed to initialize CORBA with " << buf
              << std::endl;
  }
}
...
bool Monitor::oneStep() {
  ...
  if (CORBA::is_nil(m_rhService)) {
    try {
      ...
      m_rhService =
          OpenHRP::RobotHardwareService::_narrow(m_orb->string_to_object(ior));
    } catch (...) {
      if ((loop % (5 * (1000 / m_interval))) == 0)
        std::cerr << "[monitor] RobotHardwareService could not connect ("
                  << m_rhCompName << ")" << std::endl;
    }
  } else {
    if ((loop % (5 * (1000 / m_interval))) == 0)
      std::cerr << "[monitor] RobotHardwareService is not found ("
                << m_rhCompName << ")" << std::endl;
  }
  // StateHolderService
  if (CORBA::is_nil(m_shService)) {
    try {
      ...
      m_shService =
          OpenHRP::StateHolderService::_narrow(m_orb->string_to_object(ior));
    } catch (...) {
      if ((loop % (5 * (1000 / m_interval))) == 0)
        std::cerr << "[monitor] StateHolderService could not connect ("
                  << m_shCompName << ")" << std::endl;
    }
  } else {
    if ((loop % (5 * (1000 / m_interval))) == 0)
      std::cerr << "[monitor] StateHolderService is not found (" << m_shCompName
                << ")" << std::endl;
  }
...
bool Monitor::isConnected() { return !CORBA::is_nil(m_rhService); }
...
void Monitor::showStatus(hrp::BodyPtr &body) {
  ...
  LogManager<TimedRobotState> *lm = (LogManager<TimedRobotState> *)m_log;
  ...
  std::vector<double> curr_angle, prev_angle, velocity, curr_vel, prev_vel,
  ...
  for (int i = 0; i < n; i++) {
    prev_angle[i] = lm->state().state.angle[i];
    velocity[i] = acceleration[i] = curr_vel[i] = 0;
  }
  ...
  while (m_log->index() > 0) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (fabs(acceleration[i]) < fabs(curr_acc[i]))
        acceleration[i] = curr_acc[i];
    }
    m_log->prev(1);
    for (int i = 0; i < n; i++) {
      prev_angle[i] = curr_angle[i];
      prev_vel[i] = curr_vel[i];
    }
    prev_time = curr_time;
  }
  m_log->tail();
  fprintf(stdout,
          "\e[2KID PW                 NAME    ANGLE  COMMAND    ERROR VELOCITY "
          "  ACCEL. TORQUE SERVO TEMP\n");  // greep backgroupd
  char buf[256];
  ...
  if (rstate.accel.length()) {
    ...
    for (unsigned int i = 0; i < rstate.accel.length(); i++) {
      ...
      fprintf(stdout, " %8.4f %8.4f %8.4f\n", rstate.accel[i][0],
              rstate.accel[i][1], rstate.accel[i][2]);
    }
  }
  if (rstate.rateGyro.length()) {
    ...
    for (unsigned int i = 0; i < rstate.rateGyro.length(); i++) {
      if (i > 0) fprintf(stdout, "\e[2K             ");
      fprintf(stdout, " %8.4f %8.4f %8.4f\n", rstate.rateGyro[i][0],
              rstate.rateGyro[i][1], rstate.rateGyro[i][2]);
    }
  }
  if (rstate.force.length()) {
    ...
    for (unsigned int i = 0; i < rstate.force.length(); i++) {
      ...
      fprintf(stdout, " %6.1f %6.1f %6.1f %6.2f %6.2f %6.2f\n",
              rstate.force[i][0], rstate.force[i][1], rstate.force[i][2],
              rstate.force[i][3], rstate.force[i][4], rstate.force[i][5]);
    }
  }
  fprintf(stdout, "\e[2K\n");
}
...
void Monitor::setStateHolderName(const char *i_name) { m_shCompName = i_name; }
