...
hrp::BodyPtr createBody(const std::string &name, const ModelItem &mitem,
  ...
  if (!loadBodyFromBodyInfo(body, binfo, true, GLlinkFactory)) {
    ...
    for (std::map<std::string, JointItem>::const_iterator it2 =
             mitem.joint.begin();
...
void PySimulator::initViewer() { window.start(); }
...
PyBody *PySimulator::loadBody(std::string name, std::string url) {
  ...
  ModelLoader_var modelloader = getModelLoader(
...
bool PySimulator::loadProject(std::string fname) {
  ...
  //================= setup Simulator ======================
  BodyFactory factory =
      boost::bind(::createBody, _1, _2, modelloader, &scene, useBBox);
  init(prj, factory);
  ...
  std::cout << "timestep = " << prj.timeStep()
            << ", total time = " << prj.totalTime() << std::endl;
  return true;
}
...
void PySimulator::play() { log.play(); }

void PySimulator::pause() { log.play(); }

void PySimulator::notifyChanged() { appendLog(); }
...
unsigned int PySimulator::logLength() { return log.length(); }

PyBody *PySimulator::getBody(std::string name) {
  return dynamic_cast<PyBody *>(body(name).get());
}

bool PySimulator::showSensors() { return scene.showSensors(); }

void PySimulator::setShowSensors(bool flag) { scene.showSensors(flag); }

void PySimulator::setMaxLogLength(double len) { maxLogLen = len; }

double PySimulator::maxLogLength() { return maxLogLen; }
...
void PySimulator::setUseBBox(bool flag) { useBBox = flag; }

void PySimulator::setWindowSize(int s) { window.setSize(s, s); }
...
BOOST_PYTHON_MODULE(hrpsysext) {
  ...
  class_<PySimulator, boost::noncopyable>("Simulator")
      ...
      .def("createBody", &PySimulator::createBody,
      ...
      .add_property("timeStep", &PySimulator::timeStep,
                    &PySimulator::setTimeStep)
      .add_property("time", &PySimulator::currentTime)
      .add_property("totalTime", &PySimulator::totalTime,
                    &PySimulator::setTotalTime)
      .add_property("showSensors", &PySimulator::showSensors,
                    &PySimulator::setShowSensors)
      .add_property("maxLogLength", &PySimulator::maxLogLength,
                    &PySimulator::setMaxLogLength);
  ...
  class_<PyLink, boost::noncopyable>("Link", no_init)
      ...
      .add_property("d", &PyLink::getTranslationAxis,
                    &PyLink::setTranslationAxis)
  ...
  class_<PyShape, boost::noncopyable>("Shape", no_init)
      ...
      .add_property("diffuse", &PyShape::getDiffuseColor,
                    &PyShape::setDiffuseColor);
}
