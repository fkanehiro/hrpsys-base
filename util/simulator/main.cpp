...
void print_usage(char* progname) {
  ...
  std::cerr << " -realtime          : syncronize to real world time"
            << std::endl;
  std::cerr << " -usebbox           : use bounding box for collision detection"
            << std::endl;
  std::cerr << " -endless           : endless mode" << std::endl;
  std::cerr << " -showsensors       : visualize sensors" << std::endl;
  std::cerr << " -size [pixels]     : specify window size in pixels"
            << std::endl;
  std::cerr << " -no-default-lights : disable ambient light (simulation "
               "environment will be dark)"
            << std::endl;
  std::cerr << " -max-edge-length [value] : specify maximum length of polygon "
               "edge (if exceed, polygon will be divided to improve rendering "
               "quality)"
            << std::endl;
  std::cerr << " -max-log-length [value] : specify maximum size of the log"
            << std::endl;
  std::cerr
      << " -exit-on-finish    : exit the program when the simulation finish"
      << std::endl;
  std::cerr << " -record            : record the simulation as movie"
            << std::endl;
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "-nodisplay") && strcmp(argv[i], "-realtime") &&
        strcmp(argv[i], "-usebbox") && strcmp(argv[i], "-endless") &&
        strcmp(argv[i], "-showsensors") && strcmp(argv[i], "-size") &&
        strcmp(argv[i], "-no-default-lights") &&
        strcmp(argv[i], "-max-edge-length") &&
        strcmp(argv[i], "-max-log-length") &&
        strcmp(argv[i], "-exit-on-finish") && strcmp(argv[i], "-record") &&
        strcmp(argv[i], "-bg")) {
