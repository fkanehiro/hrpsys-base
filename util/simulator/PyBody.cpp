...
const char *PyBody::pybody_spec[] = {
    "implementation_id", "PyBody", "type_name", "PyBody", "description",
    "PyBody component", "version", "0.1", "vendor", "AIST", "category",
...
void PyBody::setPosition(PyObject *v) { rootLink()->setPosition(v); }

void PyBody::setRotation(PyObject *v) { rootLink()->setRotation(v); }
...
PyObject *PyBody::getPosition() { return rootLink()->getPosition(); }

PyObject *PyBody::getRotation() { return rootLink()->getRotation(); }
...
void PyBody::calcForwardKinematics() { Body::calcForwardKinematics(); }

PyLink *PyBody::rootLink() { return (PyLink *)Body::rootLink(); }

PyLink *PyBody::link(std::string name) { return (PyLink *)Body::link(name); }
...
PyLink *PyBody::joint(int i) { return (PyLink *)Body::joint(i); }
...
std::string PyBody::getName() { return Body::name(); }
...
void PyBody::setListener(PySimulator *i_sim) { simulator = i_sim; }
...
void PyBody::moduleInit(RTC::Manager *manager) {
  ...
  manager->registerFactory(profile, RTC::Create<PyBody>, DummyDelete<PyBody>
                           // RTC::Delete<PyBody>
                           );
}
