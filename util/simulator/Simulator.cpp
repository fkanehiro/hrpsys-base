...
void Simulator::checkCollision() { checkCollision(collisions); }
...
bool Simulator::oneStep() {
  ...
  if (adjustTime) {
    ...
    if (startTimes.size() >= 2) {
      ...
      int realT =
          (last.tv_sec - first.tv_sec) * 1e6 + (last.tv_usec - first.tv_usec);
  ...
  if (m_totalTime && currentTime() > m_totalTime) {
    ...
    printf("total     :%8.3f[s], %8.3f[sim/real]\n", realT,
           m_totalTime / realT);
    printf("controller:%8.3f[s], %8.3f[ms/frame]\n", tm_control.totalTime(),
           tm_control.averageTime() * 1000);
    printf("collision :%8.3f[s], %8.3f[ms/frame]\n", tm_collision.totalTime(),
           tm_collision.averageTime() * 1000);
    printf("dynamics  :%8.3f[s], %8.3f[ms/frame]\n", tm_dynamics.totalTime(),
           tm_dynamics.averageTime() * 1000);
    for (unsigned int i = 0; i < numBodies(); i++) {
    ...
    for (size_t k = 0; k < links2.size(); ++k) {
      ...
      if (link1 && link2 && link1 != link2) {
        constraintForceSolver.addCollisionCheckLinkPair(
            bodyIndex1, link1, bodyIndex2, link2, 0.5, 0.5, 0.01, 0.0, 0.0);
...
void Simulator::kinematicsOnly(bool flag) { m_kinematicsOnly = flag; }
