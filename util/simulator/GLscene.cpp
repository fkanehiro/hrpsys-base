...
GLscene::GLscene(LogManagerBase *i_log)
    : GLsceneBase(i_log), m_showSensors(false), m_showCollision(true) {}

void GLscene::updateScene() {
  if (m_log->index() < 0) return;

  LogManager<SceneState> *lm = (LogManager<SceneState> *)m_log;
...
#define HEIGHT_STEP 12
    ...
    for (unsigned int i = 0; i < glbody->numJoints(); i++) {
      ...
      if (l) {
        sprintf(buf, "%2d %15s %8.3f", i, l->name.c_str(), q[i] * 180 / M_PI);
    ...
    if (bstate->acc.size()) {
      ...
      for (unsigned int i = 0; i < bstate->acc.size(); i++) {
        sprintf(buf, "  %8.4f %8.4f %8.4f", bstate->acc[i][0],
                bstate->acc[i][1], bstate->acc[i][2]);
        glRasterPos2f(width, height);
        height -= HEIGHT_STEP;
        drawString2(buf);
      }
    ...
    if (bstate->rate.size()) {
      ...
      for (unsigned int i = 0; i < bstate->rate.size(); i++) {
        sprintf(buf, "  %8.4f %8.4f %8.4f", bstate->rate[i][0],
                bstate->rate[i][1], bstate->rate[i][2]);
        glRasterPos2f(width, height);
        height -= HEIGHT_STEP;
        drawString2(buf);
      }
    ...
    if (bstate->force.size()) {
      ...
      for (unsigned int i = 0; i < bstate->force.size(); i++) {
        sprintf(buf, "  %6.1f %6.1f %6.1f %6.2f %6.2f %6.2f",
                bstate->force[i][0], bstate->force[i][1], bstate->force[i][2],
                bstate->force[i][3], bstate->force[i][4], bstate->force[i][5]);
        glRasterPos2f(width, height);
        height -= HEIGHT_STEP;
        drawString2(buf);
      }
...
void GLscene::drawSensorOutput(Body *body, Sensor *sensor) {
  ...
  if (bodyIndex(body->name()) < 0) {
    std::cerr << "invalid bodyIndex(" << bodyIndex(body->name()) << ") for "
              << body->name() << std::endl;
    return;
  }
...
void GLscene::showSensors(bool flag) { m_showSensors = flag; }

bool GLscene::showSensors() { return m_showSensors; }

void GLscene::showCollision(bool flag) { m_showCollision = flag; }

bool GLscene::showCollision() { return m_showCollision; }
